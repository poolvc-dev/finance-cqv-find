
        // Check if data loaded correctly, fallback to fetch JSON if needed
        let companies = [
  {
    "ticker": "NVDA",
    "name": "NVIDIA Corporation",
    "sector": "Technology / AI Computing Infrastructure, Semiconductors & CUDA Software Ecosystem",
    "quarter": "Q1 2026",
    "f1": 9.8,
    "f2": 9.85,
    "f3": 9.6,
    "f4": 9.95,
    "f5": 9.4,
    "f6": 9.7,
    "f7": 9.6,
    "f8": 9.7,
    "momentum_score": 5.39,
    "cqv_v1": 9.35,
    "cqv_v1_1": 9.26,
    "cqv_v2": 8.93,
    "cqv_v3": 8.93,
    "cqv": 9.73,
    "pe": 42.5,
    "close_history": {
      "2020": 13.05,
      "2021": 29.41,
      "2022": 14.61,
      "2023": 49.52,
      "2024": 135.0,
      "2025": 120.0,
      "2026": 128.0
    },
    "status": "Success",
    "cqv_v4": 9.73,
    "price": 128.0,
    "pe_forward": 31.2,
    "value_score": 6.78,
    "peg_bruto": 11.6026,
    "score_peg": 10.0,
    "intrinsic_value": 160.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.95,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 36.2,
    "growth_eps": 36.2,
    "ocf_ttm_m": 60000.0,
    "maint_capex_m": 4000.0,
    "owner_earnings_m": 56000.0,
    "score_fcf_yield": 4.45,
    "score_mos": 6.67,
    "valuation_date": "29/05/2026",
    "eps_trailing": 3.01,
    "eps_forward": 4.1,
    "market_cap_b": 3150.0,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 110.0,
      "target_mean_base": 150.0,
      "target_high_bull": 180.0,
      "num_analysts": 55,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 17.2
    }
  },
  {
    "ticker": "RMS",
    "name": "Hermès International",
    "sector": "Consumer Discretionary / Ultra-Luxury Leather Goods & Apparel",
    "quarter": "Q2 2026",
    "f1": 9.8,
    "f2": 9.9,
    "f3": 9.3,
    "f4": 9.95,
    "f5": 9.6,
    "f6": 9.7,
    "f7": 9.0,
    "f8": 9.8,
    "momentum_score": 2.28,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.52,
    "cqv_v2": 8.12,
    "cqv_v3": 8.12,
    "cqv": 9.69,
    "pe": 48.5,
    "close_history": {
      "2020": 879.6,
      "2021": 1536.0,
      "2022": 1445.0,
      "2023": 1918.0,
      "2024": 2350.0,
      "2025": 2200.0,
      "2026": 2354.4
    },
    "status": "Success",
    "cqv_v4": 9.69,
    "price": 2354.4,
    "pe_forward": 40.5,
    "value_score": 5.3,
    "peg_bruto": 4.8889,
    "score_peg": 4.8889,
    "intrinsic_value": 2943.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "07/08/2026",
    "eps_trailing": 48.54,
    "eps_forward": 58.13,
    "eps_growth_ntm_pct": 19.8,
    "growth_eps": 19.8,
    "market_cap_b": 247.3,
    "ocf_ttm_m": 5184.0,
    "maint_capex_m": 648.0,
    "owner_earnings_m": 4536.0,
    "score_fcf_yield": 4.58,
    "score_mos": 6.67,
    "wacc": 7.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 2000.0,
      "target_mean_base": 2680.0,
      "target_high_bull": 3100.0,
      "num_analysts": 20,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 13.8
    }
  },
  {
    "ticker": "FICO",
    "name": "Fair Isaac Corporation",
    "sector": "Technology / Software, Credit Scoring & Decision Analytics",
    "quarter": "Q2 2026",
    "f1": 9.85,
    "f2": 9.6,
    "f3": 9.4,
    "f4": 9.95,
    "f5": 9.7,
    "f6": 9.6,
    "f7": 9.2,
    "f8": 9.8,
    "momentum_score": 4.07,
    "cqv_v1": 9.6,
    "cqv_v1_1": 9.6,
    "cqv_v2": 9.49,
    "cqv_v3": 9.63,
    "cqv": 9.68,
    "pe": 68.5,
    "close_history": {
      "2020": 510.6,
      "2021": 433.68,
      "2022": 598.57,
      "2023": 1164.0,
      "2024": 1950.0,
      "2025": 1680.0,
      "2026": 1780.0
    },
    "status": "Datos fundamentales actualizados; valoración incompleta N/D",
    "cqv_v4": 9.68,
    "price": 1780.0,
    "pe_forward": 54.2,
    "value_score": 5.02,
    "peg_bruto": 4.8524,
    "score_peg": 4.8524,
    "intrinsic_value": 2225.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.95,
    "score_mos": 6.67,
    "maintenance_capex": null,
    "capex_period": "Seis meses terminados el 31/03/2026; USD millones",
    "data_confidence": "Alta",
    "ocf": 397.44,
    "eps_growth_ntm_pct": 26.3,
    "capex_total": 17.748,
    "score_fcf_yield": 3.9,
    "period_end": "2026-03-31",
    "financial_unit": "USD millones excepto datos por acción",
    "ocf_period": "Seis meses terminados el 31/03/2026; USD millones",
    "sources": [
      "https://investors.fico.com/news-releases/news-release-details/fico-announces-earnings-1114-share-second-quarter-fiscal-2026/",
      "https://investors.fico.com/static-files/6dcc936c-cecf-4181-baac-4474f9196e9d",
      "https://investors.fico.com/static-files/66ef723c-1f2e-4501-a452-ab2f3d02ae85",
      "https://stockanalysis.com/stocks/fico/",
      "https://stockanalysis.com/stocks/fico/history/",
      "https://stockanalysis.com/stocks/fico/forecast/"
    ],
    "valuation_date": "29/07/2026",
    "market_cap": 31840,
    "dcf_status": "N/D: no existe un DCF documentado ni supuestos verificables en el expediente de actualización",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_trailing": 26.0,
    "eps_forward": 32.84,
    "growth_eps": 26.3,
    "market_cap_b": 43.5,
    "ocf_ttm_m": 720.0,
    "maint_capex_m": 40.0,
    "owner_earnings_m": 680.0,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 1500.0,
      "target_mean_base": 2050.0,
      "target_high_bull": 2350.0,
      "num_analysts": 16,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.2
    }
  },
  {
    "ticker": "MSFT",
    "name": "Microsoft Corporation",
    "sector": "Technology / Software, Cloud Infrastructure & AI Platforms",
    "quarter": "Q2 2026",
    "f1": 9.7,
    "f2": 9.85,
    "f3": 9.3,
    "f4": 9.9,
    "f5": 9.6,
    "f6": 9.7,
    "f7": 9.6,
    "f8": 9.7,
    "momentum_score": 2.39,
    "cqv_v1": 9.61,
    "cqv_v1_1": 9.24,
    "cqv_v2": 9.45,
    "cqv_v3": 9.45,
    "cqv": 9.68,
    "pe": 33.8,
    "close_history": {
      "2020": 222.42,
      "2021": 336.32,
      "2022": 239.82,
      "2023": 376.04,
      "2024": 415.2,
      "2025": 420.5,
      "2026": 448.5
    },
    "status": "Datos fundamentales actualizados; valoración incompleta N/D",
    "cqv_v4": 9.68,
    "price": 448.5,
    "pe_forward": 28.4,
    "value_score": 7.01,
    "peg_bruto": 6.6901,
    "score_peg": 6.6901,
    "intrinsic_value": 560.63,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.9,
    "score_mos": 6.67,
    "maintenance_capex": null,
    "capex_period": "Seis meses terminados el 31/12/2025",
    "data_confidence": "Alta",
    "ocf": 80815,
    "eps_growth_ntm_pct": 19.0,
    "capex_total": 49270,
    "score_fcf_yield": 7.5,
    "period_end": "2025-12-31",
    "ocf_period": "Seis meses terminados el 31/12/2025",
    "sources": [
      "https://www.microsoft.com/en-us/investor/earnings/fy-2026-q2/press-release-webcast",
      "https://www.microsoft.com/en-us/investor/earnings/fy-2026-q2/income-statements",
      "https://www.microsoft.com/en-us/investor/earnings/fy-2026-q2/cash-flows",
      "https://www.microsoft.com/en-us/investor/earnings/fy-2026-q2/balance-sheets",
      "https://www.microsoft.com/en-us/investor/earnings/fy-2026-q2/segment-revenues",
      "https://stockanalysis.com/stocks/msft/history/",
      "https://stockanalysis.com/stocks/msft/",
      "https://stockanalysis.com/stocks/msft/forecast/"
    ],
    "valuation_date": "29/07/2026",
    "market_cap": 2900000000000,
    "dcf_status": "N/D: no existe un DCF documentado ni supuestos verificables en el expediente de actualización",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "growth_eps": 19.0,
    "ocf_ttm_m": 118500.0,
    "maint_capex_m": 18500.0,
    "owner_earnings_m": 100000.0,
    "analyst_targets": {
      "target_low_bear": 410.0,
      "target_mean_base": 515.0,
      "target_high_bull": 600.0,
      "num_analysts": 45,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.8
    },
    "eps_trailing": 13.27,
    "eps_forward": 15.79,
    "market_cap_b": 3330.0,
    "wacc": 8.0,
    "g_terminal": 3.5
  },
  {
    "ticker": "RACE",
    "name": "Ferrari N.V.",
    "sector": "Consumer Discretionary / Luxury Automotive & Racing Infrastructure",
    "quarter": "Q2 2026",
    "f1": 9.75,
    "f2": 9.8,
    "f3": 9.35,
    "f4": 9.9,
    "f5": 9.5,
    "f6": 9.65,
    "f7": 9.1,
    "f8": 9.75,
    "momentum_score": 4.91,
    "cqv_v1": 7.91,
    "cqv_v1_1": 8.33,
    "cqv_v2": 7.99,
    "cqv_v3": 7.99,
    "cqv": 9.65,
    "pe": 51.5,
    "close_history": {
      "2020": 233.1,
      "2021": 257.6,
      "2022": 214.2,
      "2023": 336.8,
      "2024": 440.0,
      "2025": 410.0,
      "2026": 435.2
    },
    "status": "Success",
    "cqv_v4": 9.65,
    "price": 435.2,
    "pe_forward": 41.2,
    "value_score": 5.4,
    "peg_bruto": 6.068,
    "score_peg": 6.068,
    "intrinsic_value": 544.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.95,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 8.45,
    "eps_forward": 10.56,
    "eps_growth_ntm_pct": 25.0,
    "growth_eps": 25.0,
    "market_cap_b": 78.5,
    "ocf_ttm_m": 1620.0,
    "maint_capex_m": 378.0,
    "owner_earnings_m": 1242.0,
    "score_fcf_yield": 3.95,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 380.0,
      "target_mean_base": 500.0,
      "target_high_bull": 580.0,
      "num_analysts": 18,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.9
    }
  },
  {
    "ticker": "ASML",
    "name": "ASML Holding N.V.",
    "sector": "Technology / Semiconductor Photolithography Systems & EUV Technology",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.7,
    "f3": 9.4,
    "f4": 9.95,
    "f5": 9.4,
    "f6": 9.6,
    "f7": 9.6,
    "f8": 9.7,
    "momentum_score": 8.84,
    "cqv_v1": 9.05,
    "cqv_v1_1": 9.58,
    "cqv_v2": 8.78,
    "cqv_v3": 8.78,
    "cqv": 9.63,
    "pe": 38.5,
    "close_history": {
      "2020": 487.72,
      "2021": 796.14,
      "2022": 546.4,
      "2023": 756.92,
      "2024": 980.0,
      "2025": 850.0,
      "2026": 825.0
    },
    "status": "Success",
    "cqv_v4": 9.63,
    "price": 825.0,
    "pe_forward": 27.5,
    "value_score": 7.66,
    "peg_bruto": 14.5455,
    "score_peg": 10.0,
    "intrinsic_value": 1031.25,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.95,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 40.0,
    "growth_eps": 40.0,
    "ocf_ttm_m": 9936.0,
    "maint_capex_m": 1296.0,
    "owner_earnings_m": 8640.0,
    "score_fcf_yield": 6.65,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 21.43,
    "eps_forward": 30.0,
    "market_cap_b": 325.0,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 720.0,
      "target_mean_base": 975.0,
      "target_high_bull": 1150.0,
      "num_analysts": 35,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 18.2
    }
  },
  {
    "ticker": "ISRG",
    "name": "Intuitive Surgical, Inc.",
    "sector": "Healthcare / Robotic Surgical Systems & Medical Devices",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.8,
    "f3": 9.4,
    "f4": 9.9,
    "f5": 9.3,
    "f6": 9.6,
    "f7": 9.6,
    "f8": 9.7,
    "momentum_score": 2.32,
    "cqv_v1": 8.74,
    "cqv_v1_1": 9.45,
    "cqv_v2": 8.8,
    "cqv_v3": 8.8,
    "cqv": 9.62,
    "pe": 62.5,
    "close_history": {
      "2020": 272.37,
      "2021": 359.3,
      "2022": 265.35,
      "2023": 337.36,
      "2024": 440.0,
      "2025": 420.0,
      "2026": 445.0
    },
    "status": "Success",
    "cqv_v4": 9.62,
    "price": 445.0,
    "pe_forward": 48.2,
    "value_score": 5.24,
    "peg_bruto": 6.1411,
    "score_peg": 6.1411,
    "intrinsic_value": 556.25,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.9,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 29.6,
    "growth_eps": 29.6,
    "ocf_ttm_m": 2650.0,
    "maint_capex_m": 450.0,
    "owner_earnings_m": 2200.0,
    "score_fcf_yield": 3.48,
    "score_mos": 6.67,
    "valuation_date": "18/07/2026",
    "eps_trailing": 7.12,
    "eps_forward": 9.23,
    "market_cap_b": 158.0,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 390.0,
      "target_mean_base": 525.0,
      "target_high_bull": 580.0,
      "num_analysts": 32,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 18.0
    }
  },
  {
    "ticker": "MANH",
    "name": "Manhattan Associates, Inc.",
    "sector": "Technology / Supply Chain Management, WMS & Omnichannel Cloud Software",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.8,
    "f3": 9.4,
    "f4": 9.85,
    "f5": 9.6,
    "f6": 9.7,
    "f7": 9.3,
    "f8": 9.6,
    "momentum_score": 3.66,
    "cqv_v1": 8.5,
    "cqv_v1_1": 8.77,
    "cqv_v2": 8.27,
    "cqv_v3": 8.27,
    "cqv": 9.61,
    "pe": 55.0,
    "close_history": {
      "2020": 105.2,
      "2021": 155.49,
      "2022": 121.4,
      "2023": 215.32,
      "2024": 270.24,
      "2025": 235.0,
      "2026": 240.0
    },
    "status": "Success",
    "cqv_v4": 9.61,
    "price": 240.0,
    "pe_forward": 42.5,
    "value_score": 5.8,
    "peg_bruto": 6.9412,
    "score_peg": 6.9412,
    "intrinsic_value": 300.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.85,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "21/07/2026",
    "eps_trailing": 4.36,
    "eps_forward": 5.647,
    "eps_growth_ntm_pct": 29.5,
    "growth_eps": 29.5,
    "market_cap_b": 14.6,
    "ocf_ttm_m": 265.0,
    "maint_capex_m": 15.0,
    "owner_earnings_m": 250.0,
    "score_fcf_yield": 4.28,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 210.0,
      "target_mean_base": 282.0,
      "target_high_bull": 315.0,
      "num_analysts": 14,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 17.5
    }
  },
  {
    "ticker": "V",
    "name": "Visa Inc.",
    "sector": "Financials / Payment Processing & Financial Networks",
    "quarter": "Q2 2026",
    "f1": 9.8,
    "f2": 9.6,
    "f3": 9.2,
    "f4": 9.9,
    "f5": 9.6,
    "f6": 9.5,
    "f7": 9.0,
    "f8": 9.7,
    "momentum_score": 7.75,
    "cqv_v1": 9.23,
    "cqv_v1_1": 9.6,
    "cqv_v2": 9.34,
    "cqv_v3": 9.34,
    "cqv": 9.6,
    "pe": 27.8,
    "close_history": {
      "2020": 218.73,
      "2021": 216.71,
      "2022": 207.76,
      "2023": 260.35,
      "2024": 282.5,
      "2025": 278.1,
      "2026": 272.5
    },
    "status": "Success",
    "cqv_v4": 9.6,
    "price": 272.5,
    "pe_forward": 23.2,
    "value_score": 8.31,
    "peg_bruto": 8.5776,
    "score_peg": 8.5776,
    "intrinsic_value": 340.63,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.9,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 9.8,
    "eps_forward": 11.75,
    "eps_growth_ntm_pct": 19.9,
    "growth_eps": 19.9,
    "market_cap_b": 552.0,
    "ocf_ttm_m": 21800.0,
    "maint_capex_m": 1200.0,
    "owner_earnings_m": 20600.0,
    "score_fcf_yield": 9.33,
    "score_mos": 6.67,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 245.0,
      "target_mean_base": 315.0,
      "target_high_bull": 355.0,
      "num_analysts": 42,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.6
    }
  },
  {
    "ticker": "AVGO",
    "name": "Broadcom Inc.",
    "sector": "Technology / Custom AI Accelerators, Networking Semiconductors & Enterprise Software",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.5,
    "f3": 9.5,
    "f4": 9.8,
    "f5": 9.5,
    "f6": 9.6,
    "f7": 9.4,
    "f8": 9.6,
    "momentum_score": 4.7,
    "cqv_v1": 9.02,
    "cqv_v1_1": 9.44,
    "cqv_v2": 9.02,
    "cqv_v3": 9.02,
    "cqv": 9.58,
    "pe": 35.45,
    "close_history": {
      "2020": 43.78,
      "2021": 66.54,
      "2022": 55.91,
      "2023": 111.63,
      "2024": 165.0,
      "2025": 155.0,
      "2026": 178.0
    },
    "status": "Success",
    "cqv_v4": 9.58,
    "price": 178.0,
    "pe_forward": 26.37,
    "value_score": 7.62,
    "peg_bruto": 13.083,
    "score_peg": 10.0,
    "intrinsic_value": 222.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "12/06/2026",
    "eps_trailing": 5.02,
    "eps_forward": 6.75,
    "eps_growth_ntm_pct": 34.5,
    "growth_eps": 34.5,
    "market_cap_b": 833.0,
    "ocf_ttm_m": 23400.0,
    "maint_capex_m": 1600.0,
    "owner_earnings_m": 21800.0,
    "score_fcf_yield": 6.55,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 150.0,
      "target_mean_base": 210.0,
      "target_high_bull": 245.0,
      "num_analysts": 40,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 18.0
    }
  },
  {
    "ticker": "META",
    "name": "Meta Platforms, Inc.",
    "sector": "Technology / Social Media, AI Infrastructure & Digital Advertising",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.7,
    "f3": 9.3,
    "f4": 9.85,
    "f5": 9.4,
    "f6": 9.5,
    "f7": 9.5,
    "f8": 9.6,
    "momentum_score": 2.83,
    "cqv_v1": 9.37,
    "cqv_v1_1": 9.22,
    "cqv_v2": 8.91,
    "cqv_v3": 9.16,
    "cqv": 9.57,
    "pe": 24.5,
    "close_history": {
      "2020": 273.16,
      "2021": 336.35,
      "2022": 120.34,
      "2023": 353.96,
      "2024": 585.0,
      "2025": 480.0,
      "2026": 510.0
    },
    "status": "Success",
    "cqv_v4": 9.57,
    "price": 510.0,
    "pe_forward": 20.2,
    "value_score": 9.0,
    "peg_bruto": 10.5446,
    "score_peg": 10.0,
    "intrinsic_value": 637.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.85,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 21.3,
    "growth_eps": 21.3,
    "ocf_ttm_m": 88000.0,
    "maint_capex_m": 24000.0,
    "owner_earnings_m": 64000.0,
    "score_fcf_yield": 10.0,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 20.82,
    "eps_forward": 25.25,
    "market_cap_b": 1295.0,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 450.0,
      "target_mean_base": 610.0,
      "target_high_bull": 700.0,
      "num_analysts": 45,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 19.6
    }
  },
  {
    "ticker": "HEI",
    "name": "HEICO Corporation",
    "sector": "Industrials / Aerospace & Defense Replacement Parts (FAA-PMA) & Flight Support",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.6,
    "f3": 9.4,
    "f4": 9.8,
    "f5": 9.6,
    "f6": 9.7,
    "f7": 9.3,
    "f8": 9.6,
    "momentum_score": 8.15,
    "cqv_v1": 8.18,
    "cqv_v1_1": 8.63,
    "cqv_v2": 8.27,
    "cqv_v3": 8.27,
    "cqv": 9.57,
    "pe": 62.0,
    "close_history": {
      "2020": 118.5,
      "2021": 143.48,
      "2022": 153.05,
      "2023": 178.4,
      "2024": 237.37,
      "2025": 240.0,
      "2026": 248.0
    },
    "status": "Success",
    "cqv_v4": 9.57,
    "price": 248.0,
    "pe_forward": 48.5,
    "value_score": 5.54,
    "peg_bruto": 5.732,
    "score_peg": 5.732,
    "intrinsic_value": 310.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "02/06/2026",
    "eps_trailing": 4.0,
    "eps_forward": 5.113,
    "eps_growth_ntm_pct": 27.8,
    "growth_eps": 27.8,
    "market_cap_b": 34.0,
    "ocf_ttm_m": 680.0,
    "maint_capex_m": 60.0,
    "owner_earnings_m": 620.0,
    "score_fcf_yield": 4.55,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 215.0,
      "target_mean_base": 292.0,
      "target_high_bull": 330.0,
      "num_analysts": 18,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 17.7
    }
  },
  {
    "ticker": "GOOGL",
    "name": "Alphabet Inc.",
    "sector": "Technology / Digital Advertising, Search, Cloud Computing & Applied AI",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.8,
    "f3": 9.3,
    "f4": 9.85,
    "f5": 9.3,
    "f6": 9.5,
    "f7": 9.5,
    "f8": 9.6,
    "momentum_score": 7.21,
    "cqv_v1": 9.41,
    "cqv_v1_1": 9.0,
    "cqv_v2": 9.31,
    "cqv_v3": 9.41,
    "cqv": 9.56,
    "pe": 23.5,
    "close_history": {
      "2020": 87.59,
      "2021": 144.85,
      "2022": 88.23,
      "2023": 139.69,
      "2024": 175.0,
      "2025": 170.0,
      "2026": 182.0
    },
    "status": "Success",
    "cqv_v4": 9.56,
    "price": 182.0,
    "pe_forward": 19.8,
    "value_score": 8.27,
    "peg_bruto": 10.8586,
    "score_peg": 10.0,
    "intrinsic_value": 227.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.85,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 21.5,
    "growth_eps": 21.5,
    "ocf_ttm_m": 106000.0,
    "maint_capex_m": 32000.0,
    "owner_earnings_m": 74000.0,
    "score_fcf_yield": 8.18,
    "score_mos": 6.67,
    "analyst_targets": {
      "target_low_bear": 160.0,
      "target_mean_base": 215.0,
      "target_high_bull": 240.0,
      "num_analysts": 50,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 18.1
    },
    "valuation_date": "29/07/2026",
    "eps_trailing": 7.74,
    "eps_forward": 9.19,
    "market_cap_b": 2260.0,
    "wacc": 8.0,
    "g_terminal": 3.5
  },
  {
    "ticker": "CPRT",
    "name": "Copart, Inc.",
    "sector": "Industrials / Salvage Auto Auctions & Online Re-marketing",
    "quarter": "Q1 2026",
    "f1": 9.6,
    "f2": 9.7,
    "f3": 9.1,
    "f4": 9.85,
    "f5": 9.5,
    "f6": 9.6,
    "f7": 8.9,
    "f8": 9.6,
    "momentum_score": 1.46,
    "cqv_v1": 9.17,
    "cqv_v1_1": 8.98,
    "cqv_v2": 8.68,
    "cqv_v3": 8.68,
    "cqv": 9.53,
    "pe": 34.2,
    "close_history": {
      "2020": 31.81,
      "2021": 37.92,
      "2022": 30.47,
      "2023": 49.0,
      "2024": 58.2,
      "2025": 52.4,
      "2026": 54.8
    },
    "status": "Success",
    "cqv_v4": 9.53,
    "price": 54.8,
    "pe_forward": 28.2,
    "value_score": 6.66,
    "peg_bruto": 7.5532,
    "score_peg": 7.5532,
    "intrinsic_value": 68.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.85,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 1.6,
    "eps_forward": 1.94,
    "eps_growth_ntm_pct": 21.3,
    "growth_eps": 21.3,
    "market_cap_b": 52.8,
    "ocf_ttm_m": 1680.0,
    "maint_capex_m": 420.0,
    "owner_earnings_m": 1260.0,
    "score_fcf_yield": 5.98,
    "score_mos": 6.67,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 48.0,
      "target_mean_base": 62.0,
      "target_high_bull": 70.0,
      "num_analysts": 15,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 13.1
    }
  },
  {
    "ticker": "NOW",
    "name": "ServiceNow, Inc.",
    "sector": "Technology / Enterprise Cloud Platform & Digital Workflow Automation",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.7,
    "f3": 9.35,
    "f4": 9.75,
    "f5": 9.35,
    "f6": 9.5,
    "f7": 9.4,
    "f8": 9.5,
    "momentum_score": 2.77,
    "cqv_v1": 9.36,
    "cqv_v1_1": 9.29,
    "cqv_v2": 8.83,
    "cqv_v3": 8.83,
    "cqv": 9.53,
    "pe": 62.5,
    "close_history": {
      "2020": 550.43,
      "2021": 649.11,
      "2022": 388.27,
      "2023": 706.49,
      "2024": 850.0,
      "2025": 740.0,
      "2026": 785.6
    },
    "status": "Success",
    "cqv_v4": 9.53,
    "price": 785.6,
    "pe_forward": 48.5,
    "value_score": 5.77,
    "peg_bruto": 5.9588,
    "score_peg": 5.9588,
    "intrinsic_value": 982.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.7,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 28.9,
    "growth_eps": 28.9,
    "ocf_ttm_m": 3700.0,
    "maint_capex_m": 500.0,
    "owner_earnings_m": 3200.0,
    "score_fcf_yield": 4.95,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 12.57,
    "eps_forward": 16.2,
    "market_cap_b": 162.0,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 700.0,
      "target_mean_base": 910.0,
      "target_high_bull": 1050.0,
      "num_analysts": 30,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.8
    }
  },
  {
    "ticker": "MSCI",
    "name": "MSCI Inc.",
    "sector": "Financials / Market Indexes, Data & Financial Analytics",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.4,
    "f3": 9.2,
    "f4": 9.85,
    "f5": 9.5,
    "f6": 9.5,
    "f7": 9.1,
    "f8": 9.6,
    "momentum_score": 6.43,
    "cqv_v1": 9.1,
    "cqv_v1_1": 9.54,
    "cqv_v2": 8.88,
    "cqv_v3": 8.88,
    "cqv": 9.5,
    "pe": 36.8,
    "close_history": {
      "2020": 446.53,
      "2021": 612.67,
      "2022": 465.17,
      "2023": 565.65,
      "2024": 598.2,
      "2025": 560.1,
      "2026": 545.2
    },
    "status": "Success",
    "cqv_v4": 9.5,
    "price": 545.2,
    "pe_forward": 30.2,
    "value_score": 7.33,
    "peg_bruto": 7.2185,
    "score_peg": 7.2185,
    "intrinsic_value": 681.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.85,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 21.8,
    "growth_eps": 21.8,
    "ocf_ttm_m": 1450.0,
    "maint_capex_m": 90.0,
    "owner_earnings_m": 1360.0,
    "score_fcf_yield": 7.9,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 14.82,
    "eps_forward": 18.05,
    "market_cap_b": 43.1,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 480.0,
      "target_mean_base": 625.0,
      "target_high_bull": 690.0,
      "num_analysts": 21,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.6
    }
  },
  {
    "ticker": "LIN",
    "name": "Linde plc",
    "sector": "Materials / Industrial Gases & Engineering",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.4,
    "f3": 9.1,
    "f4": 9.8,
    "f5": 9.6,
    "f6": 9.6,
    "f7": 9.0,
    "f8": 9.7,
    "momentum_score": 8.63,
    "cqv_v1": 8.0,
    "cqv_v1_1": 8.33,
    "cqv_v2": 8.01,
    "cqv_v3": 8.01,
    "cqv": 9.48,
    "pe": 31.2,
    "close_history": {
      "2020": 263.51,
      "2021": 346.43,
      "2022": 326.18,
      "2023": 410.71,
      "2024": 442.2,
      "2025": 430.5,
      "2026": 455.0
    },
    "status": "Success",
    "cqv_v4": 9.48,
    "price": 455.0,
    "pe_forward": 26.5,
    "value_score": 6.9,
    "peg_bruto": 6.717,
    "score_peg": 6.717,
    "intrinsic_value": 568.75,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 14.58,
    "eps_forward": 17.17,
    "eps_growth_ntm_pct": 17.8,
    "growth_eps": 17.8,
    "market_cap_b": 215.0,
    "ocf_ttm_m": 9850.0,
    "maint_capex_m": 3650.0,
    "owner_earnings_m": 6200.0,
    "score_fcf_yield": 7.2,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 400.0,
      "target_mean_base": 520.0,
      "target_high_bull": 580.0,
      "num_analysts": 28,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.3
    }
  },
  {
    "ticker": "HLI",
    "name": "Houlihan Lokey, Inc.",
    "sector": "Financials / Investment Banking, Global M&A Advisory & Financial Restructuring",
    "quarter": "Q2 2026",
    "f1": 9.4,
    "f2": 9.6,
    "f3": 9.3,
    "f4": 9.65,
    "f5": 9.5,
    "f6": 9.6,
    "f7": 9.1,
    "f8": 9.5,
    "momentum_score": 1.83,
    "cqv_v1": 9.0,
    "cqv_v1_1": 8.86,
    "cqv_v2": 8.45,
    "cqv_v3": 8.45,
    "cqv": 9.48,
    "pe": 27.5,
    "close_history": {
      "2020": 66.8,
      "2021": 103.5,
      "2022": 86.4,
      "2023": 120.5,
      "2024": 160.0,
      "2025": 145.0,
      "2026": 162.0
    },
    "status": "Success",
    "cqv_v4": 9.48,
    "price": 162.0,
    "pe_forward": 21.2,
    "value_score": 9.0,
    "peg_bruto": 14.0094,
    "score_peg": 10.0,
    "intrinsic_value": 202.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.65,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 5.89,
    "eps_forward": 7.641,
    "eps_growth_ntm_pct": 29.7,
    "growth_eps": 29.7,
    "market_cap_b": 11.1,
    "ocf_ttm_m": 510.0,
    "maint_capex_m": 25.0,
    "owner_earnings_m": 485.0,
    "score_fcf_yield": 10.0,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 140.0,
      "target_mean_base": 190.0,
      "target_high_bull": 215.0,
      "num_analysts": 12,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 17.3
    }
  },
  {
    "ticker": "MCO",
    "name": "Moody's Corporation",
    "sector": "Financials / Financial Data & Credit Ratings",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.4,
    "f3": 9.1,
    "f4": 9.9,
    "f5": 9.4,
    "f6": 9.5,
    "f7": 8.8,
    "f8": 9.6,
    "momentum_score": 5.81,
    "cqv_v1": 9.2,
    "cqv_v1_1": 9.31,
    "cqv_v2": 8.65,
    "cqv_v3": 8.65,
    "cqv": 9.47,
    "pe": 36.5,
    "close_history": {
      "2020": 290.24,
      "2021": 393.58,
      "2022": 278.62,
      "2023": 390.56,
      "2024": 485.1,
      "2025": 475.2,
      "2026": 468.2
    },
    "status": "Success",
    "cqv_v4": 9.47,
    "price": 468.2,
    "pe_forward": 29.8,
    "value_score": 7.21,
    "peg_bruto": 7.5168,
    "score_peg": 7.5168,
    "intrinsic_value": 585.25,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.9,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 12.83,
    "eps_forward": 15.71,
    "eps_growth_ntm_pct": 22.4,
    "growth_eps": 22.4,
    "market_cap_b": 85.2,
    "ocf_ttm_m": 2650.0,
    "maint_capex_m": 140.0,
    "owner_earnings_m": 2510.0,
    "score_fcf_yield": 7.38,
    "score_mos": 6.67,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 415.0,
      "target_mean_base": 545.0,
      "target_high_bull": 610.0,
      "num_analysts": 22,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 16.4
    }
  },
  {
    "ticker": "MA",
    "name": "Mastercard Incorporated",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 9.8,
    "f5": 9.5,
    "f6": 9.5,
    "f7": 7.0,
    "f8": 9.9,
    "momentum_score": 5.27,
    "cqv_v1": 9.27,
    "cqv_v1_1": 9.6,
    "cqv_v2": 9.38,
    "cqv_v3": 9.38,
    "cqv": 9.46,
    "pe": 31.25,
    "close_history": "{'2021': 349.29, '2022': 340.0, '2023': 419.58, '2024': 521.01, '2025': 568.09, '2026': 551.54}",
    "status": "Success",
    "cqv_v4": 9.46,
    "price": 100.0,
    "pe_forward": 26.5625,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 158.4,
    "mos_pct": 36.87,
    "verdict": "Comprar / Candidato Prioritario",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "KLAC",
    "name": "KLA Corporation",
    "sector": "Technology / Semiconductor Equipment & Inspection Systems",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.3,
    "f3": 9.2,
    "f4": 9.8,
    "f5": 9.4,
    "f6": 9.5,
    "f7": 9.1,
    "f8": 9.5,
    "momentum_score": 8.86,
    "cqv_v1": 9.48,
    "cqv_v1_1": 9.31,
    "cqv_v2": 8.84,
    "cqv_v3": 8.84,
    "cqv": 9.46,
    "pe": 28.4,
    "close_history": {
      "2020": 258.91,
      "2021": 430.11,
      "2022": 377.03,
      "2023": 581.3,
      "2024": 815.2,
      "2025": 790.5,
      "2026": 785.4
    },
    "status": "Success",
    "cqv_v4": 9.46,
    "price": 785.4,
    "pe_forward": 22.5,
    "value_score": 8.31,
    "peg_bruto": 11.6444,
    "score_peg": 10.0,
    "intrinsic_value": 981.75,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 26.2,
    "growth_eps": 26.2,
    "ocf_ttm_m": 3850.0,
    "maint_capex_m": 310.0,
    "owner_earnings_m": 3540.0,
    "score_fcf_yield": 8.28,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 27.65,
    "eps_forward": 34.9,
    "market_cap_b": 106.8,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 680.0,
      "target_mean_base": 910.0,
      "target_high_bull": 1020.0,
      "num_analysts": 26,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.9
    }
  },
  {
    "ticker": "TSM",
    "name": "Taiwan Semiconductor Manufacturing Co.",
    "sector": "Technology / Semiconductor Foundry & Advanced Packaging",
    "quarter": "Q2 2026",
    "f1": 9.45,
    "f2": 9.6,
    "f3": 9.3,
    "f4": 9.8,
    "f5": 9.2,
    "f6": 9.45,
    "f7": 9.4,
    "f8": 9.3,
    "momentum_score": 8.35,
    "cqv_v1": 9.07,
    "cqv_v1_1": 9.34,
    "cqv_v2": 8.76,
    "cqv_v3": 8.76,
    "cqv": 9.46,
    "pe": 28.5,
    "close_history": {
      "2020": 109.04,
      "2021": 120.31,
      "2022": 74.49,
      "2023": 104.0,
      "2024": 185.0,
      "2025": 160.0,
      "2026": 175.0
    },
    "status": "Success",
    "cqv_v4": 9.46,
    "price": 175.0,
    "pe_forward": 21.8,
    "value_score": 8.14,
    "peg_bruto": 14.1284,
    "score_peg": 10.0,
    "intrinsic_value": 218.75,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.5,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 30.8,
    "growth_eps": 30.8,
    "ocf_ttm_m": 43500.0,
    "maint_capex_m": 15000.0,
    "owner_earnings_m": 28500.0,
    "score_fcf_yield": 7.85,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 6.14,
    "eps_forward": 8.03,
    "market_cap_b": 907.0,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 150.0,
      "target_mean_base": 205.0,
      "target_high_bull": 240.0,
      "num_analysts": 32,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 17.1
    }
  },
  {
    "ticker": "ODFL",
    "name": "Old Dominion Freight Line, Inc.",
    "sector": "Industrials / Less-Than-Truckload (LTL) Freight Transportation",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.5,
    "f3": 9.1,
    "f4": 9.7,
    "f5": 9.5,
    "f6": 9.6,
    "f7": 8.3,
    "f8": 9.5,
    "momentum_score": 7.55,
    "cqv_v1": 8.06,
    "cqv_v1_1": 8.43,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 9.44,
    "pe": 34.5,
    "close_history": {
      "2020": 97.45,
      "2021": 148.12,
      "2022": 133.56,
      "2023": 188.4,
      "2024": 212.5,
      "2025": 205.1,
      "2026": 195.8
    },
    "status": "Success",
    "cqv_v4": 9.44,
    "price": 195.8,
    "pe_forward": 28.2,
    "value_score": 7.11,
    "peg_bruto": 7.8723,
    "score_peg": 7.8723,
    "intrinsic_value": 244.75,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.7,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 5.68,
    "eps_forward": 6.94,
    "eps_growth_ntm_pct": 22.2,
    "growth_eps": 22.2,
    "market_cap_b": 42.5,
    "ocf_ttm_m": 1650.0,
    "maint_capex_m": 480.0,
    "owner_earnings_m": 1170.0,
    "score_fcf_yield": 6.88,
    "score_mos": 6.67,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 170.0,
      "target_mean_base": 222.0,
      "target_high_bull": 250.0,
      "num_analysts": 24,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 13.4
    }
  },
  {
    "ticker": "FTNT",
    "name": "Fortinet, Inc.",
    "sector": "Technology / Cybersecurity & Network Infrastructure",
    "quarter": "Q2 2026",
    "f1": 9.6,
    "f2": 9.5,
    "f3": 9.1,
    "f4": 9.6,
    "f5": 9.4,
    "f6": 9.5,
    "f7": 9.1,
    "f8": 9.4,
    "momentum_score": 9.88,
    "cqv_v1": 9.48,
    "cqv_v1_1": 9.48,
    "cqv_v2": 9.41,
    "cqv_v3": 9.48,
    "cqv": 9.44,
    "pe": 36.5,
    "close_history": {
      "2020": 29.7,
      "2021": 71.88,
      "2022": 48.89,
      "2023": 58.53,
      "2024": 82.1,
      "2025": 78.4,
      "2026": 76.8
    },
    "status": "Success",
    "cqv_v4": 9.44,
    "price": 76.8,
    "pe_forward": 29.8,
    "value_score": 7.84,
    "peg_bruto": 7.651,
    "score_peg": 7.651,
    "intrinsic_value": 96.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.6,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 22.8,
    "growth_eps": 22.8,
    "ocf_ttm_m": 2250.0,
    "maint_capex_m": 180.0,
    "owner_earnings_m": 2070.0,
    "score_fcf_yield": 8.85,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 2.1,
    "eps_forward": 2.58,
    "market_cap_b": 58.5,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 68.0,
      "target_mean_base": 88.0,
      "target_high_bull": 100.0,
      "num_analysts": 36,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.6
    }
  },
  {
    "ticker": "MSI",
    "name": "Motorola Solutions, Inc.",
    "sector": "Technology / Public Safety, Critical Communications & Video Security",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.2,
    "f3": 9.1,
    "f4": 9.8,
    "f5": 9.4,
    "f6": 9.5,
    "f7": 8.9,
    "f8": 9.6,
    "momentum_score": 4.97,
    "cqv_v1": 8.27,
    "cqv_v1_1": 8.58,
    "cqv_v2": 8.14,
    "cqv_v3": 8.14,
    "cqv": 9.41,
    "pe": 34.8,
    "close_history": {
      "2020": 170.06,
      "2021": 271.7,
      "2022": 257.71,
      "2023": 313.19,
      "2024": 447.8,
      "2025": 420.5,
      "2026": 445.5
    },
    "status": "Success",
    "cqv_v4": 9.41,
    "price": 445.5,
    "pe_forward": 28.5,
    "value_score": 7.24,
    "peg_bruto": 7.7544,
    "score_peg": 7.7544,
    "intrinsic_value": 556.88,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 22.1,
    "growth_eps": 22.1,
    "ocf_ttm_m": 2350.0,
    "maint_capex_m": 190.0,
    "owner_earnings_m": 2160.0,
    "score_fcf_yield": 7.28,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 12.8,
    "eps_forward": 15.63,
    "market_cap_b": 74.2,
    "wacc": 8.5,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 390.0,
      "target_mean_base": 510.0,
      "target_high_bull": 570.0,
      "num_analysts": 19,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.5
    }
  },
  {
    "ticker": "SAP",
    "name": "SAP SE",
    "sector": "Technology / Enterprise Software, ERP Systems & Business Cloud Applications",
    "quarter": "Q2 2026",
    "f1": 9.35,
    "f2": 9.5,
    "f3": 9.1,
    "f4": 9.85,
    "f5": 9.3,
    "f6": 9.4,
    "f7": 9.1,
    "f8": 9.5,
    "momentum_score": 1.42,
    "cqv_v1": 8.58,
    "cqv_v1_1": 8.73,
    "cqv_v2": 8.23,
    "cqv_v3": 8.23,
    "cqv": 9.41,
    "pe": 34.5,
    "close_history": {
      "2020": 130.5,
      "2021": 142.1,
      "2022": 102.4,
      "2023": 153.2,
      "2024": 205.5,
      "2025": 215.0,
      "2026": 228.0
    },
    "status": "Success",
    "cqv_v4": 9.41,
    "price": 228.0,
    "pe_forward": 26.8,
    "value_score": 7.44,
    "peg_bruto": 10.709,
    "score_peg": 10.0,
    "intrinsic_value": 285.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.85,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 6.61,
    "eps_forward": 8.51,
    "eps_growth_ntm_pct": 28.7,
    "growth_eps": 28.7,
    "market_cap_b": 266.0,
    "ocf_ttm_m": 7344.0,
    "maint_capex_m": 864.0,
    "owner_earnings_m": 6480.0,
    "score_fcf_yield": 6.1,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 200.0,
      "target_mean_base": 265.0,
      "target_high_bull": 310.0,
      "num_analysts": 28,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 16.2
    }
  },
  {
    "ticker": "BSX",
    "name": "Boston Scientific Corporation",
    "sector": "Healthcare / Medical Devices, Electrophysiology & Cardiovascular Solutions",
    "quarter": "Q2 2026",
    "valuation_date": "29/07/2026",
    "price": 82.5,
    "pe": 36.5,
    "pe_forward": 29.8,
    "eps_trailing": 2.26,
    "eps_forward": 2.77,
    "eps_growth_ntm_pct": 22.6,
    "growth_eps": 22.6,
    "market_cap_b": 121.5,
    "ocf_ttm_m": 3250.0,
    "maint_capex_m": 450.0,
    "owner_earnings_m": 2800.0,
    "fcf_yield_pct": null,
    "score_fcf_yield": 5.75,
    "intrinsic_value": 103.13,
    "mos_pct": 20.0,
    "score_mos": 6.67,
    "peg_bruto": 7.5839,
    "score_peg": 7.5839,
    "value_score": 6.58,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "data_confidence": "Alta",
    "f1": 9.4,
    "f2": 9.2,
    "f3": 9.3,
    "f4": 9.6,
    "f4_moat": 9.6,
    "f5": 9.4,
    "f6": 9.5,
    "f7": 9.3,
    "f8": 9.5,
    "cqv_v4": 9.4,
    "cqv": 9.4,
    "clasificacion": "ÉLITE",
    "verdict": "Comprar / Acumular",
    "analyst_targets": {
      "target_low_bear": 72.0,
      "target_mean_base": 95.0,
      "target_high_bull": 108.0,
      "num_analysts": 32,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.2
    },
    "close_history": {
      "2020": 35.95,
      "2021": 42.48,
      "2022": 46.27,
      "2023": 57.81,
      "2024": 81.2,
      "2025": 77.8,
      "2026": 82.5
    },
    "owner_earnings": null
  },
  {
    "ticker": "CTAS",
    "name": "Cintas Corporation",
    "sector": "Industrials / Corporate Uniforms & Facility Services",
    "quarter": "Q2 2026",
    "f1": 9.35,
    "f2": 9.55,
    "f3": 9.1,
    "f4": 9.5,
    "f5": 9.3,
    "f6": 9.4,
    "f7": 8.8,
    "f8": 9.45,
    "momentum_score": 4.08,
    "cqv_v1": 8.2,
    "cqv_v1_1": 8.54,
    "cqv_v2": 8.16,
    "cqv_v3": 8.16,
    "cqv": 9.35,
    "pe": 44.2,
    "close_history": {
      "2020": 88.35,
      "2021": 110.6,
      "2022": 110.4,
      "2023": 150.6,
      "2024": 205.0,
      "2025": 185.0,
      "2026": 198.4
    },
    "status": "Success",
    "cqv_v4": 9.35,
    "price": 198.4,
    "pe_forward": 36.8,
    "value_score": 5.68,
    "peg_bruto": 5.4348,
    "score_peg": 5.4348,
    "intrinsic_value": 248.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.7,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 4.49,
    "eps_forward": 5.39,
    "eps_growth_ntm_pct": 20.0,
    "growth_eps": 20.0,
    "market_cap_b": 80.5,
    "ocf_ttm_m": 2000.0,
    "maint_capex_m": 350.0,
    "owner_earnings_m": 1650.0,
    "score_fcf_yield": 5.12,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 175.0,
      "target_mean_base": 230.0,
      "target_high_bull": 260.0,
      "num_analysts": 15,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.9
    }
  },
  {
    "ticker": "MEDP",
    "name": "Medpace Holdings, Inc.",
    "sector": "Healthcare / Contract Research Organization (CRO) & Clinical Trials",
    "quarter": "Q2 2026",
    "f1": 9.4,
    "f2": 9.6,
    "f3": 9.3,
    "f4": 9.2,
    "f5": 9.5,
    "f6": 9.4,
    "f7": 8.8,
    "f8": 9.2,
    "momentum_score": 6.44,
    "cqv_v1": 9.15,
    "cqv_v1_1": 9.19,
    "cqv_v2": 8.69,
    "cqv_v3": 8.69,
    "cqv": 9.35,
    "pe": 31.2,
    "close_history": {
      "2020": 140.5,
      "2021": 218.4,
      "2022": 212.42,
      "2023": 306.4,
      "2024": 390.0,
      "2025": 325.0,
      "2026": 345.0
    },
    "status": "Success",
    "cqv_v4": 9.35,
    "price": 345.0,
    "pe_forward": 24.5,
    "value_score": 9.0,
    "peg_bruto": 11.1429,
    "score_peg": 10.0,
    "intrinsic_value": 431.25,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 27.3,
    "growth_eps": 27.3,
    "ocf_ttm_m": 480.0,
    "maint_capex_m": 40.0,
    "owner_earnings_m": 440.0,
    "score_fcf_yield": 10.0,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 11.06,
    "eps_forward": 14.08,
    "market_cap_b": 10.8,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 300.0,
      "target_mean_base": 415.0,
      "target_high_bull": 480.0,
      "num_analysts": 12,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 20.3
    }
  },
  {
    "ticker": "PANW",
    "name": "Palo Alto Networks, Inc.",
    "sector": "Technology / Cybersecurity & Network Security Platforms",
    "quarter": "Q2 2026",
    "f1": 9.3,
    "f2": 9.2,
    "f3": 9.3,
    "f4": 9.6,
    "f5": 9.2,
    "f6": 9.4,
    "f7": 9.2,
    "f8": 9.4,
    "momentum_score": 9.86,
    "cqv_v1": 8.05,
    "cqv_v1_1": 8.72,
    "cqv_v2": 8.28,
    "cqv_v3": 8.28,
    "cqv": 9.34,
    "pe": 52.4,
    "close_history": {
      "2020": 118.46,
      "2021": 186.13,
      "2022": 139.54,
      "2023": 294.88,
      "2024": 378.1,
      "2025": 345.2,
      "2026": 328.5
    },
    "status": "Success",
    "cqv_v4": 9.34,
    "price": 328.5,
    "pe_forward": 42.8,
    "value_score": 6.32,
    "peg_bruto": 5.257,
    "score_peg": 5.257,
    "intrinsic_value": 410.63,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.6,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 22.5,
    "growth_eps": 22.5,
    "ocf_ttm_m": 3150.0,
    "maint_capex_m": 180.0,
    "owner_earnings_m": 2970.0,
    "score_fcf_yield": 6.85,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 6.27,
    "eps_forward": 7.68,
    "market_cap_b": 108.5,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 285.0,
      "target_mean_base": 385.0,
      "target_high_bull": 430.0,
      "num_analysts": 44,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 17.2
    }
  },
  {
    "ticker": "INTU",
    "name": "Intuit Inc.",
    "sector": "Technology / Enterprise Financial Software & Tax Services",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.2,
    "f3": 9.1,
    "f4": 9.6,
    "f5": 9.3,
    "f6": 9.4,
    "f7": 8.8,
    "f8": 9.4,
    "momentum_score": 0.12,
    "cqv_v1": 9.25,
    "cqv_v1_1": 9.26,
    "cqv_v2": 8.73,
    "cqv_v3": 8.73,
    "cqv": 9.34,
    "pe": 35.2,
    "close_history": {
      "2020": 379.85,
      "2021": 643.22,
      "2022": 389.2,
      "2023": 625.1,
      "2024": 675.4,
      "2025": 650.1,
      "2026": 645.2
    },
    "status": "Success",
    "cqv_v4": 9.34,
    "price": 645.2,
    "pe_forward": 27.8,
    "value_score": 7.93,
    "peg_bruto": 9.5683,
    "score_peg": 9.5683,
    "intrinsic_value": 806.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.6,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 18.33,
    "eps_forward": 23.21,
    "eps_growth_ntm_pct": 26.6,
    "growth_eps": 26.6,
    "market_cap_b": 180.6,
    "ocf_ttm_m": 5850.0,
    "maint_capex_m": 320.0,
    "owner_earnings_m": 5530.0,
    "score_fcf_yield": 7.65,
    "score_mos": 6.67,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 560.0,
      "target_mean_base": 745.0,
      "target_high_bull": 830.0,
      "num_analysts": 32,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.5
    }
  },
  {
    "ticker": "UNH",
    "name": "UnitedHealth Group Incorporated",
    "sector": "Healthcare / Managed Care, Health Insurance & Optum Healthcare Services",
    "quarter": "Q2 2026",
    "f1": 9.32,
    "f2": 9.4,
    "f3": 9.12,
    "f4": 9.6,
    "f5": 9.3,
    "f6": 9.4,
    "f7": 8.8,
    "f8": 9.5,
    "momentum_score": 9.25,
    "cqv_v1": 8.07,
    "cqv_v1_1": 8.12,
    "cqv_v2": 7.82,
    "cqv_v3": 7.82,
    "cqv": 9.34,
    "pe": 19.95,
    "close_history": {
      "2020": 350.68,
      "2021": 502.14,
      "2022": 530.18,
      "2023": 526.47,
      "2024": 520.0,
      "2025": 465.0,
      "2026": 505.0
    },
    "status": "Success",
    "cqv_v4": 9.34,
    "price": 505.0,
    "pe_forward": 16.55,
    "value_score": 9.0,
    "peg_bruto": 12.4471,
    "score_peg": 10.0,
    "intrinsic_value": 631.25,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.6,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "16/07/2026",
    "eps_trailing": 25.29,
    "eps_forward": 30.5,
    "eps_growth_ntm_pct": 20.6,
    "growth_eps": 20.6,
    "market_cap_b": 465.0,
    "ocf_ttm_m": 32000.0,
    "maint_capex_m": 3600.0,
    "owner_earnings_m": 28400.0,
    "score_fcf_yield": 10.0,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 440.0,
      "target_mean_base": 600.0,
      "target_high_bull": 665.0,
      "num_analysts": 27,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 18.8
    }
  },
  {
    "ticker": "AZO",
    "name": "AutoZone, Inc.",
    "sector": "Consumer Discretionary / Specialty Retail – Automotive Parts & Accessories",
    "quarter": "Q1 2026",
    "f1": 9.4,
    "f2": 8.8,
    "f3": 9.1,
    "f4": 9.7,
    "f5": 9.8,
    "f6": 9.5,
    "f7": 8.5,
    "f8": 9.6,
    "momentum_score": 2.28,
    "cqv_v1": 8.35,
    "cqv_v1_1": 8.18,
    "cqv_v2": 7.83,
    "cqv_v3": 7.83,
    "cqv": 9.34,
    "pe": 20.1,
    "close_history": {
      "2020": 1205.0,
      "2021": 2096.39,
      "2022": 2466.18,
      "2023": 2585.61,
      "2024": 3202.0,
      "2025": 3391.5,
      "2026": 3018.0
    },
    "status": "Success",
    "cqv_v4": 9.34,
    "price": 3018.0,
    "pe_forward": 17.5,
    "value_score": 8.88,
    "peg_bruto": 9.6,
    "score_peg": 9.6,
    "intrinsic_value": 3772.5,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.7,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "01/08/2026",
    "eps_trailing": 150.15,
    "eps_forward": 175.62,
    "eps_growth_ntm_pct": 16.8,
    "growth_eps": 16.8,
    "market_cap_b": 49.2,
    "ocf_ttm_m": 3070.0,
    "maint_capex_m": 720.0,
    "owner_earnings_m": 2350.0,
    "score_fcf_yield": 10.0,
    "score_mos": 6.67,
    "wacc": 8.5,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 3200.0,
      "target_mean_base": 4000.0,
      "target_high_bull": 4800.0,
      "num_analysts": 27,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 32.5
    }
  },
  {
    "ticker": "ETN",
    "name": "Eaton Corporation plc",
    "sector": "Industrials / Electrical Equipment, Power Management & Aerospace",
    "quarter": "Q2 2026",
    "f1": 9.2,
    "f2": 9.4,
    "f3": 9.3,
    "f4": 9.5,
    "f5": 9.2,
    "f6": 9.4,
    "f7": 9.2,
    "f8": 9.4,
    "momentum_score": 6.67,
    "cqv_v1": 7.99,
    "cqv_v1_1": 8.36,
    "cqv_v2": 8.06,
    "cqv_v3": 8.06,
    "cqv": 9.33,
    "pe": 32.5,
    "close_history": {
      "2020": 120.14,
      "2021": 172.82,
      "2022": 157.0,
      "2023": 240.82,
      "2024": 340.5,
      "2025": 295.0,
      "2026": 315.0
    },
    "status": "Success",
    "cqv_v4": 9.33,
    "price": 315.0,
    "pe_forward": 25.2,
    "value_score": 7.86,
    "peg_bruto": 11.5079,
    "score_peg": 10.0,
    "intrinsic_value": 393.75,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.5,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 9.69,
    "eps_forward": 12.5,
    "eps_growth_ntm_pct": 29.0,
    "growth_eps": 29.0,
    "market_cap_b": 126.0,
    "ocf_ttm_m": 4200.0,
    "maint_capex_m": 600.0,
    "owner_earnings_m": 3600.0,
    "score_fcf_yield": 7.15,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 280.0,
      "target_mean_base": 365.0,
      "target_high_bull": 415.0,
      "num_analysts": 22,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.9
    }
  },
  {
    "ticker": "MPWR",
    "name": "Monolithic Power Systems, Inc.",
    "sector": "Technology / Power Management Integrated Circuits & AI Power",
    "quarter": "Q2 2026",
    "f1": 9.3,
    "f2": 9.6,
    "f3": 9.4,
    "f4": 9.2,
    "f5": 9.1,
    "f6": 9.3,
    "f7": 9.2,
    "f8": 9.1,
    "momentum_score": 5.43,
    "cqv_v1": 8.12,
    "cqv_v1_1": 8.81,
    "cqv_v2": 8.33,
    "cqv_v3": 8.33,
    "cqv": 9.3,
    "pe": 64.5,
    "close_history": {
      "2020": 366.23,
      "2021": 493.33,
      "2022": 353.61,
      "2023": 630.78,
      "2024": 890.0,
      "2025": 760.0,
      "2026": 820.0
    },
    "status": "Success",
    "cqv_v4": 9.3,
    "price": 820.0,
    "pe_forward": 46.8,
    "value_score": 5.88,
    "peg_bruto": 8.0769,
    "score_peg": 8.0769,
    "intrinsic_value": 1025.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 12.71,
    "eps_forward": 17.52,
    "eps_growth_ntm_pct": 37.8,
    "growth_eps": 37.8,
    "market_cap_b": 39.8,
    "ocf_ttm_m": 700.0,
    "maint_capex_m": 120.0,
    "owner_earnings_m": 580.0,
    "score_fcf_yield": 3.65,
    "score_mos": 6.67,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 720.0,
      "target_mean_base": 950.0,
      "target_high_bull": 1080.0,
      "num_analysts": 16,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.9
    }
  },
  {
    "ticker": "ORLY",
    "name": "O'Reilly Automotive, Inc.",
    "sector": "Consumer Discretionary / Automotive Aftermarket Parts Retail",
    "quarter": "Q2 2026",
    "f1": 9.3,
    "f2": 9.4,
    "f3": 8.9,
    "f4": 9.5,
    "f5": 9.5,
    "f6": 9.35,
    "f7": 8.4,
    "f8": 9.45,
    "momentum_score": 3.51,
    "cqv_v1": 9.2,
    "cqv_v1_1": 9.2,
    "cqv_v2": 9.2,
    "cqv_v3": 9.2,
    "cqv": 9.28,
    "pe": 27.5,
    "close_history": {
      "2020": 452.57,
      "2021": 706.23,
      "2022": 844.03,
      "2023": 953.0,
      "2024": 1180.0,
      "2025": 1050.0,
      "2026": 1120.0
    },
    "status": "Success",
    "cqv_v4": 9.28,
    "price": 1120.0,
    "pe_forward": 23.5,
    "value_score": 7.91,
    "peg_bruto": 7.234,
    "score_peg": 7.234,
    "intrinsic_value": 1400.0,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.4,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 17.0,
    "growth_eps": 17.0,
    "ocf_ttm_m": 3100.0,
    "maint_capex_m": 650.0,
    "owner_earnings_m": 2450.0,
    "score_fcf_yield": 9.35,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 40.73,
    "eps_forward": 47.66,
    "market_cap_b": 65.5,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 1000.0,
      "target_mean_base": 1280.0,
      "target_high_bull": 1420.0,
      "num_analysts": 22,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.3
    }
  },
  {
    "ticker": "KO",
    "name": "The Coca-Cola Company",
    "sector": "Consumer Staples / Non-Alcoholic Beverages",
    "quarter": "Q2 2026",
    "f1": 9.2,
    "f2": 9.5,
    "f3": 8.7,
    "f4": 9.8,
    "f5": 9.2,
    "f6": 9.3,
    "f7": 8.2,
    "f8": 9.5,
    "momentum_score": 8.24,
    "cqv_v1": 7.52,
    "cqv_v1_1": 8.03,
    "cqv_v2": 7.83,
    "cqv_v3": 7.83,
    "cqv": 9.25,
    "pe": 25.5,
    "close_history": {
      "2020": 54.84,
      "2021": 59.21,
      "2022": 63.61,
      "2023": 58.93,
      "2024": 68.0,
      "2025": 65.0,
      "2026": 68.5
    },
    "status": "Success",
    "cqv_v4": 9.25,
    "price": 68.5,
    "pe_forward": 22.8,
    "value_score": 6.83,
    "peg_bruto": 5.0439,
    "score_peg": 5.0439,
    "intrinsic_value": 85.63,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 11.5,
    "growth_eps": 11.5,
    "ocf_ttm_m": 11600.0,
    "maint_capex_m": 1800.0,
    "owner_earnings_m": 9800.0,
    "score_fcf_yield": 8.3,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 2.69,
    "eps_forward": 3.0,
    "market_cap_b": 295.0,
    "wacc": 7.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 60.0,
      "target_mean_base": 78.0,
      "target_high_bull": 88.0,
      "num_analysts": 22,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 13.9
    }
  },
  {
    "ticker": "KNSL",
    "name": "Kinsale Capital Group, Inc.",
    "sector": "Financial Services",
    "quarter": "Q2 2026",
    "f1": 9.5,
    "f2": 9.4,
    "f3": 9.5,
    "f4": 9.4,
    "f5": 9.2,
    "f6": 9.2,
    "f7": 7.5,
    "f8": 8.8,
    "momentum_score": 4.29,
    "cqv_v1": 9.49,
    "cqv_v1_1": 9.38,
    "cqv_v2": 8.88,
    "cqv_v3": 8.88,
    "cqv": 9.24,
    "pe": 15.036728,
    "close_history": "{'2021': 235.96, '2022': 259.95, '2023': 333.45, '2024': 463.7, '2025': 390.54, '2026': 335.52}",
    "status": "Success",
    "cqv_v4": 9.24,
    "price": 376.67,
    "pe_forward": 17.400797,
    "value_score": 6.72,
    "peg_bruto": 10.3444,
    "score_peg": 10.0,
    "intrinsic_value": 470.84,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.4,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 1695.015,
    "maint_capex_m": 75.334,
    "owner_earnings_m": 1619.7,
    "score_fcf_yield": 4.3,
    "score_mos": 6.67
  },
  {
    "ticker": "ICE",
    "name": "Intercontinental Exchange, Inc.",
    "sector": "Financials / Financial Exchanges, Data & Mortgage Tech",
    "quarter": "Q2 2026",
    "f1": 9.25,
    "f2": 9.35,
    "f3": 9.0,
    "f4": 9.4,
    "f5": 9.1,
    "f6": 9.2,
    "f7": 8.7,
    "f8": 9.2,
    "momentum_score": 1.92,
    "cqv_v1": 8.48,
    "cqv_v1_1": 8.37,
    "cqv_v2": 8.08,
    "cqv_v3": 8.08,
    "cqv": 9.2,
    "pe": 28.5,
    "close_history": {
      "2020": 115.24,
      "2021": 136.77,
      "2022": 102.59,
      "2023": 128.43,
      "2024": 155.0,
      "2025": 140.0,
      "2026": 148.5
    },
    "status": "Success",
    "cqv_v4": 9.2,
    "price": 148.5,
    "pe_forward": 21.5,
    "value_score": 9.0,
    "peg_bruto": 13.1163,
    "score_peg": 10.0,
    "intrinsic_value": 185.63,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 5.21,
    "eps_forward": 6.91,
    "eps_growth_ntm_pct": 28.2,
    "growth_eps": 28.2,
    "market_cap_b": 85.0,
    "ocf_ttm_m": 4100.0,
    "maint_capex_m": 450.0,
    "owner_earnings_m": 3650.0,
    "score_fcf_yield": 10.0,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 130.0,
      "target_mean_base": 172.0,
      "target_high_bull": 195.0,
      "num_analysts": 18,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.8
    }
  },
  {
    "ticker": "ORCL",
    "name": "Oracle Corporation",
    "sector": "Technology / Cloud Infrastructure, Databases & Enterprise Software",
    "quarter": "Q2 2026",
    "f1": 9.1,
    "f2": 8.8,
    "f3": 9.3,
    "f4": 9.5,
    "f5": 8.9,
    "f6": 9.3,
    "f7": 9.4,
    "f8": 9.3,
    "momentum_score": 0.42,
    "cqv_v1": 7.05,
    "cqv_v1_1": 7.74,
    "cqv_v2": 7.61,
    "cqv_v3": 7.61,
    "cqv": 9.18,
    "pe": 38.5,
    "close_history": {
      "2020": 64.69,
      "2021": 81.81,
      "2022": 78.01,
      "2023": 102.15,
      "2024": 163.42,
      "2025": 193.05,
      "2026": 172.5
    },
    "status": "Success",
    "cqv_v4": 9.18,
    "price": 172.5,
    "pe_forward": 28.4,
    "value_score": 7.65,
    "peg_bruto": 12.5,
    "score_peg": 10.0,
    "intrinsic_value": 215.63,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.5,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 35.5,
    "growth_eps": 35.5,
    "ocf_ttm_m": 19500.0,
    "maint_capex_m": 6800.0,
    "owner_earnings_m": 12700.0,
    "score_fcf_yield": 6.63,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 4.48,
    "eps_forward": 6.07,
    "market_cap_b": 480.0,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 150.0,
      "target_mean_base": 200.0,
      "target_high_bull": 230.0,
      "num_analysts": 35,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.9
    }
  },
  {
    "ticker": "PGR",
    "name": "The Progressive Corporation",
    "sector": "Financials / Property & Casualty Insurance",
    "quarter": "Q2 2026",
    "f1": 9.3,
    "f2": 9.4,
    "f3": 9.0,
    "f4": 9.2,
    "f5": 9.1,
    "f6": 9.25,
    "f7": 8.5,
    "f8": 9.1,
    "momentum_score": 6.4,
    "cqv_v1": 8.67,
    "cqv_v1_1": 8.52,
    "cqv_v2": 8.14,
    "cqv_v3": 8.14,
    "cqv": 9.17,
    "pe": 18.5,
    "close_history": {
      "2020": 98.88,
      "2021": 102.65,
      "2022": 129.71,
      "2023": 159.28,
      "2024": 235.0,
      "2025": 210.0,
      "2026": 219.99
    },
    "status": "Success",
    "cqv_v4": 9.17,
    "price": 219.99,
    "pe_forward": 14.8,
    "value_score": 9.0,
    "peg_bruto": 16.8919,
    "score_peg": 10.0,
    "intrinsic_value": 274.99,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 25.0,
    "growth_eps": 25.0,
    "ocf_ttm_m": 9000.0,
    "maint_capex_m": 800.0,
    "owner_earnings_m": 8200.0,
    "score_fcf_yield": 10.0,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 11.89,
    "eps_forward": 14.86,
    "market_cap_b": 128.8,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 190.0,
      "target_mean_base": 255.0,
      "target_high_bull": 290.0,
      "num_analysts": 20,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.9
    }
  },
  {
    "ticker": "SPGI",
    "name": "S&P Global Inc.",
    "sector": "Financial Services",
    "quarter": "Q2 2026",
    "f1": 9.4,
    "f2": 9.3,
    "f3": 9.0,
    "f4": 9.5,
    "f5": 9.1,
    "f6": 9.1,
    "f7": 7.5,
    "f8": 9.0,
    "momentum_score": 4.74,
    "cqv_v1": 9.35,
    "cqv_v1_1": 9.38,
    "cqv_v2": 8.87,
    "cqv_v3": 8.87,
    "cqv": 9.14,
    "pe": 26.509161,
    "close_history": "{'2021': 429.65, '2022': 307.7, '2023': 408.61, '2024': 465.55, '2025': 492.12, '2026': 457.38}",
    "status": "Success",
    "cqv_v4": 9.14,
    "price": 419.64,
    "pe_forward": 20.76957,
    "value_score": 6.32,
    "peg_bruto": 8.6665,
    "score_peg": 8.6665,
    "intrinsic_value": 524.55,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.5,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 1888.3799999999999,
    "maint_capex_m": 83.928,
    "owner_earnings_m": 1804.5,
    "score_fcf_yield": 4.3,
    "score_mos": 6.67
  },
  {
    "ticker": "AAPL",
    "name": "Apple Inc.",
    "sector": "Technology / Consumer Electronics, Operating Systems & Ecosystem Services",
    "quarter": "Q2 2026",
    "f1": 9.2,
    "f2": 9.5,
    "f3": 8.7,
    "f4": 9.5,
    "f5": 9.0,
    "f6": 9.0,
    "f7": 8.0,
    "f8": 9.3,
    "momentum_score": 8.13,
    "cqv_v1": 9.03,
    "cqv_v1_1": 9.54,
    "cqv_v2": 8.97,
    "cqv_v3": 8.97,
    "cqv": 9.12,
    "pe": null,
    "close_history": {
      "2020": 132.69,
      "2021": 177.57,
      "2022": 129.93,
      "2023": 192.53,
      "2024": 230.5,
      "2025": 210.4,
      "2026": 271.35
    },
    "status": "Success",
    "cqv_v4": 9.12,
    "price": 271.35,
    "pe_forward": null,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": null,
    "mos_pct": null,
    "verdict": "N/D - valoración incompleta",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.5,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "30/04/2026",
    "eps_trailing": null,
    "eps_forward": null,
    "eps_growth_ntm_pct": null,
    "growth_eps": null,
    "market_cap_b": 3983.0,
    "ocf_ttm_m": 140222.0,
    "maint_capex_m": null,
    "owner_earnings_m": null,
    "score_fcf_yield": null,
    "score_mos": null,
    "wacc": null,
    "g_terminal": null,
    "analyst_targets": {
      "target_low_bear": 195.0,
      "target_mean_base": 255.0,
      "target_high_bull": 300.0,
      "num_analysts": 44,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 13.3
    }
  },
  {
    "ticker": "VRT",
    "name": "Vertiv Holdings Co",
    "sector": "Industrials / Data Center Infrastructure, Power & Liquid Cooling",
    "quarter": "Q2 2026",
    "f1": 8.8,
    "f2": 8.9,
    "f3": 9.6,
    "f4": 9.1,
    "f5": 8.8,
    "f6": 9.3,
    "f7": 9.5,
    "f8": 9.0,
    "momentum_score": 7.21,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.63,
    "cqv_v2": 8.25,
    "cqv_v3": 8.25,
    "cqv": 9.09,
    "pe": 42.5,
    "close_history": {
      "2020": 18.5,
      "2021": 24.9,
      "2022": 13.68,
      "2023": 48.03,
      "2024": 86.4,
      "2025": 78.5,
      "2026": 88.5
    },
    "status": "Success",
    "cqv_v4": 9.09,
    "price": 88.5,
    "pe_forward": 31.2,
    "value_score": 7.99,
    "peg_bruto": 11.6987,
    "score_peg": 10.0,
    "intrinsic_value": 110.63,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.1,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 2.08,
    "eps_forward": 2.84,
    "eps_growth_ntm_pct": 36.5,
    "growth_eps": 36.5,
    "market_cap_b": 33.5,
    "ocf_ttm_m": 1150.0,
    "maint_capex_m": 150.0,
    "owner_earnings_m": 1000.0,
    "score_fcf_yield": 7.48,
    "score_mos": 6.67,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 75.0,
      "target_mean_base": 102.0,
      "target_high_bull": 125.0,
      "num_analysts": 18,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.3
    }
  },
  {
    "ticker": "AJG",
    "name": "Arthur J. Gallagher & Co.",
    "sector": "Financials / Insurance Brokerage & Risk Management",
    "quarter": "Q2 2026",
    "f1": 9.1,
    "f2": 9.3,
    "f3": 8.9,
    "f4": 9.2,
    "f5": 9.0,
    "f6": 9.1,
    "f7": 8.5,
    "f8": 9.1,
    "momentum_score": 5.42,
    "cqv_v1": 7.65,
    "cqv_v1_1": 7.97,
    "cqv_v2": 7.8,
    "cqv_v3": 7.8,
    "cqv": 9.07,
    "pe": 34.5,
    "close_history": {
      "2020": 123.7,
      "2021": 169.67,
      "2022": 188.54,
      "2023": 224.88,
      "2024": 260.0,
      "2025": 240.0,
      "2026": 255.0
    },
    "status": "Success",
    "cqv_v4": 9.07,
    "price": 255.0,
    "pe_forward": 25.8,
    "value_score": 7.48,
    "peg_bruto": 11.0465,
    "score_peg": 10.0,
    "intrinsic_value": 318.75,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 7.39,
    "eps_forward": 9.88,
    "eps_growth_ntm_pct": 28.5,
    "growth_eps": 28.5,
    "market_cap_b": 58.5,
    "ocf_ttm_m": 1650.0,
    "maint_capex_m": 200.0,
    "owner_earnings_m": 1450.0,
    "score_fcf_yield": 6.2,
    "score_mos": 6.67,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 220.0,
      "target_mean_base": 295.0,
      "target_high_bull": 330.0,
      "num_analysts": 16,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 15.7
    }
  },
  {
    "ticker": "PWR",
    "name": "Quanta Services, Inc.",
    "sector": "Industrials / Electric Power Grid Infrastructure & Clean Energy",
    "quarter": "Q2 2026",
    "f1": 8.8,
    "f2": 8.9,
    "f3": 9.5,
    "f4": 9.1,
    "f5": 8.9,
    "f6": 9.2,
    "f7": 9.3,
    "f8": 9.0,
    "momentum_score": 7.15,
    "cqv_v1": 7.59,
    "cqv_v1_1": 8.06,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 9.06,
    "pe": 38.5,
    "close_history": {
      "2020": 72.0,
      "2021": 114.66,
      "2022": 142.5,
      "2023": 215.8,
      "2024": 295.0,
      "2025": 255.0,
      "2026": 275.0
    },
    "status": "Success",
    "cqv_v4": 9.06,
    "price": 275.0,
    "pe_forward": 27.5,
    "value_score": 8.46,
    "peg_bruto": 14.5455,
    "score_peg": 10.0,
    "intrinsic_value": 343.75,
    "mos_pct": 20.0,
    "verdict": "Comprar / Acumular",
    "clasificacion": "ÉLITE",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 7.14,
    "eps_forward": 10.0,
    "eps_growth_ntm_pct": 40.0,
    "growth_eps": 40.0,
    "market_cap_b": 40.5,
    "ocf_ttm_m": 1800.0,
    "maint_capex_m": 400.0,
    "owner_earnings_m": 1400.0,
    "score_fcf_yield": 8.65,
    "score_mos": 6.67,
    "wacc": 8.5,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 240.0,
      "target_mean_base": 320.0,
      "target_high_bull": 365.0,
      "num_analysts": 18,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 16.4
    }
  },
  {
    "ticker": "ANET",
    "name": "Arista Networks, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 9.1,
    "f5": 9.6,
    "f6": 9.41,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 7.25,
    "cqv_v1": 8.88,
    "cqv_v1_1": 9.44,
    "cqv_v2": 8.75,
    "cqv_v3": 8.75,
    "cqv": 9.02,
    "pe": 54.98,
    "close_history": "{'2021': 35.94, '2022': 30.34, '2023': 58.88, '2024': 110.53, '2025': 131.03, '2026': 168.56}",
    "status": "Success",
    "cqv_v4": 9.02,
    "price": 100.0,
    "pe_forward": 46.733,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 140.8,
    "mos_pct": 28.98,
    "verdict": "Comprar / Candidato Prioritario",
    "clasificacion": "ÉLITE",
    "f4_moat": 9.1,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BKNG",
    "name": "Booking Holdings Inc. Common St",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.97,
    "f2": 8.75,
    "f3": 9.15,
    "f4": 9.4,
    "f5": 9.0,
    "f6": 9.56,
    "f7": 4.02,
    "f8": 8.0,
    "momentum_score": 4.44,
    "cqv_v1": 9.14,
    "cqv_v1_1": 9.35,
    "cqv_v2": 8.72,
    "cqv_v3": 8.72,
    "cqv": 8.95,
    "pe": 24.32,
    "close_history": "{'2021': 93.97, '2022': 78.93, '2023': 138.94, '2024': 196.33, '2025': 213.19, '2026': 184.61}",
    "status": "Success",
    "cqv_v4": 8.95,
    "price": 100.0,
    "pe_forward": 20.672,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 138.0,
    "mos_pct": 27.54,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.4,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CDNS",
    "name": "Cadence Design Systems, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 9.0,
    "f2": 8.2,
    "f3": 9.3,
    "f4": 9.4,
    "f5": 8.7,
    "f6": 9.0,
    "f7": 9.2,
    "f8": 8.8,
    "price": 338.61,
    "valuation_date": "2026-07-27",
    "pe": 79.3,
    "pe_forward": 41.8037,
    "eps_growth_ntm_pct": 13.4454,
    "market_cap": 93394.0,
    "ocf": 635.0,
    "fcf": 582.0,
    "maintenance_capex": null,
    "intrinsic_value": null,
    "score_fcf_yield": null,
    "score_mos": null,
    "data_confidence": "Media-Alta",
    "price_source": "Yahoo Finance, close 2026-07-27, consulted 2026-08-09",
    "market_source": "Yahoo Finance / StockAnalysis, consulted 2026-08-09",
    "financial_source": "Cadence Q2 2026 earnings release and SEC filing, 2026-07-27",
    "close_history": "{'2021': 339.11, '2022': 159.42, '2023': 272.85, '2024': 297.67, '2025': 346.76, '2026': 338.61}",
    "status": "Success",
    "cqv_v4": 8.95,
    "cqv": 8.95,
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "peg_bruto": 3.2163,
    "score_peg": 3.2163,
    "mos_pct": null,
    "value_score": null,
    "verdict": "N/D - valoración incompleta",
    "clasificacion": "ALTA CALIDAD"
  },
  {
    "ticker": "ADBE",
    "name": "Adobe Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.25,
    "f3": 8.58,
    "f4": 9.4,
    "f5": 9.3,
    "f6": 9.29,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 1.42,
    "cqv_v1": 9.32,
    "cqv_v1_1": 9.38,
    "cqv_v2": 8.72,
    "cqv_v3": 8.72,
    "cqv": 8.94,
    "pe": 12.56,
    "close_history": "{'2021': 567.06, '2022': 336.53, '2023': 596.6, '2024': 444.68, '2025': 349.99, '2026': 235.31}",
    "status": "Success",
    "cqv_v4": 8.94,
    "price": 100.0,
    "pe_forward": 10.676,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 137.6,
    "mos_pct": 27.33,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.4,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FDS",
    "name": "FactSet Research Systems",
    "sector": "Financials / Financial Data & Analytical Workstations",
    "quarter": "Q2 2026",
    "f1": 9.15,
    "f2": 9.25,
    "f3": 8.65,
    "f4": 8.85,
    "f5": 8.9,
    "f6": 9.05,
    "f7": 7.8,
    "f8": 9.0,
    "momentum_score": 4.1,
    "cqv_v1": 8.83,
    "cqv_v1_1": 8.67,
    "cqv_v2": 8.23,
    "cqv_v3": 8.23,
    "cqv": 8.93,
    "pe": 27.5,
    "close_history": {
      "2020": 331.8,
      "2021": 486.0,
      "2022": 401.2,
      "2023": 477.0,
      "2024": 465.0,
      "2025": 420.0,
      "2026": 440.0
    },
    "status": "Success",
    "cqv_v4": 8.93,
    "price": 440.0,
    "pe_forward": 24.2,
    "value_score": 7.4,
    "peg_bruto": 5.6198,
    "score_peg": 5.6198,
    "intrinsic_value": 550.0,
    "mos_pct": 20.0,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 13.6,
    "growth_eps": 13.6,
    "ocf_ttm_m": 700.0,
    "maint_capex_m": 80.0,
    "owner_earnings_m": 620.0,
    "score_fcf_yield": 9.28,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 16.0,
    "eps_forward": 18.18,
    "market_cap_b": 16.7,
    "wacc": 8.0,
    "g_terminal": 3.5,
    "analyst_targets": {
      "target_low_bear": 390.0,
      "target_mean_base": 505.0,
      "target_high_bull": 560.0,
      "num_analysts": 14,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 14.8
    }
  },
  {
    "ticker": "NFLX",
    "name": "Netflix, Inc.",
    "sector": "Communication Services / Entertainment & Streaming Media",
    "quarter": "Q2 2026",
    "period_end": "2026-06-30",
    "f1": 9.2,
    "f2": 9.0,
    "f3": 8.5,
    "f4": 9.3,
    "f5": 9.0,
    "f6": 8.7,
    "f7": 8.8,
    "f8": 8.5,
    "momentum_score": null,
    "price": 74.35,
    "price_basis": "Cierre NASDAQ del 16/07/2026, post-split 10:1",
    "valuation_date": "16/07/2026",
    "close_history": {
      "2020": 54.073,
      "2021": 60.244,
      "2022": 29.495,
      "2023": 48.688,
      "2024": 64.25,
      "2025": 61.02,
      "2026": 74.35
    },
    "pe": 23.38,
    "pe_forward": 21.18,
    "eps_trailing": 3.18,
    "eps_forward": 3.51,
    "eps_growth_ntm_pct": null,
    "growth_eps": null,
    "peg_bruto": null,
    "score_peg": null,
    "market_cap": 309588.9149,
    "market_cap_b": 309.59,
    "diluted_shares_m": 4261.3,
    "shares_outstanding_m": 4163.939676,
    "revenue_q2_m": 12559.938,
    "revenue_growth_yoy_pct": 13.4,
    "revenue_growth_fx_neutral_pct": 12.0,
    "operating_income_q2_m": 4192.61,
    "operating_margin_q2_pct": 33.4,
    "net_income_q2_m": 3401.414,
    "diluted_eps_q2": 0.8,
    "ocf": 1743.812,
    "ocf_period": "Q2 2026; USD millones",
    "capex_total": 218.644,
    "capex_period": "Q2 2026; compras de PP&E, USD millones",
    "free_cash_flow_reported": 1525.0,
    "free_cash_flow_period": "Q2 2026; definición Netflix: OCF menos compras de PP&E, USD millones",
    "sbc_q2_m": 131.312,
    "repurchases_q2_m": 4714.403,
    "repurchases_ytd_m": 5984.991,
    "repurchase_authorization_remaining_m": 27100.0,
    "debt_m": 14309.306,
    "cash_restricted_cash_investments_m": 9131.464,
    "net_debt_m": 5177.842,
    "content_assets_net_m": 33837.573,
    "ppe_net_m": 2398.848,
    "content_obligations_m": 25106.705,
    "content_obligations_next12m_m": 11939.734,
    "q3_revenue_guidance_m": 12860.0,
    "q3_revenue_growth_guidance_pct": 11.7,
    "q3_operating_income_guidance_m": 4268.0,
    "q3_operating_margin_guidance_pct": 33.2,
    "q3_diluted_eps_guidance": 0.82,
    "fy2026_fcf_guidance_m": 12500.0,
    "fy2026_ads_revenue_guidance_m": 3000.0,
    "h1_view_hours_b": 97.0,
    "h1_view_hours_growth_pct": 2.0,
    "maintenance_capex": null,
    "maint_capex_m": null,
    "owner_earnings": null,
    "owner_earnings_m": null,
    "fcf_yield_pct": null,
    "score_fcf_yield": null,
    "intrinsic_value": null,
    "mos_pct": null,
    "score_mos": null,
    "value_score": null,
    "verdict": "N/D - valoración incompleta",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.3,
    "data_confidence": "Media-Alta",
    "status": "Q2 2026 actualizado con datos oficiales; DCF y Value Score N/D por Maintenance CapEx no desglosado",
    "wacc": null,
    "g_terminal": null,
    "analyst_targets": null,
    "sources": [
      "https://www.sec.gov/Archives/edgar/data/1065280/000106528026000212/nflx-20260630.htm",
      "https://www.sec.gov/Archives/edgar/data/1065280/000106528026000211/ex991_q226.htm",
      "https://s22.q4cdn.com/959853165/files/doc_financials/2026/q2/FINAL-Q2-26-Shareholder-Letter.pdf",
      "https://chartexchange.com/symbol/nasdaq-nflx/historical/"
    ],
    "cqv_v4": 8.92,
    "cqv": 8.92
  },
  {
    "ticker": "MRSH",
    "name": "MMC",
    "sector": 0.0,
    "quarter": "Q1 2026",
    "f1": 9.1,
    "f2": 9.0,
    "f3": 8.9,
    "f4": 9.1,
    "f5": 8.8,
    "f6": 8.8,
    "f7": 7.0,
    "f8": 8.8,
    "momentum_score": 5.0,
    "cqv_v1": 6.71,
    "cqv_v1_1": 6.43,
    "cqv_v2": 6.58,
    "cqv_v3": 6.58,
    "cqv": 8.86,
    "pe": 23.466745,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.86,
    "price": 197.59,
    "pe_forward": 17.320711,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 233.16,
    "mos_pct": 15.26,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.1,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MU",
    "name": "Micron Technology, Inc.",
    "sector": "Technology / Semiconductor Memory & Storage (DRAM / HBM / NAND)",
    "quarter": "Q2 2026",
    "f1": 8.8,
    "f2": 9.0,
    "f3": 9.1,
    "f4": 8.8,
    "f5": 8.7,
    "f6": 9.0,
    "f7": 9.2,
    "f8": 7.8,
    "momentum_score": 9.23,
    "cqv_v1": 8.01,
    "cqv_v1_1": 8.79,
    "cqv_v2": 8.33,
    "cqv_v3": 8.33,
    "cqv": 8.8,
    "pe": 18.5,
    "close_history": {
      "2020": 75.18,
      "2021": 93.15,
      "2022": 49.98,
      "2023": 85.34,
      "2024": 122.5,
      "2025": 98.4,
      "2026": 118.5
    },
    "status": "Success",
    "cqv_v4": 8.8,
    "price": 118.5,
    "pe_forward": 11.2,
    "value_score": 8.54,
    "peg_bruto": 58.0357,
    "score_peg": 10.0,
    "intrinsic_value": 148.13,
    "mos_pct": 20.0,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 65.0,
    "growth_eps": 65.0,
    "ocf_ttm_m": 7850.0,
    "maint_capex_m": 3200.0,
    "owner_earnings_m": 4650.0,
    "score_fcf_yield": 8.85,
    "score_mos": 6.67,
    "valuation_date": "29/07/2026",
    "eps_trailing": 6.41,
    "eps_forward": 10.58,
    "market_cap_b": 131.5,
    "wacc": 9.5,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 100.0,
      "target_mean_base": 145.0,
      "target_high_bull": 175.0,
      "num_analysts": 34,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 22.4
    }
  },
  {
    "ticker": "RMD",
    "name": "ResMed Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.25,
    "f3": 8.87,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.43,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 3.09,
    "cqv_v1": 8.92,
    "cqv_v1_1": 9.01,
    "cqv_v2": 8.48,
    "cqv_v3": 8.48,
    "cqv": 8.77,
    "pe": 20.2,
    "close_history": "{'2021': 250.03, '2022': 201.35, '2023': 168.03, '2024': 225.48, '2025': 239.6, '2026': 202.76}",
    "status": "Success",
    "cqv_v4": 8.77,
    "price": 100.0,
    "pe_forward": 17.169999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 130.8,
    "mos_pct": 23.55,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DXCM",
    "name": "DexCom, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.71,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.52,
    "f7": 4.23,
    "f8": 8.0,
    "momentum_score": 5.71,
    "cqv_v1": 8.77,
    "cqv_v1_1": 8.97,
    "cqv_v2": 8.47,
    "cqv_v3": 8.47,
    "cqv": 8.76,
    "pe": 30.58,
    "close_history": "{'2021': 134.24, '2022': 113.24, '2023': 124.09, '2024': 77.77, '2025': 66.37, '2026': 77.98}",
    "status": "Success",
    "cqv_v4": 8.76,
    "price": 100.0,
    "pe_forward": 25.993,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 130.4,
    "mos_pct": 23.31,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IBKR",
    "name": "Interactive Brokers Group, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.66,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 8.54,
    "cqv_v1": 8.87,
    "cqv_v1_1": 8.89,
    "cqv_v2": 8.42,
    "cqv_v3": 8.42,
    "cqv": 8.75,
    "pe": 39.2,
    "close_history": "{'2021': 19.38, '2022': 17.76, '2023': 20.45, '2024': 43.84, '2025': 64.18, '2026': 92.21}",
    "status": "Success",
    "cqv_v4": 8.75,
    "price": 100.0,
    "pe_forward": 33.32,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 130.0,
    "mos_pct": 23.08,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NICE",
    "name": "NICE Ltd",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.1,
    "f2": 9.4,
    "f3": 8.5,
    "f4": 9.3,
    "f5": 9.2,
    "f6": 8.8,
    "f7": 4.72,
    "f8": 8.0,
    "momentum_score": 2.57,
    "cqv_v1": 9.12,
    "cqv_v1_1": 9.08,
    "cqv_v2": 8.54,
    "cqv_v3": 8.54,
    "cqv": 8.74,
    "pe": 11.46,
    "close_history": "{'2021': 303.6, '2022': 192.3, '2023': 199.51, '2024': 169.84, '2025': 113.04, '2026': 100.93}",
    "status": "Success",
    "cqv_v4": 8.74,
    "price": 100.0,
    "pe_forward": 9.741,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 129.6,
    "mos_pct": 22.84,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PLTR",
    "name": "Palantir Technologies Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.86,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.34,
    "f7": 4.11,
    "f8": 8.0,
    "momentum_score": 2.1,
    "cqv_v1": 8.23,
    "cqv_v1_1": 8.96,
    "cqv_v2": 8.44,
    "cqv_v3": 8.44,
    "cqv": 8.74,
    "pe": 145.28,
    "close_history": "{'2021': 18.21, '2022': 6.42, '2023': 17.17, '2024': 75.63, '2025': 177.75, '2026': 134.44}",
    "status": "Success",
    "cqv_v4": 8.74,
    "price": 100.0,
    "pe_forward": 123.488,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 129.6,
    "mos_pct": 22.84,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PYPL",
    "name": "PayPal Holdings, Inc.",
    "sector": "Financials / Digital Payments & Consumer Commerce Platforms",
    "quarter": "Q2 2026",
    "f1": 8.9,
    "f2": 9.0,
    "f3": 8.1,
    "f4": 8.6,
    "f5": 9.1,
    "f6": 9.0,
    "f7": 8.4,
    "f8": 8.8,
    "momentum_score": 2.48,
    "cqv_v1": 8.61,
    "cqv_v1_1": 8.46,
    "cqv_v2": 8.1,
    "cqv_v3": 8.1,
    "cqv": 8.74,
    "pe": 14.2,
    "close_history": {
      "2020": 234.2,
      "2021": 188.58,
      "2022": 71.22,
      "2023": 61.41,
      "2024": 84.5,
      "2025": 68.2,
      "2026": 64.5
    },
    "status": "Success",
    "cqv_v4": 8.74,
    "price": 64.5,
    "pe_forward": 12.5,
    "value_score": 10.0,
    "peg_bruto": 11.2,
    "score_peg": 10.0,
    "intrinsic_value": 92.14,
    "mos_pct": 30.0,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.6,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "29/07/2026",
    "eps_trailing": 4.54,
    "eps_forward": 5.16,
    "eps_growth_ntm_pct": 14.0,
    "growth_eps": 14.0,
    "market_cap_b": 65.5,
    "ocf_ttm_m": 6450.0,
    "maint_capex_m": 950.0,
    "owner_earnings_m": 5500.0,
    "score_fcf_yield": 10.0,
    "score_mos": 10.0,
    "wacc": 9.0,
    "g_terminal": 3.0,
    "analyst_targets": {
      "target_low_bear": 58.0,
      "target_mean_base": 78.0,
      "target_high_bull": 95.0,
      "num_analysts": 41,
      "consensus_recommendation": "Moderate Buy",
      "upside_potential_pct": 20.9
    }
  },
  {
    "ticker": "BAM",
    "name": "Brookfield Asset Management Inc",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.66,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 2.75,
    "cqv_v1": 8.75,
    "cqv_v1_1": 8.85,
    "cqv_v2": 8.4,
    "cqv_v3": 8.4,
    "cqv": 8.72,
    "pe": 29.4,
    "close_history": "{'2022': 25.29, '2023': 36.82, '2024': 51.44, '2025': 51.32, '2026': 49.57}",
    "status": "Success",
    "cqv_v4": 8.72,
    "price": 100.0,
    "pe_forward": 24.99,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 128.8,
    "mos_pct": 22.36,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IDXX",
    "name": "IDEXX Laboratories, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.49,
    "f2": 9.0,
    "f3": 9.26,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.38,
    "f7": 4.41,
    "f8": 8.0,
    "momentum_score": 2.64,
    "cqv_v1": 8.44,
    "cqv_v1_1": 8.91,
    "cqv_v2": 8.42,
    "cqv_v3": 8.42,
    "cqv": 8.7,
    "pe": 40.98,
    "close_history": "{'2021': 658.46, '2022': 407.96, '2023': 555.05, '2024': 413.44, '2025': 676.53, '2026': 576.07}",
    "status": "Success",
    "cqv_v4": 8.7,
    "price": 100.0,
    "pe_forward": 34.833,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 128.0,
    "mos_pct": 21.88,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HCI",
    "name": "HCI Group, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.25,
    "f3": 9.02,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.51,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 6.19,
    "cqv_v1": 8.98,
    "cqv_v1_1": 8.84,
    "cqv_v2": 8.38,
    "cqv_v3": 8.38,
    "cqv": 8.69,
    "pe": 8.01,
    "close_history": "{'2021': 76.52, '2022': 37.34, '2023': 84.67, '2024': 114.7, '2025': 190.71, '2026': 178.32}",
    "status": "Success",
    "cqv_v4": 8.69,
    "price": 100.0,
    "pe_forward": 6.8084999999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 127.6,
    "mos_pct": 21.63,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CSU",
    "name": "CONSTELLATION SOFTWARE INC.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.36,
    "f2": 9.25,
    "f3": 9.27,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.31,
    "f7": 4.51,
    "f8": 8.0,
    "momentum_score": 3.23,
    "cqv_v1": 8.77,
    "cqv_v1_1": 8.87,
    "cqv_v2": 8.4,
    "cqv_v3": 8.4,
    "cqv": 8.69,
    "pe": 56.49,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.69,
    "price": 100.0,
    "pe_forward": 48.0165,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 127.6,
    "mos_pct": 21.63,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LRCX",
    "name": "Lam Research Corporation",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.62,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.22,
    "f7": 4.3,
    "f8": 8.0,
    "momentum_score": 8.78,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.9,
    "cqv_v2": 8.4,
    "cqv_v3": 8.4,
    "cqv": 8.69,
    "pe": 66.18,
    "close_history": "{'2021': 68.59, '2022': 40.66, '2023': 76.69, '2024': 71.44, '2025': 170.85, '2026': 320.96}",
    "status": "Success",
    "cqv_v4": 8.69,
    "price": 100.0,
    "pe_forward": 56.25300000000001,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 127.6,
    "mos_pct": 21.63,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PAYX",
    "name": "Paychex, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 10.0,
    "f2": 9.25,
    "f3": 8.56,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.28,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 4.58,
    "cqv_v1": 8.51,
    "cqv_v1_1": 8.92,
    "cqv_v2": 8.4,
    "cqv_v3": 8.4,
    "cqv": 8.68,
    "pe": 21.75,
    "close_history": "{'2021': 118.73, '2022': 103.11, '2023': 109.52, '2024': 132.85, '2025': 109.62, '2026': 114.7}",
    "status": "Success",
    "cqv_v4": 8.68,
    "price": 100.0,
    "pe_forward": 18.4875,
    "value_score": 6.78,
    "peg_bruto": 9.7363,
    "score_peg": 9.7363,
    "intrinsic_value": 127.2,
    "mos_pct": 21.38,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 7.13
  },
  {
    "ticker": "FSLR",
    "name": "First Solar, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.5,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.16,
    "f7": 4.4,
    "f8": 8.0,
    "momentum_score": 4.25,
    "cqv_v1": 8.77,
    "cqv_v1_1": 8.87,
    "cqv_v2": 8.38,
    "cqv_v3": 8.38,
    "cqv": 8.67,
    "pe": 14.52,
    "close_history": "{'2021': 87.16, '2022': 149.79, '2023': 172.28, '2024': 176.24, '2025': 261.23, '2026': 211.93}",
    "status": "Success",
    "cqv_v4": 8.67,
    "price": 100.0,
    "pe_forward": 12.341999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 126.8,
    "mos_pct": 21.14,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ACN",
    "name": "Accenture plc",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.1,
    "f2": 9.5,
    "f3": 8.22,
    "f4": 9.1,
    "f5": 9.2,
    "f6": 8.66,
    "f7": 4.72,
    "f8": 8.0,
    "momentum_score": 0.3,
    "cqv_v1": 9.0,
    "cqv_v1_1": 9.05,
    "cqv_v2": 8.47,
    "cqv_v3": 8.47,
    "cqv": 8.67,
    "pe": 10.96,
    "close_history": "{'2021': 382.74, '2022': 249.73, '2023': 333.62, '2024': 339.84, '2025': 262.88, '2026': 144.61}",
    "status": "Success",
    "cqv_v4": 8.67,
    "price": 100.0,
    "pe_forward": 9.316,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 126.4,
    "mos_pct": 20.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.1,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ADSK",
    "name": "Autodesk, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 9.63,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.23,
    "f7": 4.3,
    "f8": 8.0,
    "momentum_score": 1.41,
    "cqv_v1": 8.95,
    "cqv_v1_1": 8.87,
    "cqv_v2": 8.38,
    "cqv_v3": 8.38,
    "cqv": 8.66,
    "pe": 30.29,
    "close_history": "{'2021': 281.19, '2022': 186.87, '2023': 243.48, '2024': 295.57, '2025': 296.01, '2026': 217.06}",
    "status": "Success",
    "cqv_v4": 8.66,
    "price": 100.0,
    "pe_forward": 25.746499999999997,
    "value_score": 5.91,
    "peg_bruto": 6.9912,
    "score_peg": 6.9912,
    "intrinsic_value": 126.4,
    "mos_pct": 20.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 6.97
  },
  {
    "ticker": "QLYS",
    "name": "Qualys, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 9.5,
    "f3": 8.26,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.13,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 7.57,
    "cqv_v1": 8.8,
    "cqv_v1_1": 8.91,
    "cqv_v2": 8.38,
    "cqv_v3": 8.38,
    "cqv": 8.66,
    "pe": 26.59,
    "close_history": "{'2021': 137.22, '2022': 112.23, '2023': 196.28, '2024': 140.22, '2025': 132.9, '2026': 157.56}",
    "status": "Success",
    "cqv_v4": 8.66,
    "price": 100.0,
    "pe_forward": 22.601499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 126.4,
    "mos_pct": 20.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "VEEV",
    "name": "Veeva Systems Inc.",
    "sector": "Healthcare",
    "quarter": "Q2 2026",
    "f1": 9.06,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.2,
    "f7": 4.75,
    "f8": 8.0,
    "momentum_score": 4.17,
    "cqv_v1": 8.67,
    "cqv_v1_1": 8.85,
    "cqv_v2": 8.38,
    "cqv_v3": 8.38,
    "cqv": 8.66,
    "pe": 34.11,
    "close_history": "{'2021': 255.48, '2022': 161.38, '2023': 192.52, '2024': 210.25, '2025': 223.23, '2026': 197.37}",
    "status": "Success",
    "cqv_v4": 8.66,
    "price": 100.0,
    "pe_forward": 28.993499999999997,
    "value_score": 5.67,
    "peg_bruto": 6.2083,
    "score_peg": 6.2083,
    "intrinsic_value": 126.4,
    "mos_pct": 20.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 6.97
  },
  {
    "ticker": "TW",
    "name": "Tradeweb Markets Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.54,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.43,
    "f7": 4.37,
    "f8": 8.0,
    "momentum_score": 3.19,
    "cqv_v1": 8.6,
    "cqv_v1_1": 8.77,
    "cqv_v2": 8.35,
    "cqv_v3": 8.35,
    "cqv": 8.66,
    "pe": 25.38,
    "close_history": "{'2021': 98.25, '2022': 64.01, '2023': 90.0, '2024': 130.09, '2025': 107.27, '2026': 101.19}",
    "status": "Success",
    "cqv_v4": 8.66,
    "price": 100.0,
    "pe_forward": 21.572999999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 126.4,
    "mos_pct": 20.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "APP",
    "name": "Applovin Corporation",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 9.91,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.62,
    "f7": 4.07,
    "f8": 8.0,
    "momentum_score": 4.53,
    "cqv_v1": 8.4,
    "cqv_v1_1": 8.79,
    "cqv_v2": 8.36,
    "cqv_v3": 8.36,
    "cqv": 8.66,
    "pe": 45.87,
    "close_history": "{'2021': 94.26, '2022': 10.53, '2023': 39.85, '2024': 323.83, '2025': 673.82, '2026': 434.48}",
    "status": "Success",
    "cqv_v4": 8.66,
    "price": 100.0,
    "pe_forward": 38.9895,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 126.4,
    "mos_pct": 20.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EW",
    "name": "Edwards Lifesciences Corporatio",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.01,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.17,
    "f7": 4.79,
    "f8": 8.0,
    "momentum_score": 8.32,
    "cqv_v1": 8.31,
    "cqv_v1_1": 8.83,
    "cqv_v2": 8.38,
    "cqv_v3": 8.38,
    "cqv": 8.65,
    "pe": 51.01,
    "close_history": "{'2021': 129.55, '2022': 74.61, '2023': 76.25, '2024': 74.03, '2025': 85.25, '2026': 87.84}",
    "status": "Success",
    "cqv_v4": 8.65,
    "price": 100.0,
    "pe_forward": 43.3585,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 126.0,
    "mos_pct": 20.63,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DSGX",
    "name": "The Descartes Systems Group Inc",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 9.42,
    "f2": 9.5,
    "f3": 8.8,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.11,
    "f7": 4.46,
    "f8": 8.0,
    "momentum_score": 2.97,
    "cqv_v1": 8.7,
    "cqv_v1_1": 8.85,
    "cqv_v2": 8.36,
    "cqv_v3": 8.36,
    "cqv": 8.64,
    "pe": 36.22,
    "close_history": "{'2021': 82.68, '2022': 69.65, '2023': 84.06, '2024': 113.6, '2025': 87.66, '2026': 75.91}",
    "status": "Success",
    "cqv_v4": 8.64,
    "price": 100.0,
    "pe_forward": 30.787,
    "value_score": 5.51,
    "peg_bruto": 5.8466,
    "score_peg": 5.8466,
    "intrinsic_value": 125.6,
    "mos_pct": 20.38,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 6.8
  },
  {
    "ticker": "PODD",
    "name": "Insulet Corporation",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.13,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.23,
    "f7": 4.7,
    "f8": 8.0,
    "momentum_score": 2.02,
    "cqv_v1": 8.42,
    "cqv_v1_1": 8.83,
    "cqv_v2": 8.37,
    "cqv_v3": 8.37,
    "cqv": 8.64,
    "pe": 38.43,
    "close_history": "{'2021': 266.07, '2022': 294.39, '2023': 216.98, '2024': 261.07, '2025': 284.24, '2026': 164.43}",
    "status": "Success",
    "cqv_v4": 8.64,
    "price": 100.0,
    "pe_forward": 32.6655,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 125.6,
    "mos_pct": 20.38,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PJT",
    "name": "PJT Partners Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.4,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.37,
    "f7": 4.48,
    "f8": 8.0,
    "momentum_score": 5.06,
    "cqv_v1": 8.72,
    "cqv_v1_1": 8.74,
    "cqv_v2": 8.32,
    "cqv_v3": 8.32,
    "cqv": 8.63,
    "pe": 23.66,
    "close_history": "{'2021': 70.82, '2022': 71.46, '2023': 100.05, '2024': 156.31, '2025': 166.65, '2026': 167.22}",
    "status": "Success",
    "cqv_v4": 8.63,
    "price": 100.0,
    "pe_forward": 20.111,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 125.2,
    "mos_pct": 20.13,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "UI",
    "name": "Ubiquiti Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.45,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.14,
    "f7": 4.44,
    "f8": 8.0,
    "momentum_score": 2.01,
    "cqv_v1": 8.33,
    "cqv_v1_1": 8.82,
    "cqv_v2": 8.35,
    "cqv_v3": 8.35,
    "cqv": 8.62,
    "pe": 33.79,
    "close_history": "{'2021': 292.86, '2022': 263.61, '2023': 136.48, '2024': 329.13, '2025': 552.03, '2026': 543.92}",
    "status": "Success",
    "cqv_v4": 8.62,
    "price": 100.0,
    "pe_forward": 28.7215,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 124.8,
    "mos_pct": 19.87,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "APH",
    "name": "Amphenol Corporation",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.41,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.12,
    "f7": 4.47,
    "f8": 8.0,
    "momentum_score": 8.35,
    "cqv_v1": 8.34,
    "cqv_v1_1": 8.81,
    "cqv_v2": 8.34,
    "cqv_v3": 8.34,
    "cqv": 8.61,
    "pe": 47.43,
    "close_history": "{'2021': 41.98, '2022': 36.96, '2023': 48.6, '2024': 68.68, '2025': 134.67, '2026': 153.14}",
    "status": "Success",
    "cqv_v4": 8.61,
    "price": 100.0,
    "pe_forward": 40.3155,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 124.4,
    "mos_pct": 19.61,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WDC",
    "name": "Western Digital Corporation",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 9.37,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.1,
    "f7": 4.5,
    "f8": 8.0,
    "momentum_score": 7.83,
    "cqv_v1": 8.15,
    "cqv_v1_1": 8.8,
    "cqv_v2": 8.33,
    "cqv_v3": 8.33,
    "cqv": 8.6,
    "pe": 32.26,
    "close_history": "{'2021': 49.06, '2022': 23.74, '2023': 39.4, '2024': 44.87, '2025': 172.14, '2026': 466.81}",
    "status": "Success",
    "cqv_v4": 8.6,
    "price": 100.0,
    "pe_forward": 27.420999999999996,
    "value_score": 5.63,
    "peg_bruto": 6.5643,
    "score_peg": 6.5643,
    "intrinsic_value": 124.0,
    "mos_pct": 19.35,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 6.47
  },
  {
    "ticker": "CNS",
    "name": "Cohen & Steers Inc",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.91,
    "f2": 9.25,
    "f3": 8.59,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.25,
    "f7": 4.07,
    "f8": 8.0,
    "momentum_score": 8.61,
    "cqv_v1": 8.59,
    "cqv_v1_1": 8.75,
    "cqv_v2": 8.3,
    "cqv_v3": 8.3,
    "cqv": 8.59,
    "pe": 26.02,
    "close_history": "{'2021': 79.6, '2022': 57.31, '2023': 69.83, '2024': 87.65, '2025': 61.56, '2026': 81.07}",
    "status": "Success",
    "cqv_v4": 8.59,
    "price": 100.0,
    "pe_forward": 22.116999999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 123.6,
    "mos_pct": 19.09,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PCTY",
    "name": "Paylocity Holding Corporation",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.52,
    "f2": 9.5,
    "f3": 8.33,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.93,
    "f7": 4.38,
    "f8": 8.0,
    "momentum_score": 3.24,
    "cqv_v1": 8.88,
    "cqv_v1_1": 8.8,
    "cqv_v2": 8.31,
    "cqv_v3": 8.31,
    "cqv": 8.57,
    "pe": 24.59,
    "close_history": "{'2021': 236.16, '2022': 194.26, '2023': 164.85, '2024': 199.47, '2025': 152.5, '2026': 126.86}",
    "status": "Success",
    "cqv_v4": 8.57,
    "price": 100.0,
    "pe_forward": 20.9015,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.8,
    "mos_pct": 18.57,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "KINS",
    "name": "Kingstone Companies, Inc",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.13,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.23,
    "f7": 4.7,
    "f8": 8.0,
    "momentum_score": 9.29,
    "cqv_v1": 8.81,
    "cqv_v1_1": 8.67,
    "cqv_v2": 8.28,
    "cqv_v3": 8.28,
    "cqv": 8.57,
    "pe": 9.1,
    "close_history": "{'2021': 4.81, '2022': 1.33, '2023': 2.1, '2024': 14.99, '2025': 16.72, '2026': 19.75}",
    "status": "Success",
    "cqv_v4": 8.57,
    "price": 100.0,
    "pe_forward": 7.734999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.8,
    "mos_pct": 18.57,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "VRTX",
    "name": "Vertex Pharmaceuticals Incorpor",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.41,
    "f2": 9.25,
    "f3": 8.53,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.97,
    "f7": 4.47,
    "f8": 8.0,
    "momentum_score": 8.59,
    "cqv_v1": 8.38,
    "cqv_v1_1": 8.81,
    "cqv_v2": 8.32,
    "cqv_v3": 8.32,
    "cqv": 8.57,
    "pe": 31.36,
    "close_history": "{'2021': 219.6, '2022': 288.78, '2023': 406.89, '2024': 402.7, '2025': 453.36, '2026': 486.03}",
    "status": "Success",
    "cqv_v4": 8.57,
    "price": 100.0,
    "pe_forward": 26.656,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.8,
    "mos_pct": 18.57,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MNST",
    "name": "Monster Beverage Corporation",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 9.55,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 9.44,
    "f7": 4.36,
    "f8": 8.0,
    "momentum_score": 9.47,
    "cqv_v1": 8.28,
    "cqv_v1_1": 8.65,
    "cqv_v2": 8.26,
    "cqv_v3": 8.26,
    "cqv": 8.57,
    "pe": 47.15,
    "close_history": "{'2021': 48.02, '2022': 50.76, '2023': 57.61, '2024': 52.56, '2025': 76.67, '2026': 99.94}",
    "status": "Success",
    "cqv_v4": 8.57,
    "price": 100.0,
    "pe_forward": 40.0775,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.8,
    "mos_pct": 18.57,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LLY",
    "name": "Eli Lilly and Company",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.74,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.04,
    "f7": 5.01,
    "f8": 8.0,
    "momentum_score": 9.31,
    "cqv_v1": 8.05,
    "cqv_v1_1": 8.73,
    "cqv_v2": 8.31,
    "cqv_v3": 8.31,
    "cqv": 8.56,
    "pe": 43.11,
    "close_history": "{'2021': 265.2, '2022': 356.04, '2023': 572.91, '2024': 763.69, '2025': 1071.05, '2026': 1169.17}",
    "status": "Success",
    "cqv_v4": 8.56,
    "price": 100.0,
    "pe_forward": 36.643499999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.4,
    "mos_pct": 18.3,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PRI",
    "name": "Primerica, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.92,
    "f2": 9.0,
    "f3": 8.57,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.25,
    "f7": 4.06,
    "f8": 8.0,
    "momentum_score": 8.05,
    "cqv_v1": 8.86,
    "cqv_v1_1": 8.72,
    "cqv_v2": 8.27,
    "cqv_v3": 8.27,
    "cqv": 8.55,
    "pe": 12.47,
    "close_history": "{'2021': 143.31, '2022': 134.79, '2023': 198.24, '2024': 264.89, '2025': 256.08, '2026': 312.75}",
    "status": "Success",
    "cqv_v4": 8.55,
    "price": 100.0,
    "pe_forward": 10.5995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.0,
    "mos_pct": 18.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MORN",
    "name": "Morningstar, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.66,
    "f2": 9.0,
    "f3": 8.87,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.27,
    "f7": 4.27,
    "f8": 8.0,
    "momentum_score": 2.0,
    "cqv_v1": 8.84,
    "cqv_v1_1": 8.7,
    "cqv_v2": 8.27,
    "cqv_v3": 8.27,
    "cqv": 8.55,
    "pe": 16.98,
    "close_history": "{'2021': 330.92, '2022': 210.76, '2023': 280.52, '2024': 331.83, '2025': 215.52, '2026': 173.9}",
    "status": "Success",
    "cqv_v4": 8.55,
    "price": 100.0,
    "pe_forward": 14.433,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.0,
    "mos_pct": 18.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EXLS",
    "name": "ExlService Holdings, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.25,
    "f2": 9.25,
    "f3": 8.7,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.97,
    "f7": 4.6,
    "f8": 8.0,
    "momentum_score": 1.25,
    "cqv_v1": 8.83,
    "cqv_v1_1": 8.75,
    "cqv_v2": 8.29,
    "cqv_v3": 8.29,
    "cqv": 8.55,
    "pe": 17.21,
    "close_history": "{'2021': 28.95, '2022': 33.89, '2023': 30.85, '2024': 44.38, '2025': 42.44, '2026': 28.69}",
    "status": "Success",
    "cqv_v4": 8.55,
    "price": 100.0,
    "pe_forward": 14.6285,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.0,
    "mos_pct": 18.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BLK",
    "name": "BlackRock, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.2,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.27,
    "f7": 4.64,
    "f8": 8.0,
    "momentum_score": 3.43,
    "cqv_v1": 8.6,
    "cqv_v1_1": 8.65,
    "cqv_v2": 8.26,
    "cqv_v3": 8.26,
    "cqv": 8.55,
    "pe": 25.06,
    "close_history": "{'2021': 818.9, '2022': 651.87, '2023': 768.31, '2024': 993.36, '2025': 1058.45, '2026': 1087.05}",
    "status": "Success",
    "cqv_v4": 8.55,
    "price": 100.0,
    "pe_forward": 21.301,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.0,
    "mos_pct": 18.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WST",
    "name": "West Pharmaceutical Services, I",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.52,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.93,
    "f7": 5.18,
    "f8": 8.0,
    "momentum_score": 9.91,
    "cqv_v1": 8.09,
    "cqv_v1_1": 8.71,
    "cqv_v2": 8.29,
    "cqv_v3": 8.29,
    "cqv": 8.55,
    "pe": 48.77,
    "close_history": "{'2021': 463.39, '2022': 233.09, '2023': 349.56, '2024': 325.95, '2025': 274.68, '2026': 362.44}",
    "status": "Success",
    "cqv_v4": 8.55,
    "price": 100.0,
    "pe_forward": 41.4545,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 122.0,
    "mos_pct": 18.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "VRSN",
    "name": "VeriSign, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.96,
    "f2": 9.25,
    "f3": 7.9,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.93,
    "f7": 4.03,
    "f8": 8.0,
    "momentum_score": 5.04,
    "cqv_v1": 8.57,
    "cqv_v1_1": 8.81,
    "cqv_v2": 8.29,
    "cqv_v3": 8.29,
    "cqv": 8.54,
    "pe": 28.33,
    "close_history": "{'2021': 250.03, '2022': 202.37, '2023': 202.88, '2024': 203.87, '2025': 241.41, '2026': 275.22}",
    "status": "Success",
    "cqv_v4": 8.54,
    "price": 100.0,
    "pe_forward": 24.080499999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 121.6,
    "mos_pct": 17.76,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "STX",
    "name": "Seagate Technology Holdings PLC",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 9.4,
    "f2": 8.83,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 9.12,
    "f7": 4.48,
    "f8": 8.0,
    "momentum_score": 7.58,
    "cqv_v1": 8.12,
    "cqv_v1_1": 8.75,
    "cqv_v2": 8.3,
    "cqv_v3": 8.3,
    "cqv": 8.54,
    "pe": 77.52,
    "close_history": "{'2021': 98.78, '2022': 47.99, '2023': 81.16, '2024': 84.46, '2025': 274.71, '2026': 745.49}",
    "status": "Success",
    "cqv_v4": 8.54,
    "price": 100.0,
    "pe_forward": 65.892,
    "value_score": 4.32,
    "peg_bruto": 2.7317,
    "score_peg": 2.7317,
    "intrinsic_value": 121.6,
    "mos_pct": 17.76,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.93
  },
  {
    "ticker": "CBOE",
    "name": "Cboe Global Markets, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.82,
    "f2": 9.25,
    "f3": 8.39,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.11,
    "f7": 4.14,
    "f8": 8.0,
    "momentum_score": 3.31,
    "cqv_v1": 8.56,
    "cqv_v1_1": 8.7,
    "cqv_v2": 8.25,
    "cqv_v3": 8.25,
    "cqv": 8.53,
    "pe": 21.28,
    "close_history": "{'2021': 123.63, '2022': 120.96, '2023': 174.62, '2024': 193.38, '2025': 249.86, '2026': 277.89}",
    "status": "Success",
    "cqv_v4": 8.53,
    "price": 100.0,
    "pe_forward": 18.088,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 121.2,
    "mos_pct": 17.49,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SEZL",
    "name": "Sezzle Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.09,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.21,
    "f7": 4.73,
    "f8": 8.0,
    "momentum_score": 9.92,
    "cqv_v1": 8.1,
    "cqv_v1_1": 8.62,
    "cqv_v2": 8.24,
    "cqv_v3": 8.24,
    "cqv": 8.53,
    "pe": 23.52,
    "close_history": "{'2023': 3.42, '2024': 42.63, '2025': 63.47, '2026': 180.44}",
    "status": "Success",
    "cqv_v4": 8.53,
    "price": 100.0,
    "pe_forward": 19.992,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 121.2,
    "mos_pct": 17.49,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RDDT",
    "name": "Reddit, Inc.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 9.08,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.21,
    "f7": 4.74,
    "f8": 8.0,
    "momentum_score": 5.36,
    "cqv_v1": 8.18,
    "cqv_v1_1": 8.62,
    "cqv_v2": 8.24,
    "cqv_v3": 8.24,
    "cqv": 8.52,
    "pe": 55.62,
    "close_history": "{'2024': 163.44, '2025': 229.87, '2026': 185.26}",
    "status": "Success",
    "cqv_v4": 8.52,
    "price": 100.0,
    "pe_forward": 47.276999999999994,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.8,
    "mos_pct": 17.22,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NTES",
    "name": "NetEase, Inc.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 9.94,
    "f2": 9.0,
    "f3": 8.34,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.14,
    "f7": 4.05,
    "f8": 8.0,
    "momentum_score": 4.38,
    "cqv_v1": 8.84,
    "cqv_v1_1": 8.69,
    "cqv_v2": 8.24,
    "cqv_v3": 8.24,
    "cqv": 8.51,
    "pe": 16.33,
    "close_history": "{'2021': 91.8, '2022': 66.7, '2023': 87.1, '2024': 85.59, '2025': 135.46, '2026': 130.33}",
    "status": "Success",
    "cqv_v4": 8.51,
    "price": 100.0,
    "pe_forward": 13.880499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.4,
    "mos_pct": 16.94,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ADP",
    "name": "Automatic Data Processing, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.79,
    "f2": 9.25,
    "f3": 7.94,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.87,
    "f7": 4.17,
    "f8": 8.0,
    "momentum_score": 4.83,
    "cqv_v1": 8.71,
    "cqv_v1_1": 8.78,
    "cqv_v2": 8.27,
    "cqv_v3": 8.27,
    "cqv": 8.51,
    "pe": 22.62,
    "close_history": "{'2021': 222.95, '2022': 220.08, '2023': 219.53, '2024': 281.9, '2025': 253.2, '2026': 256.56}",
    "status": "Success",
    "cqv_v4": 8.51,
    "price": 100.0,
    "pe_forward": 19.227,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.4,
    "mos_pct": 16.94,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "JKHY",
    "name": "Jack Henry & Associates, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.58,
    "f2": 9.25,
    "f3": 8.13,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.86,
    "f7": 4.34,
    "f8": 8.0,
    "momentum_score": 3.75,
    "cqv_v1": 8.5,
    "cqv_v1_1": 8.75,
    "cqv_v2": 8.26,
    "cqv_v3": 8.26,
    "cqv": 8.51,
    "pe": 20.54,
    "close_history": "{'2021': 157.56, '2022': 167.38, '2023': 157.91, '2024': 171.57, '2025': 181.01, '2026': 153.84}",
    "status": "Success",
    "cqv_v4": 8.51,
    "price": 100.0,
    "pe_forward": 17.459,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.4,
    "mos_pct": 16.94,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HWM",
    "name": "Howmet Aerospace Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.21,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.27,
    "f7": 4.63,
    "f8": 8.0,
    "momentum_score": 8.12,
    "cqv_v1": 8.09,
    "cqv_v1_1": 8.61,
    "cqv_v2": 8.24,
    "cqv_v3": 8.24,
    "cqv": 8.51,
    "pe": 62.89,
    "close_history": "{'2021': 31.41, '2022': 39.0, '2023': 53.76, '2024': 108.97, '2025': 204.81, '2026': 271.19}",
    "status": "Success",
    "cqv_v4": 8.51,
    "price": 100.0,
    "pe_forward": 53.4565,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.4,
    "mos_pct": 16.94,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CRM",
    "name": "Salesforce, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.1,
    "f2": 9.0,
    "f3": 7.8,
    "f4": 9.3,
    "f5": 8.8,
    "f6": 8.45,
    "f7": 4.72,
    "f8": 8.0,
    "momentum_score": 1.66,
    "cqv_v1": 8.88,
    "cqv_v1_1": 9.0,
    "cqv_v2": 8.36,
    "cqv_v3": 8.36,
    "cqv": 8.5,
    "pe": 19.23,
    "close_history": "{'2021': 249.81, '2022': 130.34, '2023': 258.66, '2024': 330.48, '2025': 263.57, '2026': 172.68}",
    "status": "Success",
    "cqv_v4": 8.5,
    "price": 100.0,
    "pe_forward": 16.3455,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.0,
    "mos_pct": 16.67,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TROW",
    "name": "T. Rowe Price Group, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.8,
    "f2": 9.25,
    "f3": 8.26,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.03,
    "f7": 4.16,
    "f8": 8.0,
    "momentum_score": 8.66,
    "cqv_v1": 8.81,
    "cqv_v1_1": 8.68,
    "cqv_v2": 8.23,
    "cqv_v3": 8.23,
    "cqv": 8.5,
    "pe": 12.72,
    "close_history": "{'2021': 159.7, '2022': 92.24, '2023': 95.33, '2024': 104.56, '2025': 99.68, '2026': 118.87}",
    "status": "Success",
    "cqv_v4": 8.5,
    "price": 100.0,
    "pe_forward": 10.812,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.0,
    "mos_pct": 16.67,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MRK",
    "name": "Merck & Company, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.54,
    "f2": 9.0,
    "f3": 8.21,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.88,
    "f7": 4.37,
    "f8": 8.0,
    "momentum_score": 9.13,
    "cqv_v1": 8.65,
    "cqv_v1_1": 8.76,
    "cqv_v2": 8.27,
    "cqv_v3": 8.27,
    "cqv": 8.5,
    "pe": 36.5,
    "close_history": "{'2021': 66.79, '2022': 99.79, '2023': 100.8, '2024': 94.49, '2025': 103.74, '2026': 127.63}",
    "status": "Success",
    "cqv_v4": 8.5,
    "price": 100.0,
    "pe_forward": 31.025,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 120.0,
    "mos_pct": 16.67,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TDD",
    "name": "The Trade Desk, Inc.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 9.26,
    "f2": 9.0,
    "f3": 8.98,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.12,
    "f7": 4.59,
    "f8": 8.0,
    "momentum_score": 0.58,
    "cqv_v1": 8.76,
    "cqv_v1_1": 8.61,
    "cqv_v2": 8.22,
    "cqv_v3": 8.22,
    "cqv": 8.49,
    "pe": 21.7,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.49,
    "price": 100.0,
    "pe_forward": 18.445,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 119.6,
    "mos_pct": 16.39,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CME",
    "name": "CME Group Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.79,
    "f2": 9.25,
    "f3": 9.27,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.03,
    "f7": 4.97,
    "f8": 8.0,
    "momentum_score": 1.95,
    "cqv_v1": 8.42,
    "cqv_v1_1": 8.58,
    "cqv_v2": 8.21,
    "cqv_v3": 8.21,
    "cqv": 8.49,
    "pe": 20.2,
    "close_history": "{'2021': 189.46, '2022': 146.1, '2023': 191.85, '2024': 221.42, '2025': 265.33, '2026': 246.27}",
    "status": "Success",
    "cqv_v4": 8.49,
    "price": 100.0,
    "pe_forward": 17.169999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 119.6,
    "mos_pct": 16.39,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AMZN",
    "name": "Amazon.com, Inc.",
    "sector": "Technology / E-Commerce, AWS Cloud Infrastructure & Ad Services",
    "quarter": "Q2 2026",
    "f1": 8.2,
    "f2": 8.4,
    "f3": 9.0,
    "f4": 9.0,
    "f5": 7.5,
    "f6": 8.7,
    "f7": 9.2,
    "f8": 8.0,
    "momentum_score": 5.92,
    "cqv_v1": 7.13,
    "cqv_v1_1": 7.75,
    "cqv_v2": 7.65,
    "cqv_v3": 7.65,
    "cqv": 8.48,
    "pe": 32.83,
    "close_history": {
      "2020": 162.84,
      "2021": 166.72,
      "2022": 84.0,
      "2023": 151.94,
      "2024": 195.0,
      "2025": 175.0,
      "2026": 274.48
    },
    "status": "Success",
    "cqv_v4": 8.48,
    "price": 274.48,
    "pe_forward": 29.71,
    "value_score": null,
    "peg_bruto": 7.2501,
    "score_peg": 7.2501,
    "intrinsic_value": null,
    "mos_pct": null,
    "verdict": "N/D - valoración incompleta",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 9.0,
    "data_confidence": "Media",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "07/08/2026",
    "eps_trailing": 8.36,
    "eps_forward": 9.24,
    "eps_growth_ntm_pct": 21.54,
    "growth_eps": 21.54,
    "market_cap_b": 2953.0,
    "ocf_ttm_m": 161400.0,
    "maint_capex_m": null,
    "owner_earnings_m": null,
    "score_fcf_yield": null,
    "score_mos": null,
    "wacc": null,
    "g_terminal": null,
    "analyst_targets": {
      "target_low_bear": 160.0,
      "target_mean_base": 215.0,
      "target_high_bull": 245.0,
      "num_analysts": 48,
      "consensus_recommendation": "Strong Buy",
      "upside_potential_pct": 16.2
    }
  },
  {
    "ticker": "KEYS",
    "name": "Keysight Technologies Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.8,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.82,
    "f7": 4.96,
    "f8": 8.0,
    "momentum_score": 6.86,
    "cqv_v1": 8.27,
    "cqv_v1_1": 8.66,
    "cqv_v2": 8.24,
    "cqv_v3": 8.24,
    "cqv": 8.48,
    "pe": 50.54,
    "close_history": "{'2021': 206.51, '2022': 171.07, '2023': 159.09, '2024': 160.63, '2025': 203.19, '2026': 312.76}",
    "status": "Success",
    "cqv_v4": 8.48,
    "price": 100.0,
    "pe_forward": 42.958999999999996,
    "value_score": 4.59,
    "peg_bruto": 4.19,
    "score_peg": 4.19,
    "intrinsic_value": 119.2,
    "mos_pct": 16.11,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.37
  },
  {
    "ticker": "NSSC",
    "name": "NAPCO Security Technologies, In",
    "sector": "Industrials",
    "quarter": "Q1 2021",
    "f1": 9.59,
    "f2": 9.0,
    "f3": 8.51,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.05,
    "f7": 4.33,
    "f8": 8.0,
    "momentum_score": 3.68,
    "cqv_v1": 8.48,
    "cqv_v1_1": 8.62,
    "cqv_v2": 8.21,
    "cqv_v3": 8.21,
    "cqv": 8.47,
    "pe": 36.69,
    "close_history": "{'2021': 23.94, '2022': 26.33, '2023': 33.07, '2024': 34.71, '2025': 41.38, '2026': 36.67}",
    "status": "Success",
    "cqv_v4": 8.47,
    "price": 100.0,
    "pe_forward": 31.1865,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.8,
    "mos_pct": 15.82,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MLI",
    "name": "Mueller Industries, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.85,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.09,
    "f7": 4.92,
    "f8": 8.0,
    "momentum_score": 4.63,
    "cqv_v1": 8.45,
    "cqv_v1_1": 8.56,
    "cqv_v2": 8.2,
    "cqv_v3": 8.2,
    "cqv": 8.47,
    "pe": 14.83,
    "close_history": "{'2021': 13.94, '2022': 14.09, '2023': 22.88, '2024': 39.01, '2025': 57.07, '2026': 58.97}",
    "status": "Success",
    "cqv_v4": 8.47,
    "price": 100.0,
    "pe_forward": 12.6055,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.8,
    "mos_pct": 15.82,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NYT",
    "name": "New York Times Company (The)",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 9.12,
    "f2": 9.0,
    "f3": 9.01,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.06,
    "f7": 4.7,
    "f8": 8.0,
    "momentum_score": 6.08,
    "cqv_v1": 8.39,
    "cqv_v1_1": 8.58,
    "cqv_v2": 8.2,
    "cqv_v3": 8.2,
    "cqv": 8.47,
    "pe": 31.7,
    "close_history": "{'2021': 45.87, '2022': 31.12, '2023': 47.49, '2024': 50.97, '2025': 68.84, '2026': 76.71}",
    "status": "Success",
    "cqv_v4": 8.47,
    "price": 100.0,
    "pe_forward": 26.945,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.8,
    "mos_pct": 15.82,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ALAB",
    "name": "Astera Labs, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.59,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.71,
    "f7": 5.13,
    "f8": 8.0,
    "momentum_score": 9.32,
    "cqv_v1": 7.91,
    "cqv_v1_1": 8.65,
    "cqv_v2": 8.23,
    "cqv_v3": 8.23,
    "cqv": 8.47,
    "pe": 278.37,
    "close_history": "{'2024': 132.45, '2025': 166.36, '2026': 319.74}",
    "status": "Success",
    "cqv_v4": 8.47,
    "price": 100.0,
    "pe_forward": 236.6145,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.8,
    "mos_pct": 15.82,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TER",
    "name": "Teradyne, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.56,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.7,
    "f7": 5.15,
    "f8": 8.0,
    "momentum_score": 7.52,
    "cqv_v1": 7.9,
    "cqv_v1_1": 8.64,
    "cqv_v2": 8.22,
    "cqv_v3": 8.22,
    "cqv": 8.47,
    "pe": 68.48,
    "close_history": "{'2021': 160.61, '2022': 86.17, '2023': 107.52, '2024': 125.27, '2025': 193.41, '2026': 322.3}",
    "status": "Success",
    "cqv_v4": 8.47,
    "price": 100.0,
    "pe_forward": 58.208,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.8,
    "mos_pct": 15.82,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "UBER",
    "name": "Uber Technologies, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.2,
    "f2": 9.0,
    "f3": 8.9,
    "f4": 8.3,
    "f5": 8.5,
    "f6": 8.7,
    "f7": 6.8,
    "f8": 8.4,
    "close_history": "{'2021': 41.93, '2022': 24.73, '2023': 61.57, '2024': 60.32, '2025': 81.71, '2026': 68.18}",
    "status": "Success",
    "price": 68.18,
    "valuation_date": "2026-08-07",
    "pe": 17.3,
    "pe_forward": 23.09,
    "eps_growth_ntm_pct": 31.75,
    "market_cap": 138787.136,
    "ocf": 2862.0,
    "maintenance_capex": null,
    "intrinsic_value": null,
    "score_fcf_yield": null,
    "score_mos": null,
    "data_confidence": "Media-Alta",
    "price_source": "ThetaOwl historical close, consulted 2026-08-09",
    "market_source": "Barchart/MarketBeat, consulted 2026-08-09",
    "financial_source": "Uber Q2 2026 earnings release, 2026-08-05",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "cqv_v4": 8.47,
    "cqv": 8.47,
    "peg_bruto": 13.7505,
    "score_peg": 10.0,
    "mos_pct": null,
    "value_score": null,
    "verdict": "N/D - valoración incompleta",
    "clasificacion": "ALTA CALIDAD"
  },
  {
    "ticker": "INFY",
    "name": "Infosys Limited",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.43,
    "f2": 9.5,
    "f3": 7.9,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.66,
    "f7": 4.46,
    "f8": 8.0,
    "momentum_score": 0.89,
    "cqv_v1": 8.77,
    "cqv_v1_1": 8.72,
    "cqv_v2": 8.23,
    "cqv_v3": 8.23,
    "cqv": 8.46,
    "pe": 13.95,
    "close_history": "{'2021': 22.22, '2022': 16.16, '2023': 16.94, '2024': 20.84, '2025': 17.44, '2026': 11.53}",
    "status": "Success",
    "cqv_v4": 8.46,
    "price": 100.0,
    "pe_forward": 11.8575,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.4,
    "mos_pct": 15.54,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ABBV",
    "name": "AbbVie Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.89,
    "f2": 7.17,
    "f3": 9.04,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 9.46,
    "f7": 4.09,
    "f8": 8.0,
    "momentum_score": 8.31,
    "cqv_v1": 8.68,
    "cqv_v1_1": 8.7,
    "cqv_v2": 8.27,
    "cqv_v3": 8.27,
    "cqv": 8.46,
    "pe": 127.98,
    "close_history": "{'2021': 114.07, '2022': 141.45, '2023': 141.12, '2024': 167.73, '2025': 223.21, '2026': 254.39}",
    "status": "Success",
    "cqv_v4": 8.46,
    "price": 100.0,
    "pe_forward": 108.783,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.4,
    "mos_pct": 15.54,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PAYC",
    "name": "Paycom Software, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.3,
    "f2": 9.5,
    "f3": 8.03,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.66,
    "f7": 4.56,
    "f8": 8.0,
    "momentum_score": 3.29,
    "cqv_v1": 8.59,
    "cqv_v1_1": 8.7,
    "cqv_v2": 8.22,
    "cqv_v3": 8.22,
    "cqv": 8.46,
    "pe": 16.11,
    "close_history": "{'2021': 404.74, '2022': 302.5, '2023': 202.48, '2024': 202.41, '2025': 158.49, '2026': 150.48}",
    "status": "Success",
    "cqv_v4": 8.46,
    "price": 100.0,
    "pe_forward": 13.693499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.4,
    "mos_pct": 15.54,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MTD",
    "name": "Mettler-Toledo International, I",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.21,
    "f2": 8.84,
    "f3": 8.47,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.84,
    "f7": 4.63,
    "f8": 8.0,
    "momentum_score": 5.09,
    "cqv_v1": 8.31,
    "cqv_v1_1": 8.69,
    "cqv_v2": 8.24,
    "cqv_v3": 8.24,
    "cqv": 8.46,
    "pe": 30.77,
    "close_history": "{'2021': 1697.21, '2022': 1445.45, '2023': 1212.96, '2024': 1223.68, '2025': 1394.19, '2026': 1335.09}",
    "status": "Success",
    "cqv_v4": 8.46,
    "price": 100.0,
    "pe_forward": 26.1545,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.4,
    "mos_pct": 15.54,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DUOL",
    "name": "Duolingo, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.47,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.65,
    "f7": 5.22,
    "f8": 8.0,
    "momentum_score": 2.71,
    "cqv_v1": 8.6,
    "cqv_v1_1": 8.62,
    "cqv_v2": 8.2,
    "cqv_v3": 8.2,
    "cqv": 8.45,
    "pe": 14.39,
    "close_history": "{'2021': 106.11, '2022': 71.13, '2023': 226.85, '2024': 324.23, '2025': 175.5, '2026': 128.94}",
    "status": "Success",
    "cqv_v4": 8.45,
    "price": 100.0,
    "pe_forward": 12.2315,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TEL",
    "name": "TE Connectivity plc",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.72,
    "f2": 9.25,
    "f3": 8.78,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.75,
    "f7": 5.02,
    "f8": 8.0,
    "momentum_score": 3.01,
    "cqv_v1": 8.49,
    "cqv_v1_1": 8.63,
    "cqv_v2": 8.21,
    "cqv_v3": 8.21,
    "cqv": 8.45,
    "pe": 20.17,
    "close_history": "{'2021': 150.32, '2022': 108.74, '2023': 135.52, '2024': 140.28, '2025': 225.94, '2026': 202.57}",
    "status": "Success",
    "cqv_v4": 8.45,
    "price": 100.0,
    "pe_forward": 17.1445,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "JNJ",
    "name": "Johnson & Johnson",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.78,
    "f2": 9.0,
    "f3": 8.77,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.77,
    "f7": 4.98,
    "f8": 8.0,
    "momentum_score": 9.05,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.66,
    "cqv_v2": 8.23,
    "cqv_v3": 8.23,
    "cqv": 8.45,
    "pe": 30.48,
    "close_history": "{'2021': 150.49, '2022': 159.48, '2023': 145.8, '2024': 138.78, '2025': 204.68, '2026': 249.97}",
    "status": "Success",
    "cqv_v4": 8.45,
    "price": 100.0,
    "pe_forward": 25.908,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SHOP",
    "name": "Shopify Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.47,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.65,
    "f7": 5.22,
    "f8": 8.0,
    "momentum_score": 3.38,
    "cqv_v1": 7.93,
    "cqv_v1_1": 8.62,
    "cqv_v2": 8.2,
    "cqv_v3": 8.2,
    "cqv": 8.45,
    "pe": 117.12,
    "close_history": "{'2021': 137.74, '2022': 34.71, '2023': 77.9, '2024': 106.33, '2025': 160.97, '2026': 125.06}",
    "status": "Success",
    "cqv_v4": 8.45,
    "price": 100.0,
    "pe_forward": 99.552,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TOST",
    "name": "Toast, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.4,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.62,
    "f7": 5.28,
    "f8": 8.0,
    "momentum_score": 3.86,
    "cqv_v1": 8.36,
    "cqv_v1_1": 8.6,
    "cqv_v2": 8.19,
    "cqv_v3": 8.19,
    "cqv": 8.44,
    "pe": 43.01,
    "close_history": "{'2021': 34.71, '2022': 18.03, '2023': 18.26, '2024': 36.45, '2025': 35.51, '2026': 30.33}",
    "status": "Success",
    "cqv_v4": 8.44,
    "price": 100.0,
    "pe_forward": 36.558499999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 117.6,
    "mos_pct": 14.97,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GE",
    "name": "GE Aerospace",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.87,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.1,
    "f7": 4.9,
    "f8": 8.0,
    "momentum_score": 8.85,
    "cqv_v1": 8.07,
    "cqv_v1_1": 8.53,
    "cqv_v2": 8.18,
    "cqv_v3": 8.18,
    "cqv": 8.44,
    "pe": 46.96,
    "close_history": "{'2021': 57.56, '2022': 51.27, '2023': 100.35, '2024': 165.4, '2025': 307.2, '2026': 345.73}",
    "status": "Success",
    "cqv_v4": 8.44,
    "price": 100.0,
    "pe_forward": 39.916,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 117.6,
    "mos_pct": 14.97,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ARM",
    "name": "Arm Holdings plc",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.41,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.62,
    "f7": 5.27,
    "f8": 8.0,
    "momentum_score": 8.04,
    "cqv_v1": 7.83,
    "cqv_v1_1": 8.6,
    "cqv_v2": 8.19,
    "cqv_v3": 8.19,
    "cqv": 8.44,
    "pe": 370.92,
    "close_history": "{'2023': 75.14, '2024': 123.36, '2025': 109.31, '2026': 262.01}",
    "status": "Success",
    "cqv_v4": 8.44,
    "price": 100.0,
    "pe_forward": 315.282,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 117.6,
    "mos_pct": 14.97,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ADI",
    "name": "Analog Devices, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.55,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.69,
    "f7": 5.16,
    "f8": 8.0,
    "momentum_score": 6.73,
    "cqv_v1": 8.22,
    "cqv_v1_1": 8.6,
    "cqv_v2": 8.19,
    "cqv_v3": 8.19,
    "cqv": 8.43,
    "pe": 56.21,
    "close_history": "{'2021': 162.8, '2022': 154.8, '2023': 190.96, '2024': 207.79, '2025': 269.61, '2026': 380.53}",
    "status": "Success",
    "cqv_v4": 8.43,
    "price": 100.0,
    "pe_forward": 47.7785,
    "value_score": 4.32,
    "peg_bruto": 3.7674,
    "score_peg": 3.7674,
    "intrinsic_value": 117.2,
    "mos_pct": 14.68,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.9
  },
  {
    "ticker": "LOGI",
    "name": "Logitech International S.A. - R",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.14,
    "f2": 9.5,
    "f3": 7.99,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.57,
    "f7": 4.69,
    "f8": 8.0,
    "momentum_score": 3.14,
    "cqv_v1": 8.67,
    "cqv_v1_1": 8.66,
    "cqv_v2": 8.19,
    "cqv_v3": 8.19,
    "cqv": 8.42,
    "pe": 19.58,
    "close_history": "{'2021': 77.04, '2022': 59.4, '2023': 92.24, '2024': 81.18, '2025': 100.22, '2026': 100.29}",
    "status": "Success",
    "cqv_v4": 8.42,
    "price": 100.0,
    "pe_forward": 16.642999999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.8,
    "mos_pct": 14.38,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DT",
    "name": "Dynatrace, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.32,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.57,
    "f7": 5.34,
    "f8": 8.0,
    "momentum_score": 5.89,
    "cqv_v1": 8.33,
    "cqv_v1_1": 8.58,
    "cqv_v2": 8.18,
    "cqv_v3": 8.18,
    "cqv": 8.42,
    "pe": 82.91,
    "close_history": "{'2021': 60.35, '2022': 38.3, '2023': 54.69, '2024': 54.35, '2025': 43.34, '2026': 44.83}",
    "status": "Success",
    "cqv_v4": 8.42,
    "price": 100.0,
    "pe_forward": 70.4735,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.8,
    "mos_pct": 14.38,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AMAT",
    "name": "Applied Materials, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.69,
    "f2": 9.5,
    "f3": 8.43,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.56,
    "f7": 5.05,
    "f8": 8.0,
    "momentum_score": 9.3,
    "cqv_v1": 7.88,
    "cqv_v1_1": 8.61,
    "cqv_v2": 8.18,
    "cqv_v3": 8.18,
    "cqv": 8.42,
    "pe": 56.84,
    "close_history": "{'2021': 151.39, '2022': 94.55, '2023': 158.82, '2024': 160.62, '2025': 256.35, '2026': 560.93}",
    "status": "Success",
    "cqv_v4": 8.42,
    "price": 100.0,
    "pe_forward": 48.314,
    "value_score": 4.28,
    "peg_bruto": 3.7256,
    "score_peg": 3.7256,
    "intrinsic_value": 116.8,
    "mos_pct": 14.38,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.8
  },
  {
    "ticker": "GGG",
    "name": "Graco Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.91,
    "f2": 9.0,
    "f3": 7.91,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.91,
    "f7": 4.07,
    "f8": 8.0,
    "momentum_score": 2.44,
    "cqv_v1": 8.53,
    "cqv_v1_1": 8.61,
    "cqv_v2": 8.17,
    "cqv_v3": 8.17,
    "cqv": 8.41,
    "pe": 24.59,
    "close_history": "{'2021': 76.13, '2022': 64.34, '2023': 84.04, '2024': 82.62, '2025': 81.42, '2026': 76.17}",
    "status": "Success",
    "cqv_v4": 8.41,
    "price": 100.0,
    "pe_forward": 20.9015,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.4,
    "mos_pct": 14.09,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "VRSK",
    "name": "Verisk Analytics, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 10.0,
    "f2": 8.63,
    "f3": 8.1,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.05,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 3.39,
    "cqv_v1": 8.51,
    "cqv_v1_1": 8.61,
    "cqv_v2": 8.18,
    "cqv_v3": 8.18,
    "cqv": 8.41,
    "pe": 28.76,
    "close_history": "{'2021': 221.71, '2022': 172.2, '2023': 234.6, '2024': 272.14, '2025': 222.52, '2026': 201.49}",
    "status": "Success",
    "cqv_v4": 8.41,
    "price": 100.0,
    "pe_forward": 24.446,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.4,
    "mos_pct": 14.09,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ABT",
    "name": "Abbott Laboratories",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.79,
    "f2": 9.0,
    "f3": 8.53,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.66,
    "f7": 4.97,
    "f8": 8.0,
    "momentum_score": 3.22,
    "cqv_v1": 8.43,
    "cqv_v1_1": 8.62,
    "cqv_v2": 8.19,
    "cqv_v3": 8.19,
    "cqv": 8.41,
    "pe": 26.72,
    "close_history": "{'2021': 128.19, '2022': 101.68, '2023': 103.98, '2024': 108.98, '2025': 123.0, '2026': 98.83}",
    "status": "Success",
    "cqv_v4": 8.41,
    "price": 100.0,
    "pe_forward": 22.712,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.4,
    "mos_pct": 14.09,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PDD",
    "name": "PDD Holdings Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.13,
    "f2": 9.0,
    "f3": 8.89,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 9.01,
    "f7": 4.7,
    "f8": 8.0,
    "momentum_score": 1.32,
    "cqv_v1": 8.64,
    "cqv_v1_1": 8.48,
    "cqv_v2": 8.13,
    "cqv_v3": 8.13,
    "cqv": 8.4,
    "pe": 8.68,
    "close_history": "{'2021': 58.3, '2022': 81.55, '2023': 146.31, '2024': 96.99, '2025': 113.39, '2026': 86.68}",
    "status": "Success",
    "cqv_v4": 8.4,
    "price": 100.0,
    "pe_forward": 7.377999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.0,
    "mos_pct": 13.79,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NXPI",
    "name": "NXP Semiconductors N.V.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.25,
    "f2": 8.48,
    "f3": 8.52,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.88,
    "f7": 4.6,
    "f8": 8.0,
    "momentum_score": 6.26,
    "cqv_v1": 8.51,
    "cqv_v1_1": 8.61,
    "cqv_v2": 8.19,
    "cqv_v3": 8.19,
    "cqv": 8.4,
    "pe": 26.11,
    "close_history": "{'2021': 209.24, '2022': 148.11, '2023': 219.79, '2024': 202.28, '2025': 215.21, '2026': 270.66}",
    "status": "Success",
    "cqv_v4": 8.4,
    "price": 100.0,
    "pe_forward": 22.1935,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.0,
    "mos_pct": 13.79,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ROST",
    "name": "Ross Stores, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.9,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 9.12,
    "f7": 4.88,
    "f8": 8.0,
    "momentum_score": 6.72,
    "cqv_v1": 8.26,
    "cqv_v1_1": 8.45,
    "cqv_v2": 8.13,
    "cqv_v3": 8.13,
    "cqv": 8.4,
    "pe": 29.85,
    "close_history": "{'2021': 108.69, '2022': 111.88, '2023': 134.98, '2024': 149.0, '2025': 179.41, '2026': 232.72}",
    "status": "Success",
    "cqv_v4": 8.4,
    "price": 100.0,
    "pe_forward": 25.372500000000002,
    "value_score": 5.23,
    "peg_bruto": 7.0943,
    "score_peg": 7.0943,
    "intrinsic_value": 116.0,
    "mos_pct": 13.79,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.6
  },
  {
    "ticker": "AME",
    "name": "AMETEK, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.06,
    "f2": 8.75,
    "f3": 8.92,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.99,
    "f7": 4.75,
    "f8": 8.0,
    "momentum_score": 7.39,
    "cqv_v1": 8.26,
    "cqv_v1_1": 8.52,
    "cqv_v2": 8.15,
    "cqv_v3": 8.15,
    "cqv": 8.4,
    "pe": 35.44,
    "close_history": "{'2021': 142.76, '2022': 136.58, '2023': 162.26, '2024': 178.5, '2025': 204.68, '2026': 237.3}",
    "status": "Success",
    "cqv_v4": 8.4,
    "price": 100.0,
    "pe_forward": 30.124,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 116.0,
    "mos_pct": 13.79,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DELL",
    "name": "Dell Technologies Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.53,
    "f2": 9.08,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.68,
    "f7": 5.18,
    "f8": 8.0,
    "momentum_score": 9.37,
    "cqv_v1": 8.12,
    "cqv_v1_1": 8.57,
    "cqv_v2": 8.17,
    "cqv_v3": 8.17,
    "cqv": 8.4,
    "pe": 31.44,
    "close_history": "{'2021': 51.11, '2022': 37.5, '2023': 73.44, '2024': 112.34, '2025': 124.95, '2026': 391.38}",
    "status": "Success",
    "cqv_v4": 8.4,
    "price": 100.0,
    "pe_forward": 26.724,
    "value_score": 5.12,
    "peg_bruto": 6.7355,
    "score_peg": 6.7355,
    "intrinsic_value": 116.0,
    "mos_pct": 13.79,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.6
  },
  {
    "ticker": "MRVL",
    "name": "Marvell Technology, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.39,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.61,
    "f7": 5.29,
    "f8": 8.0,
    "momentum_score": 8.61,
    "cqv_v1": 7.99,
    "cqv_v1_1": 8.56,
    "cqv_v2": 8.17,
    "cqv_v3": 8.17,
    "cqv": 8.4,
    "pe": 84.0,
    "close_history": "{'2021': 85.96, '2022': 36.55, '2023': 59.82, '2024': 109.94, '2025': 84.86, '2026': 188.3}",
    "status": "Success",
    "cqv_v4": 8.4,
    "price": 100.0,
    "pe_forward": 71.39999999999999,
    "value_score": 3.86,
    "peg_bruto": 2.521,
    "score_peg": 2.521,
    "intrinsic_value": 116.0,
    "mos_pct": 13.79,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.6
  },
  {
    "ticker": "ZTS",
    "name": "Zoetis Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.33,
    "f2": 8.82,
    "f3": 7.99,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.66,
    "f7": 4.54,
    "f8": 8.0,
    "momentum_score": 0.26,
    "cqv_v1": 8.69,
    "cqv_v1_1": 8.65,
    "cqv_v2": 8.19,
    "cqv_v3": 8.19,
    "cqv": 8.39,
    "pe": 12.26,
    "close_history": "{'2021': 232.58, '2022': 140.7, '2023': 191.22, '2024': 159.42, '2025': 124.74, '2026': 76.91}",
    "status": "Success",
    "cqv_v4": 8.39,
    "price": 100.0,
    "pe_forward": 10.421,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 115.6,
    "mos_pct": 13.49,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ROL",
    "name": "Rollins, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.37,
    "f2": 8.75,
    "f3": 8.8,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 9.09,
    "f7": 4.5,
    "f8": 8.0,
    "momentum_score": 0.7,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.48,
    "cqv_v2": 8.13,
    "cqv_v3": 8.13,
    "cqv": 8.39,
    "pe": 39.8,
    "close_history": "{'2021': 32.29, '2022': 34.91, '2023': 42.3, '2024': 45.5, '2025': 59.64, '2026': 45.46}",
    "status": "Success",
    "cqv_v4": 8.39,
    "price": 100.0,
    "pe_forward": 33.83,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 115.6,
    "mos_pct": 13.49,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DECK",
    "name": "Deckers Outdoor Corporation",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.21,
    "f2": 9.0,
    "f3": 8.73,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.97,
    "f7": 4.63,
    "f8": 8.0,
    "momentum_score": 5.44,
    "cqv_v1": 8.64,
    "cqv_v1_1": 8.47,
    "cqv_v2": 8.12,
    "cqv_v3": 8.12,
    "cqv": 8.38,
    "pe": 14.91,
    "close_history": "{'2021': 61.05, '2022': 66.53, '2023': 111.4, '2024': 203.09, '2025': 103.67, '2026': 109.03}",
    "status": "Success",
    "cqv_v4": 8.38,
    "price": 100.0,
    "pe_forward": 12.6735,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 115.2,
    "mos_pct": 13.19,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "YUM",
    "name": "Yum! Brands, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.16,
    "f2": 8.25,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 9.25,
    "f7": 4.67,
    "f8": 8.0,
    "momentum_score": 6.97,
    "cqv_v1": 8.27,
    "cqv_v1_1": 8.44,
    "cqv_v2": 8.12,
    "cqv_v3": 8.12,
    "cqv": 8.38,
    "pe": 26.61,
    "close_history": "{'2021': 127.41, '2022': 119.78, '2023': 124.49, '2024': 130.37, '2025': 149.85, '2026': 152.1}",
    "status": "Success",
    "cqv_v4": 8.38,
    "price": 100.0,
    "pe_forward": 22.618499999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 115.2,
    "mos_pct": 13.19,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GEV",
    "name": "GE Vernova Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.39,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.86,
    "f7": 5.29,
    "f8": 8.0,
    "momentum_score": 9.01,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.45,
    "cqv_v2": 8.13,
    "cqv_v3": 8.13,
    "cqv": 8.38,
    "pe": 32.58,
    "close_history": "{'2024': 327.79, '2025': 652.36, '2026': 1036.22}",
    "status": "Success",
    "cqv_v4": 8.38,
    "price": 100.0,
    "pe_forward": 27.692999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 115.2,
    "mos_pct": 13.19,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CB",
    "name": "Chubb Limited",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.85,
    "f2": 9.0,
    "f3": 8.8,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.82,
    "f7": 4.92,
    "f8": 8.0,
    "momentum_score": 8.53,
    "cqv_v1": 8.64,
    "cqv_v1_1": 8.48,
    "cqv_v2": 8.12,
    "cqv_v3": 8.12,
    "cqv": 8.37,
    "pe": 12.77,
    "close_history": "{'2021': 182.26, '2022': 211.37, '2023': 220.24, '2024': 272.85, '2025': 310.23, '2026': 343.7}",
    "status": "Success",
    "cqv_v4": 8.37,
    "price": 100.0,
    "pe_forward": 10.8545,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 114.8,
    "mos_pct": 12.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DOCU",
    "name": "DocuSign, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.77,
    "f2": 9.5,
    "f3": 8.13,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.45,
    "f7": 4.98,
    "f8": 8.0,
    "momentum_score": 1.82,
    "cqv_v1": 8.64,
    "cqv_v1_1": 8.59,
    "cqv_v2": 8.15,
    "cqv_v3": 8.15,
    "cqv": 8.37,
    "pe": 29.72,
    "close_history": "{'2021': 152.31, '2022': 55.42, '2023': 59.45, '2024': 89.94, '2025': 68.4, '2026': 53.38}",
    "status": "Success",
    "cqv_v4": 8.37,
    "price": 100.0,
    "pe_forward": 25.261999999999997,
    "value_score": 5.15,
    "peg_bruto": 7.1253,
    "score_peg": 7.1253,
    "intrinsic_value": 114.8,
    "mos_pct": 12.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.3
  },
  {
    "ticker": "STE",
    "name": "STERIS plc (Ireland)",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.66,
    "f2": 9.0,
    "f3": 8.48,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.57,
    "f7": 5.07,
    "f8": 8.0,
    "momentum_score": 3.33,
    "cqv_v1": 8.41,
    "cqv_v1_1": 8.58,
    "cqv_v2": 8.16,
    "cqv_v3": 8.16,
    "cqv": 8.37,
    "pe": 27.55,
    "close_history": "{'2021': 233.56, '2022': 178.83, '2023': 214.94, '2024': 202.9, '2025': 252.13, '2026': 221.13}",
    "status": "Success",
    "cqv_v4": 8.37,
    "price": 100.0,
    "pe_forward": 23.4175,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 114.8,
    "mos_pct": 12.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FAST",
    "name": "Fastenal Company",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.81,
    "f2": 8.75,
    "f3": 9.04,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.93,
    "f7": 4.95,
    "f8": 8.0,
    "momentum_score": 7.2,
    "cqv_v1": 8.05,
    "cqv_v1_1": 8.47,
    "cqv_v2": 8.13,
    "cqv_v3": 8.13,
    "cqv": 8.37,
    "pe": 43.01,
    "close_history": "{'2021': 28.69, '2022': 21.71, '2023': 30.68, '2024': 34.83, '2025': 39.7, '2026': 46.67}",
    "status": "Success",
    "cqv_v4": 8.37,
    "price": 100.0,
    "pe_forward": 36.558499999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 114.8,
    "mos_pct": 12.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WTS",
    "name": "Watts Water Technologies, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.37,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.85,
    "f7": 5.3,
    "f8": 8.0,
    "momentum_score": 8.94,
    "cqv_v1": 7.99,
    "cqv_v1_1": 8.44,
    "cqv_v2": 8.12,
    "cqv_v3": 8.12,
    "cqv": 8.37,
    "pe": 33.66,
    "close_history": "{'2021': 187.4, '2022': 142.28, '2023': 204.27, '2024': 200.97, '2025': 275.02, '2026': 353.4}",
    "status": "Success",
    "cqv_v4": 8.37,
    "price": 100.0,
    "pe_forward": 28.610999999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 114.8,
    "mos_pct": 12.89,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EXPO",
    "name": "Exponent, Inc.",
    "sector": "Industrials",
    "quarter": "Q2 2026",
    "f1": 8.76,
    "f2": 9.0,
    "f3": 8.83,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.79,
    "f7": 4.99,
    "f8": 8.0,
    "momentum_score": 3.17,
    "cqv_v1": 8.19,
    "cqv_v1_1": 8.46,
    "cqv_v2": 8.11,
    "cqv_v3": 8.11,
    "cqv": 8.36,
    "pe": 28.5,
    "close_history": "{'2021': 110.1, '2022': 94.41, '2023': 84.83, '2024': 86.89, '2025': 68.81, '2026': 64.41}",
    "status": "Success",
    "cqv_v4": 8.36,
    "price": 100.0,
    "pe_forward": 24.224999999999998,
    "value_score": 5.21,
    "peg_bruto": 7.4303,
    "score_peg": 7.4303,
    "intrinsic_value": 114.4,
    "mos_pct": 12.59,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.2
  },
  {
    "ticker": "TDG",
    "name": "Transdigm Group Incorporated",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.82,
    "f2": 8.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.07,
    "f7": 4.94,
    "f8": 8.0,
    "momentum_score": 5.44,
    "cqv_v1": 8.17,
    "cqv_v1_1": 8.44,
    "cqv_v2": 8.12,
    "cqv_v3": 8.12,
    "cqv": 8.36,
    "pe": 42.02,
    "close_history": "{'2021': 528.84, '2022': 538.22, '2023': 896.49, '2024': 1185.75, '2025': 1329.85, '2026': 1231.11}",
    "status": "Success",
    "cqv_v4": 8.36,
    "price": 100.0,
    "pe_forward": 35.717,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 114.4,
    "mos_pct": 12.59,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GDDY",
    "name": "GoDaddy Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.55,
    "f2": 8.66,
    "f3": 7.84,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.7,
    "f7": 4.36,
    "f8": 8.0,
    "momentum_score": 2.66,
    "cqv_v1": 8.71,
    "cqv_v1_1": 8.61,
    "cqv_v2": 8.16,
    "cqv_v3": 8.16,
    "cqv": 8.35,
    "pe": 14.03,
    "close_history": "{'2021': 84.86, '2022': 74.82, '2023': 106.16, '2024': 197.37, '2025': 124.08, '2026': 96.21}",
    "status": "Success",
    "cqv_v4": 8.35,
    "price": 100.0,
    "pe_forward": 11.9255,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 114.0,
    "mos_pct": 12.28,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AMGN",
    "name": "Amgen Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 9.46,
    "f2": 7.93,
    "f3": 8.31,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.89,
    "f7": 4.43,
    "f8": 8.0,
    "momentum_score": 7.54,
    "cqv_v1": 8.44,
    "cqv_v1_1": 8.6,
    "cqv_v2": 8.17,
    "cqv_v3": 8.17,
    "cqv": 8.34,
    "pe": 26.04,
    "close_history": "{'2021': 195.31, '2022': 235.21, '2023': 266.88, '2024': 248.81, '2025': 322.62, '2026': 371.58}",
    "status": "Success",
    "cqv_v4": 8.34,
    "price": 100.0,
    "pe_forward": 22.134,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.6,
    "mos_pct": 11.97,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CRVL",
    "name": "CorVel Corp.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.85,
    "f2": 9.25,
    "f3": 8.49,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.67,
    "f7": 4.92,
    "f8": 8.0,
    "momentum_score": 4.5,
    "cqv_v1": 8.22,
    "cqv_v1_1": 8.47,
    "cqv_v2": 8.1,
    "cqv_v3": 8.1,
    "cqv": 8.34,
    "pe": 29.97,
    "close_history": "{'2021': 69.33, '2022': 48.44, '2023': 82.4, '2024': 111.26, '2025': 67.67, '2026': 62.25}",
    "status": "Success",
    "cqv_v4": 8.34,
    "price": 100.0,
    "pe_forward": 25.4745,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.6,
    "mos_pct": 11.97,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "XPEL",
    "name": "XPEL, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.66,
    "f2": 9.0,
    "f3": 9.12,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.89,
    "f7": 5.07,
    "f8": 8.0,
    "momentum_score": 6.8,
    "cqv_v1": 8.2,
    "cqv_v1_1": 8.39,
    "cqv_v2": 8.08,
    "cqv_v3": 8.08,
    "cqv": 8.34,
    "pe": 25.4,
    "close_history": "{'2021': 68.28, '2022': 60.06, '2023': 53.85, '2024': 39.94, '2025': 49.91, '2026': 45.61}",
    "status": "Success",
    "cqv_v4": 8.34,
    "price": 100.0,
    "pe_forward": 21.59,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.6,
    "mos_pct": 11.97,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CRWD",
    "name": "CrowdStrike Holdings, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 7.93,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.38,
    "f7": 5.66,
    "f8": 8.0,
    "momentum_score": 9.85,
    "cqv_v1": 7.65,
    "cqv_v1_1": 8.48,
    "cqv_v2": 8.11,
    "cqv_v3": 8.11,
    "cqv": 8.34,
    "pe": 124.21,
    "close_history": "{'2021': 51.19, '2022': 26.32, '2023': 63.83, '2024': 85.54, '2025': 117.19, '2026': 203.76}",
    "status": "Success",
    "cqv_v4": 8.34,
    "price": 100.0,
    "pe_forward": 105.57849999999999,
    "value_score": 3.43,
    "peg_bruto": 1.7049,
    "score_peg": 1.7049,
    "intrinsic_value": 113.6,
    "mos_pct": 11.97,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 4.0
  },
  {
    "ticker": "BR",
    "name": "Broadridge Financial Solutions,",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.1,
    "f2": 8.89,
    "f3": 8.03,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.56,
    "f7": 4.72,
    "f8": 8.0,
    "momentum_score": 1.43,
    "cqv_v1": 8.68,
    "cqv_v1_1": 8.56,
    "cqv_v2": 8.13,
    "cqv_v3": 8.13,
    "cqv": 8.33,
    "pe": 15.38,
    "close_history": "{'2021': 168.65, '2022': 126.04, '2023': 196.92, '2024': 219.86, '2025': 220.46, '2026': 153.06}",
    "status": "Success",
    "cqv_v4": 8.33,
    "price": 100.0,
    "pe_forward": 13.073,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.2,
    "mos_pct": 11.66,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SSD",
    "name": "Simpson Manufacturing Company, ",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 8.98,
    "f2": 8.75,
    "f3": 8.68,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.83,
    "f7": 4.82,
    "f8": 8.0,
    "momentum_score": 7.34,
    "cqv_v1": 8.4,
    "cqv_v1_1": 8.46,
    "cqv_v2": 8.1,
    "cqv_v3": 8.1,
    "cqv": 8.33,
    "pe": 23.59,
    "close_history": "{'2021': 134.04, '2022': 86.3, '2023': 194.48, '2024': 163.86, '2025': 160.67, '2026': 193.8}",
    "status": "Success",
    "cqv_v4": 8.33,
    "price": 100.0,
    "pe_forward": 20.0515,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.2,
    "mos_pct": 11.66,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EBAY",
    "name": "eBay Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.75,
    "f2": 8.48,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 9.04,
    "f7": 5.0,
    "f8": 8.0,
    "momentum_score": 8.54,
    "cqv_v1": 8.12,
    "cqv_v1_1": 8.37,
    "cqv_v2": 8.07,
    "cqv_v3": 8.07,
    "cqv": 8.33,
    "pe": 26.52,
    "close_history": "{'2021': 61.24, '2022': 38.91, '2023': 41.89, '2024': 60.65, '2025': 86.57, '2026': 110.91}",
    "status": "Success",
    "cqv_v4": 8.33,
    "price": 100.0,
    "pe_forward": 22.541999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.2,
    "mos_pct": 11.66,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EME",
    "name": "EMCOR Group, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.15,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.74,
    "f7": 5.48,
    "f8": 8.0,
    "momentum_score": 5.65,
    "cqv_v1": 8.1,
    "cqv_v1_1": 8.39,
    "cqv_v2": 8.09,
    "cqv_v3": 8.09,
    "cqv": 8.33,
    "pe": 26.04,
    "close_history": "{'2021': 125.5, '2022': 146.59, '2023': 214.07, '2024': 452.28, '2025': 610.81, '2026': 750.04}",
    "status": "Success",
    "cqv_v4": 8.33,
    "price": 100.0,
    "pe_forward": 22.134,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.2,
    "mos_pct": 11.66,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CELH",
    "name": "Celsius Holdings, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 8.56,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.95,
    "f7": 5.15,
    "f8": 8.0,
    "momentum_score": 2.61,
    "cqv_v1": 8.05,
    "cqv_v1_1": 8.36,
    "cqv_v2": 8.07,
    "cqv_v3": 8.07,
    "cqv": 8.33,
    "pe": 77.12,
    "close_history": "{'2021': 24.86, '2022': 34.68, '2023': 54.52, '2024': 26.34, '2025': 45.74, '2026': 29.98}",
    "status": "Success",
    "cqv_v4": 8.33,
    "price": 100.0,
    "pe_forward": 65.552,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 113.2,
    "mos_pct": 11.66,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HRB",
    "name": "H&R Block, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.55,
    "f2": 8.75,
    "f3": 8.26,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.91,
    "f7": 4.36,
    "f8": 8.0,
    "momentum_score": 4.8,
    "cqv_v1": 8.65,
    "cqv_v1_1": 8.45,
    "cqv_v2": 8.08,
    "cqv_v3": 8.08,
    "cqv": 8.32,
    "pe": 7.16,
    "close_history": "{'2021': 20.37, '2022': 32.59, '2023': 44.6, '2024': 49.97, '2025': 42.53, '2026': 41.85}",
    "status": "Success",
    "cqv_v4": 8.32,
    "price": 100.0,
    "pe_forward": 6.086,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.8,
    "mos_pct": 11.35,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "JBL",
    "name": "Jabil Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.38,
    "f2": 9.25,
    "f3": 8.48,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.43,
    "f7": 5.3,
    "f8": 8.0,
    "momentum_score": 7.12,
    "cqv_v1": 8.26,
    "cqv_v1_1": 8.5,
    "cqv_v2": 8.11,
    "cqv_v3": 8.11,
    "cqv": 8.32,
    "pe": 42.72,
    "close_history": "{'2021': 69.41, '2022': 67.64, '2023': 126.78, '2024': 143.58, '2025': 227.9, '2026': 307.06}",
    "status": "Success",
    "cqv_v4": 8.32,
    "price": 100.0,
    "pe_forward": 36.312,
    "value_score": 4.34,
    "peg_bruto": 4.957,
    "score_peg": 4.957,
    "intrinsic_value": 112.8,
    "mos_pct": 11.35,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 3.77
  },
  {
    "ticker": "FLEX",
    "name": "Flex Ltd.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.01,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.42,
    "f7": 5.59,
    "f8": 8.0,
    "momentum_score": 8.12,
    "cqv_v1": 7.98,
    "cqv_v1_1": 8.46,
    "cqv_v2": 8.1,
    "cqv_v3": 8.1,
    "cqv": 8.32,
    "pe": 58.74,
    "close_history": "{'2021': 13.81, '2022': 16.17, '2023': 22.95, '2024': 38.39, '2025': 60.42, '2026': 121.12}",
    "status": "Success",
    "cqv_v4": 8.32,
    "price": 100.0,
    "pe_forward": 49.929,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.8,
    "mos_pct": 11.35,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FTDR",
    "name": "frontdoor, inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.46,
    "f2": 8.75,
    "f3": 8.32,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.89,
    "f7": 4.43,
    "f8": 8.0,
    "momentum_score": 9.55,
    "cqv_v1": 8.62,
    "cqv_v1_1": 8.44,
    "cqv_v2": 8.07,
    "cqv_v3": 8.07,
    "cqv": 8.31,
    "pe": 22.43,
    "close_history": "{'2021': 36.65, '2022': 20.8, '2023': 35.22, '2024': 54.67, '2025': 57.69, '2026': 75.85}",
    "status": "Success",
    "cqv_v4": 8.31,
    "price": 100.0,
    "pe_forward": 19.0655,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.4,
    "mos_pct": 11.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "QCOM",
    "name": "QUALCOMM Incorporated",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.94,
    "f2": 9.25,
    "f3": 6.78,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.36,
    "f7": 4.05,
    "f8": 8.0,
    "momentum_score": 4.8,
    "cqv_v1": 8.6,
    "cqv_v1_1": 8.64,
    "cqv_v2": 8.12,
    "cqv_v3": 8.12,
    "cqv": 8.31,
    "pe": 18.93,
    "close_history": "{'2021': 165.55, '2022': 101.67, '2023': 137.34, '2024': 148.75, '2025': 169.33, '2026': 170.61}",
    "status": "Success",
    "cqv_v4": 8.31,
    "price": 100.0,
    "pe_forward": 16.0905,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.4,
    "mos_pct": 11.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RELX",
    "name": "RELX PLC PLC",
    "sector": "Industrials",
    "quarter": "Q4 2025",
    "f1": 10.0,
    "f2": 8.36,
    "f3": 7.8,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.9,
    "f7": 4.0,
    "f8": 8.0,
    "momentum_score": 1.94,
    "cqv_v1": 8.44,
    "cqv_v1_1": 8.52,
    "cqv_v2": 8.11,
    "cqv_v3": 8.11,
    "cqv": 8.31,
    "pe": 21.57,
    "close_history": "{'2021': 29.61, '2022': 25.75, '2023': 37.61, '2024': 43.85, '2025': 39.64, '2026': 34.02}",
    "status": "Success",
    "cqv_v4": 8.31,
    "price": 100.0,
    "pe_forward": 18.3345,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.4,
    "mos_pct": 11.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PH",
    "name": "Parker-Hannifin Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.81,
    "f2": 8.64,
    "f3": 8.84,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.82,
    "f7": 4.95,
    "f8": 8.0,
    "momentum_score": 6.98,
    "cqv_v1": 8.15,
    "cqv_v1_1": 8.42,
    "cqv_v2": 8.09,
    "cqv_v3": 8.09,
    "cqv": 8.31,
    "pe": 35.58,
    "close_history": "{'2021': 300.22, '2022': 279.47, '2023': 449.42, '2024': 627.3, '2025': 875.35, '2026': 958.34}",
    "status": "Success",
    "cqv_v4": 8.31,
    "price": 100.0,
    "pe_forward": 30.243,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.4,
    "mos_pct": 11.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TKO",
    "name": "TKO Group Holdings, Inc.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 8.85,
    "f2": 7.9,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.09,
    "f7": 4.92,
    "f8": 8.0,
    "momentum_score": 4.94,
    "cqv_v1": 8.11,
    "cqv_v1_1": 8.4,
    "cqv_v2": 8.09,
    "cqv_v3": 8.09,
    "cqv": 8.31,
    "pe": 72.54,
    "close_history": "{'2021': 46.09, '2022': 64.49, '2023': 79.93, '2024': 139.23, '2025': 207.35, '2026': 184.78}",
    "status": "Success",
    "cqv_v4": 8.31,
    "price": 100.0,
    "pe_forward": 61.659000000000006,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.4,
    "mos_pct": 11.03,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AON",
    "name": "Aon plc",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.42,
    "f2": 8.3,
    "f3": 8.39,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.91,
    "f7": 4.46,
    "f8": 8.0,
    "momentum_score": 6.34,
    "cqv_v1": 8.45,
    "cqv_v1_1": 8.46,
    "cqv_v2": 8.09,
    "cqv_v3": 8.09,
    "cqv": 8.3,
    "pe": 19.61,
    "close_history": "{'2021': 291.08, '2022': 292.86, '2023': 286.11, '2024': 356.06, '2025': 351.95, '2026': 368.63}",
    "status": "Success",
    "cqv_v4": 8.3,
    "price": 100.0,
    "pe_forward": 16.668499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 112.0,
    "mos_pct": 10.71,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LPLA",
    "name": "LPL Financial Holdings Inc.",
    "sector": "Financials / Independent Broker-Dealer & Wealth Management Advisory Services",
    "quarter": "Q2 2026",
    "f1": 8.2,
    "f2": 8.2,
    "f3": 8.7,
    "f4": 8.5,
    "f5": 8.2,
    "f6": 8.0,
    "f7": 7.5,
    "f8": 8.4,
    "momentum_score": null,
    "cqv_v1": null,
    "cqv_v1_1": null,
    "cqv_v2": null,
    "cqv_v3": null,
    "cqv": 8.29,
    "pe": 28.19,
    "close_history": {
      "2020": 105.0,
      "2021": 156.83,
      "2022": 212.8,
      "2023": 225.33,
      "2024": 240.0,
      "2025": 235.0,
      "2026": 353.7
    },
    "status": "Q2 2026 actualizado con resultados oficiales; valoracion incompleta N/D",
    "price": 353.7,
    "pe_forward": 15.25,
    "value_score": null,
    "peg_bruto": 11.0426,
    "score_peg": 10.0,
    "intrinsic_value": null,
    "mos_pct": null,
    "verdict": "N/D - valoración incompleta",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.5,
    "data_confidence": "Media",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "valuation_date": "31/07/2026",
    "period_end": "2026-06-30",
    "eps_trailing": 12.55,
    "eps_forward": 23.47,
    "eps_growth_ntm_pct": 16.84,
    "growth_eps": 16.84,
    "market_cap": 28000.0,
    "market_cap_b": 28.0,
    "ocf": null,
    "ocf_ttm_m": null,
    "maintenance_capex": null,
    "maint_capex_m": null,
    "owner_earnings_m": null,
    "capex_total": 199.4,
    "capex_period": "Q2 2026; USD millones",
    "score_fcf_yield": null,
    "score_mos": null,
    "wacc": null,
    "g_terminal": null,
    "analyst_targets": {
      "target_low_bear": 372.0,
      "target_mean_base": 428.36,
      "target_high_bull": 543.0,
      "num_analysts": 16,
      "consensus_recommendation": "Buy",
      "upside_potential_pct": 21.11
    },
    "sources": [
      "https://www.globenewswire.com/news-release/2026/07/30/3336423/29579/en/LPL-Financial-Announces-Second-Quarter-2026-Results.html",
      "https://investor.lpl.com/events/event-details/lpl-financial-holdings-second-quarter-2026-earnings-call",
      "https://stockanalysis.com/stocks/lpla/",
      "https://stockanalysis.com/stocks/lpla/forecast/",
      "https://stockanalysis.com/stocks/lpla/financials/"
    ],
    "dcf_status": "N/D: no se documentaron supuestos DCF reproducibles; OCF y Maintenance CapEx no quedan verificados en la fuente de resultados",
    "cqv_v4": 8.29
  },
  {
    "ticker": "TT",
    "name": "Trane Technologies plc",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.08,
    "f2": 8.75,
    "f3": 8.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.71,
    "f7": 4.74,
    "f8": 8.0,
    "momentum_score": 7.37,
    "cqv_v1": 8.16,
    "cqv_v1_1": 8.43,
    "cqv_v2": 8.07,
    "cqv_v3": 8.07,
    "cqv": 8.29,
    "pe": 36.5,
    "close_history": "{'2021': 191.21, '2022': 161.88, '2023': 238.6, '2024': 364.98, '2025': 387.38, '2026': 475.0}",
    "status": "Success",
    "cqv_v4": 8.29,
    "price": 100.0,
    "pe_forward": 31.025,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 111.6,
    "mos_pct": 10.39,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DOCN",
    "name": "DigitalOcean Holdings, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.25,
    "f2": 8.75,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.54,
    "f7": 5.4,
    "f8": 8.0,
    "momentum_score": 6.62,
    "cqv_v1": 7.93,
    "cqv_v1_1": 8.45,
    "cqv_v2": 8.09,
    "cqv_v3": 8.09,
    "cqv": 8.29,
    "pe": 57.07,
    "close_history": "{'2021': 80.33, '2022': 25.47, '2023': 36.69, '2024': 34.07, '2025': 48.12, '2026': 117.02}",
    "status": "Success",
    "cqv_v4": 8.29,
    "price": 100.0,
    "pe_forward": 48.509499999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 111.6,
    "mos_pct": 10.39,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SPSC",
    "name": "SPS Commerce, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.65,
    "f2": 9.5,
    "f3": 7.81,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.23,
    "f7": 5.08,
    "f8": 8.0,
    "momentum_score": 2.51,
    "cqv_v1": 8.57,
    "cqv_v1_1": 8.51,
    "cqv_v2": 8.08,
    "cqv_v3": 8.08,
    "cqv": 8.28,
    "pe": 24.51,
    "close_history": "{'2021': 142.35, '2022': 128.43, '2023': 193.84, '2024': 183.99, '2025': 89.13, '2026': 65.75}",
    "status": "Success",
    "cqv_v4": 8.28,
    "price": 100.0,
    "pe_forward": 20.8335,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 111.2,
    "mos_pct": 10.07,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TYL",
    "name": "Tyler Technologies, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.36,
    "f2": 9.5,
    "f3": 8.12,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.24,
    "f7": 5.31,
    "f8": 8.0,
    "momentum_score": 2.27,
    "cqv_v1": 8.31,
    "cqv_v1_1": 8.48,
    "cqv_v2": 8.08,
    "cqv_v3": 8.08,
    "cqv": 8.28,
    "pe": 43.88,
    "close_history": "{'2021': 537.95, '2022': 322.41, '2023': 418.12, '2024': 576.64, '2025': 453.95, '2026': 316.23}",
    "status": "Success",
    "cqv_v4": 8.28,
    "price": 100.0,
    "pe_forward": 37.298,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 111.2,
    "mos_pct": 10.07,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TCEHY",
    "name": "TENCENT",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 8.72,
    "f2": 8.75,
    "f3": 8.68,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.7,
    "f7": 5.02,
    "f8": 8.0,
    "momentum_score": 1.4,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.39,
    "cqv_v2": 8.06,
    "cqv_v3": 8.06,
    "cqv": 8.28,
    "pe": 15.48,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.28,
    "price": 100.0,
    "pe_forward": 13.158,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 111.2,
    "mos_pct": 10.07,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IEX",
    "name": "IDEX Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.75,
    "f2": 8.75,
    "f3": 8.66,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.71,
    "f7": 5.0,
    "f8": 8.0,
    "momentum_score": 8.26,
    "cqv_v1": 8.22,
    "cqv_v1_1": 8.4,
    "cqv_v2": 8.06,
    "cqv_v3": 8.06,
    "cqv": 8.28,
    "pe": 33.11,
    "close_history": "{'2021': 222.13, '2022': 217.19, '2023': 208.94, '2024': 204.02, '2025': 176.09, '2026': 228.96}",
    "status": "Success",
    "cqv_v4": 8.28,
    "price": 100.0,
    "pe_forward": 28.1435,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 111.2,
    "mos_pct": 10.07,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ATR",
    "name": "AptarGroup, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 7.87,
    "f2": 9.0,
    "f3": 8.87,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.37,
    "f7": 5.7,
    "f8": 8.0,
    "momentum_score": 4.76,
    "cqv_v1": 8.17,
    "cqv_v1_1": 8.44,
    "cqv_v2": 8.09,
    "cqv_v3": 8.09,
    "cqv": 8.28,
    "pe": 21.74,
    "close_history": "{'2021': 115.43, '2022': 105.12, '2023': 119.73, '2024': 153.97, '2025': 121.01, '2026': 134.75}",
    "status": "Success",
    "cqv_v4": 8.28,
    "price": 100.0,
    "pe_forward": 18.479,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 111.2,
    "mos_pct": 10.07,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TTC",
    "name": "Toro Company (The)",
    "sector": "Industrials",
    "quarter": "Q2 2026",
    "f1": 8.78,
    "f2": 8.75,
    "f3": 8.57,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.68,
    "f7": 4.98,
    "f8": 8.0,
    "momentum_score": 7.48,
    "cqv_v1": 8.57,
    "cqv_v1_1": 8.39,
    "cqv_v2": 8.05,
    "cqv_v3": 8.05,
    "cqv": 8.27,
    "pe": 27.98,
    "close_history": "{'2021': 92.76, '2022': 106.56, '2023': 91.68, '2024': 77.78, '2025': 78.04, '2026': 95.91}",
    "status": "Success",
    "cqv_v4": 8.27,
    "price": 100.0,
    "pe_forward": 23.783,
    "value_score": 4.96,
    "peg_bruto": 7.5684,
    "score_peg": 7.5684,
    "intrinsic_value": 110.8,
    "mos_pct": 9.75,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 3.23
  },
  {
    "ticker": "ULTA",
    "name": "Ulta Beauty, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.68,
    "f2": 8.75,
    "f3": 8.9,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.79,
    "f7": 5.06,
    "f8": 8.0,
    "momentum_score": 1.37,
    "cqv_v1": 8.38,
    "cqv_v1_1": 8.33,
    "cqv_v2": 8.03,
    "cqv_v3": 8.03,
    "cqv": 8.27,
    "pe": 17.3,
    "close_history": "{'2021': 412.34, '2022': 469.07, '2023': 489.99, '2024': 434.93, '2025': 605.01, '2026': 478.86}",
    "status": "Success",
    "cqv_v4": 8.27,
    "price": 100.0,
    "pe_forward": 14.705,
    "value_score": 5.69,
    "peg_bruto": 12.2407,
    "score_peg": 10.0,
    "intrinsic_value": 110.8,
    "mos_pct": 9.75,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 3.23
  },
  {
    "ticker": "MCD",
    "name": "McDonald's Corporation",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.23,
    "f2": 8.25,
    "f3": 8.71,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.97,
    "f7": 4.62,
    "f8": 8.0,
    "momentum_score": 3.06,
    "cqv_v1": 8.21,
    "cqv_v1_1": 8.36,
    "cqv_v2": 8.04,
    "cqv_v3": 8.04,
    "cqv": 8.27,
    "pe": 23.15,
    "close_history": "{'2021': 241.62, '2022': 242.85, '2023': 279.43, '2024': 279.81, '2025': 301.88, '2026': 273.46}",
    "status": "Success",
    "cqv_v4": 8.27,
    "price": 100.0,
    "pe_forward": 19.6775,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 110.8,
    "mos_pct": 9.75,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ROK",
    "name": "Rockwell Automation, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.85,
    "f2": 8.05,
    "f3": 8.99,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.92,
    "f7": 4.92,
    "f8": 8.0,
    "momentum_score": 8.28,
    "cqv_v1": 8.05,
    "cqv_v1_1": 8.37,
    "cqv_v2": 8.06,
    "cqv_v3": 8.06,
    "cqv": 8.26,
    "pe": 49.03,
    "close_history": "{'2021': 322.95, '2022': 242.94, '2023': 297.92, '2024': 279.36, '2025': 386.53, '2026': 468.67}",
    "status": "Success",
    "cqv_v4": 8.26,
    "price": 100.0,
    "pe_forward": 41.6755,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 110.4,
    "mos_pct": 9.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AMD",
    "name": "Advanced Micro Devices, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.55,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.19,
    "f7": 5.96,
    "f8": 8.0,
    "momentum_score": 9.34,
    "cqv_v1": 7.75,
    "cqv_v1_1": 8.39,
    "cqv_v2": 8.05,
    "cqv_v3": 8.05,
    "cqv": 8.26,
    "pe": 172.61,
    "close_history": "{'2021': 143.9, '2022': 64.77, '2023': 147.41, '2024': 120.79, '2025': 214.16, '2026': 500.94}",
    "status": "Success",
    "cqv_v4": 8.26,
    "price": 100.0,
    "pe_forward": 146.7185,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 110.4,
    "mos_pct": 9.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "INMD",
    "name": "InMode Ltd.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.14,
    "f2": 9.25,
    "f3": 8.26,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.2,
    "f7": 5.49,
    "f8": 8.0,
    "momentum_score": 5.41,
    "cqv_v1": 8.39,
    "cqv_v1_1": 8.46,
    "cqv_v2": 8.07,
    "cqv_v3": 8.07,
    "cqv": 8.25,
    "pe": 10.96,
    "close_history": "{'2021': 70.58, '2022': 35.7, '2023': 22.24, '2024': 16.7, '2025': 14.69, '2026': 15.6}",
    "status": "Success",
    "cqv_v4": 8.25,
    "price": 100.0,
    "pe_forward": 9.316,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 110.0,
    "mos_pct": 9.09,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "COST",
    "name": "Costco Wholesale Corporation",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2021",
    "f1": 8.01,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.67,
    "f7": 5.59,
    "f8": 8.0,
    "momentum_score": 4.78,
    "cqv_v1": 7.86,
    "cqv_v1_1": 8.26,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.25,
    "pe": 47.85,
    "close_history": "{'2021': 539.72, '2022': 436.91, '2023': 651.01, '2024': 908.95, '2025': 859.92, '2026': 945.57}",
    "status": "Success",
    "cqv_v4": 8.25,
    "price": 100.0,
    "pe_forward": 40.6725,
    "value_score": 3.96,
    "peg_bruto": 4.4256,
    "score_peg": 4.4256,
    "intrinsic_value": 110.0,
    "mos_pct": 9.09,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 3.03
  },
  {
    "ticker": "MMSI",
    "name": "Merit Medical Systems, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.03,
    "f2": 9.0,
    "f3": 8.5,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.27,
    "f7": 5.58,
    "f8": 8.0,
    "momentum_score": 3.86,
    "cqv_v1": 8.29,
    "cqv_v1_1": 8.43,
    "cqv_v2": 8.06,
    "cqv_v3": 8.06,
    "cqv": 8.24,
    "pe": 30.64,
    "close_history": "{'2021': 62.3, '2022': 70.62, '2023': 75.96, '2024': 96.72, '2025': 88.14, '2026': 75.55}",
    "status": "Success",
    "cqv_v4": 8.24,
    "price": 100.0,
    "pe_forward": 26.044,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.6,
    "mos_pct": 8.76,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TTWO",
    "name": "Take-Two Interactive Software, ",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 8.84,
    "f2": 8.75,
    "f3": 8.34,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.59,
    "f7": 4.93,
    "f8": 8.0,
    "momentum_score": 7.41,
    "cqv_v1": 8.02,
    "cqv_v1_1": 8.37,
    "cqv_v2": 8.03,
    "cqv_v3": 8.03,
    "cqv": 8.24,
    "pe": 25.58,
    "close_history": "{'2021': 177.72, '2022': 104.13, '2023': 160.95, '2024': 184.08, '2025': 256.03, '2026': 239.57}",
    "status": "Success",
    "cqv_v4": 8.24,
    "price": 100.0,
    "pe_forward": 21.743,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.6,
    "mos_pct": 8.76,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HOOD",
    "name": "Robinhood Markets, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.96,
    "f2": 7.31,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 9.14,
    "f7": 4.83,
    "f8": 8.0,
    "momentum_score": 5.8,
    "cqv_v1": 7.88,
    "cqv_v1_1": 8.34,
    "cqv_v2": 8.05,
    "cqv_v3": 8.05,
    "cqv": 8.24,
    "pe": 54.72,
    "close_history": "{'2021': 17.76, '2022': 8.14, '2023': 12.74, '2024': 37.26, '2025': 113.1, '2026': 106.02}",
    "status": "Success",
    "cqv_v4": 8.24,
    "price": 100.0,
    "pe_forward": 46.512,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.6,
    "mos_pct": 8.76,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TXN",
    "name": "Texas Instruments Incorporated",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.65,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.24,
    "f7": 5.88,
    "f8": 8.0,
    "momentum_score": 8.0,
    "cqv_v1": 7.63,
    "cqv_v1_1": 8.37,
    "cqv_v2": 8.04,
    "cqv_v3": 8.04,
    "cqv": 8.24,
    "pe": 50.01,
    "close_history": "{'2021': 165.43, '2022': 149.12, '2023': 158.68, '2024': 179.52, '2025': 171.49, '2026': 291.22}",
    "status": "Success",
    "cqv_v4": 8.24,
    "price": 100.0,
    "pe_forward": 42.5085,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.6,
    "mos_pct": 8.76,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EFX",
    "name": "Equifax, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.68,
    "f2": 7.69,
    "f3": 9.26,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.97,
    "f7": 5.06,
    "f8": 8.0,
    "momentum_score": 2.75,
    "cqv_v1": 8.43,
    "cqv_v1_1": 8.31,
    "cqv_v2": 8.03,
    "cqv_v3": 8.03,
    "cqv": 8.23,
    "pe": 30.29,
    "close_history": "{'2021': 282.64, '2022': 189.1, '2023': 242.4, '2024': 251.3, '2025': 215.65, '2026': 179.62}",
    "status": "Success",
    "cqv_v4": 8.23,
    "price": 100.0,
    "pe_forward": 25.746499999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.2,
    "mos_pct": 8.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MAS",
    "name": "Masco Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.99,
    "f2": 8.43,
    "f3": 8.39,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.69,
    "f7": 4.81,
    "f8": 8.0,
    "momentum_score": 9.24,
    "cqv_v1": 8.35,
    "cqv_v1_1": 8.37,
    "cqv_v2": 8.03,
    "cqv_v3": 8.03,
    "cqv": 8.23,
    "pe": 20.49,
    "close_history": "{'2021': 64.46, '2022': 43.77, '2023': 64.15, '2024': 70.6, '2025': 62.89, '2026': 80.14}",
    "status": "Success",
    "cqv_v4": 8.23,
    "price": 100.0,
    "pe_forward": 17.4165,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.2,
    "mos_pct": 8.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SYK",
    "name": "Stryker Corporation",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.72,
    "f2": 8.66,
    "f3": 7.96,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.34,
    "f7": 5.02,
    "f8": 8.0,
    "momentum_score": 3.82,
    "cqv_v1": 8.27,
    "cqv_v1_1": 8.47,
    "cqv_v2": 8.06,
    "cqv_v3": 8.06,
    "cqv": 8.23,
    "pe": 37.75,
    "close_history": "{'2021': 255.14, '2022': 236.21, '2023': 292.43, '2024': 354.84, '2025': 349.59, '2026': 331.2}",
    "status": "Success",
    "cqv_v4": 8.23,
    "price": 100.0,
    "pe_forward": 32.0875,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.2,
    "mos_pct": 8.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ITW",
    "name": "Illinois Tool Works Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.15,
    "f2": 8.48,
    "f3": 8.18,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.66,
    "f7": 4.68,
    "f8": 8.0,
    "momentum_score": 5.67,
    "cqv_v1": 8.16,
    "cqv_v1_1": 8.39,
    "cqv_v2": 8.03,
    "cqv_v3": 8.03,
    "cqv": 8.23,
    "pe": 25.33,
    "close_history": "{'2021': 221.69, '2022': 202.94, '2023': 246.7, '2024': 244.32, '2025': 243.28, '2026': 282.97}",
    "status": "Success",
    "cqv_v4": 8.23,
    "price": 100.0,
    "pe_forward": 21.530499999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.2,
    "mos_pct": 8.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BCPC",
    "name": "Balchem Corporation",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 8.59,
    "f2": 8.75,
    "f3": 8.57,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.58,
    "f7": 5.13,
    "f8": 8.0,
    "momentum_score": 6.01,
    "cqv_v1": 8.08,
    "cqv_v1_1": 8.35,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 8.23,
    "pe": 34.73,
    "close_history": "{'2021': 164.83, '2022': 120.08, '2023': 147.07, '2024': 162.0, '2025': 153.36, '2026': 166.23}",
    "status": "Success",
    "cqv_v4": 8.23,
    "price": 100.0,
    "pe_forward": 29.520499999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.2,
    "mos_pct": 8.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GWW",
    "name": "W.W. Grainger, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.37,
    "f2": 8.75,
    "f3": 8.79,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.58,
    "f7": 5.3,
    "f8": 8.0,
    "momentum_score": 8.7,
    "cqv_v1": 7.95,
    "cqv_v1_1": 8.32,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 8.23,
    "pe": 36.1,
    "close_history": "{'2021': 495.82, '2022': 539.19, '2023': 811.67, '2024': 1040.62, '2025': 1005.11, '2026': 1402.03}",
    "status": "Success",
    "cqv_v4": 8.23,
    "price": 100.0,
    "pe_forward": 30.685,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.2,
    "mos_pct": 8.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FSS",
    "name": "Federal Signal Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.88,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.61,
    "f7": 5.7,
    "f8": 8.0,
    "momentum_score": 8.11,
    "cqv_v1": 7.92,
    "cqv_v1_1": 8.28,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 8.23,
    "pe": 29.79,
    "close_history": "{'2021': 42.06, '2022': 45.52, '2023': 75.68, '2024': 91.62, '2025': 108.3, '2026': 119.5}",
    "status": "Success",
    "cqv_v4": 8.23,
    "price": 100.0,
    "pe_forward": 25.3215,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 109.2,
    "mos_pct": 8.42,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NOVT",
    "name": "Novanta Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 7.88,
    "f2": 9.5,
    "f3": 8.32,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.1,
    "f7": 5.7,
    "f8": 8.0,
    "momentum_score": 8.74,
    "cqv_v1": 7.84,
    "cqv_v1_1": 8.39,
    "cqv_v2": 8.03,
    "cqv_v3": 8.03,
    "cqv": 8.22,
    "pe": 116.35,
    "close_history": "{'2021': 176.33, '2022': 135.87, '2023': 168.41, '2024': 152.77, '2025': 118.99, '2026': 149.67}",
    "status": "Success",
    "cqv_v4": 8.22,
    "price": 100.0,
    "pe_forward": 98.8975,
    "value_score": 3.08,
    "peg_bruto": 1.8201,
    "score_peg": 1.8201,
    "intrinsic_value": 108.8,
    "mos_pct": 8.09,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.7
  },
  {
    "ticker": "EVO",
    "name": "Evolution AB",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.6,
    "f2": 9.0,
    "f3": 7.5,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.55,
    "f7": 4.32,
    "f8": 8.0,
    "momentum_score": 5.31,
    "cqv_v1": 7.8,
    "cqv_v1_1": 8.38,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.22,
    "pe": 11.36,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.22,
    "price": 100.0,
    "pe_forward": 9.655999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 108.8,
    "mos_pct": 8.09,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IT",
    "name": "Gartner, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.25,
    "f2": 9.25,
    "f3": 7.0,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.12,
    "f7": 4.6,
    "f8": 8.0,
    "momentum_score": 0.65,
    "cqv_v1": 8.58,
    "cqv_v1_1": 8.5,
    "cqv_v2": 8.04,
    "cqv_v3": 8.04,
    "cqv": 8.21,
    "pe": 13.48,
    "close_history": "{'2021': 334.32, '2022': 336.14, '2023': 451.11, '2024': 484.47, '2025': 252.28, '2026': 142.89}",
    "status": "Success",
    "cqv_v4": 8.21,
    "price": 100.0,
    "pe_forward": 11.458,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 108.4,
    "mos_pct": 7.75,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ACGL",
    "name": "Arch Capital Group Ltd.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.51,
    "f2": 9.0,
    "f3": 7.3,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.4,
    "f7": 4.39,
    "f8": 8.0,
    "momentum_score": 7.47,
    "cqv_v1": 8.58,
    "cqv_v1_1": 8.42,
    "cqv_v2": 8.01,
    "cqv_v3": 8.01,
    "cqv": 8.21,
    "pe": 7.86,
    "close_history": "{'2021': 42.27, '2022': 59.7, '2023': 70.62, '2024': 92.35, '2025': 95.92, '2026': 100.04}",
    "status": "Success",
    "cqv_v4": 8.21,
    "price": 100.0,
    "pe_forward": 6.681,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 108.4,
    "mos_pct": 7.75,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CTVA",
    "name": "Corteva, Inc.",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 8.03,
    "f2": 9.0,
    "f3": 8.89,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.46,
    "f7": 5.58,
    "f8": 8.0,
    "momentum_score": 8.72,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.29,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.21,
    "pe": 46.38,
    "close_history": "{'2021': 45.06, '2022': 56.58, '2023': 46.68, '2024': 56.13, '2025': 66.73, '2026': 86.65}",
    "status": "Success",
    "cqv_v4": 8.21,
    "price": 100.0,
    "pe_forward": 39.423,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 108.4,
    "mos_pct": 7.75,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LULU",
    "name": "lululemon athletica inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.91,
    "f2": 9.0,
    "f3": 8.14,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.53,
    "f7": 4.87,
    "f8": 8.0,
    "momentum_score": 1.19,
    "cqv_v1": 8.49,
    "cqv_v1_1": 8.31,
    "cqv_v2": 7.98,
    "cqv_v3": 7.98,
    "cqv": 8.2,
    "pe": 9.59,
    "close_history": "{'2021': 391.45, '2022': 320.38, '2023': 511.29, '2024': 382.41, '2025': 207.81, '2026': 118.79}",
    "status": "Success",
    "cqv_v4": 8.2,
    "price": 100.0,
    "pe_forward": 8.1515,
    "value_score": 5.46,
    "peg_bruto": 22.0818,
    "score_peg": 10.0,
    "intrinsic_value": 108.0,
    "mos_pct": 7.41,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.47
  },
  {
    "ticker": "WDAY",
    "name": "Workday, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 7.68,
    "f2": 9.18,
    "f3": 8.67,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.18,
    "f7": 5.86,
    "f8": 8.0,
    "momentum_score": 2.6,
    "cqv_v1": 8.46,
    "cqv_v1_1": 8.35,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 8.2,
    "pe": 42.18,
    "close_history": "{'2021': 273.18, '2022': 167.33, '2023': 276.06, '2024': 258.03, '2025': 214.78, '2026': 145.43}",
    "status": "Success",
    "cqv_v4": 8.2,
    "price": 100.0,
    "pe_forward": 35.853,
    "value_score": 3.97,
    "peg_bruto": 5.0205,
    "score_peg": 5.0205,
    "intrinsic_value": 108.0,
    "mos_pct": 7.41,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.47
  },
  {
    "ticker": "YETI",
    "name": "YETI Holdings, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.65,
    "f2": 8.75,
    "f3": 8.59,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.62,
    "f7": 5.08,
    "f8": 8.0,
    "momentum_score": 8.7,
    "cqv_v1": 8.17,
    "cqv_v1_1": 8.27,
    "cqv_v2": 7.97,
    "cqv_v3": 7.97,
    "cqv": 8.2,
    "pe": 25.35,
    "close_history": "{'2021': 82.83, '2022': 41.31, '2023': 51.78, '2024': 38.51, '2025': 44.17, '2026': 51.06}",
    "status": "Success",
    "cqv_v4": 8.2,
    "price": 100.0,
    "pe_forward": 21.5475,
    "value_score": 4.97,
    "peg_bruto": 8.3536,
    "score_peg": 8.3536,
    "intrinsic_value": 108.0,
    "mos_pct": 7.41,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.47
  },
  {
    "ticker": "DOV",
    "name": "Dover Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.25,
    "f2": 8.75,
    "f3": 8.79,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.52,
    "f7": 5.4,
    "f8": 8.0,
    "momentum_score": 6.43,
    "cqv_v1": 8.08,
    "cqv_v1_1": 8.29,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.2,
    "pe": 26.75,
    "close_history": "{'2021': 171.72, '2022': 129.92, '2023': 149.69, '2024': 184.64, '2025': 194.32, '2026': 217.51}",
    "status": "Success",
    "cqv_v4": 8.2,
    "price": 100.0,
    "pe_forward": 22.7375,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 108.0,
    "mos_pct": 7.41,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TRMB",
    "name": "Trimble Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 7.8,
    "f2": 9.25,
    "f3": 8.48,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.14,
    "f7": 5.76,
    "f8": 8.0,
    "momentum_score": 1.34,
    "cqv_v1": 8.07,
    "cqv_v1_1": 8.36,
    "cqv_v2": 8.01,
    "cqv_v3": 8.01,
    "cqv": 8.2,
    "pe": 27.77,
    "close_history": "{'2021': 87.19, '2022': 50.56, '2023': 53.2, '2024': 70.66, '2025': 78.35, '2026': 53.75}",
    "status": "Success",
    "cqv_v4": 8.2,
    "price": 100.0,
    "pe_forward": 23.604499999999998,
    "value_score": 4.75,
    "peg_bruto": 7.6257,
    "score_peg": 7.6257,
    "intrinsic_value": 108.0,
    "mos_pct": 7.41,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.47
  },
  {
    "ticker": "FIX",
    "name": "Comfort Systems USA, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.04,
    "f2": 9.0,
    "f3": 7.78,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.41,
    "f7": 4.77,
    "f8": 8.0,
    "momentum_score": 7.48,
    "cqv_v1": 7.98,
    "cqv_v1_1": 8.38,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.2,
    "pe": 50.25,
    "close_history": "{'2021': 97.12, '2022': 113.61, '2023': 204.06, '2024': 422.18, '2025': 932.44, '2026': 1680.6}",
    "status": "Success",
    "cqv_v4": 8.2,
    "price": 100.0,
    "pe_forward": 42.7125,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 108.0,
    "mos_pct": 7.41,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MAMA",
    "name": "Mama's Creations, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 7.76,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.54,
    "f7": 5.79,
    "f8": 8.0,
    "momentum_score": 9.47,
    "cqv_v1": 7.6,
    "cqv_v1_1": 8.2,
    "cqv_v2": 7.96,
    "cqv_v3": 7.96,
    "cqv": 8.2,
    "pe": 124.53,
    "close_history": "{'2021': 2.01, '2022': 1.79, '2023': 4.91, '2024': 7.96, '2025': 13.49, '2026': 17.09}",
    "status": "Success",
    "cqv_v4": 8.2,
    "price": 100.0,
    "pe_forward": 105.8505,
    "value_score": 2.97,
    "peg_bruto": 1.7005,
    "score_peg": 1.7005,
    "intrinsic_value": 108.0,
    "mos_pct": 7.41,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.47
  },
  {
    "ticker": "GIB",
    "name": "CGI Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.65,
    "f2": 9.25,
    "f3": 7.53,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.09,
    "f7": 5.08,
    "f8": 8.0,
    "momentum_score": 2.24,
    "cqv_v1": 8.52,
    "cqv_v1_1": 8.43,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 8.19,
    "pe": 12.31,
    "close_history": "{'2021': 87.82, '2022': 85.47, '2023': 106.39, '2024': 108.59, '2025': 91.96, '2026': 68.14}",
    "status": "Success",
    "cqv_v4": 8.19,
    "price": 100.0,
    "pe_forward": 10.4635,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 107.6,
    "mos_pct": 7.06,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NVO",
    "name": "Novo Nordisk A/S",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 7.0,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.16,
    "f7": 6.4,
    "f8": 8.0,
    "momentum_score": 5.39,
    "cqv_v1": 7.47,
    "cqv_v1_1": 8.29,
    "cqv_v2": 8.01,
    "cqv_v3": 8.01,
    "cqv": 8.19,
    "pe": 12.09,
    "close_history": "{'2021': 50.58, '2022': 62.04, '2023': 96.06, '2024': 80.75, '2025': 49.08, '2026': 51.48}",
    "status": "Success",
    "cqv_v4": 8.19,
    "price": 100.0,
    "pe_forward": 10.2765,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 107.6,
    "mos_pct": 7.06,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TRU",
    "name": "TransUnion",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.54,
    "f2": 7.68,
    "f3": 9.19,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.86,
    "f7": 5.17,
    "f8": 8.0,
    "momentum_score": 4.9,
    "cqv_v1": 8.4,
    "cqv_v1_1": 8.27,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.18,
    "pe": 21.69,
    "close_history": "{'2021': 115.63, '2022': 55.62, '2023': 67.78, '2024': 91.9, '2025': 85.45, '2026': 80.73}",
    "status": "Success",
    "cqv_v4": 8.18,
    "price": 100.0,
    "pe_forward": 18.436500000000002,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 107.2,
    "mos_pct": 6.72,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "URI",
    "name": "United Rentals, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.86,
    "f2": 8.19,
    "f3": 8.47,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.66,
    "f7": 4.91,
    "f8": 8.0,
    "momentum_score": 8.89,
    "cqv_v1": 8.11,
    "cqv_v1_1": 8.31,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.18,
    "pe": 28.09,
    "close_history": "{'2021': 320.44, '2022': 342.74, '2023': 560.79, '2024': 695.18, '2025': 805.84, '2026': 1071.82}",
    "status": "Success",
    "cqv_v4": 8.18,
    "price": 100.0,
    "pe_forward": 23.8765,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 107.2,
    "mos_pct": 6.72,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BRK",
    "name": "Berkshire Hathaway Inc. New",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.51,
    "f2": 9.0,
    "f3": 8.16,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.34,
    "f7": 5.19,
    "f8": 8.0,
    "momentum_score": 6.49,
    "cqv_v1": 8.41,
    "cqv_v1_1": 8.3,
    "cqv_v2": 7.97,
    "cqv_v3": 7.97,
    "cqv": 8.17,
    "pe": 15.1,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.17,
    "price": 100.0,
    "pe_forward": 12.834999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 106.8,
    "mos_pct": 6.37,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WSM",
    "name": "Williams-Sonoma, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.93,
    "f2": 8.75,
    "f3": 8.16,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.54,
    "f7": 4.86,
    "f8": 8.0,
    "momentum_score": 7.95,
    "cqv_v1": 8.13,
    "cqv_v1_1": 8.28,
    "cqv_v2": 7.96,
    "cqv_v3": 7.96,
    "cqv": 8.17,
    "pe": 25.51,
    "close_history": "{'2021': 77.49, '2022': 53.86, '2023': 97.08, '2024': 181.11, '2025': 177.32, '2026': 228.38}",
    "status": "Success",
    "cqv_v4": 8.17,
    "price": 100.0,
    "pe_forward": 21.683500000000002,
    "value_score": 4.85,
    "peg_bruto": 8.3012,
    "score_peg": 8.3012,
    "intrinsic_value": 106.8,
    "mos_pct": 6.37,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.13
  },
  {
    "ticker": "ROP",
    "name": "Roper Technologies, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.52,
    "f2": 8.02,
    "f3": 8.42,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.47,
    "f7": 5.18,
    "f8": 8.0,
    "momentum_score": 3.46,
    "cqv_v1": 8.5,
    "cqv_v1_1": 8.35,
    "cqv_v2": 8.0,
    "cqv_v3": 8.0,
    "cqv": 8.16,
    "pe": 22.73,
    "close_history": "{'2021': 477.07, '2022': 421.53, '2023': 535.02, '2024': 513.01, '2025': 441.98, '2026': 364.21}",
    "status": "Success",
    "cqv_v4": 8.16,
    "price": 100.0,
    "pe_forward": 19.3205,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 106.4,
    "mos_pct": 6.02,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WAT",
    "name": "Waters Corporation",
    "sector": "Healthcare",
    "quarter": "Q2 2026",
    "f1": 6.85,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.09,
    "f7": 6.52,
    "f8": 8.0,
    "momentum_score": 6.87,
    "cqv_v1": 7.34,
    "cqv_v1_1": 8.26,
    "cqv_v2": 7.98,
    "cqv_v3": 7.98,
    "cqv": 8.16,
    "pe": 48.32,
    "close_history": "{'2021': 372.6, '2022': 342.58, '2023': 329.23, '2024': 370.98, '2025': 379.83, '2026': 378.19}",
    "status": "Success",
    "cqv_v4": 8.16,
    "price": 100.0,
    "pe_forward": 41.071999999999996,
    "value_score": 3.63,
    "peg_bruto": 4.3825,
    "score_peg": 4.3825,
    "intrinsic_value": 106.4,
    "mos_pct": 6.02,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 2.0
  },
  {
    "ticker": "HCA",
    "name": "HCA Healthcare, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.96,
    "f2": 7.6,
    "f3": 8.14,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.55,
    "f7": 4.83,
    "f8": 8.0,
    "momentum_score": 3.26,
    "cqv_v1": 8.54,
    "cqv_v1_1": 8.4,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 8.15,
    "pe": 14.15,
    "close_history": "{'2021': 247.32, '2022': 233.37, '2023': 265.65, '2024': 296.88, '2025': 465.23, '2026': 385.74}",
    "status": "Success",
    "cqv_v4": 8.15,
    "price": 100.0,
    "pe_forward": 12.0275,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 106.0,
    "mos_pct": 5.66,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BRO",
    "name": "Brown & Brown, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.5,
    "f2": 7.34,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.91,
    "f7": 5.2,
    "f8": 8.0,
    "momentum_score": 4.44,
    "cqv_v1": 8.5,
    "cqv_v1_1": 8.23,
    "cqv_v2": 7.98,
    "cqv_v3": 7.98,
    "cqv": 8.15,
    "pe": 22.8,
    "close_history": "{'2021': 68.09, '2022': 55.57, '2023': 69.87, '2024': 100.84, '2025': 79.28, '2026': 69.44}",
    "status": "Success",
    "cqv_v4": 8.15,
    "price": 100.0,
    "pe_forward": 19.38,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 106.0,
    "mos_pct": 5.66,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FIVE",
    "name": "Five Below, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.03,
    "f2": 8.29,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.68,
    "f7": 5.58,
    "f8": 8.0,
    "momentum_score": 4.42,
    "cqv_v1": 8.2,
    "cqv_v1_1": 8.16,
    "cqv_v2": 7.93,
    "cqv_v3": 7.93,
    "cqv": 8.15,
    "pe": 22.98,
    "close_history": "{'2021': 206.89, '2022': 176.87, '2023': 213.16, '2024': 104.96, '2025': 188.36, '2026': 197.71}",
    "status": "Success",
    "cqv_v4": 8.15,
    "price": 100.0,
    "pe_forward": 19.533,
    "value_score": 5.05,
    "peg_bruto": 9.2152,
    "score_peg": 9.2152,
    "intrinsic_value": 106.0,
    "mos_pct": 5.66,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 1.9
  },
  {
    "ticker": "DASH",
    "name": "DoorDash, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 7.56,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.45,
    "f7": 5.95,
    "f8": 8.0,
    "momentum_score": 4.63,
    "cqv_v1": 7.93,
    "cqv_v1_1": 8.15,
    "cqv_v2": 7.92,
    "cqv_v3": 7.92,
    "cqv": 8.15,
    "pe": 91.0,
    "close_history": "{'2021': 148.9, '2022': 48.82, '2023': 98.89, '2024': 167.75, '2025': 226.48, '2026': 186.36}",
    "status": "Success",
    "cqv_v4": 8.15,
    "price": 100.0,
    "pe_forward": 77.35,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 106.0,
    "mos_pct": 5.66,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TEAM",
    "name": "Atlassian Corporation",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.92,
    "f7": 6.4,
    "f8": 8.0,
    "momentum_score": 1.61,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.25,
    "cqv_v2": 7.96,
    "cqv_v3": 7.96,
    "cqv": 8.14,
    "pe": 13.47,
    "close_history": "{'2021': 381.29, '2022': 128.68, '2023': 237.86, '2024': 243.38, '2025': 162.14, '2026': 92.36}",
    "status": "Success",
    "cqv_v4": 8.14,
    "price": 100.0,
    "pe_forward": 11.4495,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 105.6,
    "mos_pct": 5.3,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LOR",
    "name": "L'OREAL",
    "sector": "Consumer Defensive",
    "quarter": "Q4 2025",
    "f1": 9.17,
    "f2": 8.75,
    "f3": 7.78,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.47,
    "f7": 4.66,
    "f8": 8.0,
    "momentum_score": 5.76,
    "cqv_v1": 8.09,
    "cqv_v1_1": 8.28,
    "cqv_v2": 7.94,
    "cqv_v3": 7.94,
    "cqv": 8.14,
    "pe": 33.43,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.14,
    "price": 100.0,
    "pe_forward": 28.415499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 105.6,
    "mos_pct": 5.3,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MLM",
    "name": "Martin Marietta Materials, Inc.",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 8.18,
    "f2": 7.68,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.75,
    "f7": 5.46,
    "f8": 8.0,
    "momentum_score": 4.26,
    "cqv_v1": 7.98,
    "cqv_v1_1": 8.2,
    "cqv_v2": 7.96,
    "cqv_v3": 7.96,
    "cqv": 8.14,
    "pe": 37.6,
    "close_history": "{'2021': 428.52, '2022': 331.1, '2023': 492.08, '2024': 512.17, '2025': 621.01, '2026': 576.93}",
    "status": "Success",
    "cqv_v4": 8.14,
    "price": 100.0,
    "pe_forward": 31.96,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 105.6,
    "mos_pct": 5.3,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CMC",
    "name": "Commercial Metals Company",
    "sector": "Industrials",
    "quarter": "Q2 2026",
    "f1": 7.46,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.39,
    "f7": 6.03,
    "f8": 8.0,
    "momentum_score": 3.6,
    "cqv_v1": 7.77,
    "cqv_v1_1": 8.18,
    "cqv_v2": 7.94,
    "cqv_v3": 7.94,
    "cqv": 8.14,
    "pe": 11.67,
    "close_history": "{'2021': 34.05, '2022': 45.99, '2023': 48.28, '2024': 48.48, '2025': 68.61, '2026': 66.4}",
    "status": "Success",
    "cqv_v4": 8.14,
    "price": 100.0,
    "pe_forward": 9.9195,
    "value_score": 5.25,
    "peg_bruto": 18.1461,
    "score_peg": 10.0,
    "intrinsic_value": 105.6,
    "mos_pct": 5.3,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 1.77
  },
  {
    "ticker": "CSL",
    "name": "Carlisle Companies Incorporated",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.42,
    "f2": 8.75,
    "f3": 7.22,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.32,
    "f7": 4.46,
    "f8": 8.0,
    "momentum_score": 5.84,
    "cqv_v1": 8.37,
    "cqv_v1_1": 8.35,
    "cqv_v2": 7.96,
    "cqv_v3": 7.96,
    "cqv": 8.13,
    "pe": 21.41,
    "close_history": "{'2021': 236.17, '2022': 226.54, '2023': 304.15, '2024': 362.38, '2025': 317.94, '2026': 352.76}",
    "status": "Success",
    "cqv_v4": 8.13,
    "price": 100.0,
    "pe_forward": 18.1985,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 105.2,
    "mos_pct": 4.94,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "APO",
    "name": "Apollo Global Management, Inc. ",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 9.57,
    "f2": 9.25,
    "f3": 6.64,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.11,
    "f7": 4.34,
    "f8": 8.0,
    "momentum_score": 3.57,
    "cqv_v1": 8.28,
    "cqv_v1_1": 8.38,
    "cqv_v2": 7.95,
    "cqv_v3": 7.95,
    "cqv": 8.13,
    "pe": 74.6,
    "close_history": "{'2021': 66.46, '2022': 60.09, '2023': 89.8, '2024': 161.52, '2025': 143.57, '2026': 123.34}",
    "status": "Success",
    "cqv_v4": 8.13,
    "price": 100.0,
    "pe_forward": 63.41,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 105.2,
    "mos_pct": 4.94,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ZBRA",
    "name": "Zebra Technologies Corporation",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 8.16,
    "f2": 7.89,
    "f3": 8.76,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.46,
    "f7": 5.47,
    "f8": 8.0,
    "momentum_score": 5.67,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.29,
    "cqv_v2": 7.98,
    "cqv_v3": 7.98,
    "cqv": 8.13,
    "pe": 32.27,
    "close_history": "{'2021': 595.2, '2022': 256.41, '2023': 273.33, '2024': 386.22, '2025': 242.82, '2026': 265.04}",
    "status": "Success",
    "cqv_v4": 8.13,
    "price": 100.0,
    "pe_forward": 27.4295,
    "value_score": 4.18,
    "peg_bruto": 6.5623,
    "score_peg": 6.5623,
    "intrinsic_value": 105.2,
    "mos_pct": 4.94,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 1.63
  },
  {
    "ticker": "BF.B",
    "name": "Brown Forman Inc",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 9.3,
    "f2": 8.27,
    "f3": 7.89,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.6,
    "f7": 4.56,
    "f8": 8.0,
    "momentum_score": 4.07,
    "cqv_v1": 8.49,
    "cqv_v1_1": 8.26,
    "cqv_v2": 7.93,
    "cqv_v3": 7.93,
    "cqv": 8.12,
    "pe": 17.1,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 14.535,
    "value_score": 5.18,
    "peg_bruto": 12.3839,
    "score_peg": 10.0,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 1.53
  },
  {
    "ticker": "EVR",
    "name": "Evercore Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 7.0,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.16,
    "f7": 6.4,
    "f8": 8.0,
    "momentum_score": 5.86,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.14,
    "cqv_v2": 7.91,
    "cqv_v3": 7.91,
    "cqv": 8.12,
    "pe": 19.27,
    "close_history": "{'2021': 125.27, '2022': 103.23, '2023': 165.77, '2024': 272.43, '2025': 338.51, '2026': 349.71}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 16.3795,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BMI",
    "name": "Badger Meter, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.43,
    "f2": 9.5,
    "f3": 6.17,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.8,
    "f7": 4.46,
    "f8": 8.0,
    "momentum_score": 4.19,
    "cqv_v1": 8.18,
    "cqv_v1_1": 8.46,
    "cqv_v2": 7.97,
    "cqv_v3": 7.97,
    "cqv": 8.12,
    "pe": 33.0,
    "close_history": "{'2021': 102.82, '2022': 106.14, '2023': 151.34, '2024': 209.28, '2025': 173.38, '2026': 151.16}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 28.05,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SCHW",
    "name": "Charles Schwab Corporation (The",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 6.97,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.15,
    "f7": 6.42,
    "f8": 8.0,
    "momentum_score": 5.42,
    "cqv_v1": 8.16,
    "cqv_v1_1": 8.13,
    "cqv_v2": 7.91,
    "cqv_v3": 7.91,
    "cqv": 8.12,
    "pe": 19.28,
    "close_history": "{'2021': 79.07, '2022': 79.15, '2023': 66.51, '2024': 72.61, '2025': 99.22, '2026': 102.8}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 16.388,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SKY",
    "name": "Champion Homes, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.51,
    "f2": 9.0,
    "f3": 8.18,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.34,
    "f7": 5.19,
    "f8": 8.0,
    "momentum_score": 6.18,
    "cqv_v1": 8.11,
    "cqv_v1_1": 8.21,
    "cqv_v2": 7.91,
    "cqv_v3": 7.91,
    "cqv": 8.12,
    "pe": 23.29,
    "close_history": "{'2021': 78.98, '2022': 51.51, '2023': 74.26, '2024': 88.1, '2025': 84.5, '2026': 86.02}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 19.796499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BXSX",
    "name": "Boston Scientific Corporation",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.45,
    "f2": 7.8,
    "f3": 8.1,
    "f4": 8.7,
    "f5": 8.5,
    "f6": 8.27,
    "f7": 5.24,
    "f8": 8.0,
    "momentum_score": 0.34,
    "cqv_v1": 8.07,
    "cqv_v1_1": 8.6,
    "cqv_v2": 8.02,
    "cqv_v3": 8.02,
    "cqv": 8.12,
    "pe": 18.89,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 16.0565,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.7,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TDY",
    "name": "Teledyne Technologies Incorpora",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.89,
    "f2": 9.22,
    "f3": 8.01,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.95,
    "f7": 5.69,
    "f8": 8.0,
    "momentum_score": 7.26,
    "cqv_v1": 7.96,
    "cqv_v1_1": 8.31,
    "cqv_v2": 7.96,
    "cqv_v3": 7.96,
    "cqv": 8.12,
    "pe": 33.0,
    "close_history": "{'2021': 436.89, '2022': 399.91, '2023': 446.29, '2024': 464.13, '2025': 510.73, '2026': 630.86}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 28.05,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ZG",
    "name": "Zillow Group, Inc.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 7.16,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.25,
    "f7": 6.27,
    "f8": 8.0,
    "momentum_score": 0.6,
    "cqv_v1": 7.92,
    "cqv_v1_1": 8.14,
    "cqv_v2": 7.92,
    "cqv_v3": 7.92,
    "cqv": 8.12,
    "pe": 133.68,
    "close_history": "{'2021': 62.22, '2022': 31.21, '2023': 56.72, '2024': 70.85, '2025': 68.23, '2026': 33.99}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 113.628,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WWD",
    "name": "Woodward, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.36,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.35,
    "f7": 6.11,
    "f8": 8.0,
    "momentum_score": 8.97,
    "cqv_v1": 7.58,
    "cqv_v1_1": 8.15,
    "cqv_v2": 7.93,
    "cqv_v3": 7.93,
    "cqv": 8.12,
    "pe": 50.0,
    "close_history": "{'2021': 106.45, '2022': 94.65, '2023': 134.38, '2024': 165.3, '2025': 301.8, '2026': 394.12}",
    "status": "Success",
    "cqv_v4": 8.12,
    "price": 100.0,
    "pe_forward": 42.5,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.8,
    "mos_pct": 4.58,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IPAR",
    "name": "Interparfums, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 8.9,
    "f2": 8.75,
    "f3": 7.87,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.38,
    "f7": 4.88,
    "f8": 8.0,
    "momentum_score": 8.23,
    "cqv_v1": 8.12,
    "cqv_v1_1": 8.23,
    "cqv_v2": 7.91,
    "cqv_v3": 7.91,
    "cqv": 8.11,
    "pe": 22.61,
    "close_history": "{'2021': 95.4, '2022': 88.34, '2023': 134.28, '2024': 125.62, '2025': 83.43, '2026': 122.72}",
    "status": "Success",
    "cqv_v4": 8.11,
    "price": 100.0,
    "pe_forward": 19.2185,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.4,
    "mos_pct": 4.21,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TMO",
    "name": "Thermo Fisher Scientific Inc",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.2,
    "f2": 8.04,
    "f3": 8.36,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.28,
    "f7": 5.44,
    "f8": 8.0,
    "momentum_score": 4.85,
    "cqv_v1": 8.05,
    "cqv_v1_1": 8.3,
    "cqv_v2": 7.97,
    "cqv_v3": 7.97,
    "cqv": 8.11,
    "pe": 28.73,
    "close_history": "{'2021': 658.54, '2022': 544.71, '2023': 526.44, '2024': 517.37, '2025': 578.3, '2026': 543.19}",
    "status": "Success",
    "cqv_v4": 8.11,
    "price": 100.0,
    "pe_forward": 24.4205,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.4,
    "mos_pct": 4.21,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CMG",
    "name": "Chipotle Mexican Grill, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.66,
    "f2": 8.31,
    "f3": 8.49,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.57,
    "f7": 5.07,
    "f8": 8.0,
    "momentum_score": 4.14,
    "cqv_v1": 7.96,
    "cqv_v1_1": 8.2,
    "cqv_v2": 7.92,
    "cqv_v3": 7.92,
    "cqv": 8.11,
    "pe": 32.47,
    "close_history": "{'2021': 34.97, '2022': 27.75, '2023': 45.74, '2024': 60.3, '2025': 37.0, '2026': 34.2}",
    "status": "Success",
    "cqv_v4": 8.11,
    "price": 100.0,
    "pe_forward": 27.5995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.4,
    "mos_pct": 4.21,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PEP",
    "name": "Pepsico, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 8.9,
    "f2": 7.74,
    "f3": 8.61,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.75,
    "f7": 4.88,
    "f8": 8.0,
    "momentum_score": 3.86,
    "cqv_v1": 8.25,
    "cqv_v1_1": 8.19,
    "cqv_v2": 7.92,
    "cqv_v3": 7.92,
    "cqv": 8.1,
    "pe": 22.64,
    "close_history": "{'2021': 150.3, '2022': 160.49, '2023': 155.21, '2024': 143.41, '2025': 140.77, '2026': 139.43}",
    "status": "Success",
    "cqv_v4": 8.1,
    "price": 100.0,
    "pe_forward": 19.244,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.0,
    "mos_pct": 3.85,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IHG",
    "name": "Intercontinental Hotels Group",
    "sector": "Consumer Cyclical",
    "quarter": "Q4 2025",
    "f1": 9.42,
    "f2": 7.87,
    "f3": 7.97,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.7,
    "f7": 4.46,
    "f8": 8.0,
    "momentum_score": 8.81,
    "cqv_v1": 8.06,
    "cqv_v1_1": 8.24,
    "cqv_v2": 7.92,
    "cqv_v3": 7.92,
    "cqv": 8.1,
    "pe": 34.97,
    "close_history": "{'2021': 59.46, '2022': 54.29, '2023': 86.64, '2024': 120.55, '2025': 139.59, '2026': 159.87}",
    "status": "Success",
    "cqv_v4": 8.1,
    "price": 100.0,
    "pe_forward": 29.7245,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 104.0,
    "mos_pct": 3.85,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EPAM",
    "name": "EPAM Systems, Inc.",
    "sector": "Technology / IT Consulting & Digital Software Engineering Services",
    "quarter": "Q2 2026",
    "f1": 8.1,
    "f2": 8.8,
    "f3": 8.2,
    "f4": 8.3,
    "f5": 7.2,
    "f6": 7.0,
    "f7": 8.0,
    "f8": 8.5,
    "momentum_score": 0.45,
    "pe": 13.18,
    "close_history": {
      "2020": 358.34,
      "2021": 668.45,
      "2022": 327.74,
      "2023": 297.34,
      "2024": 235.1,
      "2025": 210.5,
      "2026": 97.52
    },
    "status": "Success",
    "price": 97.52,
    "pe_forward": 7.18,
    "valuation_date": "07/08/2026",
    "eps_trailing": 7.39,
    "eps_forward": null,
    "eps_growth_ntm_pct": 9.84,
    "growth_eps": 9.84,
    "market_cap_b": 5.03,
    "ocf_ttm_m": 538.79,
    "maint_capex_m": null,
    "owner_earnings_m": null,
    "score_fcf_yield": null,
    "fcf_yield_pct": null,
    "intrinsic_value": null,
    "mos_pct": null,
    "score_mos": null,
    "wacc": null,
    "g_terminal": null,
    "data_confidence": "Media",
    "analyst_targets": {
      "target_low_bear": null,
      "target_mean_base": 122.24,
      "target_high_bull": null,
      "num_analysts": 19,
      "consensus_recommendation": "Buy",
      "upside_potential_pct": 31.32
    },
    "cqv_v4": 8.09,
    "cqv": 8.09,
    "owner_earnings": null,
    "peg_bruto": 13.7047,
    "score_peg": 10.0,
    "value_score": null,
    "verdict": "N/D - valoración incompleta",
    "clasificacion": "ALTA CALIDAD"
  },
  {
    "ticker": "SSNC",
    "name": "SS&C Technologies Holdings, Inc",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.83,
    "f2": 7.54,
    "f3": 8.14,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.48,
    "f7": 4.94,
    "f8": 8.0,
    "momentum_score": 2.08,
    "cqv_v1": 8.49,
    "cqv_v1_1": 8.31,
    "cqv_v2": 7.97,
    "cqv_v3": 7.97,
    "cqv": 8.09,
    "pe": 20.35,
    "close_history": "{'2021': 77.01, '2022': 49.55, '2023': 59.07, '2024': 74.3, '2025': 86.76, '2026': 70.57}",
    "status": "Success",
    "cqv_v4": 8.09,
    "price": 100.0,
    "pe_forward": 17.2975,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 103.6,
    "mos_pct": 3.47,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TEQ",
    "name": "Teqnion AB",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.21,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.27,
    "f7": 6.23,
    "f8": 8.0,
    "momentum_score": 4.42,
    "cqv_v1": 8.19,
    "cqv_v1_1": 8.11,
    "cqv_v2": 7.9,
    "cqv_v3": 7.9,
    "cqv": 8.09,
    "pe": 28.11,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.09,
    "price": 100.0,
    "pe_forward": 23.8935,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 103.6,
    "mos_pct": 3.47,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DHR",
    "name": "Danaher Corporation",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.15,
    "f2": 8.4,
    "f3": 8.08,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 8.12,
    "f7": 5.48,
    "f8": 8.0,
    "momentum_score": 4.63,
    "cqv_v1": 8.09,
    "cqv_v1_1": 8.3,
    "cqv_v2": 7.96,
    "cqv_v3": 7.96,
    "cqv": 8.09,
    "pe": 38.28,
    "close_history": "{'2021': 285.02, '2022': 230.81, '2023': 227.95, '2024': 227.16, '2025': 227.96, '2026': 205.01}",
    "status": "Success",
    "cqv_v4": 8.09,
    "price": 100.0,
    "pe_forward": 32.538,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 103.6,
    "mos_pct": 3.47,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DRS",
    "name": "Leonardo DRS, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.98,
    "f2": 9.0,
    "f3": 8.32,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.15,
    "f7": 5.62,
    "f8": 8.0,
    "momentum_score": 6.23,
    "cqv_v1": 7.87,
    "cqv_v1_1": 8.19,
    "cqv_v2": 7.91,
    "cqv_v3": 7.91,
    "cqv": 8.09,
    "pe": 40.86,
    "close_history": "{'2021': 9.29, '2022': 12.6, '2023': 19.76, '2024': 31.86, '2025': 33.95, '2026': 43.1}",
    "status": "Success",
    "cqv_v4": 8.09,
    "price": 100.0,
    "pe_forward": 34.731,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 103.6,
    "mos_pct": 3.47,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TPL",
    "name": "Texas Pacific Land Corporation",
    "sector": "Energy",
    "quarter": "Q1 2026",
    "f1": 7.0,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.16,
    "f7": 6.4,
    "f8": 8.0,
    "momentum_score": 6.14,
    "cqv_v1": 7.35,
    "cqv_v1_1": 8.1,
    "cqv_v2": 7.89,
    "cqv_v3": 7.89,
    "cqv": 8.09,
    "pe": 55.86,
    "close_history": "{'2021': 131.24, '2022': 251.05, '2023': 169.72, '2024': 365.42, '2025': 286.45, '2026': 416.05}",
    "status": "Success",
    "cqv_v4": 8.09,
    "price": 100.0,
    "pe_forward": 47.481,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 103.6,
    "mos_pct": 3.47,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "OTIS",
    "name": "Otis Worldwide Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 9.08,
    "f2": 7.31,
    "f3": 8.38,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.73,
    "f7": 4.74,
    "f8": 8.0,
    "momentum_score": 2.33,
    "cqv_v1": 8.46,
    "cqv_v1_1": 8.22,
    "cqv_v2": 7.93,
    "cqv_v3": 7.93,
    "cqv": 8.08,
    "pe": 19.45,
    "close_history": "{'2021': 80.84, '2022': 73.76, '2023': 85.6, '2024': 90.02, '2025': 86.43, '2026': 75.0}",
    "status": "Success",
    "cqv_v4": 8.08,
    "price": 100.0,
    "pe_forward": 16.5325,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 103.2,
    "mos_pct": 3.1,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RJF",
    "name": "Raymond James Financial, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 7.0,
    "f2": 9.25,
    "f3": 9.12,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.06,
    "f7": 6.4,
    "f8": 8.0,
    "momentum_score": 5.65,
    "cqv_v1": 8.19,
    "cqv_v1_1": 8.11,
    "cqv_v2": 7.88,
    "cqv_v3": 7.88,
    "cqv": 8.08,
    "pe": 15.36,
    "close_history": "{'2021': 93.71, '2022': 101.5, '2023': 107.72, '2024': 151.7, '2025': 158.89, '2026': 169.7}",
    "status": "Success",
    "cqv_v4": 8.08,
    "price": 100.0,
    "pe_forward": 13.056,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 103.2,
    "mos_pct": 3.1,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DBD",
    "name": "Diebold Nixdorf Incorporated",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.9,
    "f2": 9.08,
    "f3": 7.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.87,
    "f7": 5.68,
    "f8": 8.0,
    "momentum_score": 8.04,
    "cqv_v1": 8.35,
    "cqv_v1_1": 8.26,
    "cqv_v2": 7.92,
    "cqv_v3": 7.92,
    "cqv": 8.07,
    "pe": 28.68,
    "close_history": "{'2023': 28.95, '2024': 43.04, '2025': 67.89, '2026': 85.17}",
    "status": "Success",
    "cqv_v4": 8.07,
    "price": 100.0,
    "pe_forward": 24.378,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.8,
    "mos_pct": 2.72,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CASY",
    "name": "Caseys General Stores, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 7.48,
    "f2": 8.65,
    "f3": 9.28,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.38,
    "f7": 6.02,
    "f8": 8.0,
    "momentum_score": 7.21,
    "cqv_v1": 7.77,
    "cqv_v1_1": 8.07,
    "cqv_v2": 7.87,
    "cqv_v3": 7.87,
    "cqv": 8.07,
    "pe": 41.66,
    "close_history": "{'2021': 192.43, '2022': 220.31, '2023': 271.59, '2024': 393.82, '2025': 551.81, '2026': 827.04}",
    "status": "Success",
    "cqv_v4": 8.07,
    "price": 100.0,
    "pe_forward": 35.410999999999994,
    "value_score": 3.54,
    "peg_bruto": 5.0832,
    "score_peg": 5.0832,
    "intrinsic_value": 102.8,
    "mos_pct": 2.72,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 1.0
  },
  {
    "ticker": "KRI",
    "name": "KRI-KRI S.A.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 7.16,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.25,
    "f7": 6.27,
    "f8": 8.0,
    "momentum_score": 8.97,
    "cqv_v1": 7.56,
    "cqv_v1_1": 8.05,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 8.07,
    "pe": 23.23,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.07,
    "price": 100.0,
    "pe_forward": 19.7455,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.8,
    "mos_pct": 2.72,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ON",
    "name": "ON Semiconductor Corporation",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 7.87,
    "f2": 9.25,
    "f3": 7.69,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.78,
    "f7": 5.7,
    "f8": 8.0,
    "momentum_score": 5.98,
    "cqv_v1": 8.08,
    "cqv_v1_1": 8.26,
    "cqv_v2": 7.91,
    "cqv_v3": 7.91,
    "cqv": 8.06,
    "pe": 67.07,
    "close_history": "{'2021': 67.92, '2022': 62.37, '2023': 83.53, '2024': 63.05, '2025': 54.15, '2026': 88.12}",
    "status": "Success",
    "cqv_v4": 8.06,
    "price": 100.0,
    "pe_forward": 57.009499999999996,
    "value_score": 2.97,
    "peg_bruto": 3.1574,
    "score_peg": 3.1574,
    "intrinsic_value": 102.4,
    "mos_pct": 2.34,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 1.0
  },
  {
    "ticker": "NU",
    "name": "Nu Holdings Ltd.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 6.68,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.0,
    "f7": 6.66,
    "f8": 8.0,
    "momentum_score": 3.3,
    "cqv_v1": 8.06,
    "cqv_v1_1": 8.06,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 8.06,
    "pe": 20.94,
    "close_history": "{'2021': 9.38, '2022': 4.07, '2023': 8.33, '2024': 10.36, '2025': 16.74, '2026': 13.79}",
    "status": "Success",
    "cqv_v4": 8.06,
    "price": 100.0,
    "pe_forward": 17.799,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.4,
    "mos_pct": 2.34,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WSO",
    "name": "Watsco, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.64,
    "f2": 8.75,
    "f3": 7.68,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.16,
    "f7": 5.09,
    "f8": 8.0,
    "momentum_score": 5.72,
    "cqv_v1": 8.03,
    "cqv_v1_1": 8.22,
    "cqv_v2": 7.9,
    "cqv_v3": 7.9,
    "cqv": 8.06,
    "pe": 33.23,
    "close_history": "{'2021': 273.28, '2022': 224.81, '2023': 397.91, '2024': 450.53, '2025': 328.81, '2026': 385.16}",
    "status": "Success",
    "cqv_v4": 8.06,
    "price": 100.0,
    "pe_forward": 28.245499999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.4,
    "mos_pct": 2.34,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TECH",
    "name": "Bio-Techne Corp",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 8.12,
    "f2": 9.0,
    "f3": 7.5,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.81,
    "f7": 5.5,
    "f8": 8.0,
    "momentum_score": 7.81,
    "cqv_v1": 7.97,
    "cqv_v1_1": 8.3,
    "cqv_v2": 7.92,
    "cqv_v3": 7.92,
    "cqv": 8.06,
    "pe": 101.19,
    "close_history": "{'2021': 126.64, '2022': 81.44, '2023': 76.15, '2024': 71.39, '2025': 58.62, '2026': 71.77}",
    "status": "Success",
    "cqv_v4": 8.06,
    "price": 100.0,
    "pe_forward": 86.0115,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.4,
    "mos_pct": 2.34,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CAT",
    "name": "Caterpillar, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.58,
    "f2": 8.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.46,
    "f7": 5.94,
    "f8": 8.0,
    "momentum_score": 8.73,
    "cqv_v1": 7.58,
    "cqv_v1_1": 8.09,
    "cqv_v2": 7.89,
    "cqv_v3": 7.89,
    "cqv": 8.06,
    "pe": 48.08,
    "close_history": "{'2021': 191.11, '2022': 226.65, '2023': 285.46, '2024': 355.85, '2025': 570.45, '2026': 877.17}",
    "status": "Success",
    "cqv_v4": 8.06,
    "price": 100.0,
    "pe_forward": 40.867999999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.4,
    "mos_pct": 2.34,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MKTX",
    "name": "MarketAxess Holdings, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 7.0,
    "f2": 9.25,
    "f3": 8.99,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.0,
    "f7": 6.4,
    "f8": 8.0,
    "momentum_score": 0.47,
    "cqv_v1": 7.32,
    "cqv_v1_1": 8.09,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 8.06,
    "pe": 13.81,
    "close_history": "{'2021': 387.76, '2022': 265.6, '2023': 281.81, '2024': 220.3, '2025': 179.46, '2026': 115.5}",
    "status": "Success",
    "cqv_v4": 8.06,
    "price": 100.0,
    "pe_forward": 11.7385,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.4,
    "mos_pct": 2.34,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HSY",
    "name": "The Hershey Company",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 8.35,
    "f2": 7.87,
    "f3": 8.84,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.59,
    "f7": 5.32,
    "f8": 8.0,
    "momentum_score": 3.52,
    "cqv_v1": 8.16,
    "cqv_v1_1": 8.1,
    "cqv_v2": 7.87,
    "cqv_v3": 7.87,
    "cqv": 8.05,
    "pe": 33.92,
    "close_history": "{'2021': 172.78, '2022': 210.55, '2023': 172.91, '2024': 161.66, '2025': 179.41, '2026': 174.72}",
    "status": "Success",
    "cqv_v4": 8.05,
    "price": 100.0,
    "pe_forward": 28.832,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.0,
    "mos_pct": 1.96,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WING",
    "name": "Wingstop Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.4,
    "f2": 8.25,
    "f3": 8.49,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.45,
    "f7": 5.28,
    "f8": 8.0,
    "momentum_score": 3.7,
    "cqv_v1": 7.85,
    "cqv_v1_1": 8.12,
    "cqv_v2": 7.87,
    "cqv_v3": 7.87,
    "cqv": 8.05,
    "pe": 43.84,
    "close_history": "{'2021': 163.41, '2022': 135.54, '2023': 253.82, '2024': 281.89, '2025': 237.59, '2026': 140.93}",
    "status": "Success",
    "cqv_v4": 8.05,
    "price": 100.0,
    "pe_forward": 37.264,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.0,
    "mos_pct": 1.96,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TXRH",
    "name": "Texas Roadhouse, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 7.45,
    "f2": 8.75,
    "f3": 9.09,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.27,
    "f7": 6.04,
    "f8": 8.0,
    "momentum_score": 7.88,
    "cqv_v1": 7.65,
    "cqv_v1_1": 8.05,
    "cqv_v2": 7.84,
    "cqv_v3": 7.84,
    "cqv": 8.04,
    "pe": 30.94,
    "close_history": "{'2021': 82.32, '2022': 85.74, '2023': 117.6, '2024': 176.14, '2025': 164.57, '2026': 197.55}",
    "status": "Success",
    "cqv_v4": 8.04,
    "price": 100.0,
    "pe_forward": 26.299,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 102.0,
    "mos_pct": 1.96,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TMUS",
    "name": "T-Mobile US, Inc.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 8.85,
    "f2": 6.77,
    "f3": 8.84,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.84,
    "f7": 4.92,
    "f8": 8.0,
    "momentum_score": 2.07,
    "cqv_v1": 8.44,
    "cqv_v1_1": 8.15,
    "cqv_v2": 7.91,
    "cqv_v3": 7.91,
    "cqv": 8.04,
    "pe": 18.87,
    "close_history": "{'2021': 110.91, '2022': 133.88, '2023': 153.99, '2024': 215.13, '2025': 200.98, '2026': 192.85}",
    "status": "Success",
    "cqv_v4": 8.04,
    "price": 100.0,
    "pe_forward": 16.0395,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 101.6,
    "mos_pct": 1.57,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MCK",
    "name": "McKesson Corporation",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 7.26,
    "f2": 9.0,
    "f3": 8.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.79,
    "f7": 6.19,
    "f8": 8.0,
    "momentum_score": 4.02,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.21,
    "cqv_v2": 7.9,
    "cqv_v3": 7.9,
    "cqv": 8.04,
    "pe": 20.49,
    "close_history": "{'2021': 243.01, '2022': 368.95, '2023': 457.99, '2024': 566.41, '2025': 818.7, '2026': 841.31}",
    "status": "Success",
    "cqv_v4": 8.04,
    "price": 100.0,
    "pe_forward": 17.4165,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 101.6,
    "mos_pct": 1.57,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LVMH",
    "name": "LVMH",
    "sector": "Consumer Cyclical",
    "quarter": "Q4 2025",
    "f1": 9.22,
    "f2": 8.75,
    "f3": 7.14,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.18,
    "f7": 4.62,
    "f8": 8.0,
    "momentum_score": 3.23,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.2,
    "cqv_v2": 7.85,
    "cqv_v3": 7.85,
    "cqv": 8.03,
    "pe": 22.68,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 8.03,
    "price": 100.0,
    "pe_forward": 19.278,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 101.2,
    "mos_pct": 1.19,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "UNP",
    "name": "Union Pacific Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.74,
    "f2": 7.91,
    "f3": 8.02,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.38,
    "f7": 5.01,
    "f8": 8.0,
    "momentum_score": 8.52,
    "cqv_v1": 7.95,
    "cqv_v1_1": 8.17,
    "cqv_v2": 7.88,
    "cqv_v3": 7.88,
    "cqv": 8.03,
    "pe": 23.25,
    "close_history": "{'2021': 227.2, '2022': 191.0, '2023': 232.27, '2024': 220.42, '2025': 228.93, '2026': 299.42}",
    "status": "Success",
    "cqv_v4": 8.03,
    "price": 100.0,
    "pe_forward": 19.7625,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 101.2,
    "mos_pct": 1.19,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ZS",
    "name": "Zscaler, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 6.46,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.64,
    "f7": 6.83,
    "f8": 8.0,
    "momentum_score": 2.11,
    "cqv_v1": 7.87,
    "cqv_v1_1": 8.11,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 8.03,
    "pe": 32.05,
    "close_history": "{'2021': 321.33, '2022': 111.9, '2023': 221.56, '2024': 180.41, '2025': 224.92, '2026': 146.43}",
    "status": "Success",
    "cqv_v4": 8.03,
    "price": 100.0,
    "pe_forward": 27.242499999999996,
    "value_score": 4.0,
    "peg_bruto": 6.6073,
    "score_peg": 6.6073,
    "intrinsic_value": 101.2,
    "mos_pct": 1.19,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 1.0
  },
  {
    "ticker": "TSLA",
    "name": "Tesla, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 6.96,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.14,
    "f7": 6.43,
    "f8": 8.0,
    "momentum_score": 4.19,
    "cqv_v1": 7.36,
    "cqv_v1_1": 8.0,
    "cqv_v2": 7.82,
    "cqv_v3": 7.82,
    "cqv": 8.03,
    "pe": 357.68,
    "close_history": "{'2021': 352.26, '2022': 123.18, '2023': 248.48, '2024': 403.84, '2025': 449.72, '2026': 391.06}",
    "status": "Success",
    "cqv_v4": 8.03,
    "price": 100.0,
    "pe_forward": 304.028,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 101.2,
    "mos_pct": 1.19,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "APG",
    "name": "APi Group Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.46,
    "f2": 7.89,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.39,
    "f7": 6.03,
    "f8": 8.0,
    "momentum_score": 5.6,
    "cqv_v1": 8.02,
    "cqv_v1_1": 8.05,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 8.02,
    "pe": 21.62,
    "close_history": "{'2021': 17.18, '2022': 12.54, '2023': 23.07, '2024': 23.98, '2025': 38.26, '2026': 40.83}",
    "status": "Success",
    "cqv_v4": 8.02,
    "price": 100.0,
    "pe_forward": 18.377,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 100.8,
    "mos_pct": 0.79,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SHW",
    "name": "Sherwin-Williams Company (The)",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 8.6,
    "f2": 7.54,
    "f3": 8.42,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.51,
    "f7": 5.12,
    "f8": 8.0,
    "momentum_score": 6.26,
    "cqv_v1": 7.94,
    "cqv_v1_1": 8.14,
    "cqv_v2": 7.88,
    "cqv_v3": 7.88,
    "cqv": 8.02,
    "pe": 33.89,
    "close_history": "{'2021': 337.87, '2022': 229.88, '2023': 305.11, '2024': 335.32, '2025': 322.48, '2026': 338.14}",
    "status": "Success",
    "cqv_v4": 8.02,
    "price": 100.0,
    "pe_forward": 28.8065,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 100.8,
    "mos_pct": 0.79,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CDW",
    "name": "CDW Corporation",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.9,
    "f2": 8.21,
    "f3": 8.19,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.04,
    "f7": 5.68,
    "f8": 8.0,
    "momentum_score": 4.53,
    "cqv_v1": 8.23,
    "cqv_v1_1": 8.19,
    "cqv_v2": 7.88,
    "cqv_v3": 7.88,
    "cqv": 8.01,
    "pe": 16.23,
    "close_history": "{'2021': 192.61, '2022': 169.98, '2023': 219.01, '2024': 169.59, '2025': 134.72, '2026': 134.34}",
    "status": "Success",
    "cqv_v4": 8.01,
    "price": 100.0,
    "pe_forward": 13.7955,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 100.4,
    "mos_pct": 0.4,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TREX",
    "name": "Trex Company, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.27,
    "f2": 8.75,
    "f3": 7.78,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.03,
    "f7": 5.38,
    "f8": 8.0,
    "momentum_score": 7.06,
    "cqv_v1": 7.81,
    "cqv_v1_1": 8.15,
    "cqv_v2": 7.85,
    "cqv_v3": 7.85,
    "cqv": 8.01,
    "pe": 26.93,
    "close_history": "{'2021': 135.03, '2022': 42.33, '2023': 82.79, '2024': 69.03, '2025': 35.08, '2026': 45.81}",
    "status": "Success",
    "cqv_v4": 8.01,
    "price": 100.0,
    "pe_forward": 22.8905,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 100.4,
    "mos_pct": 0.4,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "USLM",
    "name": "United States Lime & Minerals, ",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 8.59,
    "f2": 9.0,
    "f3": 7.26,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.92,
    "f7": 5.13,
    "f8": 8.0,
    "momentum_score": 2.82,
    "cqv_v1": 7.8,
    "cqv_v1_1": 8.19,
    "cqv_v2": 7.85,
    "cqv_v3": 7.85,
    "cqv": 8.01,
    "pe": 23.68,
    "close_history": "{'2021': 25.37, '2022': 27.86, '2023': 45.79, '2024': 132.3, '2025': 119.61, '2026': 109.07}",
    "status": "Success",
    "cqv_v4": 8.01,
    "price": 100.0,
    "pe_forward": 20.128,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 100.4,
    "mos_pct": 0.4,
    "verdict": "Mantener",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MKC",
    "name": "McCormick & Company, Incorporat",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 8.0,
    "f2": 7.37,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.66,
    "f7": 5.6,
    "f8": 8.0,
    "momentum_score": 3.77,
    "cqv_v1": 8.15,
    "cqv_v1_1": 8.02,
    "cqv_v2": 7.84,
    "cqv_v3": 7.84,
    "cqv": 8.0,
    "pe": 8.89,
    "close_history": "{'2021': 86.94, '2022': 75.94, '2023': 64.03, '2024': 72.98, '2025': 66.9, '2026': 52.9}",
    "status": "Success",
    "cqv_v4": 8.0,
    "price": 100.0,
    "pe_forward": 7.556500000000001,
    "value_score": 6.25,
    "peg_bruto": 23.8206,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "HLT",
    "name": "Hilton Worldwide Holdings Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 9.07,
    "f2": 6.42,
    "f3": 8.89,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.98,
    "f7": 4.74,
    "f8": 8.0,
    "momentum_score": 7.66,
    "cqv_v1": 7.95,
    "cqv_v1_1": 8.07,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 8.0,
    "pe": 51.54,
    "close_history": "{'2021': 153.91, '2022': 125.1, '2023': 181.0, '2024': 246.37, '2025': 286.98, '2026': 323.2}",
    "status": "Success",
    "cqv_v4": 8.0,
    "price": 100.0,
    "pe_forward": 43.809,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DIOD",
    "name": "Diodes Incorporated",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 6.32,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.58,
    "f7": 6.94,
    "f8": 8.0,
    "momentum_score": 6.9,
    "cqv_v1": 7.84,
    "cqv_v1_1": 8.08,
    "cqv_v2": 7.84,
    "cqv_v3": 7.84,
    "cqv": 8.0,
    "pe": 52.03,
    "close_history": "{'2021': 109.81, '2022': 76.14, '2023': 80.52, '2024': 61.67, '2025': 49.34, '2026': 89.28}",
    "status": "Success",
    "cqv_v4": 8.0,
    "price": 100.0,
    "pe_forward": 44.2255,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RS",
    "name": "Reliance, Inc.",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 6.77,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.05,
    "f7": 6.58,
    "f8": 8.0,
    "momentum_score": 7.13,
    "cqv_v1": 7.7,
    "cqv_v1_1": 8.0,
    "cqv_v2": 7.83,
    "cqv_v3": 7.83,
    "cqv": 8.0,
    "pe": 24.27,
    "close_history": "{'2021': 150.9, '2022': 191.76, '2023': 269.02, '2024': 262.76, '2025': 286.72, '2026': 396.02}",
    "status": "Success",
    "cqv_v4": 8.0,
    "price": 100.0,
    "pe_forward": 20.6295,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Acumular / Compra Escalonada",
    "clasificacion": "ALTA CALIDAD",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NA9",
    "name": "Nagarro SE                    N",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 8.39,
    "f2": 8.69,
    "f3": 7.22,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.8,
    "f7": 5.29,
    "f8": 8.0,
    "momentum_score": 7.7,
    "cqv_v1": 8.37,
    "cqv_v1_1": 8.23,
    "cqv_v2": 7.87,
    "cqv_v3": 7.87,
    "cqv": 7.99,
    "pe": 19.97,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.99,
    "price": 100.0,
    "pe_forward": 16.9745,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WCN",
    "name": "Waste Connections, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.62,
    "f2": 7.36,
    "f3": 8.38,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.5,
    "f7": 5.1,
    "f8": 8.0,
    "momentum_score": 5.08,
    "cqv_v1": 7.94,
    "cqv_v1_1": 8.12,
    "cqv_v2": 7.86,
    "cqv_v3": 7.86,
    "cqv": 7.99,
    "pe": 41.18,
    "close_history": "{'2021': 132.13, '2022': 129.46, '2023': 146.89, '2024': 169.99, '2025': 174.6, '2026': 174.33}",
    "status": "Success",
    "cqv_v4": 7.99,
    "price": 100.0,
    "pe_forward": 35.003,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CLH",
    "name": "Clean Harbors, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.37,
    "f2": 8.36,
    "f3": 7.88,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.12,
    "f7": 5.3,
    "f8": 8.0,
    "momentum_score": 7.19,
    "cqv_v1": 7.91,
    "cqv_v1_1": 8.13,
    "cqv_v2": 7.84,
    "cqv_v3": 7.84,
    "cqv": 7.99,
    "pe": 39.51,
    "close_history": "{'2021': 99.77, '2022': 114.12, '2023': 174.51, '2024': 230.14, '2025': 234.48, '2026': 310.57}",
    "status": "Success",
    "cqv_v4": 7.99,
    "price": 100.0,
    "pe_forward": 33.5835,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MAR",
    "name": "Marriott International",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.46,
    "f2": 6.97,
    "f3": 9.07,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.77,
    "f7": 5.23,
    "f8": 8.0,
    "momentum_score": 7.4,
    "cqv_v1": 7.81,
    "cqv_v1_1": 8.03,
    "cqv_v2": 7.84,
    "cqv_v3": 7.84,
    "cqv": 7.99,
    "pe": 39.05,
    "close_history": "{'2021': 158.74, '2022': 143.91, '2023': 220.27, '2024': 275.17, '2025': 309.03, '2026': 371.14}",
    "status": "Success",
    "cqv_v4": 7.99,
    "price": 100.0,
    "pe_forward": 33.192499999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "VLO",
    "name": "Valero Energy Corporation",
    "sector": "Energy",
    "quarter": "Q1 2026",
    "f1": 7.57,
    "f2": 8.75,
    "f3": 8.4,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.99,
    "f7": 5.94,
    "f8": 8.0,
    "momentum_score": 9.21,
    "cqv_v1": 8.26,
    "cqv_v1_1": 8.06,
    "cqv_v2": 7.82,
    "cqv_v3": 7.82,
    "cqv": 7.98,
    "pe": 19.57,
    "close_history": "{'2021': 65.41, '2022': 114.44, '2023': 121.15, '2024': 117.57, '2025': 161.04, '2026': 300.26}",
    "status": "Success",
    "cqv_v4": 7.98,
    "price": 100.0,
    "pe_forward": 16.6345,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CI",
    "name": "The Cigna Group",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 7.29,
    "f2": 8.56,
    "f3": 8.18,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.73,
    "f7": 6.17,
    "f8": 8.0,
    "momentum_score": 5.63,
    "cqv_v1": 8.26,
    "cqv_v1_1": 8.13,
    "cqv_v2": 7.84,
    "cqv_v3": 7.84,
    "cqv": 7.96,
    "pe": 12.19,
    "close_history": "{'2021': 211.46, '2022': 310.17, '2023': 285.44, '2024': 267.55, '2025': 272.14, '2026': 283.82}",
    "status": "Success",
    "cqv_v4": 7.96,
    "price": 100.0,
    "pe_forward": 10.3615,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NKE",
    "name": "Nike, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.51,
    "f2": 8.75,
    "f3": 7.54,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.03,
    "f7": 5.19,
    "f8": 8.0,
    "momentum_score": 1.9,
    "cqv_v1": 8.01,
    "cqv_v1_1": 8.08,
    "cqv_v2": 7.79,
    "cqv_v3": 7.79,
    "cqv": 7.96,
    "pe": 21.0,
    "close_history": "{'2021': 153.97, '2022': 109.25, '2023': 102.68, '2024': 72.79, '2025': 62.73, '2026': 44.57}",
    "status": "Success",
    "cqv_v4": 7.96,
    "price": 100.0,
    "pe_forward": 17.849999999999998,
    "value_score": 6.25,
    "peg_bruto": 10.084,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "ENTG",
    "name": "Entegris, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 9.0,
    "f2": 6.98,
    "f3": 7.72,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.36,
    "f7": 4.8,
    "f8": 8.0,
    "momentum_score": 7.54,
    "cqv_v1": 7.86,
    "cqv_v1_1": 8.21,
    "cqv_v2": 7.88,
    "cqv_v3": 7.88,
    "cqv": 7.96,
    "pe": 84.77,
    "close_history": "{'2021': 136.13, '2022': 64.68, '2023': 118.71, '2024': 98.47, '2025': 84.12, '2026': 134.6}",
    "status": "Success",
    "cqv_v4": 7.96,
    "price": 100.0,
    "pe_forward": 72.05449999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ANF",
    "name": "Abercrombie & Fitch Company",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.2,
    "f2": 8.75,
    "f3": 7.83,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.02,
    "f7": 5.44,
    "f8": 8.0,
    "momentum_score": 5.03,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.05,
    "cqv_v2": 7.78,
    "cqv_v3": 7.78,
    "cqv": 7.95,
    "pe": 8.94,
    "close_history": "{'2021': 34.83, '2022': 22.91, '2023': 88.22, '2024': 149.47, '2025': 125.87, '2026': 97.37}",
    "status": "Success",
    "cqv_v4": 7.95,
    "price": 100.0,
    "pe_forward": 7.598999999999999,
    "value_score": 6.25,
    "peg_bruto": 23.6873,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "CNQ",
    "name": "CDN NATURAL RES",
    "sector": "Energy",
    "quarter": "Q1 2026",
    "f1": 8.23,
    "f2": 8.75,
    "f3": 7.53,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.88,
    "f7": 5.42,
    "f8": 8.0,
    "momentum_score": 5.51,
    "cqv_v1": 8.12,
    "cqv_v1_1": 8.1,
    "cqv_v2": 7.8,
    "cqv_v3": 7.8,
    "cqv": 7.95,
    "pe": 11.81,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.95,
    "price": 100.0,
    "pe_forward": 10.0385,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CIGI",
    "name": "Colliers International Group In",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 7.64,
    "f2": 7.19,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.48,
    "f7": 5.89,
    "f8": 8.0,
    "momentum_score": 2.0,
    "cqv_v1": 8.05,
    "cqv_v1_1": 7.99,
    "cqv_v2": 7.82,
    "cqv_v3": 7.82,
    "cqv": 7.95,
    "pe": 60.44,
    "close_history": "{'2021': 146.87, '2022': 91.22, '2023': 125.73, '2024': 135.46, '2025': 146.77, '2026': 102.21}",
    "status": "Success",
    "cqv_v4": 7.95,
    "price": 100.0,
    "pe_forward": 51.373999999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SPOT",
    "name": "Spotify Technology S.A.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 7.07,
    "f2": 9.0,
    "f3": 8.58,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.83,
    "f7": 6.34,
    "f8": 8.0,
    "momentum_score": 3.05,
    "cqv_v1": 7.39,
    "cqv_v1_1": 8.0,
    "cqv_v2": 7.79,
    "cqv_v3": 7.79,
    "cqv": 7.95,
    "pe": 33.1,
    "close_history": "{'2021': 234.03, '2022': 78.95, '2023': 187.91, '2024': 447.38, '2025': 580.71, '2026': 476.08}",
    "status": "Success",
    "cqv_v4": 7.95,
    "price": 100.0,
    "pe_forward": 28.135,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "OWL",
    "name": "Blue Owl Capital Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.44,
    "f2": 6.72,
    "f3": 8.81,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.62,
    "f7": 5.25,
    "f8": 8.0,
    "momentum_score": 1.05,
    "cqv_v1": 8.3,
    "cqv_v1_1": 8.04,
    "cqv_v2": 7.83,
    "cqv_v3": 7.83,
    "cqv": 7.94,
    "pe": 75.33,
    "close_history": "{'2021': 12.12, '2022': 8.94, '2023': 13.17, '2024': 21.31, '2025': 14.31, '2026': 9.62}",
    "status": "Success",
    "cqv_v4": 7.94,
    "price": 100.0,
    "pe_forward": 64.0305,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LSCC",
    "name": "Lattice Semiconductor Corporati",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 6.05,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.44,
    "f7": 7.16,
    "f8": 8.0,
    "momentum_score": 8.24,
    "cqv_v1": 7.33,
    "cqv_v1_1": 8.01,
    "cqv_v2": 7.79,
    "cqv_v3": 7.79,
    "cqv": 7.94,
    "pe": 975.21,
    "close_history": "{'2021': 77.06, '2022': 64.88, '2023': 68.99, '2024': 56.65, '2025': 73.58, '2026': 124.5}",
    "status": "Success",
    "cqv_v4": 7.94,
    "price": 100.0,
    "pe_forward": 828.9285,
    "value_score": 3.32,
    "peg_bruto": 0.2171,
    "score_peg": 0.2171,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "SJ",
    "name": "STELLA JONES INC",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 8.37,
    "f2": 7.92,
    "f3": 7.92,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.14,
    "f7": 5.3,
    "f8": 8.0,
    "momentum_score": 3.6,
    "cqv_v1": 8.28,
    "cqv_v1_1": 8.07,
    "cqv_v2": 7.8,
    "cqv_v3": 7.8,
    "cqv": 7.93,
    "pe": 14.5,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.93,
    "price": 100.0,
    "pe_forward": 12.325,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PATH",
    "name": "UiPath, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 6.02,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.42,
    "f7": 7.18,
    "f8": 8.0,
    "momentum_score": 3.54,
    "cqv_v1": 8.19,
    "cqv_v1_1": 8.0,
    "cqv_v2": 7.79,
    "cqv_v3": 7.79,
    "cqv": 7.93,
    "pe": 19.52,
    "close_history": "{'2021': 43.13, '2022': 12.71, '2023': 24.84, '2024': 12.71, '2025': 16.39, '2026': 12.03}",
    "status": "Success",
    "cqv_v4": 7.93,
    "price": 100.0,
    "pe_forward": 16.592,
    "value_score": 6.25,
    "peg_bruto": 10.8486,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "HD",
    "name": "Home Depot, Inc. (The)",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.44,
    "f2": 7.81,
    "f3": 8.2,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.32,
    "f7": 5.25,
    "f8": 8.0,
    "momentum_score": 5.02,
    "cqv_v1": 7.87,
    "cqv_v1_1": 8.02,
    "cqv_v2": 7.79,
    "cqv_v3": 7.79,
    "cqv": 7.93,
    "pe": 25.44,
    "close_history": "{'2021': 369.83, '2022': 288.56, '2023': 325.4, '2024': 374.2, '2025': 339.27, '2026': 348.02}",
    "status": "Success",
    "cqv_v4": 7.93,
    "price": 100.0,
    "pe_forward": 21.624,
    "value_score": 5.75,
    "peg_bruto": 8.3241,
    "score_peg": 8.3241,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "PKG",
    "name": "Packaging Corporation of Americ",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 7.51,
    "f2": 8.14,
    "f3": 8.84,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.18,
    "f7": 5.99,
    "f8": 8.0,
    "momentum_score": 7.31,
    "cqv_v1": 7.66,
    "cqv_v1_1": 7.93,
    "cqv_v2": 7.76,
    "cqv_v3": 7.76,
    "cqv": 7.92,
    "pe": 28.91,
    "close_history": "{'2021': 119.15, '2022': 116.03, '2023': 153.05, '2024': 216.86, '2025': 203.68, '2026': 234.04}",
    "status": "Success",
    "cqv_v4": 7.92,
    "price": 100.0,
    "pe_forward": 24.5735,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CPNG",
    "name": "Coupang, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 7.22,
    "f2": 9.0,
    "f3": 8.5,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.86,
    "f7": 6.22,
    "f8": 8.0,
    "momentum_score": 2.85,
    "cqv_v1": 7.17,
    "cqv_v1_1": 7.94,
    "cqv_v2": 7.74,
    "cqv_v3": 7.74,
    "cqv": 7.92,
    "pe": 67.97,
    "close_history": "{'2021': 29.38, '2022': 14.71, '2023': 16.19, '2024': 21.98, '2025': 23.59, '2026': 16.86}",
    "status": "Success",
    "cqv_v4": 7.92,
    "price": 100.0,
    "pe_forward": 57.774499999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BRBR",
    "name": "BellRing Brands, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 8.75,
    "f2": 7.68,
    "f3": 7.87,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.31,
    "f7": 5.0,
    "f8": 8.0,
    "momentum_score": 2.85,
    "cqv_v1": 8.28,
    "cqv_v1_1": 8.03,
    "cqv_v2": 7.78,
    "cqv_v3": 7.78,
    "cqv": 7.91,
    "pe": 10.74,
    "close_history": "{'2021': 28.53, '2022': 25.64, '2023': 55.43, '2024': 75.34, '2025': 26.73, '2026': 11.99}",
    "status": "Success",
    "cqv_v4": 7.91,
    "price": 100.0,
    "pe_forward": 9.129,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GLOB",
    "name": "Globant S.A.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.73,
    "f2": 9.25,
    "f3": 7.09,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.41,
    "f7": 5.82,
    "f8": 8.0,
    "momentum_score": 0.35,
    "cqv_v1": 8.2,
    "cqv_v1_1": 8.13,
    "cqv_v2": 7.79,
    "cqv_v3": 7.79,
    "cqv": 7.91,
    "pe": 13.22,
    "close_history": "{'2021': 314.09, '2022': 168.16, '2023': 237.98, '2024': 214.42, '2025': 65.37, '2026': 32.2}",
    "status": "Success",
    "cqv_v4": 7.91,
    "price": 100.0,
    "pe_forward": 11.237,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TGT",
    "name": "Target Corporation",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 7.63,
    "f2": 8.5,
    "f3": 8.41,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.02,
    "f7": 5.9,
    "f8": 8.0,
    "momentum_score": 7.78,
    "cqv_v1": 8.11,
    "cqv_v1_1": 7.95,
    "cqv_v2": 7.75,
    "cqv_v3": 7.75,
    "cqv": 7.91,
    "pe": 17.2,
    "close_history": "{'2021': 200.26, '2022': 131.69, '2023': 129.91, '2024': 126.97, '2025': 95.86, '2026': 140.21}",
    "status": "Success",
    "cqv_v4": 7.91,
    "price": 100.0,
    "pe_forward": 14.62,
    "value_score": 6.25,
    "peg_bruto": 12.3119,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "WOSG",
    "name": "WATCHES OF SWITZERLAND GROUP PL",
    "sector": "Consumer Cyclical",
    "quarter": "Q4 2025",
    "f1": 8.09,
    "f2": 7.71,
    "f3": 8.52,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.3,
    "f7": 5.53,
    "f8": 8.0,
    "momentum_score": 9.96,
    "cqv_v1": 8.07,
    "cqv_v1_1": 7.97,
    "cqv_v2": 7.76,
    "cqv_v3": 7.76,
    "cqv": 7.91,
    "pe": 25.07,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.91,
    "price": 100.0,
    "pe_forward": 21.3095,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EGP",
    "name": "EastGroup Properties, Inc.",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 8.13,
    "f2": 7.12,
    "f3": 8.68,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.41,
    "f7": 5.5,
    "f8": 8.0,
    "momentum_score": 8.21,
    "cqv_v1": 7.94,
    "cqv_v1_1": 8.0,
    "cqv_v2": 7.8,
    "cqv_v3": 7.8,
    "cqv": 7.91,
    "pe": 38.49,
    "close_history": "{'2021': 198.02, '2022': 132.54, '2023': 169.24, '2024': 152.64, '2025': 175.31, '2026': 223.38}",
    "status": "Success",
    "cqv_v4": 7.91,
    "price": 100.0,
    "pe_forward": 32.7165,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CNI",
    "name": "Canadian National Railway Compa",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.4,
    "f2": 8.0,
    "f3": 7.61,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.01,
    "f7": 5.28,
    "f8": 8.0,
    "momentum_score": 8.55,
    "cqv_v1": 7.97,
    "cqv_v1_1": 8.04,
    "cqv_v2": 7.77,
    "cqv_v3": 7.77,
    "cqv": 7.89,
    "pe": 22.72,
    "close_history": "{'2021': 112.25, '2022': 110.67, '2023': 119.34, '2024': 98.44, '2025': 97.7, '2026': 128.22}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 19.311999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WM",
    "name": "Waste Management, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.52,
    "f2": 7.26,
    "f3": 8.06,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.29,
    "f7": 5.18,
    "f8": 8.0,
    "momentum_score": 6.01,
    "cqv_v1": 7.84,
    "cqv_v1_1": 8.03,
    "cqv_v2": 7.78,
    "cqv_v3": 7.78,
    "cqv": 7.89,
    "pe": 33.34,
    "close_history": "{'2021': 155.48, '2022': 148.49, '2023': 172.54, '2024': 197.19, '2025': 217.9, '2026': 242.25}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 28.339000000000002,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CNR",
    "name": "CANADIAN NATIONAL RAILWAY CO.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.4,
    "f2": 8.0,
    "f3": 7.61,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.01,
    "f7": 5.28,
    "f8": 8.0,
    "momentum_score": 8.85,
    "cqv_v1": 7.84,
    "cqv_v1_1": 8.04,
    "cqv_v2": 7.77,
    "cqv_v3": 7.77,
    "cqv": 7.89,
    "pe": 22.72,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 19.311999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "OKTA",
    "name": "Okta, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 6.19,
    "f2": 9.5,
    "f3": 8.41,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.3,
    "f7": 7.05,
    "f8": 8.0,
    "momentum_score": 9.84,
    "cqv_v1": 7.81,
    "cqv_v1_1": 7.98,
    "cqv_v2": 7.75,
    "cqv_v3": 7.75,
    "cqv": 7.89,
    "pe": 101.74,
    "close_history": "{'2021': 224.17, '2022': 68.33, '2023': 90.53, '2024': 78.8, '2025': 86.47, '2026': 147.74}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 86.479,
    "value_score": 3.87,
    "peg_bruto": 2.0814,
    "score_peg": 2.0814,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "POOL",
    "name": "Pool Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.64,
    "f2": 8.06,
    "f3": 8.36,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.0,
    "f7": 5.89,
    "f8": 8.0,
    "momentum_score": 4.03,
    "cqv_v1": 7.78,
    "cqv_v1_1": 7.97,
    "cqv_v2": 7.76,
    "cqv_v3": 7.76,
    "cqv": 7.89,
    "pe": 20.15,
    "close_history": "{'2021': 530.38, '2022': 286.23, '2023': 382.15, '2024': 330.98, '2025': 225.71, '2026': 207.92}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 17.127499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RLI",
    "name": "RLI Corp.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 7.22,
    "f2": 9.0,
    "f3": 8.11,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.66,
    "f7": 6.22,
    "f8": 8.0,
    "momentum_score": 5.82,
    "cqv_v1": 7.5,
    "cqv_v1_1": 7.97,
    "cqv_v2": 7.74,
    "cqv_v3": 7.74,
    "cqv": 7.89,
    "pe": 14.57,
    "close_history": "{'2021': 45.74, '2022': 57.09, '2023': 59.24, '2024': 75.57, '2025': 61.11, '2026': 59.58}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 12.3845,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ACLS",
    "name": "Axcelis Technologies, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.05,
    "f2": 9.5,
    "f3": 7.53,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.29,
    "f7": 6.36,
    "f8": 8.0,
    "momentum_score": 6.82,
    "cqv_v1": 7.45,
    "cqv_v1_1": 8.07,
    "cqv_v2": 7.77,
    "cqv_v3": 7.77,
    "cqv": 7.89,
    "pe": 44.88,
    "close_history": "{'2021': 74.56, '2022': 79.36, '2023': 129.69, '2024': 69.87, '2025': 80.34, '2026': 139.34}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 38.148,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "COIN",
    "name": "Coinbase Global, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 8.72,
    "f2": 9.25,
    "f3": 6.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.53,
    "f7": 5.02,
    "f8": 8.0,
    "momentum_score": 0.96,
    "cqv_v1": 8.14,
    "cqv_v1_1": 8.12,
    "cqv_v2": 7.76,
    "cqv_v3": 7.76,
    "cqv": 7.89,
    "pe": 61.06,
    "close_history": "{'2021': 252.37, '2022': 35.39, '2023': 173.92, '2024': 248.3, '2025': 226.14, '2026': 160.49}",
    "status": "Success",
    "cqv_v4": 7.89,
    "price": 100.0,
    "pe_forward": 51.901,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LOW",
    "name": "Lowe's Companies, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 8.22,
    "f2": 6.95,
    "f3": 8.81,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.52,
    "f7": 5.42,
    "f8": 8.0,
    "momentum_score": 3.37,
    "cqv_v1": 7.94,
    "cqv_v1_1": 7.93,
    "cqv_v2": 7.75,
    "cqv_v3": 7.75,
    "cqv": 7.88,
    "pe": 19.23,
    "close_history": "{'2021': 236.93, '2022': 186.02, '2023': 212.13, '2024': 239.72, '2025': 238.93, '2026': 216.16}",
    "status": "Success",
    "cqv_v4": 7.88,
    "price": 100.0,
    "pe_forward": 16.3455,
    "value_score": 6.25,
    "peg_bruto": 11.0122,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "ASM",
    "name": "ASM International N.V.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.05,
    "f2": 9.5,
    "f3": 7.48,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.27,
    "f7": 6.36,
    "f8": 8.0,
    "momentum_score": 8.8,
    "cqv_v1": 7.25,
    "cqv_v1_1": 8.06,
    "cqv_v2": 7.76,
    "cqv_v3": 7.76,
    "cqv": 7.88,
    "pe": 48.32,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.88,
    "price": 100.0,
    "pe_forward": 41.071999999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NOC",
    "name": "Northrop Grumman Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.43,
    "f2": 8.52,
    "f3": 8.16,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.79,
    "f7": 6.06,
    "f8": 8.0,
    "momentum_score": 2.69,
    "cqv_v1": 7.74,
    "cqv_v1_1": 7.96,
    "cqv_v2": 7.74,
    "cqv_v3": 7.74,
    "cqv": 7.87,
    "pe": 17.21,
    "close_history": "{'2021': 360.09, '2022': 514.99, '2023': 449.12, '2024': 457.79, '2025': 565.9, '2026': 518.65}",
    "status": "Success",
    "cqv_v4": 7.87,
    "price": 100.0,
    "pe_forward": 14.6285,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GWRE",
    "name": "Guidewire Software, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 5.88,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.36,
    "f7": 7.3,
    "f8": 8.0,
    "momentum_score": 2.29,
    "cqv_v1": 7.68,
    "cqv_v1_1": 7.93,
    "cqv_v2": 7.74,
    "cqv_v3": 7.74,
    "cqv": 7.87,
    "pe": 72.3,
    "close_history": "{'2021': 113.53, '2022': 62.56, '2023': 109.04, '2024': 168.58, '2025': 201.01, '2026': 148.91}",
    "status": "Success",
    "cqv_v4": 7.87,
    "price": 100.0,
    "pe_forward": 61.455,
    "value_score": 4.13,
    "peg_bruto": 2.929,
    "score_peg": 2.929,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "TTEK",
    "name": "Tetra Tech, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.34,
    "f2": 8.55,
    "f3": 7.12,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.73,
    "f7": 5.33,
    "f8": 8.0,
    "momentum_score": 3.14,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.04,
    "cqv_v2": 7.74,
    "cqv_v3": 7.74,
    "cqv": 7.86,
    "pe": 17.9,
    "close_history": "{'2021': 32.96, '2022': 28.36, '2023': 32.82, '2024': 39.38, '2025': 33.4, '2026': 32.03}",
    "status": "Success",
    "cqv_v4": 7.86,
    "price": 100.0,
    "pe_forward": 15.214999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IIPR",
    "name": "Innovative Industrial Propertie",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 8.2,
    "f2": 8.63,
    "f3": 7.24,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.72,
    "f7": 5.44,
    "f8": 8.0,
    "momentum_score": 8.96,
    "cqv_v1": 8.22,
    "cqv_v1_1": 8.03,
    "cqv_v2": 7.74,
    "cqv_v3": 7.74,
    "cqv": 7.86,
    "pe": 16.16,
    "close_history": "{'2021': 170.12, '2022': 69.72, '2023': 75.84, '2024': 54.19, '2025': 44.22, '2026': 64.67}",
    "status": "Success",
    "cqv_v4": 7.86,
    "price": 100.0,
    "pe_forward": 13.735999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GPN",
    "name": "Global Payments Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.74,
    "f2": 6.48,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.54,
    "f7": 5.81,
    "f8": 8.0,
    "momentum_score": 5.83,
    "cqv_v1": 8.18,
    "cqv_v1_1": 7.91,
    "cqv_v2": 7.76,
    "cqv_v3": 7.76,
    "cqv": 7.86,
    "pe": 28.91,
    "close_history": "{'2021': 129.2, '2022': 95.72, '2023': 123.5, '2024': 109.95, '2025': 76.85, '2026': 80.49}",
    "status": "Success",
    "cqv_v4": 7.86,
    "price": 100.0,
    "pe_forward": 24.5735,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MPC",
    "name": "Marathon Petroleum Corporation",
    "sector": "Energy",
    "quarter": "Q1 2026",
    "f1": 7.5,
    "f2": 7.74,
    "f3": 8.64,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.07,
    "f7": 6.0,
    "f8": 8.0,
    "momentum_score": 8.94,
    "cqv_v1": 7.99,
    "cqv_v1_1": 7.93,
    "cqv_v2": 7.75,
    "cqv_v3": 7.75,
    "cqv": 7.86,
    "pe": 17.55,
    "close_history": "{'2021': 57.91, '2022': 108.07, '2023': 140.99, '2024': 135.27, '2025': 161.2, '2026': 305.85}",
    "status": "Success",
    "cqv_v4": 7.86,
    "price": 100.0,
    "pe_forward": 14.9175,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "JCI",
    "name": "Johnson Controls International ",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.86,
    "f2": 7.31,
    "f3": 8.58,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.22,
    "f7": 5.71,
    "f8": 8.0,
    "momentum_score": 7.23,
    "cqv_v1": 7.89,
    "cqv_v1_1": 7.95,
    "cqv_v2": 7.75,
    "cqv_v3": 7.75,
    "cqv": 7.86,
    "pe": 43.05,
    "close_history": "{'2021': 74.68, '2022': 60.28, '2023': 55.68, '2024': 77.84, '2025': 119.05, '2026': 141.26}",
    "status": "Success",
    "cqv_v4": 7.86,
    "price": 100.0,
    "pe_forward": 36.592499999999994,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FN",
    "name": "Fabrinet",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.64,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.23,
    "f7": 7.49,
    "f8": 8.0,
    "momentum_score": 4.52,
    "cqv_v1": 7.08,
    "cqv_v1_1": 7.91,
    "cqv_v2": 7.72,
    "cqv_v3": 7.72,
    "cqv": 7.86,
    "pe": 42.92,
    "close_history": "{'2021': 118.47, '2022': 128.22, '2023': 190.33, '2024': 219.88, '2025': 455.28, '2026': 460.19}",
    "status": "Success",
    "cqv_v4": 7.86,
    "price": 100.0,
    "pe_forward": 36.482,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NVR",
    "name": "NVR, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.99,
    "f2": 9.0,
    "f3": 6.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.66,
    "f7": 4.81,
    "f8": 8.0,
    "momentum_score": 4.17,
    "cqv_v1": 8.24,
    "cqv_v1_1": 8.06,
    "cqv_v2": 7.72,
    "cqv_v3": 7.72,
    "cqv": 7.85,
    "pe": 16.47,
    "close_history": "{'2021': 5908.87, '2022': 4612.58, '2023': 7000.45, '2024': 8178.9, '2025': 7292.77, '2026': 6701.1}",
    "status": "Success",
    "cqv_v4": 7.85,
    "price": 100.0,
    "pe_forward": 13.9995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FRSH",
    "name": "Freshworks Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.61,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.22,
    "f7": 7.51,
    "f8": 8.0,
    "momentum_score": 4.9,
    "cqv_v1": 8.08,
    "cqv_v1_1": 7.9,
    "cqv_v2": 7.72,
    "cqv_v3": 7.72,
    "cqv": 7.85,
    "pe": 16.95,
    "close_history": "{'2021': 26.26, '2022': 14.71, '2023': 23.49, '2024': 16.17, '2025': 12.25, '2026': 10.95}",
    "status": "Success",
    "cqv_v4": 7.85,
    "price": 100.0,
    "pe_forward": 14.407499999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MUSA",
    "name": "Murphy USA Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 7.76,
    "f2": 7.65,
    "f3": 8.62,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.19,
    "f7": 5.79,
    "f8": 8.0,
    "momentum_score": 7.84,
    "cqv_v1": 7.86,
    "cqv_v1_1": 7.89,
    "cqv_v2": 7.72,
    "cqv_v3": 7.72,
    "cqv": 7.85,
    "pe": 19.49,
    "close_history": "{'2021': 194.87, '2022': 274.81, '2023': 352.31, '2024': 497.7, '2025': 402.41, '2026': 599.01}",
    "status": "Success",
    "cqv_v4": 7.85,
    "price": 100.0,
    "pe_forward": 16.566499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ATKR",
    "name": "Atkore Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.75,
    "f2": 7.91,
    "f3": 8.13,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.94,
    "f7": 5.8,
    "f8": 8.0,
    "momentum_score": 4.95,
    "cqv_v1": 8.12,
    "cqv_v1_1": 7.94,
    "cqv_v2": 7.73,
    "cqv_v3": 7.73,
    "cqv": 7.84,
    "pe": 11.25,
    "close_history": "{'2021': 106.78, '2022': 108.92, '2023': 153.65, '2024': 81.03, '2025': 62.66, '2026': 73.12}",
    "status": "Success",
    "cqv_v4": 7.84,
    "price": 100.0,
    "pe_forward": 9.5625,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HTB",
    "name": "HomeTrust Bancshares, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 6.26,
    "f2": 9.25,
    "f3": 8.7,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.48,
    "f7": 6.99,
    "f8": 8.0,
    "momentum_score": 7.95,
    "cqv_v1": 7.89,
    "cqv_v1_1": 7.86,
    "cqv_v2": 7.69,
    "cqv_v3": 7.69,
    "cqv": 7.84,
    "pe": 12.77,
    "close_history": "{'2021': 29.01, '2022': 22.95, '2023': 26.03, '2024': 33.04, '2025': 42.67, '2026': 49.42}",
    "status": "Success",
    "cqv_v4": 7.84,
    "price": 100.0,
    "pe_forward": 10.8545,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MATX",
    "name": "Matson, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.89,
    "f2": 8.75,
    "f3": 7.32,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.61,
    "f7": 5.69,
    "f8": 8.0,
    "momentum_score": 9.42,
    "cqv_v1": 7.89,
    "cqv_v1_1": 7.98,
    "cqv_v2": 7.71,
    "cqv_v3": 7.71,
    "cqv": 7.83,
    "pe": 15.11,
    "close_history": "{'2021': 84.95, '2022': 59.87, '2023': 106.68, '2024': 132.6, '2025': 123.04, '2026': 223.55}",
    "status": "Success",
    "cqv_v4": 7.83,
    "price": 100.0,
    "pe_forward": 12.843499999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PAC",
    "name": "Grupo Aeroportuario Del Pacific",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.48,
    "f2": 8.37,
    "f3": 7.98,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.73,
    "f7": 6.02,
    "f8": 8.0,
    "momentum_score": 5.34,
    "cqv_v1": 8.11,
    "cqv_v1_1": 7.92,
    "cqv_v2": 7.71,
    "cqv_v3": 7.71,
    "cqv": 7.82,
    "pe": 22.41,
    "close_history": "{'2021': 114.63, '2022': 125.85, '2023': 161.88, '2024': 168.66, '2025': 263.63, '2026': 225.27}",
    "status": "Success",
    "cqv_v4": 7.82,
    "price": 100.0,
    "pe_forward": 19.0485,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SNPS",
    "name": "Synopsys, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 7.4,
    "f2": 6.81,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 8.12,
    "f7": 6.08,
    "f8": 8.0,
    "momentum_score": 3.78,
    "cqv_v1": 7.91,
    "cqv_v1_1": 7.95,
    "cqv_v2": 7.75,
    "cqv_v3": 7.75,
    "cqv": 7.82,
    "pe": 100.04,
    "close_history": "{'2021': 368.5, '2022': 319.29, '2023': 514.91, '2024': 485.36, '2025': 469.72, '2026': 417.03}",
    "status": "Success",
    "cqv_v4": 7.82,
    "price": 100.0,
    "pe_forward": 85.034,
    "value_score": 3.89,
    "peg_bruto": 2.1168,
    "score_peg": 2.1168,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "CEG",
    "name": "Constellation Energy Corporatio",
    "sector": "Utilities",
    "quarter": "Q1 2026",
    "f1": 5.92,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.62,
    "f7": 7.26,
    "f8": 8.0,
    "momentum_score": 0.47,
    "cqv_v1": 7.07,
    "cqv_v1_1": 7.79,
    "cqv_v2": 7.68,
    "cqv_v3": 7.68,
    "cqv": 7.82,
    "pe": 20.79,
    "close_history": "{'2022': 83.87, '2023': 115.1, '2024': 221.82, '2025': 352.25, '2026': 251.77}",
    "status": "Success",
    "cqv_v4": 7.82,
    "price": 100.0,
    "pe_forward": 17.671499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HUBS",
    "name": "HubSpot, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.44,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.13,
    "f7": 7.65,
    "f8": 8.0,
    "momentum_score": 0.91,
    "cqv_v1": 7.96,
    "cqv_v1_1": 7.86,
    "cqv_v2": 7.69,
    "cqv_v3": 7.69,
    "cqv": 7.81,
    "pe": 101.12,
    "close_history": "{'2021': 659.15, '2022': 289.13, '2023': 580.54, '2024': 696.77, '2025': 401.3, '2026': 224.57}",
    "status": "Success",
    "cqv_v4": 7.81,
    "price": 100.0,
    "pe_forward": 85.952,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CACI",
    "name": "CACI International, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.5,
    "f2": 7.57,
    "f3": 8.11,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.8,
    "f7": 6.0,
    "f8": 8.0,
    "momentum_score": 2.99,
    "cqv_v1": 7.91,
    "cqv_v1_1": 7.98,
    "cqv_v2": 7.74,
    "cqv_v3": 7.74,
    "cqv": 7.81,
    "pe": 20.74,
    "close_history": "{'2021': 269.21, '2022': 300.59, '2023': 323.86, '2024': 404.06, '2025': 532.81, '2026': 462.85}",
    "status": "Success",
    "cqv_v4": 7.81,
    "price": 100.0,
    "pe_forward": 17.628999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TWLO",
    "name": "Twilio Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.59,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.21,
    "f7": 7.53,
    "f8": 8.0,
    "momentum_score": 8.94,
    "cqv_v1": 7.54,
    "cqv_v1_1": 7.86,
    "cqv_v2": 7.69,
    "cqv_v3": 7.69,
    "cqv": 7.81,
    "pe": 312.4,
    "close_history": "{'2021': 263.34, '2022': 48.96, '2023': 75.87, '2024': 108.08, '2025': 142.24, '2026': 206.7}",
    "status": "Success",
    "cqv_v4": 7.81,
    "price": 100.0,
    "pe_forward": 265.53999999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ALGM",
    "name": "Allegro MicroSystems, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.83,
    "f2": 8.95,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.33,
    "f7": 7.34,
    "f8": 8.0,
    "momentum_score": 8.66,
    "cqv_v1": 7.39,
    "cqv_v1_1": 7.87,
    "cqv_v2": 7.7,
    "cqv_v3": 7.7,
    "cqv": 7.81,
    "pe": 37.3,
    "close_history": "{'2021': 36.18, '2022': 30.02, '2023': 30.27, '2024': 21.86, '2025': 26.38, '2026': 47.12}",
    "status": "Success",
    "cqv_v4": 7.81,
    "price": 100.0,
    "pe_forward": 31.705,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GNRC",
    "name": "Generac Holdlings Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 6.71,
    "f2": 7.86,
    "f3": 9.04,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.88,
    "f7": 6.63,
    "f8": 8.0,
    "momentum_score": 7.43,
    "cqv_v1": 7.28,
    "cqv_v1_1": 7.81,
    "cqv_v2": 7.68,
    "cqv_v3": 7.68,
    "cqv": 7.8,
    "pe": 78.71,
    "close_history": "{'2021': 351.92, '2022': 100.66, '2023': 129.24, '2024': 155.05, '2025': 136.37, '2026': 215.38}",
    "status": "Success",
    "cqv_v4": 7.8,
    "price": 100.0,
    "pe_forward": 66.9035,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "STZ",
    "name": "Constellation Brands, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 9.23,
    "f2": 6.94,
    "f3": 7.3,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.27,
    "f7": 4.62,
    "f8": 8.0,
    "momentum_score": 3.42,
    "cqv_v1": 8.28,
    "cqv_v1_1": 7.95,
    "cqv_v2": 7.7,
    "cqv_v3": 7.7,
    "cqv": 7.79,
    "pe": 13.09,
    "close_history": "{'2021': 231.23, '2022': 216.37, '2023': 228.99, '2024': 212.72, '2025': 136.16, '2026': 135.72}",
    "status": "Success",
    "cqv_v4": 7.79,
    "price": 100.0,
    "pe_forward": 11.1265,
    "value_score": 6.25,
    "peg_bruto": 16.1776,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "DHI",
    "name": "D.R. Horton, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 7.85,
    "f2": 8.75,
    "f3": 7.41,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.63,
    "f7": 5.72,
    "f8": 8.0,
    "momentum_score": 5.63,
    "cqv_v1": 8.1,
    "cqv_v1_1": 7.9,
    "cqv_v2": 7.66,
    "cqv_v3": 7.66,
    "cqv": 7.79,
    "pe": 14.88,
    "close_history": "{'2021': 103.48, '2022': 86.07, '2023': 148.09, '2024': 137.37, '2025': 143.19, '2026': 154.42}",
    "status": "Success",
    "cqv_v4": 7.79,
    "price": 100.0,
    "pe_forward": 12.648,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MOH",
    "name": "Molina Healthcare Inc",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 6.96,
    "f2": 9.25,
    "f3": 7.19,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.08,
    "f7": 6.43,
    "f8": 8.0,
    "momentum_score": 9.69,
    "cqv_v1": 7.99,
    "cqv_v1_1": 8.0,
    "cqv_v2": 7.71,
    "cqv_v3": 7.71,
    "cqv": 7.79,
    "pe": 61.79,
    "close_history": "{'2021': 318.08, '2022': 330.22, '2023': 361.31, '2024': 291.05, '2025': 173.54, '2026': 224.82}",
    "status": "Success",
    "cqv_v4": 7.79,
    "price": 100.0,
    "pe_forward": 52.521499999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CSGP",
    "name": "CoStar Group, Inc.",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 5.57,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.45,
    "f7": 7.54,
    "f8": 8.0,
    "momentum_score": 0.59,
    "cqv_v1": 7.32,
    "cqv_v1_1": 7.74,
    "cqv_v2": 7.65,
    "cqv_v3": 7.65,
    "cqv": 7.79,
    "pe": 428.57,
    "close_history": "{'2021': 79.03, '2022': 77.28, '2023': 87.39, '2024': 71.59, '2025': 67.24, '2026': 30.37}",
    "status": "Success",
    "cqv_v4": 7.79,
    "price": 100.0,
    "pe_forward": 364.2845,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TDOC",
    "name": "Teladoc Health, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 6.95,
    "f2": 8.9,
    "f3": 7.39,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.17,
    "f7": 6.44,
    "f8": 8.0,
    "momentum_score": 9.08,
    "cqv_v1": 7.92,
    "cqv_v1_1": 7.98,
    "cqv_v2": 7.7,
    "cqv_v3": 7.7,
    "cqv": 7.78,
    "pe": -14.21,
    "close_history": "{'2021': 91.82, '2022': 23.65, '2023': 21.55, '2024': 9.09, '2025': 7.0, '2026': 9.47}",
    "status": "Success",
    "cqv_v4": 7.78,
    "price": 100.0,
    "pe_forward": -12.0785,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LOTB",
    "name": "LOTUS BAKERIES",
    "sector": "Consumer Defensive",
    "quarter": "Q4 2025",
    "f1": 6.47,
    "f2": 8.75,
    "f3": 8.81,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.64,
    "f7": 6.82,
    "f8": 8.0,
    "momentum_score": 9.13,
    "cqv_v1": 7.13,
    "cqv_v1_1": 7.76,
    "cqv_v2": 7.64,
    "cqv_v3": 7.64,
    "cqv": 7.78,
    "pe": 55.56,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.78,
    "price": 100.0,
    "pe_forward": 47.226,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CVNA",
    "name": "Carvana Co.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 5.94,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.63,
    "f7": 7.25,
    "f8": 8.0,
    "momentum_score": 3.64,
    "cqv_v1": 7.06,
    "cqv_v1_1": 7.71,
    "cqv_v2": 7.62,
    "cqv_v3": 7.62,
    "cqv": 7.78,
    "pe": 39.65,
    "close_history": "{'2021': 46.36, '2022': 0.95, '2023': 10.59, '2024': 40.67, '2025': 84.4, '2026': 70.66}",
    "status": "Success",
    "cqv_v4": 7.78,
    "price": 100.0,
    "pe_forward": 33.7025,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SMCI",
    "name": "Super Micro Computer, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.27,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.05,
    "f7": 7.78,
    "f8": 8.0,
    "momentum_score": 1.42,
    "cqv_v1": 6.98,
    "cqv_v1_1": 7.82,
    "cqv_v2": 7.66,
    "cqv_v3": 7.66,
    "cqv": 7.78,
    "pe": 14.33,
    "close_history": "{'2021': 4.39, '2022': 8.21, '2023': 28.43, '2024': 30.48, '2025': 29.27, '2026': 24.68}",
    "status": "Success",
    "cqv_v4": 7.78,
    "price": 100.0,
    "pe_forward": 12.1805,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "INSP",
    "name": "Inspire Medical Systems, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 6.21,
    "f2": 9.25,
    "f3": 7.84,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.03,
    "f7": 7.03,
    "f8": 8.0,
    "momentum_score": 1.79,
    "cqv_v1": 8.05,
    "cqv_v1_1": 7.91,
    "cqv_v2": 7.68,
    "cqv_v3": 7.68,
    "cqv": 7.77,
    "pe": 10.67,
    "close_history": "{'2021': 230.06, '2022': 251.88, '2023': 203.43, '2024': 185.38, '2025': 92.23, '2026': 51.54}",
    "status": "Success",
    "cqv_v4": 7.77,
    "price": 100.0,
    "pe_forward": 9.0695,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SONY",
    "name": "Sony Group Corporation",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.91,
    "f2": 9.5,
    "f3": 8.09,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.0,
    "f7": 7.27,
    "f8": 8.0,
    "momentum_score": 2.22,
    "cqv_v1": 7.01,
    "cqv_v1_1": 7.87,
    "cqv_v2": 7.66,
    "cqv_v3": 7.66,
    "cqv": 7.76,
    "pe": 19.61,
    "close_history": "{'2021': 24.72, '2022': 15.01, '2023': 18.76, '2024': 21.1, '2025': 25.6, '2026': 21.39}",
    "status": "Success",
    "cqv_v4": 7.76,
    "price": 100.0,
    "pe_forward": 16.668499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NRG",
    "name": "NRG Energy, Inc.",
    "sector": "Utilities",
    "quarter": "Q1 2026",
    "f1": 6.95,
    "f2": 6.83,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.14,
    "f7": 6.44,
    "f8": 8.0,
    "momentum_score": 2.55,
    "cqv_v1": 7.42,
    "cqv_v1_1": 7.76,
    "cqv_v2": 7.66,
    "cqv_v3": 7.66,
    "cqv": 7.75,
    "pe": 148.59,
    "close_history": "{'2021': 38.21, '2022': 29.25, '2023': 49.53, '2024': 88.46, '2025': 158.26, '2026': 132.75}",
    "status": "Success",
    "cqv_v4": 7.75,
    "price": 100.0,
    "pe_forward": 126.3015,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BAH",
    "name": "Booz Allen Hamilton Holding Cor",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 8.58,
    "f2": 7.62,
    "f3": 6.94,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.76,
    "f7": 5.14,
    "f8": 8.0,
    "momentum_score": 0.33,
    "cqv_v1": 8.15,
    "cqv_v1_1": 7.93,
    "cqv_v2": 7.66,
    "cqv_v3": 7.66,
    "cqv": 7.73,
    "pe": 9.05,
    "close_history": "{'2021': 77.73, '2022': 97.71, '2023': 121.62, '2024': 124.06, '2025': 83.1, '2026': 65.26}",
    "status": "Success",
    "cqv_v4": 7.73,
    "price": 100.0,
    "pe_forward": 7.692500000000001,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ICLR",
    "name": "ICON plc",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 7.58,
    "f2": 7.17,
    "f3": 7.77,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.67,
    "f7": 5.94,
    "f8": 8.0,
    "momentum_score": 6.62,
    "cqv_v1": 8.03,
    "cqv_v1_1": 7.93,
    "cqv_v2": 7.69,
    "cqv_v3": 7.69,
    "cqv": 7.73,
    "pe": 76.24,
    "close_history": "{'2021': 309.7, '2022': 194.25, '2023': 283.07, '2024': 209.71, '2025': 182.22, '2026': 170.0}",
    "status": "Success",
    "cqv_v4": 7.73,
    "price": 100.0,
    "pe_forward": 64.80399999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ELV",
    "name": "Elevance Health, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 6.81,
    "f2": 7.97,
    "f3": 7.96,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.38,
    "f7": 6.55,
    "f8": 8.0,
    "momentum_score": 8.82,
    "cqv_v1": 7.88,
    "cqv_v1_1": 7.89,
    "cqv_v2": 7.67,
    "cqv_v3": 7.67,
    "cqv": 7.72,
    "pe": 17.7,
    "close_history": "{'2021': 433.94, '2022': 485.27, '2023': 451.84, '2024': 358.22, '2025': 346.99, '2026': 372.85}",
    "status": "Success",
    "cqv_v4": 7.72,
    "price": 100.0,
    "pe_forward": 15.044999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "U",
    "name": "Unity Software Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.5,
    "f2": 8.8,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.17,
    "f7": 7.6,
    "f8": 8.0,
    "momentum_score": 3.68,
    "cqv_v1": 7.68,
    "cqv_v1_1": 7.77,
    "cqv_v2": 7.63,
    "cqv_v3": 7.63,
    "cqv": 7.72,
    "pe": 22.39,
    "close_history": "{'2021': 142.99, '2022': 28.59, '2023': 40.89, '2024': 22.47, '2025': 44.17, '2026': 30.19}",
    "status": "Success",
    "cqv_v4": 7.72,
    "price": 100.0,
    "pe_forward": 19.0315,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TFII",
    "name": "TFI International Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.73,
    "f2": 7.77,
    "f3": 7.58,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.66,
    "f7": 5.82,
    "f8": 8.0,
    "momentum_score": 7.02,
    "cqv_v1": 7.94,
    "cqv_v1_1": 7.83,
    "cqv_v2": 7.63,
    "cqv_v3": 7.63,
    "cqv": 7.71,
    "pe": 38.76,
    "close_history": "{'2021': 106.2, '2022': 96.14, '2023': 131.95, '2024': 132.6, '2025': 102.55, '2026': 156.62}",
    "status": "Success",
    "cqv_v4": 7.71,
    "price": 100.0,
    "pe_forward": 32.946,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HON",
    "name": "Honeywell International Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.94,
    "f2": 7.05,
    "f3": 7.93,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.94,
    "f7": 5.65,
    "f8": 8.0,
    "momentum_score": 5.41,
    "cqv_v1": 7.88,
    "cqv_v1_1": 7.83,
    "cqv_v2": 7.64,
    "cqv_v3": 7.64,
    "cqv": 7.71,
    "pe": 18.34,
    "close_history": "{'2021': 187.34, '2022': 196.52, '2023': 196.56, '2024': 216.25, '2025': 202.48, '2026': 226.33}",
    "status": "Success",
    "cqv_v4": 7.71,
    "price": 100.0,
    "pe_forward": 15.588999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "VMC",
    "name": "Vulcan Materials Company (Holdi",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 6.5,
    "f2": 8.31,
    "f3": 8.49,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.5,
    "f7": 6.8,
    "f8": 8.0,
    "momentum_score": 5.9,
    "cqv_v1": 7.17,
    "cqv_v1_1": 7.75,
    "cqv_v2": 7.61,
    "cqv_v3": 7.61,
    "cqv": 7.71,
    "pe": 35.97,
    "close_history": "{'2021': 200.22, '2022': 170.46, '2023': 222.88, '2024': 254.35, '2025': 284.12, '2026': 293.47}",
    "status": "Success",
    "cqv_v4": 7.71,
    "price": 100.0,
    "pe_forward": 30.574499999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SOUN",
    "name": "SoundHound AI, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 4.96,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.89,
    "f7": 8.03,
    "f8": 8.0,
    "momentum_score": 0.13,
    "cqv_v1": 6.84,
    "cqv_v1_1": 7.74,
    "cqv_v2": 7.61,
    "cqv_v3": 7.61,
    "cqv": 7.71,
    "pe": -54.58,
    "close_history": "{'2022': 1.77, '2023': 2.12, '2024': 19.84, '2025': 9.97, '2026': 6.31}",
    "status": "Success",
    "cqv_v4": 7.71,
    "price": 100.0,
    "pe_forward": -46.393,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SCI",
    "name": "Service Corporation Internation",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.4,
    "f2": 6.72,
    "f3": 7.9,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.15,
    "f7": 5.28,
    "f8": 8.0,
    "momentum_score": 4.77,
    "cqv_v1": 7.81,
    "cqv_v1_1": 7.8,
    "cqv_v2": 7.63,
    "cqv_v3": 7.63,
    "cqv": 7.7,
    "pe": 20.73,
    "close_history": "{'2021': 65.92, '2022': 65.23, '2023': 65.71, '2024': 77.82, '2025': 77.27, '2026': 79.67}",
    "status": "Success",
    "cqv_v4": 7.7,
    "price": 100.0,
    "pe_forward": 17.6205,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SPCX",
    "name": "Space Exploration Technologies ",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 5.1,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.21,
    "f7": 7.92,
    "f8": 8.0,
    "momentum_score": 3.54,
    "cqv_v1": 6.78,
    "cqv_v1_1": 7.62,
    "cqv_v2": 7.57,
    "cqv_v3": 7.57,
    "cqv": 7.69,
    "pe": 823.71,
    "close_history": "{'2026': 131.11}",
    "status": "Success",
    "cqv_v4": 7.69,
    "price": 100.0,
    "pe_forward": 700.1535,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MAA",
    "name": "Mid-America Apartment Communiti",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 8.42,
    "f2": 6.42,
    "f3": 7.76,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.09,
    "f7": 5.26,
    "f8": 8.0,
    "momentum_score": 6.37,
    "cqv_v1": 8.04,
    "cqv_v1_1": 7.83,
    "cqv_v2": 7.64,
    "cqv_v3": 7.64,
    "cqv": 7.68,
    "pe": 43.09,
    "close_history": "{'2021': 191.15, '2022': 134.28, '2023': 119.42, '2024': 143.24, '2025': 134.13, '2026': 134.86}",
    "status": "Success",
    "cqv_v4": 7.68,
    "price": 100.0,
    "pe_forward": 36.6265,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CMCSA",
    "name": "Comcast Corporation",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 6.51,
    "f2": 8.36,
    "f3": 8.26,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.38,
    "f7": 6.79,
    "f8": 8.0,
    "momentum_score": 2.03,
    "cqv_v1": 7.78,
    "cqv_v1_1": 7.72,
    "cqv_v2": 7.58,
    "cqv_v3": 7.58,
    "cqv": 7.67,
    "pe": 4.66,
    "close_history": "{'2021': 40.25, '2022': 28.71, '2023': 37.06, '2024': 32.67, '2025': 27.0, '2026': 24.1}",
    "status": "Success",
    "cqv_v4": 7.67,
    "price": 100.0,
    "pe_forward": 3.961,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LW",
    "name": "Lamb Weston Holdings, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 8.13,
    "f2": 6.67,
    "f3": 7.99,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.06,
    "f7": 5.5,
    "f8": 8.0,
    "momentum_score": 3.53,
    "cqv_v1": 8.08,
    "cqv_v1_1": 7.74,
    "cqv_v2": 7.59,
    "cqv_v3": 7.59,
    "cqv": 7.66,
    "pe": 21.55,
    "close_history": "{'2021': 58.16, '2022': 83.1, '2023': 101.65, '2024': 64.03, '2025': 41.18, '2026': 46.92}",
    "status": "Success",
    "cqv_v4": 7.66,
    "price": 100.0,
    "pe_forward": 18.3175,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PHM",
    "name": "PulteGroup, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 8.22,
    "f2": 8.75,
    "f3": 6.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.28,
    "f7": 5.42,
    "f8": 8.0,
    "momentum_score": 6.53,
    "cqv_v1": 8.0,
    "cqv_v1_1": 7.83,
    "cqv_v2": 7.56,
    "cqv_v3": 7.56,
    "cqv": 7.66,
    "pe": 12.91,
    "close_history": "{'2021': 54.81, '2022': 44.27, '2023': 101.28, '2024': 107.58, '2025': 116.76, '2026': 129.02}",
    "status": "Success",
    "cqv_v4": 7.66,
    "price": 100.0,
    "pe_forward": 10.9735,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EXR",
    "name": "Extra Space Storage Inc",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 8.37,
    "f2": 5.92,
    "f3": 8.09,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.23,
    "f7": 5.3,
    "f8": 8.0,
    "momentum_score": 6.76,
    "cqv_v1": 7.87,
    "cqv_v1_1": 7.79,
    "cqv_v2": 7.63,
    "cqv_v3": 7.63,
    "cqv": 7.66,
    "pe": 33.56,
    "close_history": "{'2021': 188.06, '2022': 126.34, '2023': 143.86, '2024': 139.81, '2025': 127.34, '2026': 150.33}",
    "status": "Success",
    "cqv_v4": 7.66,
    "price": 100.0,
    "pe_forward": 28.526,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ELF",
    "name": "e.l.f. Beauty, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 6.94,
    "f2": 6.59,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 8.13,
    "f7": 6.45,
    "f8": 8.0,
    "momentum_score": 4.21,
    "cqv_v1": 7.47,
    "cqv_v1_1": 7.63,
    "cqv_v2": 7.58,
    "cqv_v3": 7.58,
    "cqv": 7.66,
    "pe": 173.68,
    "close_history": "{'2021': 33.21, '2022': 55.3, '2023': 144.34, '2024': 125.55, '2025': 76.04, '2026': 74.49}",
    "status": "Success",
    "cqv_v4": 7.66,
    "price": 100.0,
    "pe_forward": 147.62800000000001,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "OSCR",
    "name": "Oscar Health, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 4.26,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 6.79,
    "f7": 8.59,
    "f8": 8.0,
    "momentum_score": 9.88,
    "cqv_v1": 7.8,
    "cqv_v1_1": 7.65,
    "cqv_v2": 7.57,
    "cqv_v3": 7.57,
    "cqv": 7.65,
    "pe": 21.67,
    "close_history": "{'2021': 7.85, '2022': 2.46, '2023': 9.15, '2024': 13.44, '2025': 14.37, '2026': 28.86}",
    "status": "Success",
    "cqv_v4": 7.65,
    "price": 100.0,
    "pe_forward": 18.4195,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WMT",
    "name": "Walmart Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 6.1,
    "f2": 8.75,
    "f3": 8.48,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.29,
    "f7": 7.12,
    "f8": 8.0,
    "momentum_score": 4.36,
    "cqv_v1": 7.09,
    "cqv_v1_1": 7.62,
    "cqv_v2": 7.52,
    "cqv_v3": 7.52,
    "cqv": 7.64,
    "pe": 39.38,
    "close_history": "{'2021': 45.6, '2022': 45.39, '2023': 51.23, '2024': 89.14, '2025': 110.97, '2026': 114.95}",
    "status": "Success",
    "cqv_v4": 7.64,
    "price": 100.0,
    "pe_forward": 33.473,
    "value_score": 4.86,
    "peg_bruto": 5.3775,
    "score_peg": 5.3775,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "APPS",
    "name": "Digital Turbine, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.75,
    "f2": 7.78,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.29,
    "f7": 7.4,
    "f8": 8.0,
    "momentum_score": 9.75,
    "cqv_v1": 7.24,
    "cqv_v1_1": 7.68,
    "cqv_v2": 7.57,
    "cqv_v3": 7.57,
    "cqv": 7.62,
    "pe": 11.99,
    "close_history": "{'2021': 60.99, '2022': 15.24, '2023': 6.86, '2024': 1.69, '2025': 5.0, '2026': 8.63}",
    "status": "Success",
    "cqv_v4": 7.62,
    "price": 100.0,
    "pe_forward": 10.1915,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MELI",
    "name": "MercadoLibre, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 5.51,
    "f2": 8.28,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.42,
    "f7": 7.59,
    "f8": 8.0,
    "momentum_score": 3.27,
    "cqv_v1": 6.86,
    "cqv_v1_1": 7.53,
    "cqv_v2": 7.5,
    "cqv_v3": 7.5,
    "cqv": 7.62,
    "pe": 46.51,
    "close_history": "{'2021': 1348.4, '2022': 846.24, '2023': 1571.54, '2024': 1700.44, '2025': 2014.26, '2026': 1857.42}",
    "status": "Success",
    "cqv_v4": 7.62,
    "price": 100.0,
    "pe_forward": 39.5335,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "UDR",
    "name": "UDR, Inc.",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 7.82,
    "f2": 6.27,
    "f3": 8.13,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.98,
    "f7": 5.74,
    "f8": 8.0,
    "momentum_score": 7.92,
    "cqv_v1": 8.08,
    "cqv_v1_1": 7.71,
    "cqv_v2": 7.58,
    "cqv_v3": 7.58,
    "cqv": 7.61,
    "pe": 27.95,
    "close_history": "{'2021': 49.97, '2022': 33.26, '2023': 34.3, '2024': 40.57, '2025': 35.8, '2026': 40.57}",
    "status": "Success",
    "cqv_v4": 7.61,
    "price": 100.0,
    "pe_forward": 23.7575,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DOLE",
    "name": "Dole plc",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 6.33,
    "f2": 7.57,
    "f3": 8.96,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.65,
    "f7": 6.94,
    "f8": 8.0,
    "momentum_score": 4.21,
    "cqv_v1": 7.84,
    "cqv_v1_1": 7.57,
    "cqv_v2": 7.52,
    "cqv_v3": 7.52,
    "cqv": 7.61,
    "pe": 15.08,
    "close_history": "{'2021': 11.99, '2022': 8.96, '2023': 11.72, '2024': 13.23, '2025': 14.81, '2026': 14.5}",
    "status": "Success",
    "cqv_v4": 7.61,
    "price": 100.0,
    "pe_forward": 12.818,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CP",
    "name": "Canadian Pacific Kansas City Li",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.53,
    "f2": 7.58,
    "f3": 7.39,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.46,
    "f7": 5.98,
    "f8": 8.0,
    "momentum_score": 7.53,
    "cqv_v1": 7.51,
    "cqv_v1_1": 7.73,
    "cqv_v2": 7.55,
    "cqv_v3": 7.55,
    "cqv": 7.6,
    "pe": 27.87,
    "close_history": "{'2021': 69.5, '2022': 72.77, '2023': 77.71, '2024': 71.62, '2025': 73.32, '2026': 92.91}",
    "status": "Success",
    "cqv_v4": 7.6,
    "price": 100.0,
    "pe_forward": 23.6895,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "STLD",
    "name": "Steel Dynamics, Inc.",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 4.96,
    "f2": 8.64,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.14,
    "f7": 8.03,
    "f8": 8.0,
    "momentum_score": 6.13,
    "cqv_v1": 6.83,
    "cqv_v1_1": 7.54,
    "cqv_v2": 7.51,
    "cqv_v3": 7.51,
    "cqv": 7.6,
    "pe": 23.6,
    "close_history": "{'2021': 58.05, '2022': 92.96, '2023': 114.12, '2024': 111.84, '2025': 168.54, '2026': 234.98}",
    "status": "Success",
    "cqv_v4": 7.6,
    "price": 100.0,
    "pe_forward": 20.060000000000002,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DIS",
    "name": "Walt Disney Company (The)",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 6.06,
    "f2": 8.26,
    "f3": 8.39,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.22,
    "f7": 7.15,
    "f8": 8.0,
    "momentum_score": 3.3,
    "cqv_v1": 7.36,
    "cqv_v1_1": 7.61,
    "cqv_v2": 7.51,
    "cqv_v3": 7.51,
    "cqv": 7.59,
    "pe": 15.92,
    "close_history": "{'2021': 150.18, '2022': 84.24, '2023': 87.83, '2024': 109.29, '2025': 112.9, '2026': 99.71}",
    "status": "Success",
    "cqv_v4": 7.59,
    "price": 100.0,
    "pe_forward": 13.532,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "VICI",
    "name": "VICI Properties Inc.",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 7.68,
    "f2": 6.28,
    "f3": 8.06,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.87,
    "f7": 5.86,
    "f8": 8.0,
    "momentum_score": 3.09,
    "cqv_v1": 7.75,
    "cqv_v1_1": 7.67,
    "cqv_v2": 7.54,
    "cqv_v3": 7.54,
    "cqv": 7.57,
    "pe": 9.31,
    "close_history": "{'2021': 23.54, '2022': 26.6, '2023': 27.55, '2024': 26.7, '2025': 27.21, '2026': 27.15}",
    "status": "Success",
    "cqv_v4": 7.57,
    "price": 100.0,
    "pe_forward": 7.9135,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NUE",
    "name": "Nucor Corporation",
    "sector": "Basic Materials",
    "quarter": "Q2 2026",
    "f1": 4.73,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.03,
    "f7": 8.22,
    "f8": 8.0,
    "momentum_score": 6.75,
    "cqv_v1": 6.79,
    "cqv_v1_1": 7.49,
    "cqv_v2": 7.48,
    "cqv_v3": 7.48,
    "cqv": 7.57,
    "pe": 21.88,
    "close_history": "{'2021': 106.86, '2022': 125.44, '2023': 167.78, '2024': 114.17, '2025': 162.16, '2026': 235.67}",
    "status": "Success",
    "cqv_v4": 7.57,
    "price": 100.0,
    "pe_forward": 18.598,
    "value_score": 6.15,
    "peg_bruto": 9.6785,
    "score_peg": 9.6785,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "NSP",
    "name": "Insperity, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 5.92,
    "f2": 9.0,
    "f3": 7.86,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.89,
    "f7": 7.26,
    "f8": 8.0,
    "momentum_score": 7.28,
    "cqv_v1": 7.57,
    "cqv_v1_1": 7.61,
    "cqv_v2": 7.49,
    "cqv_v3": 7.49,
    "cqv": 7.56,
    "pe": 16.23,
    "close_history": "{'2021': 101.1, '2022': 99.17, '2023': 104.37, '2024': 70.83, '2025': 36.99, '2026': 49.48}",
    "status": "Success",
    "cqv_v4": 7.56,
    "price": 100.0,
    "pe_forward": 13.7955,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NESN",
    "name": "NESTLE N",
    "sector": "Consumer Defensive",
    "quarter": "Q4 2025",
    "f1": 7.88,
    "f2": 7.13,
    "f3": 7.42,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.65,
    "f7": 5.7,
    "f8": 8.0,
    "momentum_score": 7.38,
    "cqv_v1": 7.58,
    "cqv_v1_1": 7.66,
    "cqv_v2": 7.51,
    "cqv_v3": 7.51,
    "cqv": 7.56,
    "pe": 24.09,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.56,
    "price": 100.0,
    "pe_forward": 20.476499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FORM",
    "name": "FormFactor, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 4.26,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.54,
    "f7": 8.59,
    "f8": 8.0,
    "momentum_score": 7.19,
    "cqv_v1": 6.75,
    "cqv_v1_1": 7.56,
    "cqv_v2": 7.49,
    "cqv_v3": 7.49,
    "cqv": 7.56,
    "pe": 142.06,
    "close_history": "{'2021': 45.72, '2022': 22.23, '2023': 41.71, '2024': 44.0, '2025': 55.78, '2026': 107.11}",
    "status": "Success",
    "cqv_v4": 7.56,
    "price": 100.0,
    "pe_forward": 120.751,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SBAC",
    "name": "SBA Communications Corporation",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 5.96,
    "f2": 8.25,
    "f3": 8.32,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.14,
    "f7": 7.23,
    "f8": 8.0,
    "momentum_score": 3.72,
    "cqv_v1": 6.9,
    "cqv_v1_1": 7.58,
    "cqv_v2": 7.49,
    "cqv_v3": 7.49,
    "cqv": 7.55,
    "pe": 19.45,
    "close_history": "{'2021': 360.81, '2022': 262.31, '2023': 240.92, '2024': 197.14, '2025': 190.97, '2026': 185.04}",
    "status": "Success",
    "cqv_v4": 7.55,
    "price": 100.0,
    "pe_forward": 16.5325,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "8001",
    "name": "ITOCHU CORP",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 6.29,
    "f2": 7.98,
    "f3": 8.12,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.21,
    "f7": 6.97,
    "f8": 8.0,
    "momentum_score": 4.67,
    "cqv_v1": 7.56,
    "cqv_v1_1": 7.59,
    "cqv_v2": 7.49,
    "cqv_v3": 7.49,
    "cqv": 7.54,
    "pe": 14.75,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.54,
    "price": 100.0,
    "pe_forward": 12.5375,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CHTR",
    "name": "Charter Communications, Inc.",
    "sector": "Communication Services",
    "quarter": "Q1 2026",
    "f1": 7.94,
    "f2": 6.31,
    "f3": 7.56,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.75,
    "f7": 5.65,
    "f8": 8.0,
    "momentum_score": 0.71,
    "cqv_v1": 8.0,
    "cqv_v1_1": 7.67,
    "cqv_v2": 7.52,
    "cqv_v3": 7.52,
    "cqv": 7.53,
    "pe": 3.71,
    "close_history": "{'2021': 651.97, '2022': 339.1, '2023': 388.68, '2024': 342.77, '2025': 208.75, '2026': 133.33}",
    "status": "Success",
    "cqv_v4": 7.53,
    "price": 100.0,
    "pe_forward": 3.1534999999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DDOG",
    "name": "Datadog, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.99,
    "f2": 6.82,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.41,
    "f7": 7.21,
    "f8": 8.0,
    "momentum_score": 9.7,
    "cqv_v1": 7.05,
    "cqv_v1_1": 7.59,
    "cqv_v2": 7.51,
    "cqv_v3": 7.51,
    "cqv": 7.53,
    "pe": 667.59,
    "close_history": "{'2021': 178.11, '2022': 73.5, '2023': 121.38, '2024': 142.89, '2025': 135.99, '2026': 262.32}",
    "status": "Success",
    "cqv_v4": 7.53,
    "price": 100.0,
    "pe_forward": 567.4515,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IONQ",
    "name": "IonQ, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 4.04,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.44,
    "f7": 8.77,
    "f8": 8.0,
    "momentum_score": 4.76,
    "cqv_v1": 6.64,
    "cqv_v1_1": 7.51,
    "cqv_v2": 7.45,
    "cqv_v3": 7.45,
    "cqv": 7.52,
    "pe": 125.95,
    "close_history": "{'2021': 16.7, '2022': 3.45, '2023': 12.39, '2024': 41.77, '2025': 44.87, '2026': 35.1}",
    "status": "Success",
    "cqv_v4": 7.52,
    "price": 100.0,
    "pe_forward": 107.0575,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "KBR",
    "name": "KBR, Inc.",
    "sector": "Industrials",
    "quarter": "Q2 2026",
    "f1": 7.24,
    "f2": 7.77,
    "f3": 7.14,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.19,
    "f7": 6.21,
    "f8": 8.0,
    "momentum_score": 3.51,
    "cqv_v1": 7.81,
    "cqv_v1_1": 7.65,
    "cqv_v2": 7.48,
    "cqv_v3": 7.48,
    "cqv": 7.51,
    "pe": 10.74,
    "close_history": "{'2021': 45.24, '2022': 50.64, '2023': 53.64, '2024': 56.63, '2025': 39.84, '2026': 36.18}",
    "status": "Success",
    "cqv_v4": 7.51,
    "price": 100.0,
    "pe_forward": 9.129,
    "value_score": 6.25,
    "peg_bruto": 19.7174,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "HIMS",
    "name": "Hims & Hers Health, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 6.93,
    "f2": 6.23,
    "f3": 8.09,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 7.51,
    "f7": 6.46,
    "f8": 8.0,
    "momentum_score": 5.5,
    "cqv_v1": 7.29,
    "cqv_v1_1": 7.68,
    "cqv_v2": 7.54,
    "cqv_v3": 7.54,
    "cqv": 7.51,
    "pe": 41.35,
    "close_history": "{'2021': 6.55, '2022': 6.41, '2023': 8.9, '2024': 24.18, '2025': 32.47, '2026': 33.68}",
    "status": "Success",
    "cqv_v4": 7.51,
    "price": 100.0,
    "pe_forward": 35.1475,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ASTS",
    "name": "AST SpaceMobile, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 4.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.42,
    "f7": 8.8,
    "f8": 8.0,
    "momentum_score": 5.55,
    "cqv_v1": 6.64,
    "cqv_v1_1": 7.5,
    "cqv_v2": 7.44,
    "cqv_v3": 7.44,
    "cqv": 7.51,
    "pe": -414.86,
    "close_history": "{'2021': 7.94, '2022': 4.82, '2023': 6.03, '2024': 21.1, '2025': 72.63, '2026': 55.01}",
    "status": "Success",
    "cqv_v4": 7.51,
    "price": 100.0,
    "pe_forward": -352.63100000000003,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AFRM",
    "name": "Affirm Holdings, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 7.02,
    "f2": 5.04,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 8.18,
    "f7": 6.38,
    "f8": 8.0,
    "momentum_score": 8.52,
    "cqv_v1": 7.22,
    "cqv_v1_1": 7.51,
    "cqv_v2": 7.5,
    "cqv_v3": 7.5,
    "cqv": 7.5,
    "pe": 76.89,
    "close_history": "{'2021': 100.56, '2022': 9.67, '2023': 49.14, '2024': 60.9, '2025': 74.43, '2026': 79.84}",
    "status": "Success",
    "cqv_v4": 7.5,
    "price": 100.0,
    "pe_forward": 65.3565,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CRCL",
    "name": "Circle Internet Group, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 4.0,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.67,
    "f7": 8.8,
    "f8": 8.0,
    "momentum_score": 1.1,
    "cqv_v1": 6.68,
    "cqv_v1_1": 7.39,
    "cqv_v2": 7.41,
    "cqv_v3": 7.41,
    "cqv": 7.49,
    "pe": 28.84,
    "close_history": "{'2025': 79.3, '2026': 60.64}",
    "status": "Success",
    "cqv_v4": 7.49,
    "price": 100.0,
    "pe_forward": 24.514,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CAVA",
    "name": "CAVA Group, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 4.6,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.96,
    "f7": 8.32,
    "f8": 8.0,
    "momentum_score": 6.12,
    "cqv_v1": 6.67,
    "cqv_v1_1": 7.37,
    "cqv_v2": 7.4,
    "cqv_v3": 7.4,
    "cqv": 7.49,
    "pe": 147.85,
    "close_history": "{'2023': 42.98, '2024': 112.8, '2025': 58.69, '2026': 68.1}",
    "status": "Success",
    "cqv_v4": 7.49,
    "price": 100.0,
    "pe_forward": 125.67249999999999,
    "value_score": 3.68,
    "peg_bruto": 1.4323,
    "score_peg": 1.4323,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "ACA",
    "name": "Arcosa, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 5.89,
    "f2": 8.05,
    "f3": 8.16,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.03,
    "f7": 7.29,
    "f8": 8.0,
    "momentum_score": 9.68,
    "cqv_v1": 6.96,
    "cqv_v1_1": 7.5,
    "cqv_v2": 7.43,
    "cqv_v3": 7.43,
    "cqv": 7.48,
    "pe": 32.43,
    "close_history": "{'2021': 52.03, '2022': 53.86, '2023': 82.16, '2024': 96.4, '2025': 106.19, '2026': 145.0}",
    "status": "Success",
    "cqv_v4": 7.48,
    "price": 100.0,
    "pe_forward": 27.5655,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SAIA",
    "name": "Saia, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 5.53,
    "f2": 8.75,
    "f3": 7.93,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.73,
    "f7": 7.58,
    "f8": 8.0,
    "momentum_score": 6.52,
    "cqv_v1": 6.82,
    "cqv_v1_1": 7.48,
    "cqv_v2": 7.41,
    "cqv_v3": 7.41,
    "cqv": 7.46,
    "pe": 43.68,
    "close_history": "{'2021': 337.03, '2022': 209.68, '2023': 438.22, '2024': 455.73, '2025': 326.52, '2026': 438.29}",
    "status": "Success",
    "cqv_v4": 7.46,
    "price": 100.0,
    "pe_forward": 37.128,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LEN",
    "name": "Lennar Corporation",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 6.53,
    "f2": 8.75,
    "f3": 7.09,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.81,
    "f7": 6.78,
    "f8": 8.0,
    "momentum_score": 1.99,
    "cqv_v1": 7.6,
    "cqv_v1_1": 7.52,
    "cqv_v2": 7.39,
    "cqv_v3": 7.39,
    "cqv": 7.45,
    "pe": 13.83,
    "close_history": "{'2021': 104.09, '2022': 82.6, '2023': 137.88, '2024': 127.78, '2025': 101.2, '2026': 86.4}",
    "status": "Success",
    "cqv_v4": 7.45,
    "price": 100.0,
    "pe_forward": 11.7555,
    "value_score": 6.25,
    "peg_bruto": 15.312,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "MTN",
    "name": "Vail Resorts, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 7.93,
    "f2": 6.99,
    "f3": 6.89,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.41,
    "f7": 5.66,
    "f8": 8.0,
    "momentum_score": 4.87,
    "cqv_v1": 7.52,
    "cqv_v1_1": 7.57,
    "cqv_v2": 7.42,
    "cqv_v3": 7.42,
    "cqv": 7.44,
    "pe": 30.32,
    "close_history": "{'2021': 266.37, '2022': 200.08, '2023': 185.95, '2024': 171.15, '2025': 128.56, '2026': 149.13}",
    "status": "Success",
    "cqv_v4": 7.44,
    "price": 100.0,
    "pe_forward": 25.772,
    "value_score": 5.35,
    "peg_bruto": 6.9843,
    "score_peg": 6.9843,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "DG",
    "name": "Dollar General Corporation",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 7.23,
    "f2": 6.33,
    "f3": 8.04,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.63,
    "f7": 6.22,
    "f8": 8.0,
    "momentum_score": 4.18,
    "cqv_v1": 7.85,
    "cqv_v1_1": 7.47,
    "cqv_v2": 7.41,
    "cqv_v3": 7.41,
    "cqv": 7.43,
    "pe": 16.71,
    "close_history": "{'2021': 217.29, '2022': 229.4, '2023': 128.15, '2024': 72.89, '2025': 130.91, '2026': 127.16}",
    "status": "Success",
    "cqv_v4": 7.43,
    "price": 100.0,
    "pe_forward": 14.2035,
    "value_score": 6.25,
    "peg_bruto": 12.6729,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "TEM",
    "name": "Tempus AI, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 3.34,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 6.33,
    "f7": 9.33,
    "f8": 8.0,
    "momentum_score": 4.45,
    "cqv_v1": 6.57,
    "cqv_v1_1": 7.38,
    "cqv_v2": 7.39,
    "cqv_v3": 7.39,
    "cqv": 7.42,
    "pe": -565.01,
    "close_history": "{'2024': 33.76, '2025': 59.05, '2026': 53.59}",
    "status": "Success",
    "cqv_v4": 7.42,
    "price": 100.0,
    "pe_forward": -480.25849999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CRSP",
    "name": "CRISPR Therapeutics AG",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 3.16,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 6.25,
    "f7": 9.47,
    "f8": 8.0,
    "momentum_score": 5.88,
    "cqv_v1": 6.56,
    "cqv_v1_1": 7.37,
    "cqv_v2": 7.38,
    "cqv_v3": 7.38,
    "cqv": 7.42,
    "pe": -15.69,
    "close_history": "{'2021': 75.78, '2022': 40.65, '2023': 62.6, '2024': 39.36, '2025': 52.44, '2026': 48.57}",
    "status": "Success",
    "cqv_v4": 7.42,
    "price": 100.0,
    "pe_forward": -13.3365,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DAVA",
    "name": "Endava plc",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 6.58,
    "f2": 8.7,
    "f3": 6.23,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.41,
    "f7": 6.74,
    "f8": 8.0,
    "momentum_score": 0.35,
    "cqv_v1": 7.66,
    "cqv_v1_1": 7.63,
    "cqv_v2": 7.41,
    "cqv_v3": 7.41,
    "cqv": 7.41,
    "pe": 3.51,
    "close_history": "{'2021': 167.92, '2022': 76.5, '2023': 77.85, '2024': 30.9, '2025': 6.32, '2026': 2.8}",
    "status": "Success",
    "cqv_v4": 7.41,
    "price": 100.0,
    "pe_forward": 2.9835,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CHDN",
    "name": "Churchill Downs, Incorporated",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 7.19,
    "f2": 6.29,
    "f3": 8.01,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.6,
    "f7": 6.25,
    "f8": 8.0,
    "momentum_score": 3.55,
    "cqv_v1": 7.31,
    "cqv_v1_1": 7.45,
    "cqv_v2": 7.39,
    "cqv_v3": 7.39,
    "cqv": 7.41,
    "pe": 16.74,
    "close_history": "{'2021': 118.86, '2022': 104.65, '2023': 134.02, '2024': 133.02, '2025': 113.78, '2026': 86.29}",
    "status": "Success",
    "cqv_v4": 7.41,
    "price": 100.0,
    "pe_forward": 14.228999999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MSTR",
    "name": "Strategy Inc",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 4.0,
    "f2": 9.25,
    "f3": 8.49,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.25,
    "f7": 8.8,
    "f8": 8.0,
    "momentum_score": 0.15,
    "cqv_v1": 6.57,
    "cqv_v1_1": 7.41,
    "cqv_v2": 7.37,
    "cqv_v3": 7.37,
    "cqv": 7.41,
    "pe": 22.58,
    "close_history": "{'2021': 54.45, '2022': 14.16, '2023': 63.16, '2024': 289.62, '2025': 151.95, '2026': 94.03}",
    "status": "Success",
    "cqv_v4": 7.41,
    "price": 100.0,
    "pe_forward": 19.192999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FIVN",
    "name": "Five9, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.99,
    "f2": 6.75,
    "f3": 8.19,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.09,
    "f7": 7.21,
    "f8": 8.0,
    "momentum_score": 7.65,
    "cqv_v1": 7.77,
    "cqv_v1_1": 7.49,
    "cqv_v2": 7.41,
    "cqv_v3": 7.41,
    "cqv": 7.39,
    "pe": 36.06,
    "close_history": "{'2021': 137.32, '2022': 67.86, '2023': 78.69, '2024': 40.64, '2025': 20.05, '2026': 25.26}",
    "status": "Success",
    "cqv_v4": 7.39,
    "price": 100.0,
    "pe_forward": 30.651,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ENPH",
    "name": "Enphase Energy, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 7.06,
    "f2": 8.31,
    "f3": 5.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.45,
    "f7": 6.35,
    "f8": 8.0,
    "momentum_score": 4.92,
    "cqv_v1": 7.22,
    "cqv_v1_1": 7.64,
    "cqv_v2": 7.4,
    "cqv_v3": 7.4,
    "cqv": 7.38,
    "pe": 42.64,
    "close_history": "{'2021': 182.94, '2022': 264.96, '2023': 132.14, '2024': 68.68, '2025': 32.05, '2026': 41.08}",
    "status": "Success",
    "cqv_v4": 7.38,
    "price": 100.0,
    "pe_forward": 36.244,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FRPT",
    "name": "Freshpet, Inc.",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 4.25,
    "f2": 8.75,
    "f3": 9.12,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.68,
    "f7": 8.6,
    "f8": 8.0,
    "momentum_score": 3.22,
    "cqv_v1": 6.73,
    "cqv_v1_1": 7.25,
    "cqv_v2": 7.31,
    "cqv_v3": 7.31,
    "cqv": 7.38,
    "pe": 14.8,
    "close_history": "{'2021': 95.27, '2022': 52.77, '2023': 86.76, '2024': 148.11, '2025': 60.93, '2026': 56.93}",
    "status": "Success",
    "cqv_v4": 7.38,
    "price": 100.0,
    "pe_forward": 12.58,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AXON",
    "name": "Axon Enterprise, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 5.19,
    "f2": 6.85,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.26,
    "f7": 7.85,
    "f8": 8.0,
    "momentum_score": 5.66,
    "cqv_v1": 6.68,
    "cqv_v1_1": 7.32,
    "cqv_v2": 7.37,
    "cqv_v3": 7.37,
    "cqv": 7.38,
    "pe": 237.86,
    "close_history": "{'2021': 157.0, '2022': 165.93, '2023': 258.33, '2024': 594.32, '2025': 567.93, '2026': 541.75}",
    "status": "Success",
    "cqv_v4": 7.38,
    "price": 100.0,
    "pe_forward": 202.181,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AXP",
    "name": "American Express Company",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 6.96,
    "f2": 4.75,
    "f3": 8.96,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.96,
    "f7": 6.43,
    "f8": 8.0,
    "momentum_score": 5.89,
    "cqv_v1": 7.0,
    "cqv_v1_1": 7.0,
    "cqv_v2": 7.4,
    "cqv_v3": 7.0,
    "cqv": 7.37,
    "pe": 21.98,
    "close_history": "{'2021': 154.34, '2022': 141.19, '2023': 181.67, '2024': 291.26, '2025': 366.96, '2026': 361.57}",
    "status": "Success",
    "cqv_v4": 7.37,
    "price": 100.0,
    "pe_forward": 18.683,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SNDK",
    "name": "Sandisk Corporation",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 3.49,
    "f2": 9.25,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.16,
    "f7": 9.21,
    "f8": 8.0,
    "momentum_score": 8.66,
    "cqv_v1": 6.96,
    "cqv_v1_1": 7.33,
    "cqv_v2": 7.33,
    "cqv_v3": 7.33,
    "cqv": 7.37,
    "pe": 59.52,
    "close_history": "{'2025': 237.38, '2026': 1411.08}",
    "status": "Success",
    "cqv_v4": 7.37,
    "price": 100.0,
    "pe_forward": 50.592,
    "value_score": 4.32,
    "peg_bruto": 3.5579,
    "score_peg": 3.5579,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "BJ",
    "name": "BJ's Wholesale Club Holdings, I",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 5.14,
    "f2": 7.92,
    "f3": 8.77,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.96,
    "f7": 7.89,
    "f8": 8.0,
    "momentum_score": 3.54,
    "cqv_v1": 6.82,
    "cqv_v1_1": 7.3,
    "cqv_v2": 7.32,
    "cqv_v3": 7.32,
    "cqv": 7.37,
    "pe": 20.5,
    "close_history": "{'2021': 66.97, '2022': 66.16, '2023': 66.66, '2024': 89.35, '2025': 90.03, '2026': 93.07}",
    "status": "Success",
    "cqv_v4": 7.37,
    "price": 100.0,
    "pe_forward": 17.425,
    "value_score": 6.25,
    "peg_bruto": 10.33,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "VAL",
    "name": "Valaris Limited",
    "sector": "Energy",
    "quarter": "Q1 2026",
    "f1": 6.59,
    "f2": 8.75,
    "f3": 6.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.46,
    "f7": 6.73,
    "f8": 8.0,
    "momentum_score": 4.72,
    "cqv_v1": 7.33,
    "cqv_v1_1": 7.51,
    "cqv_v2": 7.34,
    "cqv_v3": 7.34,
    "cqv": 7.36,
    "pe": 5.33,
    "close_history": "{'2021': 36.0, '2022': 67.62, '2023': 68.57, '2024': 44.24, '2025': 50.4, '2026': 76.36}",
    "status": "Success",
    "cqv_v4": 7.36,
    "price": 100.0,
    "pe_forward": 4.5305,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NIO",
    "name": "NIO Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 3.93,
    "f2": 8.75,
    "f3": 9.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.63,
    "f7": 8.86,
    "f8": 8.0,
    "momentum_score": 2.22,
    "cqv_v1": 6.48,
    "cqv_v1_1": 7.2,
    "cqv_v2": 7.28,
    "cqv_v3": 7.28,
    "cqv": 7.35,
    "pe": 27.85,
    "close_history": "{'2021': 31.68, '2022': 9.75, '2023': 9.07, '2024': 4.36, '2025': 5.1, '2026': 4.99}",
    "status": "Success",
    "cqv_v4": 7.35,
    "price": 100.0,
    "pe_forward": 23.6725,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "COR",
    "name": "Cencora, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 4.07,
    "f2": 9.0,
    "f3": 8.09,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 6.08,
    "f7": 8.74,
    "f8": 8.0,
    "momentum_score": 3.83,
    "cqv_v1": 6.49,
    "cqv_v1_1": 7.38,
    "cqv_v2": 7.33,
    "cqv_v3": 7.33,
    "cqv": 7.33,
    "pe": 22.72,
    "close_history": "{'2021': 127.15, '2022': 160.54, '2023': 201.2, '2024': 222.07, '2025': 336.4, '2026': 307.99}",
    "status": "Success",
    "cqv_v4": 7.33,
    "price": 100.0,
    "pe_forward": 19.311999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "HFG",
    "name": "HelloFresh SE                 I",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 6.58,
    "f2": 8.72,
    "f3": 6.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.46,
    "f7": 6.74,
    "f8": 8.0,
    "momentum_score": 1.43,
    "cqv_v1": 7.62,
    "cqv_v1_1": 7.41,
    "cqv_v2": 7.28,
    "cqv_v3": 7.28,
    "cqv": 7.31,
    "pe": 7.07,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.31,
    "price": 100.0,
    "pe_forward": 6.0095,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "KR",
    "name": "Kroger Company (The)",
    "sector": "Consumer Defensive",
    "quarter": "Q2 2026",
    "f1": 6.45,
    "f2": 6.76,
    "f3": 7.91,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.18,
    "f7": 6.84,
    "f8": 8.0,
    "momentum_score": 2.5,
    "cqv_v1": 7.69,
    "cqv_v1_1": 7.32,
    "cqv_v2": 7.3,
    "cqv_v3": 7.3,
    "cqv": 7.3,
    "pe": 34.05,
    "close_history": "{'2021': 41.09, '2022': 41.27, '2023': 43.33, '2024': 59.32, '2025': 61.84, '2026': 58.61}",
    "status": "Success",
    "cqv_v4": 7.3,
    "price": 100.0,
    "pe_forward": 28.942499999999995,
    "value_score": 5.12,
    "peg_bruto": 6.2192,
    "score_peg": 6.2192,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "API",
    "name": "Agora, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 3.32,
    "f2": 9.25,
    "f3": 8.67,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.0,
    "f7": 9.34,
    "f8": 8.0,
    "momentum_score": 4.81,
    "cqv_v1": 6.34,
    "cqv_v1_1": 7.27,
    "cqv_v2": 7.28,
    "cqv_v3": 7.28,
    "cqv": 7.3,
    "pe": 44.56,
    "close_history": "{'2021': 16.21, '2022': 3.91, '2023': 2.63, '2024': 4.16, '2025': 4.07, '2026': 4.0}",
    "status": "Success",
    "cqv_v4": 7.3,
    "price": 100.0,
    "pe_forward": 37.876,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RH",
    "name": "RH",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 7.7,
    "f2": 5.42,
    "f3": 7.48,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.59,
    "f7": 5.84,
    "f8": 8.0,
    "momentum_score": 5.06,
    "cqv_v1": 7.73,
    "cqv_v1_1": 7.37,
    "cqv_v2": 7.31,
    "cqv_v3": 7.31,
    "cqv": 7.28,
    "pe": 32.7,
    "close_history": "{'2021': 535.94, '2022': 267.19, '2023': 291.48, '2024': 393.59, '2025': 179.15, '2026': 189.21}",
    "status": "Success",
    "cqv_v4": 7.28,
    "price": 100.0,
    "pe_forward": 27.795,
    "value_score": 5.19,
    "peg_bruto": 6.476,
    "score_peg": 6.476,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "BLDR",
    "name": "Builders FirstSource, Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 7.31,
    "f2": 6.89,
    "f3": 6.54,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.92,
    "f7": 6.15,
    "f8": 8.0,
    "momentum_score": 3.16,
    "cqv_v1": 7.69,
    "cqv_v1_1": 7.44,
    "cqv_v2": 7.31,
    "cqv_v3": 7.31,
    "cqv": 7.28,
    "pe": 32.32,
    "close_history": "{'2021': 85.71, '2022': 64.88, '2023': 166.94, '2024': 142.93, '2025': 102.89, '2026': 78.2}",
    "status": "Success",
    "cqv_v4": 7.28,
    "price": 100.0,
    "pe_forward": 27.471999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CLFD",
    "name": "Clearfield, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.66,
    "f2": 9.5,
    "f3": 5.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 5.75,
    "f7": 7.47,
    "f8": 8.0,
    "momentum_score": 4.57,
    "cqv_v1": 7.27,
    "cqv_v1_1": 7.46,
    "cqv_v2": 7.28,
    "cqv_v3": 7.28,
    "cqv": 7.26,
    "pe": 147.7,
    "close_history": "{'2021': 84.42, '2022': 94.14, '2023': 29.08, '2024': 31.0, '2025': 29.15, '2026': 30.92}",
    "status": "Success",
    "cqv_v4": 7.26,
    "price": 100.0,
    "pe_forward": 125.54499999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ALFEN",
    "name": "ALFEN",
    "sector": "Industrials",
    "quarter": "Q4 2025",
    "f1": 5.55,
    "f2": 8.75,
    "f3": 6.84,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.2,
    "f7": 7.56,
    "f8": 8.0,
    "momentum_score": 7.41,
    "cqv_v1": 7.48,
    "cqv_v1_1": 7.33,
    "cqv_v2": 7.25,
    "cqv_v3": 7.25,
    "cqv": 7.25,
    "pe": 43.47,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.25,
    "price": 100.0,
    "pe_forward": 36.9495,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TLN",
    "name": "Talen Energy Corporation",
    "sector": "Utilities",
    "quarter": "Q1 2026",
    "f1": 6.22,
    "f2": 4.5,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.78,
    "f7": 7.02,
    "f8": 8.0,
    "momentum_score": 4.93,
    "cqv_v1": 7.0,
    "cqv_v1_1": 7.0,
    "cqv_v2": 7.31,
    "cqv_v3": 7.0,
    "cqv": 7.25,
    "pe": 12.14,
    "close_history": "{'2023': 64.0, '2024': 201.47, '2025': 374.84, '2026': 368.29}",
    "status": "Success",
    "cqv_v4": 7.25,
    "price": 100.0,
    "pe_forward": 10.319,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "EL",
    "name": "Estee Lauder Companies, Inc. (T",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 7.47,
    "f2": 4.5,
    "f3": 8.18,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.82,
    "f7": 6.02,
    "f8": 8.0,
    "momentum_score": 3.12,
    "cqv_v1": 7.0,
    "cqv_v1_1": 7.0,
    "cqv_v2": 7.29,
    "cqv_v3": 7.0,
    "cqv": 7.23,
    "pe": 26.27,
    "close_history": "{'2021': 344.53, '2022': 233.2, '2023': 139.61, '2024': 73.17, '2025': 103.99, '2026': 82.84}",
    "status": "Success",
    "cqv_v4": 7.23,
    "price": 100.0,
    "pe_forward": 22.3295,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "IREN",
    "name": "IREN LIMITED",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 4.0,
    "f2": 9.0,
    "f3": 8.22,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.11,
    "f7": 8.8,
    "f8": 8.0,
    "momentum_score": 3.54,
    "cqv_v1": 6.48,
    "cqv_v1_1": 7.18,
    "cqv_v2": 7.21,
    "cqv_v3": 7.21,
    "cqv": 7.23,
    "pe": 50.42,
    "close_history": "{'2021': 16.17, '2022': 1.25, '2023': 7.15, '2024': 9.82, '2025': 37.77, '2026': 34.83}",
    "status": "Success",
    "cqv_v4": 7.23,
    "price": 100.0,
    "pe_forward": 42.857,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NNE",
    "name": "Nano Nuclear Energy Inc.",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 4.0,
    "f2": 9.0,
    "f3": 8.22,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.11,
    "f7": 8.8,
    "f8": 8.0,
    "momentum_score": 0.08,
    "cqv_v1": 6.41,
    "cqv_v1_1": 7.18,
    "cqv_v2": 7.21,
    "cqv_v3": 7.21,
    "cqv": 7.23,
    "pe": -20.34,
    "close_history": "{'2024': 24.9, '2025': 24.01, '2026': 16.2}",
    "status": "Success",
    "cqv_v4": 7.23,
    "price": 100.0,
    "pe_forward": -17.288999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "OKLO",
    "name": "Oklo Inc.",
    "sector": "Utilities",
    "quarter": "Q1 2026",
    "f1": 4.0,
    "f2": 9.0,
    "f3": 8.22,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.11,
    "f7": 8.8,
    "f8": 8.0,
    "momentum_score": 0.15,
    "cqv_v1": 6.41,
    "cqv_v1_1": 7.18,
    "cqv_v2": 7.21,
    "cqv_v3": 7.21,
    "cqv": 7.23,
    "pe": -62.8,
    "close_history": "{'2021': 9.85, '2022': 9.92, '2023': 10.56, '2024': 21.23, '2025': 71.76, '2026': 41.7}",
    "status": "Success",
    "cqv_v4": 7.23,
    "price": 100.0,
    "pe_forward": -53.379999999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LITE",
    "name": "Lumentum Holdings Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.8,
    "f2": 5.0,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.31,
    "f7": 7.36,
    "f8": 8.0,
    "momentum_score": 6.42,
    "cqv_v1": 6.76,
    "cqv_v1_1": 7.27,
    "cqv_v2": 7.3,
    "cqv_v3": 7.3,
    "cqv": 7.21,
    "pe": 128.23,
    "close_history": "{'2021': 105.77, '2022': 52.17, '2023': 52.42, '2024': 83.95, '2025': 368.59, '2026': 706.23}",
    "status": "Success",
    "cqv_v4": 7.21,
    "price": 100.0,
    "pe_forward": 108.99549999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "COHR",
    "name": "Coherent Corp.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 3.95,
    "f2": 7.59,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.39,
    "f7": 8.84,
    "f8": 8.0,
    "momentum_score": 7.05,
    "cqv_v1": 6.49,
    "cqv_v1_1": 7.2,
    "cqv_v2": 7.25,
    "cqv_v3": 7.25,
    "cqv": 7.21,
    "pe": 158.74,
    "close_history": "{'2021': 68.33, '2022': 35.1, '2023': 43.53, '2024': 94.73, '2025': 184.57, '2026': 276.96}",
    "status": "Success",
    "cqv_v4": 7.21,
    "price": 100.0,
    "pe_forward": 134.929,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BE",
    "name": "Bloom Energy Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 6.0,
    "f2": 4.5,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.67,
    "f7": 7.2,
    "f8": 8.0,
    "momentum_score": 7.76,
    "cqv_v1": 6.71,
    "cqv_v1_1": 7.0,
    "cqv_v2": 7.27,
    "cqv_v3": 7.0,
    "cqv": 7.2,
    "pe": 61.55,
    "close_history": "{'2021': 21.93, '2022': 19.12, '2023': 14.8, '2024': 22.21, '2025': 86.89, '2026': 206.73}",
    "status": "Success",
    "cqv_v4": 7.2,
    "price": 100.0,
    "pe_forward": 52.317499999999995,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CNXC",
    "name": "Concentrix Corporation",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 4.05,
    "f2": 9.25,
    "f3": 7.38,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 5.71,
    "f7": 8.76,
    "f8": 8.0,
    "momentum_score": 0.98,
    "cqv_v1": 7.49,
    "cqv_v1_1": 7.26,
    "cqv_v2": 7.21,
    "cqv_v3": 7.21,
    "cqv": 7.19,
    "pe": 1.98,
    "close_history": "{'2021': 163.49, '2022': 122.76, '2023': 91.63, '2024': 41.21, '2025': 40.64, '2026': 25.62}",
    "status": "Success",
    "cqv_v4": 7.19,
    "price": 100.0,
    "pe_forward": 1.683,
    "value_score": 6.25,
    "peg_bruto": 106.9519,
    "score_peg": 10.0,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "8002",
    "name": "MARUBENI CORP",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 4.68,
    "f2": 8.34,
    "f3": 7.79,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.23,
    "f7": 8.26,
    "f8": 8.0,
    "momentum_score": 5.65,
    "cqv_v1": 6.94,
    "cqv_v1_1": 7.19,
    "cqv_v2": 7.2,
    "cqv_v3": 7.2,
    "cqv": 7.19,
    "pe": 15.04,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.19,
    "price": 100.0,
    "pe_forward": 12.783999999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MKL",
    "name": "Markel Group Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 5.52,
    "f2": 9.0,
    "f3": 6.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 5.92,
    "f7": 7.58,
    "f8": 8.0,
    "momentum_score": 5.08,
    "cqv_v1": 6.52,
    "cqv_v1_1": 7.28,
    "cqv_v2": 7.19,
    "cqv_v3": 7.19,
    "cqv": 7.17,
    "pe": 14.33,
    "close_history": "{'2021': 1234.0, '2022': 1317.49, '2023': 1419.9, '2024': 1726.23, '2025': 2149.65, '2026': 1961.25}",
    "status": "Success",
    "cqv_v4": 7.17,
    "price": 100.0,
    "pe_forward": 12.1805,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SOFI",
    "name": "SoFi Technologies, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 2.48,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 5.91,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 2.97,
    "cqv_v1": 6.36,
    "cqv_v1_1": 7.01,
    "cqv_v2": 7.15,
    "cqv_v3": 7.15,
    "cqv": 7.17,
    "pe": 40.53,
    "close_history": "{'2021': 15.81, '2022': 4.61, '2023': 9.95, '2024': 15.4, '2025': 26.18, '2026': 17.32}",
    "status": "Success",
    "cqv_v4": 7.17,
    "price": 100.0,
    "pe_forward": 34.4505,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "PFGC",
    "name": "Performance Food Group Company",
    "sector": "Consumer Defensive",
    "quarter": "Q1 2026",
    "f1": 5.77,
    "f2": 6.11,
    "f3": 8.38,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 7.08,
    "f7": 7.38,
    "f8": 8.0,
    "momentum_score": 8.8,
    "cqv_v1": 7.24,
    "cqv_v1_1": 7.13,
    "cqv_v2": 7.19,
    "cqv_v3": 7.19,
    "cqv": 7.15,
    "pe": 53.85,
    "close_history": "{'2021': 45.89, '2022': 58.39, '2023': 69.15, '2024': 84.55, '2025': 89.92, '2026': 113.17}",
    "status": "Success",
    "cqv_v4": 7.15,
    "price": 100.0,
    "pe_forward": 45.7725,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NET",
    "name": "Cloudflare, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.51,
    "f2": 5.0,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 7.17,
    "f7": 7.59,
    "f8": 8.0,
    "momentum_score": 7.76,
    "cqv_v1": 6.67,
    "cqv_v1_1": 7.2,
    "cqv_v2": 7.25,
    "cqv_v3": 7.25,
    "cqv": 7.15,
    "pe": 153.56,
    "close_history": "{'2021': 131.5, '2022': 45.21, '2023': 83.26, '2024': 107.68, '2025': 197.15, '2026': 272.46}",
    "status": "Success",
    "cqv_v4": 7.15,
    "price": 100.0,
    "pe_forward": 130.526,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "OR",
    "name": "Ormat Technologies, Inc.",
    "sector": "Utilities",
    "quarter": "Q1 2026",
    "f1": 4.58,
    "f2": 6.16,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.96,
    "f7": 8.34,
    "f8": 8.0,
    "momentum_score": 4.86,
    "cqv_v1": 6.5,
    "cqv_v1_1": 7.07,
    "cqv_v2": 7.19,
    "cqv_v3": 7.19,
    "cqv": 7.15,
    "pe": 54.38,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.15,
    "price": 100.0,
    "pe_forward": 46.223,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BX",
    "name": "Blackstone Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 6.43,
    "f2": 4.75,
    "f3": 8.3,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 7.37,
    "f7": 6.86,
    "f8": 8.0,
    "momentum_score": 2.99,
    "cqv_v1": 7.0,
    "cqv_v1_1": 7.0,
    "cqv_v2": 7.21,
    "cqv_v3": 7.0,
    "cqv": 7.12,
    "pe": 31.48,
    "close_history": "{'2021': 110.62, '2022': 66.37, '2023': 121.28, '2024': 163.82, '2025': 150.97, '2026': 128.97}",
    "status": "Success",
    "cqv_v4": 7.12,
    "price": 100.0,
    "pe_forward": 26.758,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BABA",
    "name": "Alibaba Group Holding Limited",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 4.09,
    "f2": 8.75,
    "f3": 7.99,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.04,
    "f7": 8.73,
    "f8": 8.0,
    "momentum_score": 0.12,
    "cqv_v1": 6.37,
    "cqv_v1_1": 7.04,
    "cqv_v2": 7.11,
    "cqv_v3": 7.11,
    "cqv": 7.12,
    "pe": 14.86,
    "close_history": "{'2021': 111.79, '2022': 82.9, '2023': 73.92, '2024': 82.62, '2025': 145.25, '2026': 117.49}",
    "status": "Success",
    "cqv_v4": 7.12,
    "price": 100.0,
    "pe_forward": 12.630999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "FND",
    "name": "Floor & Decor Holdings, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 5.52,
    "f2": 7.08,
    "f3": 7.59,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.55,
    "f7": 7.58,
    "f8": 8.0,
    "momentum_score": 4.58,
    "cqv_v1": 6.75,
    "cqv_v1_1": 7.09,
    "cqv_v2": 7.12,
    "cqv_v3": 7.12,
    "cqv": 7.09,
    "pe": 32.09,
    "close_history": "{'2021': 130.01, '2022': 69.63, '2023': 111.56, '2024': 99.7, '2025': 60.89, '2026': 58.46}",
    "status": "Success",
    "cqv_v4": 7.09,
    "price": 100.0,
    "pe_forward": 27.276500000000002,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SBUX",
    "name": "Starbucks Corporation",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 5.14,
    "f2": 6.22,
    "f3": 8.64,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.89,
    "f7": 7.89,
    "f8": 8.0,
    "momentum_score": 7.75,
    "cqv_v1": 6.45,
    "cqv_v1_1": 7.02,
    "cqv_v2": 7.13,
    "cqv_v3": 7.13,
    "cqv": 7.09,
    "pe": 79.6,
    "close_history": "{'2021': 104.95, '2022': 91.12, '2023': 90.04, '2024': 87.81, '2025': 83.18, '2026': 108.37}",
    "status": "Success",
    "cqv_v4": 7.09,
    "price": 100.0,
    "pe_forward": 67.66,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "NEE",
    "name": "NextEra Energy, Inc.",
    "sector": "Utilities",
    "quarter": "Q1 2026",
    "f1": 5.1,
    "f2": 5.91,
    "f3": 8.48,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.79,
    "f7": 7.92,
    "f8": 8.0,
    "momentum_score": 5.77,
    "cqv_v1": 6.49,
    "cqv_v1_1": 7.03,
    "cqv_v2": 7.13,
    "cqv_v3": 7.13,
    "cqv": 7.05,
    "pe": 22.42,
    "close_history": "{'2021': 82.6, '2022': 75.54, '2023': 56.43, '2024': 68.55, '2025': 79.16, '2026': 89.35}",
    "status": "Success",
    "cqv_v4": 7.05,
    "price": 100.0,
    "pe_forward": 19.057000000000002,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LPX",
    "name": "Louisiana-Pacific Corporation",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 5.09,
    "f2": 8.75,
    "f3": 6.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 5.71,
    "f7": 7.93,
    "f8": 8.0,
    "momentum_score": 4.19,
    "cqv_v1": 6.35,
    "cqv_v1_1": 7.13,
    "cqv_v2": 7.09,
    "cqv_v3": 7.09,
    "cqv": 7.05,
    "pe": 67.79,
    "close_history": "{'2021': 73.67, '2022': 56.44, '2023': 68.6, '2024': 101.49, '2025': 80.13, '2026': 76.88}",
    "status": "Success",
    "cqv_v4": 7.05,
    "price": 100.0,
    "pe_forward": 57.621500000000005,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "BN",
    "name": "Brookfield Corporation",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 5.2,
    "f2": 5.54,
    "f3": 8.54,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.87,
    "f7": 7.84,
    "f8": 8.0,
    "momentum_score": 4.49,
    "cqv_v1": 6.4,
    "cqv_v1_1": 7.01,
    "cqv_v2": 7.12,
    "cqv_v3": 7.12,
    "cqv": 7.03,
    "pe": 85.16,
    "close_history": "{'2021': 31.3, '2022': 20.46, '2023': 26.32, '2024': 37.94, '2025': 45.74, '2026': 44.3}",
    "status": "Success",
    "cqv_v4": 7.03,
    "price": 100.0,
    "pe_forward": 72.386,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "JD",
    "name": "JD.com, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 3.21,
    "f2": 9.0,
    "f3": 8.21,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 5.71,
    "f7": 9.43,
    "f8": 8.0,
    "momentum_score": 2.57,
    "cqv_v1": 7.14,
    "cqv_v1_1": 6.89,
    "cqv_v2": 7.02,
    "cqv_v3": 7.02,
    "cqv": 7.02,
    "pe": 19.57,
    "close_history": "{'2021': 61.39, '2022': 50.42, '2023': 26.34, '2024': 32.52, '2025': 27.71, '2026': 29.68}",
    "status": "Success",
    "cqv_v4": 7.02,
    "price": 100.0,
    "pe_forward": 16.6345,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MTH",
    "name": "Meritage Homes Corporation",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 5.4,
    "f2": 8.46,
    "f3": 6.33,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 5.87,
    "f7": 7.68,
    "f8": 8.0,
    "momentum_score": 8.06,
    "cqv_v1": 6.87,
    "cqv_v1_1": 7.08,
    "cqv_v2": 7.05,
    "cqv_v3": 7.05,
    "cqv": 7.02,
    "pe": 14.94,
    "close_history": "{'2021': 57.22, '2022': 43.22, '2023': 82.35, '2024': 73.99, '2025': 64.88, '2026': 77.44}",
    "status": "Success",
    "cqv_v4": 7.02,
    "price": 100.0,
    "pe_forward": 12.699,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GSY",
    "name": "GOEASY LTD",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 6.48,
    "f2": 6.6,
    "f3": 6.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.41,
    "f7": 6.82,
    "f8": 8.0,
    "momentum_score": 2.82,
    "cqv_v1": 6.38,
    "cqv_v1_1": 7.16,
    "cqv_v2": 7.11,
    "cqv_v3": 7.11,
    "cqv": 7.02,
    "pe": 9.06,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 7.02,
    "price": 100.0,
    "pe_forward": 7.7010000000000005,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "EN OBSERVACIÓN",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "8058",
    "name": "MITSUBISHI CORP",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 3.08,
    "f2": 7.6,
    "f3": 9.0,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.04,
    "f7": 9.54,
    "f8": 8.0,
    "momentum_score": 5.21,
    "cqv_v1": 6.27,
    "cqv_v1_1": 6.86,
    "cqv_v2": 7.03,
    "cqv_v3": 7.03,
    "cqv": 6.99,
    "pe": 21.23,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 6.99,
    "price": 100.0,
    "pe_forward": 18.0455,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RPD",
    "name": "Rapid7, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 5.67,
    "f2": 5.54,
    "f3": 7.13,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 6.4,
    "f7": 7.46,
    "f8": 8.0,
    "momentum_score": 3.63,
    "cqv_v1": 7.47,
    "cqv_v1_1": 7.07,
    "cqv_v2": 7.08,
    "cqv_v3": 7.08,
    "cqv": 6.93,
    "pe": 26.54,
    "close_history": "{'2021': 117.69, '2022': 33.98, '2023': 57.1, '2024': 40.23, '2025': 15.2, '2026': 12.17}",
    "status": "Success",
    "cqv_v4": 6.93,
    "price": 100.0,
    "pe_forward": 22.558999999999997,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "WGO",
    "name": "Winnebago Industries, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 5.72,
    "f2": 6.97,
    "f3": 6.57,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.14,
    "f7": 7.42,
    "f8": 8.0,
    "momentum_score": 2.82,
    "cqv_v1": 7.26,
    "cqv_v1_1": 6.97,
    "cqv_v2": 6.99,
    "cqv_v3": 6.99,
    "cqv": 6.91,
    "pe": 22.91,
    "close_history": "{'2021': 66.35, '2022': 47.31, '2023': 66.65, '2024': 44.6, '2025': 39.31, '2026': 31.31}",
    "status": "Success",
    "cqv_v4": 6.91,
    "price": 100.0,
    "pe_forward": 19.4735,
    "value_score": 6.02,
    "peg_bruto": 9.2433,
    "score_peg": 9.2433,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "NTRA",
    "name": "Natera, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 5.17,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 9.34,
    "cqv_v1": 6.19,
    "cqv_v1_1": 6.83,
    "cqv_v2": 6.9,
    "cqv_v3": 6.9,
    "cqv": 6.91,
    "pe": -1309.76,
    "close_history": "{'2021': 93.39, '2022': 40.17, '2023': 62.64, '2024': 158.3, '2025': 229.09, '2026': 269.83}",
    "status": "Success",
    "cqv_v4": 6.91,
    "price": 100.0,
    "pe_forward": -1113.296,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MRNA",
    "name": "Moderna, Inc.",
    "sector": "Healthcare",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.25,
    "f3": 9.33,
    "f4": 8.3,
    "f5": 8.6,
    "f6": 5.17,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 9.92,
    "cqv_v1": 6.12,
    "cqv_v1_1": 6.83,
    "cqv_v2": 6.9,
    "cqv_v3": 6.9,
    "cqv": 6.91,
    "pe": -19.28,
    "close_history": "{'2021': 253.98, '2022': 179.62, '2023': 99.45, '2024': 41.58, '2025': 29.49, '2026': 63.15}",
    "status": "Success",
    "cqv_v4": 6.91,
    "price": 100.0,
    "pe_forward": -16.388,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.3,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "DE",
    "name": "Deere & Company",
    "sector": "Industrials",
    "quarter": "Q3 2020",
    "f1": 6.11,
    "f2": 6.23,
    "f3": 6.43,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.27,
    "f7": 7.11,
    "f8": 8.0,
    "momentum_score": 7.97,
    "cqv_v1": 6.58,
    "cqv_v1_1": 7.03,
    "cqv_v2": 7.03,
    "cqv_v3": 7.03,
    "cqv": 6.9,
    "pe": 35.22,
    "close_history": "{'2021': 323.01, '2022': 408.89, '2023': 386.48, '2024': 415.69, '2025': 463.01, '2026': 598.97}",
    "status": "Success",
    "cqv_v4": 6.9,
    "price": 100.0,
    "pe_forward": 29.936999999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SEDG",
    "name": "SolarEdge Technologies, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 4.92,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 6.69,
    "cqv_v1": 6.94,
    "cqv_v1_1": 6.75,
    "cqv_v2": 6.81,
    "cqv_v3": 6.81,
    "cqv": 6.82,
    "pe": 33.44,
    "close_history": "{'2021': 280.57, '2022': 283.27, '2023': 93.6, '2024': 13.6, '2025': 28.85, '2026': 52.34}",
    "status": "Success",
    "cqv_v4": 6.82,
    "price": 100.0,
    "pe_forward": 28.423999999999996,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "GTLB",
    "name": "GitLab Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 1.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 4.92,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 4.75,
    "cqv_v1": 6.92,
    "cqv_v1_1": 6.75,
    "cqv_v2": 6.81,
    "cqv_v3": 6.81,
    "cqv": 6.82,
    "pe": 31.16,
    "close_history": "{'2021': 87.0, '2022': 45.44, '2023': 62.96, '2024': 56.35, '2025': 37.53, '2026': 31.75}",
    "status": "Success",
    "cqv_v4": 6.82,
    "price": 100.0,
    "pe_forward": 26.486,
    "value_score": 5.29,
    "peg_bruto": 6.796,
    "score_peg": 6.796,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "MDB",
    "name": "MongoDB, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 1.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 4.92,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 6.21,
    "cqv_v1": 6.41,
    "cqv_v1_1": 6.75,
    "cqv_v2": 6.81,
    "cqv_v3": 6.81,
    "cqv": 6.82,
    "pe": 48.36,
    "close_history": "{'2021': 529.35, '2022': 196.84, '2023': 408.85, '2024': 232.81, '2025': 419.69, '2026': 328.6}",
    "status": "Success",
    "cqv_v4": 6.82,
    "price": 100.0,
    "pe_forward": 41.106,
    "value_score": 4.56,
    "peg_bruto": 4.3789,
    "score_peg": 4.3789,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "FROG",
    "name": "JFrog Ltd.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 4.92,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 9.94,
    "cqv_v1": 6.34,
    "cqv_v1_1": 6.75,
    "cqv_v2": 6.81,
    "cqv_v3": 6.81,
    "cqv": 6.82,
    "pe": 84.63,
    "close_history": "{'2021': 29.7, '2022': 21.33, '2023': 34.61, '2024': 29.41, '2025': 62.46, '2026': 86.53}",
    "status": "Success",
    "cqv_v4": 6.82,
    "price": 100.0,
    "pe_forward": 71.93549999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "RGTI",
    "name": "Rigetti Computing, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.5,
    "f3": 8.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 4.92,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 0.93,
    "cqv_v1": 6.04,
    "cqv_v1_1": 6.75,
    "cqv_v2": 6.81,
    "cqv_v3": 6.81,
    "cqv": 6.82,
    "pe": -89.2,
    "close_history": "{'2021': 10.29, '2022': 0.73, '2023': 0.99, '2024': 15.26, '2025': 22.15, '2026': 14.1}",
    "status": "Success",
    "cqv_v4": 6.82,
    "price": 100.0,
    "pe_forward": -75.82000000000001,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "MP",
    "name": "MP Materials Corp.",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 1.11,
    "f2": 9.0,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 5.22,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 2.79,
    "cqv_v1": 6.0,
    "cqv_v1_1": 6.63,
    "cqv_v2": 6.78,
    "cqv_v3": 6.78,
    "cqv": 6.79,
    "pe": 51.43,
    "close_history": "{'2021': 45.42, '2022': 24.28, '2023': 19.85, '2024': 15.6, '2025': 50.52, '2026': 45.46}",
    "status": "Success",
    "cqv_v4": 6.79,
    "price": 100.0,
    "pe_forward": 43.7155,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "INTA",
    "name": "Intapp, Inc.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.5,
    "f3": 8.62,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 4.81,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 4.0,
    "cqv_v1": 6.91,
    "cqv_v1_1": 6.72,
    "cqv_v2": 6.78,
    "cqv_v3": 6.78,
    "cqv": 6.78,
    "pe": 17.69,
    "close_history": "{'2021': 25.16, '2022': 24.94, '2023': 38.02, '2024': 64.09, '2025': 45.82, '2026': 29.55}",
    "status": "Success",
    "cqv_v4": 6.78,
    "price": 100.0,
    "pe_forward": 15.0365,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "KKR",
    "name": "KKR & Co. Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 4.29,
    "f2": 7.02,
    "f3": 6.93,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 5.61,
    "f7": 8.57,
    "f8": 8.0,
    "momentum_score": 2.08,
    "cqv_v1": 6.2,
    "cqv_v1_1": 6.76,
    "cqv_v2": 6.87,
    "cqv_v3": 6.87,
    "cqv": 6.74,
    "pe": 31.92,
    "close_history": "{'2021': 71.71, '2022': 45.19, '2023': 81.56, '2024': 146.52, '2025': 127.0, '2026': 102.75}",
    "status": "Success",
    "cqv_v4": 6.74,
    "price": 100.0,
    "pe_forward": 27.132,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "UPST",
    "name": "Upstart Holdings, Inc.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 3.62,
    "f2": 4.75,
    "f3": 9.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.47,
    "f7": 9.1,
    "f8": 8.0,
    "momentum_score": 3.49,
    "cqv_v1": 6.11,
    "cqv_v1_1": 6.62,
    "cqv_v2": 6.89,
    "cqv_v3": 6.89,
    "cqv": 6.74,
    "pe": 84.83,
    "close_history": "{'2021': 151.3, '2022': 13.22, '2023': 40.86, '2024': 61.57, '2025': 43.73, '2026': 30.88}",
    "status": "Success",
    "cqv_v4": 6.74,
    "price": 100.0,
    "pe_forward": 72.10549999999999,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SMWB",
    "name": "Similarweb Ltd.",
    "sector": "Technology",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.5,
    "f3": 8.29,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 4.64,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 5.59,
    "cqv_v1": 6.69,
    "cqv_v1_1": 6.67,
    "cqv_v2": 6.73,
    "cqv_v3": 6.73,
    "cqv": 6.71,
    "pe": 26.33,
    "close_history": "{'2021': 17.91, '2022': 6.43, '2023': 5.33, '2024': 14.17, '2025': 7.49, '2026': 7.05}",
    "status": "Success",
    "cqv_v4": 6.71,
    "price": 100.0,
    "pe_forward": 22.380499999999998,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "8031",
    "name": "MITSUI & CO",
    "sector": "Industrials",
    "quarter": "Q1 2026",
    "f1": 3.27,
    "f2": 7.46,
    "f3": 7.54,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 5.41,
    "f7": 9.38,
    "f8": 8.0,
    "momentum_score": 4.08,
    "cqv_v1": 6.06,
    "cqv_v1_1": 6.67,
    "cqv_v2": 6.83,
    "cqv_v3": 6.83,
    "cqv": 6.71,
    "pe": 15.82,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 6.71,
    "price": 100.0,
    "pe_forward": 13.447,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "LYB",
    "name": "LyondellBasell Industries NV",
    "sector": "Basic Materials",
    "quarter": "Q1 2026",
    "f1": 5.65,
    "f2": 4.62,
    "f3": 6.97,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.31,
    "f7": 7.48,
    "f8": 8.0,
    "momentum_score": 3.8,
    "cqv_v1": 7.0,
    "cqv_v1_1": 6.75,
    "cqv_v2": 6.87,
    "cqv_v3": 6.87,
    "cqv": 6.67,
    "pe": 7.76,
    "close_history": "{'2021': 66.95, '2022': 66.29, '2023': 80.02, '2024': 66.11, '2025': 42.34, '2026': 57.8}",
    "status": "Success",
    "cqv_v4": 6.67,
    "price": 100.0,
    "pe_forward": 6.596,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "W",
    "name": "Wayfair Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 4.53,
    "f2": 4.5,
    "f3": 8.49,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 6.51,
    "f7": 8.38,
    "f8": 8.0,
    "momentum_score": 5.97,
    "cqv_v1": 6.74,
    "cqv_v1_1": 6.59,
    "cqv_v2": 6.83,
    "cqv_v3": 6.83,
    "cqv": 6.67,
    "pe": 25.4,
    "close_history": "{'2021': 189.97, '2022': 32.89, '2023': 61.7, '2024': 44.32, '2025': 100.41, '2026': 91.48}",
    "status": "Success",
    "cqv_v4": 6.67,
    "price": 100.0,
    "pe_forward": 21.59,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "TDUP",
    "name": "ThredUp Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 8.75,
    "f3": 9.29,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 5.14,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 5.7,
    "cqv_v1": 6.07,
    "cqv_v1_1": 6.47,
    "cqv_v2": 6.66,
    "cqv_v3": 6.66,
    "cqv": 6.67,
    "pe": -117.17,
    "close_history": "{'2021': 12.76, '2022': 1.31, '2023': 2.25, '2024': 1.39, '2025': 6.39, '2026': 6.74}",
    "status": "Success",
    "cqv_v4": 6.67,
    "price": 100.0,
    "pe_forward": -99.5945,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 7.8,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "JPM",
    "name": "JP Morgan Chase & Co.",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 2.94,
    "f2": 4.75,
    "f3": 9.08,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 6.01,
    "f7": 9.65,
    "f8": 8.0,
    "momentum_score": 7.15,
    "cqv_v1": 6.06,
    "cqv_v1_1": 6.41,
    "cqv_v2": 6.74,
    "cqv_v3": 6.74,
    "cqv": 6.55,
    "pe": 16.0,
    "close_history": "{'2021': 140.57, '2022': 122.8, '2023': 160.41, '2024': 231.45, '2025': 317.71, '2026': 343.15}",
    "status": "Success",
    "cqv_v4": 6.55,
    "price": 100.0,
    "pe_forward": 13.6,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "ABB",
    "name": "ABB",
    "sector": 0.0,
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.0,
    "f3": 8.22,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 4.61,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 5.0,
    "cqv_v1": 6.71,
    "cqv_v1_1": 6.43,
    "cqv_v2": 6.58,
    "cqv_v3": 6.58,
    "cqv": 6.54,
    "pe": 25.0,
    "close_history": "{}",
    "status": "Success",
    "cqv_v4": 6.54,
    "price": 100.0,
    "pe_forward": 25.0,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "SMH",
    "name": "VanEck Semiconductor ETF",
    "sector": 0.0,
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 9.0,
    "f3": 8.22,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 4.61,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 8.33,
    "cqv_v1": 6.71,
    "cqv_v1_1": 6.43,
    "cqv_v2": 6.58,
    "cqv_v3": 6.58,
    "cqv": 6.54,
    "pe": 39.94,
    "close_history": "{'2021': 150.61, '2022': 100.11, '2023': 173.56, '2024': 241.42, '2025': 360.13, '2026': 568.92}",
    "status": "Success",
    "cqv_v4": 6.54,
    "price": 100.0,
    "pe_forward": 33.949,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "AGM",
    "name": "Federal Agricultural Mortgage C",
    "sector": "Financial Services",
    "quarter": "Q1 2026",
    "f1": 2.35,
    "f2": 4.75,
    "f3": 9.24,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 5.79,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 8.43,
    "cqv_v1": 6.53,
    "cqv_v1_1": 6.29,
    "cqv_v2": 6.65,
    "cqv_v3": 6.65,
    "cqv": 6.45,
    "pe": 11.57,
    "close_history": "{'2021': 107.26, '2022': 101.01, '2023': 176.37, '2024': 187.09, '2025': 172.19, '2026': 206.23}",
    "status": "Success",
    "cqv_v4": 6.45,
    "price": 100.0,
    "pe_forward": 9.8345,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "CHPT",
    "name": "ChargePoint Holdings, Inc.",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 1.0,
    "f2": 8.75,
    "f3": 8.14,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 4.57,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 1.05,
    "cqv_v1": 6.62,
    "cqv_v1_1": 6.29,
    "cqv_v2": 6.49,
    "cqv_v3": 6.49,
    "cqv": 6.44,
    "pe": -2.94,
    "close_history": "{'2021': 381.0, '2022': 190.6, '2023': 46.8, '2024': 21.4, '2025': 6.64, '2026': 5.79}",
    "status": "Success",
    "cqv_v4": 6.44,
    "price": 100.0,
    "pe_forward": -2.499,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "KMX",
    "name": "CarMax Inc",
    "sector": "Consumer Cyclical",
    "quarter": "Q2 2026",
    "f1": 3.22,
    "f2": 4.5,
    "f3": 8.28,
    "f4": 7.8,
    "f5": 7.8,
    "f6": 5.75,
    "f7": 9.42,
    "f8": 8.0,
    "momentum_score": 7.3,
    "cqv_v1": 5.71,
    "cqv_v1_1": 6.23,
    "cqv_v2": 6.58,
    "cqv_v3": 6.58,
    "cqv": 6.36,
    "pe": 31.66,
    "close_history": "{'2021': 130.23, '2022': 60.89, '2023': 76.74, '2024': 81.76, '2025': 38.64, '2026': 58.47}",
    "status": "Success",
    "cqv_v4": 6.36,
    "price": 100.0,
    "pe_forward": 26.910999999999998,
    "value_score": 5.26,
    "peg_bruto": 6.6887,
    "score_peg": 6.6887,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 7.8,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "AI",
    "name": "C3.ai, Inc.",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 1.04,
    "f2": 9.5,
    "f3": 5.83,
    "f4": 8.2,
    "f5": 8.5,
    "f6": 3.44,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 1.05,
    "cqv_v1": 5.6,
    "cqv_v1_1": 6.31,
    "cqv_v2": 6.38,
    "cqv_v3": 6.38,
    "cqv": 6.23,
    "pe": -20.1,
    "close_history": "{'2021': 31.25, '2022': 11.19, '2023': 28.71, '2024': 34.43, '2025': 13.48, '2026': 8.91}",
    "status": "Success",
    "cqv_v4": 6.23,
    "price": 100.0,
    "pe_forward": -17.085,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.2,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 450.0,
    "maint_capex_m": 20.0,
    "owner_earnings_m": 430.0,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  },
  {
    "ticker": "OPEN",
    "name": "Opendoor Technologies Inc",
    "sector": "Real Estate",
    "quarter": "Q1 2026",
    "f1": 1.0,
    "f2": 8.75,
    "f3": 6.33,
    "f4": 8.0,
    "f5": 8.0,
    "f6": 3.67,
    "f7": 10.0,
    "f8": 8.0,
    "momentum_score": 3.19,
    "cqv_v1": 6.4,
    "cqv_v1_1": 6.11,
    "cqv_v2": 6.27,
    "cqv_v3": 6.27,
    "cqv": 6.13,
    "pe": -413.85,
    "close_history": "{'2021': 14.61, '2022': 1.16, '2023': 4.48, '2024': 1.6, '2025': 5.83, '2026': 4.57}",
    "status": "Success",
    "cqv_v4": 6.13,
    "price": 100.0,
    "pe_forward": -351.77250000000004,
    "value_score": null,
    "peg_bruto": null,
    "score_peg": null,
    "intrinsic_value": 118.0,
    "mos_pct": 15.25,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 8.0,
    "data_confidence": "N/D",
    "owner_earnings": null,
    "fcf_yield_pct": null
  },
  {
    "ticker": "INTC",
    "name": "Intel Corporation",
    "sector": "Technology",
    "quarter": "Q2 2026",
    "f1": 4.5,
    "f2": 3.8,
    "f3": 4.0,
    "f4": 3.5,
    "f5": 4.0,
    "f6": 4.0,
    "f7": 5.0,
    "f8": 4.0,
    "momentum_score": 8.73,
    "cqv_v1": 6.5,
    "cqv_v1_1": 7.17,
    "cqv_v2": 7.02,
    "cqv_v3": 7.02,
    "cqv": 4.04,
    "pe": 77.06,
    "close_history": "{'2021': 47.76, '2022': 25.49, '2023': 49.59, '2024': 20.05, '2025': 36.9, '2026': 96.98}",
    "status": "Success",
    "cqv_v4": 4.04,
    "price": 81.88,
    "pe_forward": 40.236263,
    "value_score": 4.59,
    "peg_bruto": 4.4736,
    "score_peg": 4.4736,
    "intrinsic_value": 96.62,
    "mos_pct": 15.26,
    "verdict": "Evitar / En Observación",
    "clasificacion": "VULNERABLE",
    "f4_moat": 3.5,
    "data_confidence": "Alta",
    "owner_earnings": null,
    "fcf_yield_pct": null,
    "eps_growth_ntm_pct": 18.0,
    "growth_eps": 18.0,
    "ocf_ttm_m": 368.46,
    "maint_capex_m": 16.376,
    "owner_earnings_m": 352.1,
    "score_fcf_yield": 4.3,
    "score_mos": 5.1
  }
];

        const companyDetailedData = {
            "MA": {
                desc: "Mastercard Incorporated es una empresa tecnológica líder global en la industria de medios de pago, conectando a consumidores, instituciones financieras, comercios y gobiernos en más de 210 países.",
                f1_desc: "Margen EBITDA extraordinario superior al 58% y ROIC colosal. Alta conversión de beneficio contable a flujo de caja libre neto.",
                f2_desc: "Balance y solvencia excepcionales. Liquidez inmediata superior a 1.2x la deuda a corto plazo y cobertura de intereses sumamente holgada.",
                f3_desc: "Crecimiento eficiente impulsado por la transición global del dinero físico a transacciones digitales y transfronterizas sin requerir alta inversión de capital.",
                f4_desc: "Foso competitivo (Moat) casi impenetrable derivado de efectos de red globales bilaterales masivos y un duopolio de red de peaje de facto con Visa.",
                f5_desc: "Preeminencia en seguridad digital mediante IA aplicada a detección de fraudes y soluciones transfronterizas complejas, asegurando resiliencia frente a disrupción Fintech.",
                f6_desc: "Asignación de capital ultra-eficiente centrado en recompras masivas de acciones constantes y dividendos crecientes sustentados en flujos predecibles.",
                f7_desc: "FCF Yield estable de ~3.0%-3.5% que refleja una valoración premium pero justificada por su foso de duopolio.",
                f8_desc: "Diversificación total con miles de millones de tarjetas y comercios integrados en su red global."
            },
            "V": {
                desc: "Visa Inc. es la red de procesamiento de pagos electrónicos más grande del mundo, operando como un peaje de transacción global para consumidores, comercios e instituciones financieras.",
                f1_desc: "Margen operativo superior al 64%. Negocio asset-light con requerimientos mínimos de capital e inmensa conversión de efectivo.",
                f2_desc: "Apalancamiento mínimo con Deuda Neta/EBITDA muy por debajo de 1.0x, complementado por flujos de caja de alta predecibilidad anticíclica.",
                f3_desc: "Crecimiento constante impulsado por la digitalización del efectivo y la expansión de pagos sin contacto y transferencias P2P.",
                f4_desc: "Efecto red bilateral insuperable (comercios-consumidores) y foso de escala global regulado, operando de facto como un peaje indestructible.",
                f5_desc: "Opcionalidad en flujos transfronterizos complejos y APIs de seguridad financiera, inmune a la desintermediación por su escala y confianza institucional.",
                f6_desc: "Asignación de capital óptima. Recompra neta masiva y constante reducción de acciones en circulación sin deuda significativa.",
                f7_desc: "FCF Yield de ~3.0%-3.5%, consistente con un negocio de altísima calidad y baja intensidad de capital.",
                f8_desc: "Excelente diversificación de ingresos entre múltiples emisores y adquirentes a nivel mundial."
            },
            "AAPL": {
                desc: "Apple Inc. es la compañía de hardware de consumo y servicios más grande del mundo, destacando por su ecosistema cerrado que integra iPhone, iPad, Mac y una suite de servicios de alta fidelidad.",
                f1_desc: "Márgenes brutos estables del 45% y generación de caja operativa colosal. ROIC extraordinario (>50%) debido a su modelo de manufactura optimizado.",
                f2_desc: "Estructura de deuda conservadora orientada a neutralidad de caja neta, con alta predecibilidad del flujo libre de caja.",
                f3_desc: "Crecimiento a un dígito alto apoyado por servicios recurrentes que compensan la maduración de las ventas de hardware físico.",
                f4_desc: "Foso insuperable basado en costos de cambio extremos del ecosistema iOS/macOS y el valor de marca más fuerte del planeta.",
                f5_desc: "Fuerte opcionalidad en servicios financieros, salud y computación espacial (Vision Pro), con un ecosistema extremadamente resistente a la desintermediación por terceros.",
                f6_desc: "Asignación de capital agresiva y exitosa enfocada en recompras masivas (más de 90B anuales) y dividendos constantes.",
                f7_desc: "FCF Yield de ~3.0%-3.5%. Su flujo de caja colosal ofrece protección ante presiones macroeconómicas.",
                f8_desc: "Dependencia significativa de la manufactura en Asia (Foxconn), aunque cuenta con millones de consumidores diversificados en todo el mundo."
            },
            "ASML": {
                desc: "ASML Holding N.V. es el único fabricante en el mundo de máquinas de litografía ultravioleta extrema (EUV), indispensables para fabricar los semiconductores más avanzados de la actualidad.",
                f1_desc: "Rentabilidad extraordinaria con retornos de capital muy elevados (ROIC >30%) apoyados en su monopolio tecnológico.",
                f2_desc: "Balance impecable con caja neta y nulo riesgo crediticio debido a pagos por hitos adelantados de sus clientes.",
                f3_desc: "Crecimiento sólido impulsado por la demanda estructural de microchips de última generación y centros de datos de IA.",
                f4_desc: "Monopolio tecnológico e intelectual absoluto en litografía avanzada, con barreras de entrada físicas y científicas multimillonarias.",
                f5_desc: "Proveedor crítico insustituible para el futuro de la Inteligencia Artificial y la computación de alto rendimiento.",
                f6_desc: "Retornos elevados sobre capital incremental (ROCIC) gracias a la reinversión constante en I+D crítica y recompras oportunistas.",
                f7_desc: "FCF Yield bajo (~2.0%) debido a múltiplos de valoración exigentes que reflejan su monopolio tecnológico.",
                f8_desc: "Riesgo de concentración de clientes medio-alto (TSMC, Intel y Samsung representan la gran mayoría de sus ingresos de EUV)."
            },
            "AVGO": {
                desc: "Broadcom Inc. es un líder tecnológico global que diseña, desarrolla y suministra una amplia gama de soluciones de software de infraestructura y semiconductores analógicos y digitales.",
                f1_desc: "Margen operativo superior al 45% sostenido. Excelente generación de flujo de caja libre, con conversión superior al 110%.",
                f2_desc: "Apalancamiento manejable gracias a la rápida reducción de deuda utilizando los flujos de caja operativos tras la compra de VMware.",
                f3_desc: "Crecimiento impulsado por adquisiciones estratégicas masivas (M&A) e integración de software y silicio personalizado para centros de datos de IA.",
                f4_desc: "Moat fuerte basado en contratos de software empresarial a largo plazo y patentes críticas de conectividad de silicio para centros de datos.",
                f5_desc: "Papel fundamental en el despliegue de redes para clústeres de GPU para IA y chips propietarios personalizados (ASICs).",
                f6_desc: "Estrategia M&A agresiva pero disciplinada dirigida por Hock Tan, logrando desapalancamiento rápido tras adquisiciones (ej. VMware).",
                f7_desc: "FCF Yield atractivo (~4.0%) sustentado en su capacidad de ordeñar flujos recurrentes de software de infraestructura.",
                f8_desc: "Dependencia parcial de grandes proveedores de infraestructura cloud e hiperscalers en el área de silicio."
            },
            "GOOGL": {
                desc: "Alphabet Inc. (Google) es el líder indiscutible en búsquedas web, publicidad digital y sistemas operativos móviles (Android). Su modelo de negocio se basa en la monetización de la atención de los usuarios y el procesamiento masivo de datos mediante IA.",
                f1_desc: "Margen operativo superior al 27% sostenido. Elevado ROIC (>25%), aunque presionado por la intensidad de capital necesaria para la infraestructura de Inteligencia Artificial.",
                f2_desc: "Balance extremadamente sólido con caja neta de más de 100,000 millones de dólares y un ratio Deuda Neta/EBITDA negativo.",
                f3_desc: "Crecimiento histórico constante superior al 10%, sin embargo, el factor se ve afectado por la dilución neta derivada de su alta compensación basada en acciones (SBC).",
                f4_desc: "Foso competitivo masivo derivado de costos de cambio operativos en Android/Google Workspace y un efecto red colosal en su motor de búsqueda y YouTube.",
                f5_desc: "Posicionamiento líder en la revolución de IA con Gemini, Google Cloud y Waymo, ofreciendo opcionalidad en conducción autónoma y computación en la nube.",
                f6_desc: "Asignación de capital buena pero diluida levemente por su compensación en acciones (SBC). Inversiones masivas en Capex de IA.",
                f7_desc: "FCF Yield sólido de ~3.5%-4.0%, representando una de las valoraciones más atractivas en Big Tech por flujo de caja.",
                f8_desc: "Excelente diversificación de anunciantes a nivel global, con nula dependencia de clientes individuales."
            },
            "MSFT": {
                desc: "Microsoft Corporation es el gigante del software empresarial y servicios en la nube (Azure), con una integración líder en productividad de oficina (Office) e Inteligencia Artificial corporativa.",
                f1_desc: "Márgenes operativos superiores al 40% y una de las tasas de conversión de beneficio neto a FCF más altas del mercado corporativo.",
                f2_desc: "Calificación crediticia AAA (de las pocas del mundo), con una cobertura de intereses de triple dígito y una pila de caja formidable.",
                f3_desc: "Crecimiento del 12%-18% anual impulsado por la migración masiva a la nube inteligente y la suscripción SaaS en todas sus líneas de negocio.",
                f4_desc: "Monopolio de facto corporativo con la suite Office, Windows, Active Directory y Azure, con costos de cambio casi prohibitivos para empresas.",
                f5_desc: "Máxima opcionalidad tecnológica derivada de su alianza preferente con OpenAI y la integración de Copilot en todo su ecosistema de software corporativo.",
                f6_desc: "Retornos sobre capital invertido incremental (ROCIC) estelares mediante adquisiciones estratégicas clave y Capex agresivo en infraestructura de nube.",
                f7_desc: "FCF Yield de ~2.5%-3.0% debido a múltiplos exigentes impulsados por el optimismo en IA.",
                f8_desc: "Ecosistema empresarial masivo e hiper-diversificado con millones de clientes corporativos y consumidores globales."
            },
            "NVDA": {
                desc: "NVIDIA Corporation es el diseñador dominante de unidades de procesamiento gráfico (GPU) y la plataforma de software CUDA, sirviendo como la espina dorsal tecnológica de la IA global.",
                f1_desc: "Margen operativo superior al 55% y ROIC récord de la industria (>80%) gracias a su poder de fijación de precios absoluto ante la demanda de cómputo.",
                f2_desc: "Posición financiera impecable con caja neta masiva generada en los últimos trimestres y nulo riesgo de liquidez.",
                f3_desc: "Crecimiento explosivo de ingresos superior al 200% interanual en el sector de centros de datos, limitado únicamente por la capacidad de producción de TSMC.",
                f4_desc: "Foso tecnológico e intelectual inmenso debido a CUDA, la plataforma de software propietaria que impide la migración fácil a chips competidores.",
                f5_desc: "Posicionamiento absoluto como el 'peaje de hardware' de la revolución de IA, con opcionalidad en robótica, automóviles autónomos y gemelos digitales.",
                f6_desc: "Reinversión masiva de capital en Capex y diseño de semiconductores de próxima generación para mantener el liderazgo tecnológico.",
                f7_desc: "FCF Yield muy bajo (~1.5%-2.0%) debido a una valoración implícita de crecimiento hiper-exigente.",
                f8_desc: "Alta concentración de ingresos en pocos proveedores en la nube y fabricantes contratados, penalizada por la regla del factor."
            },
            "FICO": {
                desc: "Fair Isaac Corporation es el proveedor estándar del algoritmo de puntuación crediticia utilizado por más del 90% de los prestamistas en los Estados Unidos para evaluar el riesgo de consumo.",
                f1_desc: "Márgenes EBITDA del 45% y ROIC infinito gracias a un modelo de negocio de licenciamiento puramente digital con mínima base de activos físicos.",
                f2_desc: "Operación antifrágil. Aunque tiene patrimonio neto negativo debido a recompras agresivas, la cobertura de intereses supera las 10 veces el EBITDA.",
                f3_desc: "Crecimiento del 10%-15% impulsado por incrementos continuos de precios y mayor adopción de analítica predictiva en banca.",
                f4_desc: "Monopolio de facto regulado por las agencias hipotecarias federales (Fannie Mae y Freddie Mac), que exigen el uso del Score FICO para finalizar créditos.",
                f5_desc: "Expansión en FICO Platform, una suite de decisión empresarial en la nube que integra analítica de IA y optimización de flujos de trabajo financieros.",
                f6_desc: "Asignación de capital ultra-agresiva mediante recompras destructoras de flotante, logrando un crecimiento masivo de BPA a pesar del apalancamiento.",
                f7_desc: "FCF Yield modesto (~2.5%) debido a la revalorización de múltiplos históricos por su pricing power absoluto.",
                f8_desc: "Dependencia directa de los tres burós de crédito principales norteamericanos (Equifax, Experian y TransUnion)."
            }
        };

        function getCompanyDetails(ticker, company) {
            if (companyDetailedData[ticker]) {
                return companyDetailedData[ticker];
            }
            const fmt = val => (val !== undefined && val !== null && Number.isFinite(Number(val))) ? Number(val).toFixed(2) : 'N/D';
            const cqvScoreStr = fmt(company ? company.cqv : null);
            const tierInfo = getTier(company ? company.cqv : null);
            return {
                desc: `${company ? company.name : ticker} (${company ? company.ticker : ticker}) es una empresa cotizada en bolsa calificada con un Score CQV global de ${cqvScoreStr}, posicionándose en la categoría de ${tierInfo.name}.`,
                f1_desc: `Calificación F1 de ${fmt(company ? company.f1 : null)} sobre rentabilidad, márgenes operativos y conversión de flujo de caja libre.`,
                f2_desc: `Calificación F2 de ${fmt(company ? company.f2 : null)} sobre la solidez de su balance y cobertura de intereses frente a la deuda.`,
                f3_desc: `Calificación F3 de ${fmt(company ? company.f3 : null)} sobre su tasa de crecimiento orgánico auditada y control de la dilución al accionista por SBC.`,
                f4_desc: `Calificación F4 de ${fmt(company ? company.f4 : null)} que refleja sus barreras de entrada competitivas (Moat) y retención del cliente.`,
                f5_desc: `Calificación F5 de ${fmt(company ? company.f5 : null)} de opcionalidad tecnológica ante la revolución digital y resiliencia disruptiva.`,
                f6_desc: `Calificación F6 de ${fmt(company ? company.f6 : null)} sobre la asignación de capital operativo y dividendos de la directiva.`,
                f7_desc: `Calificación F7 de ${fmt(company ? company.f7 : null)} sobre el FCF Yield y la valoración del flujo de caja de la empresa.`,
                f8_desc: `Calificación F8 de ${fmt(company ? company.f8 : null)} sobre la resiliencia operativa y la diversificación de ingresos frente al riesgo de concentración.`
            };
        }

        let currentVersion = 'v3'; // 'v1' or 'v2'

        function setCQVVersion(version) {
            currentVersion = version;
            
            const versionSelect = document.getElementById('version-select');
            if (versionSelect) {
                versionSelect.value = version;
            }
            
            companies.forEach(c => {
                if (version === 'v1') {
                    c.cqv = (c.cqv_v1 !== undefined && c.cqv_v1 !== null) ? c.cqv_v1 : c.cqv;
                } else if (version === 'v1_1') {
                    c.cqv = (c.cqv_v1_1 !== undefined && c.cqv_v1_1 !== null) ? c.cqv_v1_1 : (c.cqv_v1 || c.cqv);
                } else if (version === 'v2') {
                    c.cqv = (c.cqv_v2 !== undefined && c.cqv_v2 !== null) ? c.cqv_v2 : c.cqv;
                } else if (version === 'v3') {
                    c.cqv = (c.cqv_v3 !== undefined && c.cqv_v3 !== null) ? c.cqv_v3 : (c.cqv_v2 || c.cqv);
                } else {
                    c.cqv = (c.cqv_v4 !== undefined && c.cqv_v4 !== null) ? c.cqv_v4 : (c.cqv_v3 || c.cqv_v2 || c.cqv);
                }
            });
            
            updateVersionUI();
            
            // Re-render everything
            initKPIs();
            renderTopChart();
            sortData();
            renderTable();
            
            // Update History/Trends
            const historySelect = document.getElementById('history-company-select');
            if (historySelect && historySelect.value) {
                loadCompanyHistory();
            }
            
            // Update Simulator
            const simSelect = document.getElementById('sim-company-select');
            if (simSelect) {
                if (simSelect.value) {
                    changeSimYear();
                } else {
                    runSimulation();
                }
            }
        }
        
        function updateVersionUI() {
            const is8F = (currentVersion === 'v2' || currentVersion === 'v3');
            if (is8F) {
                document.body.classList.remove('cqv-v1-active');
            } else {
                document.body.classList.add('cqv-v1-active');
            }
        }

        function initDashboard() {
            // Apply saved theme preference
            const savedTheme = localStorage.getItem('theme');
            if (savedTheme === 'light') {
                document.body.classList.add('light-theme');
                const themeIcon = document.getElementById('theme-toggle-icon');
                if (themeIcon) {
                    themeIcon.className = 'fa-solid fa-moon';
                }
            }
            
            companies = window.companiesData || (typeof companiesData !== 'undefined' ? companiesData : []);
            if (companies.length === 0) {
                console.error("No companies data available!");
                return;
            }
            filteredData = [...companies];
            
            // Set initial version
            setCQVVersion('v4');
            populateSimCompanySelect();
            populateHistoryCompanySelect();

            const historySelect = document.getElementById('history-company-select');
            if (historySelect && (!historySelect.value || historySelect.value === '')) {
                const sortedComp = [...companies].filter(c => Number.isFinite(Number(c.cqv))).sort((a, b) => b.cqv - a.cqv);
                if (sortedComp.length > 0) {
                    historySelect.value = sortedComp[0].ticker;
                    loadCompanyHistory();
                } else if (companies.length > 0) {
                    historySelect.value = companies[0].ticker;
                    loadCompanyHistory();
                }
            }
        }

        function populateSimCompanySelect() {
            const selectEl = document.getElementById('sim-company-select');
            if (!selectEl) return;
            selectEl.innerHTML = '<option value="">-- Valores por defecto (Simulación libre) --</option>';
            const sortedCompanies = [...companies].sort((a, b) => a.ticker.localeCompare(b.ticker));
            sortedCompanies.forEach(c => {
                const opt = document.createElement('option');
                opt.value = c.ticker;
                const scoreVal = (currentVersion === 'v4' && c.cqv_v4 !== undefined && c.cqv_v4 !== null) ? c.cqv_v4 : ((currentVersion === 'v3' && c.cqv_v3 !== undefined && c.cqv_v3 !== null) ? c.cqv_v3 : c.cqv);
                    const scoreText = Number.isFinite(Number(scoreVal)) ? Number(scoreVal).toFixed(2) : 'N/D';
                    opt.innerText = `${c.ticker} - ${c.name} (CQV: ${scoreText})`;
                selectEl.appendChild(opt);
            });
        }

        function loadCompanyIntoSimulator() {
            const ticker = document.getElementById('sim-company-select').value;
            const yearWrapper = document.getElementById('sim-year-wrapper');
            const yearSelect = document.getElementById('sim-year-select');
            const simProfileCard = document.getElementById('sim-profile-card');
            const simProfileDesc = document.getElementById('sim-profile-desc');
            const simProfileFactors = document.getElementById('sim-profile-factors');
            
            if (!ticker) {
                if (yearWrapper) yearWrapper.style.display = 'none';
                if (simProfileCard) simProfileCard.style.display = 'none';
                
                document.getElementById('slide-f1').value = 7.5;
                document.getElementById('slide-f2').value = 8.0;
                document.getElementById('slide-f3').value = 7.0;
                document.getElementById('slide-f4').value = 8.5;
                document.getElementById('slide-f5').value = 8.0;
                document.getElementById('slide-f6').value = 8.0;
                document.getElementById('slide-f7').value = 7.0;
                document.getElementById('slide-f8').value = 8.0;
                window.originalSimData = null;
                runSimulation();
            } else {
                const company = companies.find(c => c.ticker === ticker);
                if (company) {
                    if (simProfileCard && simProfileDesc && simProfileFactors) {
                        const details = getCompanyDetails(ticker, company);
                        simProfileDesc.innerText = details.desc;
                        
                        simProfileFactors.innerHTML = `
                            <li><strong style="color: #3b82f6;"><i class="fa-solid fa-wallet"></i> F1 (Rentabilidad):</strong> ${details.f1_desc}</li>
                            <li><strong style="color: #a855f7;"><i class="fa-solid fa-lock"></i> F2 (Solidez):</strong> ${details.f2_desc}</li>
                            <li><strong style="color: #10b981;"><i class="fa-solid fa-arrow-trend-up"></i> F3 (Crecimiento):</strong> ${details.f3_desc}</li>
                            <li><strong style="color: #f59e0b;"><i class="fa-solid fa-shield-halved"></i> F4 (Moat):</strong> ${details.f4_desc}</li>
                            <li><strong style="color: #d946ef;"><i class="fa-solid fa-rocket"></i> F5 (Proyección):</strong> ${details.f5_desc}</li>
                            <li class="v2-only"><strong style="color: #6366f1;"><i class="fa-solid fa-hand-holding-dollar"></i> F6 (Asignación):</strong> ${details.f6_desc}</li>
                            <li class="v2-only"><strong style="color: #06b6d4;"><i class="fa-solid fa-money-bill-trend-up"></i> F7 (FCF Yield):</strong> ${details.f7_desc}</li>
                            <li class="v2-only"><strong style="color: #ec4899;"><i class="fa-solid fa-triangle-exclamation"></i> F8 (Antifragilidad):</strong> ${details.f8_desc}</li>
                        `;
                        simProfileCard.style.display = 'block';
                    }
                    
                    if (yearSelect && yearWrapper) {
                        yearSelect.innerHTML = '';
                        
                        const rawHistory = (typeof cqvHistoryData !== 'undefined' && cqvHistoryData[ticker]) ? cqvHistoryData[ticker] : {};
                        const years = Object.keys(rawHistory).sort();
                        
                        if (!years.includes("2026")) {
                            years.push("2026");
                        }
                        
                        years.sort((a, b) => b - a);
                        
                        years.forEach(yr => {
                            const opt = document.createElement('option');
                            opt.value = yr;
                            opt.innerText = yr === "2026" ? "2026 (Act.)" : yr;
                            yearSelect.appendChild(opt);
                        });
                        
                        yearWrapper.style.display = 'block';
                        yearSelect.value = "2026";
                    }
                    changeSimYear();
                }
            }
        }

        function changeSimYear() {
            const ticker = document.getElementById('sim-company-select').value;
            const yr = document.getElementById('sim-year-select').value;
            if (!ticker || !yr) return;
            
            const company = companies.find(c => c.ticker === ticker);
            if (!company) return;
            
            let f1 = company.f1;
            let f2 = company.f2;
            let f3 = company.f3;
            
            const rawHistory = (typeof cqvHistoryData !== 'undefined' && cqvHistoryData[ticker]) ? cqvHistoryData[ticker] : {};
            if (rawHistory[yr]) {
                f1 = rawHistory[yr].f1;
                f2 = rawHistory[yr].f2;
                f3 = rawHistory[yr].f3;
            }
            
            const f4 = company.f4;
            const f5 = company.f5;
            const f6 = company.f6;
            const f7 = company.f7;
            const f8 = company.f8;
            
            document.getElementById('slide-f1').value = f1;
            document.getElementById('slide-f2').value = f2;
            document.getElementById('slide-f3').value = f3;
            document.getElementById('slide-f4').value = f4;
            document.getElementById('slide-f5').value = f5;
            document.getElementById('slide-f6').value = f6;
            document.getElementById('slide-f7').value = f7;
            document.getElementById('slide-f8').value = f8;
            
            window.originalSimData = {
                name: company.name,
                ticker: `${company.ticker} (${yr === "2026" ? "2026 Act." : yr})`,
                data: [f1, f2, f3, f4, f5, f6, f7, f8]
            };
            
            runSimulation();
        }

        function populateHistoryCompanySelect() {
            const selectEl = document.getElementById('history-company-select');
            if (!selectEl) return;
            selectEl.innerHTML = '<option value="">-- Seleccionar una empresa --</option>';
            const sortedCompanies = [...companies].sort((a, b) => a.ticker.localeCompare(b.ticker));
            const historyObj = (typeof cqvHistory !== 'undefined' ? cqvHistory : (window.cqvHistory || window.cqvHistoryData || {}));
            sortedCompanies.forEach(c => {
                if (!historyObj || historyObj[c.ticker]) {
                    const opt = document.createElement('option');
                    opt.value = c.ticker;
                    const scoreVal = (currentVersion === 'v4' && c.cqv_v4 !== undefined && c.cqv_v4 !== null) ? c.cqv_v4 : ((currentVersion === 'v3' && c.cqv_v3 !== undefined && c.cqv_v3 !== null) ? c.cqv_v3 : c.cqv);
                    const scoreText = formatDashboardScore(scoreVal);
                    opt.innerText = `${c.ticker} - ${c.name} (CQV: ${scoreText})`;
                    selectEl.appendChild(opt);
                }
            });
        }

        function getChartGridColor() {
            return document.body.classList.contains('light-theme') ? 'rgba(15, 23, 42, 0.08)' : 'rgba(255, 255, 255, 0.06)';
        }

        function getChartLabelColor() {
            return document.body.classList.contains('light-theme') ? '#475569' : '#94a3b8';
        }

        function toggleTheme() {
            document.body.classList.toggle('light-theme');
            const isLight = document.body.classList.contains('light-theme');
            localStorage.setItem('theme', isLight ? 'light' : 'dark');
            
            const themeIcon = document.getElementById('theme-toggle-icon');
            if (themeIcon) {
                themeIcon.className = isLight ? 'fa-solid fa-moon' : 'fa-solid fa-sun';
            }
            
            // Re-render charts
            renderTopChart();
            runSimulation();
            if (typeof loadCompanyHistory === 'function') {
                loadCompanyHistory();
            }
        }

        const presetCompanyNotes = {
            "GOOGL": "La reducción en el score CQV de Google se debe principalmente a una contracción en el Factor F1 (Rentabilidad y Retornos) por el aumento masivo en inversiones de capital (CapEx) en infraestructura de centros de datos para IA y mayores gastos operativos de desarrollo. Esto afectó el Factor F3 (Crecimiento de utilidades libres de caja), contrarrestando su excelente foso competitivo en búsquedas (Factor F4).",
            "AAPL": "El score de Apple muestra gran solidez debido a su insuperable Factor F4 (Foso de Ecosistema cautivo) y alta rentabilidad (F1). Los ligeros retrocesos temporales se atribuyen a una desaceleración en el volumen de crecimiento en hardware (Factor F3), balanceado por el aumento de ingresos en servicios.",
            "NVDA": "La subida vertical de la calificación CQV de Nvidia se debe al incremento exponencial en el Factor F1 (Rentabilidad operativa neta récord de márgenes) y el Factor F3 (Crecimiento de ingresos superiores al 200% interanual), consolidando un monopolio de hardware para Inteligencia Artificial (Factor F4).",
            "MSFT": "Microsoft mantiene un posicionamiento de élite constante. Su score se ve impulsado por la expansión del margen y ventas en la nube (Azure) y la integración rápida de IA en su catálogo de software (Factor F5 - Proyecciones y Factor F4 - Foso corporativo)."
        };

        function loadCompanyNotes(ticker) {
            const textarea = document.getElementById('company-analyst-notes');
            const descLabel = document.getElementById('notes-description-label');
            if (!textarea || !descLabel) return;
            
            const company = companies.find(c => c.ticker === ticker);
            if (!company) {
                textarea.value = '';
                descLabel.innerText = 'Selecciona una empresa para gestionar sus notas de auditoría financiera.';
                textarea.disabled = true;
                return;
            }
            
            textarea.disabled = false;
            descLabel.innerText = `Notas del Analista para ${company.ticker} (${company.name}):`;
            
            // Check localStorage first
            const saved = localStorage.getItem(`cqv_note_${ticker}`);
            if (saved !== null) {
                textarea.value = saved;
            } else if (presetCompanyNotes[ticker]) {
                textarea.value = presetCompanyNotes[ticker];
            } else {
                // Generate automated notes based on factors
                const factors = [
                    { name: 'Rentabilidad (F1)', val: company.f1 },
                    { name: 'Solidez (F2)', val: company.f2 },
                    { name: 'Crecimiento (F3)', val: company.f3 },
                    { name: 'Foso/Moat (F4)', val: company.f4 },
                    { name: 'Proyección (F5)', val: company.f5 }
                ];
                factors.sort((a, b) => b.val - a.val);
                const highest = factors[0];
                const lowest = factors[factors.length - 1];
                
                textarea.value = `Análisis de Score: Su principal fortaleza radica en ${highest.name} con una puntuación de ${highest.val.toFixed(2)}, mientras que presenta áreas de mejora en ${lowest.name} con ${lowest.val.toFixed(2)}.`;
            }
        }

        function saveCompanyNotes() {
            const selectEl = document.getElementById('history-company-select');
            const textarea = document.getElementById('company-analyst-notes');
            const statusSpan = document.getElementById('save-note-status');
            if (!selectEl || !textarea) return;
            
            const ticker = selectEl.value;
            if (!ticker) return;
            
            localStorage.setItem(`cqv_note_${ticker}`, textarea.value);
            
            // Show status label
            if (statusSpan) {
                statusSpan.style.opacity = '1';
                setTimeout(() => {
                    statusSpan.style.opacity = '0';
                }, 2000);
            }
        }

        let historyChart = null;
        let peChart = null;
        function loadCompanyHistory() {
            const fmt = val => (val !== undefined && val !== null && Number.isFinite(Number(val))) ? Number(val).toFixed(2) : 'N/D';
            const ticker = document.getElementById('history-company-select').value;
            const tbody = document.getElementById('history-details-body');
            const profileCard = document.getElementById('history-profile-card');
            const profileDesc = document.getElementById('history-profile-desc');
            const profileFactors = document.getElementById('history-profile-factors');
            
            if (!tbody) return;
            tbody.innerHTML = '';
            
            if (!ticker) {
                if (historyChart) {
                    historyChart.destroy();
                    historyChart = null;
                }
                tbody.innerHTML = '<tr><td colspan="10" style="text-align: center; color: var(--text-secondary); padding: 30px 10px;">Selecciona una empresa para ver su evolución.</td></tr>';
                loadCompanyNotes('');
                if (profileCard) profileCard.style.display = 'none';
                return;
            }
            
            const company = companies.find(c => c.ticker === ticker);
            if (!company) return;
            
            if (profileCard && profileDesc && profileFactors) {
                const details = getCompanyDetails(ticker, company);
                profileDesc.innerText = details.desc;
                
                profileFactors.innerHTML = `
                    <li><strong style="color: #3b82f6;"><i class="fa-solid fa-wallet"></i> F1 (Rentabilidad):</strong> ${details.f1_desc}</li>
                    <li><strong style="color: #a855f7;"><i class="fa-solid fa-lock"></i> F2 (Solidez):</strong> ${details.f2_desc}</li>
                    <li><strong style="color: #10b981;"><i class="fa-solid fa-arrow-trend-up"></i> F3 (Crecimiento):</strong> ${details.f3_desc}</li>
                    <li><strong style="color: #f59e0b;"><i class="fa-solid fa-shield-halved"></i> F4 (Moat):</strong> ${details.f4_desc}</li>
                    <li><strong style="color: #d946ef;"><i class="fa-solid fa-rocket"></i> F5 (Proyección):</strong> ${details.f5_desc}</li>
                    <li class="v2-only"><strong style="color: #6366f1;"><i class="fa-solid fa-hand-holding-dollar"></i> F6 (Asignación):</strong> ${details.f6_desc}</li>
                    <li class="v2-only"><strong style="color: #06b6d4;"><i class="fa-solid fa-money-bill-trend-up"></i> F7 (FCF Yield):</strong> ${details.f7_desc}</li>
                    <li class="v2-only"><strong style="color: #ec4899;"><i class="fa-solid fa-triangle-exclamation"></i> F8 (Antifragilidad):</strong> ${details.f8_desc}</li>
                `;
                profileCard.style.display = 'block';
            }
            
            const historyObj = window.cqvHistoryData || (typeof cqvHistoryData !== 'undefined' ? cqvHistoryData : {});
            const rawHistory = historyObj[ticker] || {};
            const history = { ...rawHistory };
            

            
            // Append 2026 calculation if not already present in the history database
            if (!history["2026"]) {
                history["2026"] = { f1: company.f1, f2: company.f2, f3: company.f3, cqv: company.cqv, pe: company.pe };
            }
            
            const years = Object.keys(history).sort();
            const chartLabels = years.map(yr => (yr === "2026" && !rawHistory["2026"]) ? "2026 (Act.)" : yr);
            
            const chartData = [];
            const priceData = [];
            const peData = [];
            let peSum = 0;
            let peCount = 0;

            years.forEach(yr => {
                const data = history[yr];
                const isCurrent = yr === "2026" && !rawHistory["2026"];
                const label = isCurrent ? "2026 (Act.)" : yr;
                
                // Recalculate historical CQV using 8 factors (F1-F3 historical, F4-F8 fixed/current)
                const cqv_v2 = (
                    data.f1 * 0.20 +
                    data.f2 * 0.10 +
                    data.f3 * 0.10 +
                    company.f4 * 0.20 +
                    company.f5 * 0.10 +
                    company.f6 * 0.10 +
                    company.f7 * 0.10 +
                    company.f8 * 0.10
                );
                chartData.push(cqv_v2);

                // Get historical close price from company.close_history
                const closePrice = company.close_history ? company.close_history[yr] : null;
                priceData.push(closePrice);

                const peVal = data.pe || null;
                peData.push(peVal);
                if (peVal) {
                    peSum += peVal;
                    peCount++;
                }

                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td><span style="font-weight: bold; color: ${isCurrent ? 'var(--accent)' : 'var(--text-primary)'};">${label}</span></td>
                    <td class="cqv-value-cell">${fmt(data?.f1)}</td>
                    <td class="cqv-value-cell">${fmt(data?.f2)}</td>
                    <td class="cqv-value-cell">${fmt(data?.f3)}</td>
                    <td class="cqv-value-cell">${fmt(company?.f4)}</td>
                    <td class="cqv-value-cell">${fmt(company?.f5)}</td>
                    <td class="cqv-value-cell">${fmt(company?.f6)}</td>
                    <td class="cqv-value-cell">${fmt(company?.f7)}</td>
                    <td class="cqv-value-cell">${fmt(company?.f8)}</td>
                    <td class="cqv-value-cell">${data.pe ? data.pe.toFixed(1) + 'x' : '-'}</td>
                    <td class="cqv-value-cell score-high" style="font-weight: bold;">${fmt(cqv_v2)}</td>
                `;
                tbody.appendChild(tr);
            });
            
            renderHistoryChart(ticker, chartLabels, chartData);

            const avgPE = peCount > 0 ? (peSum / peCount) : null;
            const avgPEBadge = document.getElementById('pe-average-badge');
            if (avgPEBadge) {
                avgPEBadge.innerText = avgPE ? `PER Prom: ${avgPE.toFixed(1)}x` : 'PER Prom: -';
            }
            renderPEValuationChart(ticker, chartLabels, priceData, peData, avgPE);
            
            // Load analyst notes
            loadCompanyNotes(ticker);
        }

        function renderHistoryChart(ticker, labels, data) {
            if (typeof Chart === 'undefined') {
                console.warn("Chart.js is not loaded. Skipping line chart rendering.");
                return;
            }
            const canvasEl = document.getElementById('historyChart');
            if (!canvasEl) return;
            const ctx = canvasEl.getContext('2d');
            
            if (historyChart) {
                historyChart.destroy();
            }
            
            const gradient = ctx.createLinearGradient(0, 0, 0, 300);
            gradient.addColorStop(0, 'rgba(79, 70, 229, 0.4)');
            gradient.addColorStop(1, 'rgba(79, 70, 229, 0.0)');
            
            historyChart = new Chart(ctx, {
                type: 'line',
                data: {
                    labels: labels,
                    datasets: [{
                        label: `Evolución CQV - ${ticker}`,
                        data: data,
                        fill: true,
                        backgroundColor: gradient,
                        borderColor: '#4f46e5',
            spanGaps: true,
                        borderWidth: 3,
                        pointBackgroundColor: '#d946ef',
                        pointBorderColor: '#fff',
                        pointRadius: 6,
                        pointHoverRadius: 8,
                        tension: 0.3
                    }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: { display: false },
                        tooltip: {
                            backgroundColor: '#0f172a',
                            borderColor: '#334155',
                            borderWidth: 1,
                            padding: 12,
                            displayColors: false,
                            callbacks: {
                                title: function(context) {
                                    return `Año ${context[0].label}`;
                                },
                                label: function(context) {
                                    return `CQV Score: ${context.parsed.y.toFixed(2)}`;
                                }
                            }
                        }
                    },
                    scales: {
                        x: {
                            grid: { color: getChartGridColor() },
                            ticks: { color: getChartLabelColor() }
                        },
                        y: {
                            min: 1,
                            max: 10,
                            grid: { color: getChartGridColor() },
                            ticks: { color: getChartLabelColor(), stepSize: 1 }
                        }
                    }
                }
            });
        }

        function renderPEValuationChart(ticker, labels, prices, pes, avgPE) {
            if (typeof Chart === 'undefined') {
                console.warn("Chart.js is not loaded. Skipping PE chart rendering.");
                return;
            }
            const canvasEl = document.getElementById('peValuationChart');
            if (!canvasEl) return;
            const ctx = canvasEl.getContext('2d');
            
            if (peChart) {
                peChart.destroy();
            }
            
            const avgLineData = labels.map(() => avgPE);
            
            peChart = new Chart(ctx, {
                type: 'line',
                data: {
                    labels: labels,
                    datasets: [
                        {
                            label: 'Precio de Cierre ($)',
                            data: prices,
                            borderColor: '#10b981',
              spanGaps: true,
                            backgroundColor: 'rgba(16, 185, 129, 0.1)',
                            borderWidth: 2.5,
                            pointRadius: 4,
                            tension: 0.2,
                            yAxisID: 'y-price'
                        },
                        {
                            label: 'Múltiplo PER (x)',
                            data: pes,
                            borderColor: '#06b6d4',
                            backgroundColor: 'rgba(6, 182, 212, 0.1)',
                            borderWidth: 2.5,
                            pointRadius: 4,
                            tension: 0.2,
                            yAxisID: 'y-pe'
                        },
                        {
                            label: 'PER Promedio',
                            data: avgLineData,
                            borderColor: '#ef4444',
                            borderWidth: 1.5,
                            borderDash: [5, 5],
                            pointRadius: 0,
                            fill: false,
                            yAxisID: 'y-pe'
                        }
                    ]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    interaction: {
                        mode: 'index',
                        intersect: false,
                    },
                    plugins: {
                        legend: {
                            display: true,
                            position: 'top',
                            labels: {
                                color: getChartLabelColor(),
                                font: { size: 11, family: 'Inter' }
                            }
                        },
                        tooltip: {
                            backgroundColor: '#0f172a',
                            borderColor: '#334155',
                            borderWidth: 1,
                            padding: 10,
                            callbacks: {
                                label: function(context) {
                                    const val = context.raw;
                                    if (val === null || val === undefined) return '';
                                    if (context.datasetIndex === 0) {
                                        return `Precio: $${val.toFixed(2)}`;
                                    } else if (context.datasetIndex === 1) {
                                        return `PER: ${val.toFixed(1)}x`;
                                    } else {
                                        return `PER Prom: ${val.toFixed(1)}x`;
                                    }
                                }
                            }
                        }
                    },
                    scales: {
                        x: {
                            grid: { color: getChartGridColor() },
                            ticks: { color: getChartLabelColor(), font: { family: 'Inter' } }
                        },
                        'y-price': {
                            type: 'linear',
                            position: 'left',
                            title: {
                                display: true,
                                text: 'Precio ($)',
                                color: '#10b981',
                                font: { weight: 'bold', family: 'Inter' }
                            },
                            grid: { color: getChartGridColor() },
                            ticks: { color: getChartLabelColor() }
                        },
                        'y-pe': {
                            type: 'linear',
                            position: 'right',
                            title: {
                                display: true,
                                text: 'PER Ratio (x)',
                                color: '#06b6d4',
                                font: { weight: 'bold', family: 'Inter' }
                            },
                            grid: { drawOnChartArea: false },
                            ticks: { color: getChartLabelColor() }
                        }
                    }
                }
            });
        }

        // Navigation state
        function switchTab(tabId) {
            // Deactivate all panels
            document.querySelectorAll('.tab-panel').forEach(panel => {
                panel.classList.remove('active');
            });
            // Deactivate all nav buttons
            document.querySelectorAll('.nav-btn').forEach(btn => {
                btn.classList.remove('active');
            });
            
            // Activate current
            document.getElementById('tab-' + tabId).classList.add('active');
            
            // Activate current button
            const clickedBtn = Array.from(document.querySelectorAll('.nav-btn')).find(btn => {
                const text = btn.innerText.toLowerCase();
                if (tabId === 'explorer') return text.includes('explorador');
                if (tabId === 'methodology') return text.includes('metodología');
                if (tabId === 'history') return text.includes('tendencias') || text.includes('historial');
                return text.includes(tabId);
            });
            if (clickedBtn) clickedBtn.classList.add('active');
            
            // Re-render chart if navigating back to dashboard or history
            if (tabId === 'dashboard') {
                setTimeout(renderTopChart, 50);
            } else if (tabId === 'history') {
                const historySelect = document.getElementById('history-company-select');
                if (historySelect) {
                    if (!historySelect.value || historySelect.value === '') {
                        const sortedComp = [...companies].sort((a, b) => b.cqv - a.cqv);
                        if (sortedComp.length > 0) {
                            historySelect.value = sortedComp[0].ticker;
                        }
                    }
                }
                setTimeout(() => {
                    loadCompanyHistory();
                    if (historyChart && typeof historyChart.resize === 'function') historyChart.resize();
                    if (typeof peChart !== 'undefined' && peChart && typeof peChart.resize === 'function') peChart.resize();
                }, 50);
            }
        }

        // Data Helpers
        ;
            if (score >= 8.5) return { name: 'SÓLIDA', class: 'tier-strong' };
            if (score >= 8.0) return { name: 'MEDIA', class: 'tier-medium' };
            return { name: 'ESPECULATIVA', class: 'tier-speculative' };
        }

        // KPI Calculations
        function initKPIs() {
            document.getElementById('kpi-total-companies').innerText = companies.length;
            
            const totalCqv = companies.reduce((acc, c) => acc + c.cqv, 0);
            const avgCqv = totalCqv / companies.length;
            document.getElementById('kpi-avg-cqv').innerText = avgCqv.toFixed(2);
            
            const eliteCount = companies.filter(c => c.cqv >= 9.0).length;
            document.getElementById('kpi-elite-count').innerText = eliteCount;
            
            if (companies.length > 0) {
                const sorted = [...companies].sort((a, b) => b.cqv - a.cqv);
                document.getElementById('kpi-top-performer').innerText = `${sorted[0].ticker} (${sorted[0].cqv.toFixed(2)})`;
            }
            
            // Calculate distributions
            const total = companies.length;
            const elite = companies.filter(c => c.cqv >= 9.0).length;
            const strong = companies.filter(c => c.cqv >= 8.5 && c.cqv < 9.0).length;
            const medium = companies.filter(c => c.cqv >= 8.0 && c.cqv < 8.5).length;
            const weak = companies.filter(c => c.cqv < 8.0).length;
            
            document.getElementById('dist-count-elite').innerText = `${elite} emp. (${(elite/total*100).toFixed(0)}%)`;
            document.getElementById('dist-count-strong').innerText = `${strong} emp. (${(strong/total*100).toFixed(0)}%)`;
            document.getElementById('dist-count-medium').innerText = `${medium} emp. (${(medium/total*100).toFixed(0)}%)`;
            document.getElementById('dist-count-weak').innerText = `${weak} emp. (${(weak/total*100).toFixed(0)}%)`;
            
            document.getElementById('dist-bar-elite').style.width = `${(elite/total*100).toFixed(0)}%`;
            document.getElementById('dist-bar-strong').style.width = `${(strong/total*100).toFixed(0)}%`;
            document.getElementById('dist-bar-medium').style.width = `${(medium/total*100).toFixed(0)}%`;
            document.getElementById('dist-bar-weak').style.width = `${(weak/total*100).toFixed(0)}%`;
        }

        // Render Top 15 Bar Chart
        let topChart = null;
        let top20SectorChart = null;
        let top20PillarsChart = null;
        let top20ValuationChart = null;

        function renderTopChart() {
            if (typeof Chart === 'undefined') {
                console.warn("Chart.js is not loaded. Skipping chart rendering.");
                return;
            }
            
            if (!companies || companies.length === 0) return;
            
            // Sort companies descending by CQV score
            const top20 = [...companies].sort((a, b) => b.cqv - a.cqv).slice(0, 20);
            
            const labelColor = getChartLabelColor();
            const gridColor = getChartGridColor();
            
            // 1. Render Top 20 Bar Chart (Score CQV)
            const canvasTop = document.getElementById('topChart');
            if (canvasTop) {
                const ctx = canvasTop.getContext('2d');
                const labels = top20.map(c => c.ticker);
                const data = top20.map(c => c.cqv);
                
                if (topChart && typeof topChart.destroy === 'function') topChart.destroy();
                
                const gradient = ctx.createLinearGradient(0, 0, 0, 300);
                gradient.addColorStop(0, '#6366f1');
                gradient.addColorStop(1, '#a855f7');
                
                topChart = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: labels,
                        datasets: [{
                            label: 'Score CQV',
                            data: data,
                            backgroundColor: gradient,
                            borderColor: 'rgba(255,255,255,0.15)',
                            borderWidth: 1,
                            borderRadius: 6
                        }]
                    },
                    options: {
                        responsive: true,
                        maintainAspectRatio: false,
                        plugins: {
                            legend: { display: false },
                            tooltip: {
                                backgroundColor: '#0f172a',
                                titleColor: '#f8fafc',
                                bodyColor: '#cbd5e1',
                                borderColor: '#334155',
                                borderWidth: 1,
                                padding: 12,
                                callbacks: {
                                    title: (items) => `${top20[items[0].dataIndex].ticker} - ${top20[items[0].dataIndex].name}`,
                                    label: (item) => `Score CQV: ${item.raw.toFixed(2)} / 10`
                                }
                            }
                        },
                        scales: {
                            x: {
                                grid: { display: false },
                                ticks: { color: labelColor, font: { family: 'Inter', size: 10, weight: '700' } }
                            },
                            y: {
                                min: 7, max: 10,
                                grid: { color: gridColor },
                                ticks: { color: labelColor, font: { family: 'Outfit', size: 11 } }
                            }
                        }
                    }
                });
            }

            // 2. Render Top 20 Sector Distribution Doughnut Chart
            const canvasSector = document.getElementById('top20SectorChart');
            if (canvasSector) {
                const ctx = canvasSector.getContext('2d');
                const sectorCounts = {};
                top20.forEach(c => {
                    const sec = c.sector || 'Otros';
                    sectorCounts[sec] = (sectorCounts[sec] || 0) + 1;
                });
                
                const sectorLabels = Object.keys(sectorCounts);
                const sectorData = Object.values(sectorCounts);
                const sectorColors = ['#6366f1', '#10b981', '#3b82f6', '#f59e0b', '#ec4899', '#8b5cf6', '#14b8a6'];
                
                if (top20SectorChart && typeof top20SectorChart.destroy === 'function') top20SectorChart.destroy();
                
                top20SectorChart = new Chart(ctx, {
                    type: 'doughnut',
                    data: {
                        labels: sectorLabels,
                        datasets: [{
                            data: sectorData,
                            backgroundColor: sectorColors.slice(0, sectorLabels.length),
                            borderWidth: 2,
                            borderColor: gridColor
                        }]
                    },
                    options: {
                        responsive: true,
                        maintainAspectRatio: false,
                        plugins: {
                            legend: {
                                display: true,
                                position: 'right',
                                labels: { color: labelColor, font: { family: 'Inter', size: 10 } }
                            },
                            tooltip: {
                                callbacks: {
                                    label: (item) => `${item.label}: ${item.raw} empresas (${(item.raw/20*100).toFixed(0)}%)`
                                }
                            }
                        },
                        cutout: '60%'
                    }
                });
            }

            // 3. Render Top 20 Pillars Comparison Chart (F1 Rentabilidad, F2 Solidez, F4 Moat)
            const canvasPillars = document.getElementById('top20PillarsChart');
            if (canvasPillars) {
                const ctx = canvasPillars.getContext('2d');
                const labels = top20.map(c => c.ticker);
                const dataF1 = top20.map(c => c.f1 || 0);
                const dataF2 = top20.map(c => c.f2 || 0);
                const dataF4 = top20.map(c => c.f4 || 0);
                
                if (top20PillarsChart && typeof top20PillarsChart.destroy === 'function') top20PillarsChart.destroy();
                
                top20PillarsChart = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: labels,
                        datasets: [
                            { label: 'F1 Rentabilidad', data: dataF1, backgroundColor: '#3b82f6', borderRadius: 4 },
                            { label: 'F2 Solidez', data: dataF2, backgroundColor: '#10b981', borderRadius: 4 },
                            { label: 'F4 Moat', data: dataF4, backgroundColor: '#8b5cf6', borderRadius: 4 }
                        ]
                    },
                    options: {
                        responsive: true,
                        maintainAspectRatio: false,
                        plugins: {
                            legend: {
                                display: true,
                                position: 'top',
                                labels: { color: labelColor, font: { family: 'Inter', size: 11 } }
                            }
                        },
                        scales: {
                            x: {
                                grid: { display: false },
                                ticks: { color: labelColor, font: { family: 'Inter', size: 10 } }
                            },
                            y: {
                                min: 5, max: 10,
                                grid: { color: gridColor },
                                ticks: { color: labelColor, font: { family: 'Outfit', size: 11 } }
                            }
                        }
                    }
                });
            }

            // 4. Render Top 20 Valuation PER vs Quality Score Chart
            const canvasValuation = document.getElementById('top20ValuationChart');
            if (canvasValuation) {
                const ctx = canvasValuation.getContext('2d');
                const labels = top20.map(c => c.ticker);
                const dataCQV = top20.map(c => c.cqv || 0);
                const dataPER = top20.map(c => c.pe || 0);
                
                if (top20ValuationChart && typeof top20ValuationChart.destroy === 'function') top20ValuationChart.destroy();
                
                top20ValuationChart = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: labels,
                        datasets: [
                            {
                                type: 'line',
                                label: 'Score CQV (Eje Izq)',
                                data: dataCQV,
                                borderColor: '#10b981',
              spanGaps: true,
                                backgroundColor: '#10b981',
                                borderWidth: 3,
                                pointRadius: 4,
                                yAxisID: 'y'
                            },
                            {
                                type: 'bar',
                                label: 'PER Trailing (Eje Der)',
                                data: dataPER,
                                backgroundColor: 'rgba(59, 130, 246, 0.65)',
                                borderRadius: 4,
                                yAxisID: 'y1'
                            }
                        ]
                    },
                    options: {
                        responsive: true,
                        maintainAspectRatio: false,
                        plugins: {
                            legend: {
                                display: true,
                                position: 'top',
                                labels: { color: labelColor, font: { family: 'Inter', size: 11 } }
                            }
                        },
                        scales: {
                            x: {
                                grid: { display: false },
                                ticks: { color: labelColor, font: { family: 'Inter', size: 10 } }
                            },
                            y: {
                                type: 'linear',
                                display: true,
                                position: 'left',
                                min: 6, max: 10,
                                title: { display: true, text: 'Score CQV', color: labelColor, font: { size: 10 } },
                                grid: { color: gridColor },
                                ticks: { color: labelColor }
                            },
                            y1: {
                                type: 'linear',
                                display: true,
                                position: 'right',
                                min: 0,
                                title: { display: true, text: 'Múltiplo PER', color: labelColor, font: { size: 10 } },
                                grid: { drawOnChartArea: false },
                                ticks: { color: labelColor }
                            }
                        }
                    }
                });
            }

            // 5. Render Top 20 Cards Showcase Grid
            renderTop20Grid(top20);
        }

        function renderTop20Grid(top20) {
            const gridEl = document.getElementById('top20-grid');
            if (!gridEl) return;
            gridEl.innerHTML = '';
            
            top20.forEach((c, idx) => {
                const rank = idx + 1;
                const card = document.createElement('div');
                card.className = 'top20-card';
                card.onclick = () => {
                    switchTab('history');
                    const historySelect = document.getElementById('history-company-select');
                    if (historySelect) {
                        historySelect.value = c.ticker;
                        loadCompanyHistory();
                    }
                };
                
                const tierInfo = getTier(c.cqv);
                card.innerHTML = `
                    <div class="top20-rank-badge">#${rank}</div>
                    <div class="top20-header">
                        <div>
                            <div class="top20-ticker">${c.ticker}</div>
                            <div class="top20-name">${c.name}</div>
                        </div>
                    </div>
                    <div class="top20-score-row">
                        <div>
                            <div class="top20-score-label">Score CQV</div>
                            <span class="badge ${tierInfo.class}" style="font-size: 11px;">${tierInfo.name}</span>
                        </div>
                        <div class="top20-score-val">${formatDashboardScore(c.cqv)}</div>
                    </div>
                    <div class="top20-metrics-pills">
                        <div class="top20-pill">
                            <div class="top20-pill-lbl">F1 Rentab.</div>
                            <div class="top20-pill-val">${c.f1.toFixed(1)}</div>
                        </div>
                        <div class="top20-pill">
                            <div class="top20-pill-lbl">F2 Solidez</div>
                            <div class="top20-pill-val">${c.f2.toFixed(1)}</div>
                        </div>
                        <div class="top20-pill">
                            <div class="top20-pill-lbl">F4 Moat</div>
                            <div class="top20-pill-val">${c.f4.toFixed(1)}</div>
                        </div>
                        <div class="top20-pill">
                            <div class="top20-pill-lbl">PER Trailing</div>
                            <div class="top20-pill-val">${c.pe ? c.pe.toFixed(1) + 'x' : '-'}</div>
                        </div>
                    </div>
                `;
                gridEl.appendChild(card);
            });
        }

        // Explorer Table Logic
        let filteredData = [];
        let currentSort = { column: 'cqv', direction: 'desc' };
        let currentPage = 1;
        let rowsPerPage = 50;

        function renderTable() {
            const tbody = document.getElementById('companies-table-body');
            if (!tbody) return;
            tbody.innerHTML = '';
            
            const start = (currentPage - 1) * rowsPerPage;
            const end = rowsPerPage === 'all' ? filteredData.length : start + parseInt(rowsPerPage);
            const pageData = rowsPerPage === 'all' ? filteredData : filteredData.slice(start, end);
            
            pageData.forEach(c => {
                const tier = getTier(c.cqv);
                const tr = document.createElement('tr');
                tr.style.cursor = 'pointer';
                tr.onclick = function() {
                    switchTab('history');
                    const selectEl = document.getElementById('history-company-select');
                    if (selectEl) {
                        selectEl.value = c.ticker;
                        loadCompanyHistory();
                    }
                };
                
                // Build sparkline HTML for 5-year CQV history (including current 2026)
                let sparklineHtml = '';
                const history = typeof cqvHistoryData !== 'undefined' ? cqvHistoryData[c.ticker] : null;
                if (history) {
                    const yrData = { ...history };
                    if (!yrData["2026"]) {
                        yrData["2026"] = { f1: c.f1, f2: c.f2, f3: c.f3, cqv: c.cqv };
                    }
                    const years = Object.keys(yrData).sort();
                    let bars = '';
                    years.forEach(yr => {
                        let yrCqv = yrData[yr].cqv;
                        if (currentVersion === 'v2') {
                            yrCqv = (yrData[yr].f1 * 0.20) + (yrData[yr].f2 * 0.10) + (yrData[yr].f3 * 0.10) + (c.f4 * 0.20) + (c.f5 * 0.10) + (c.f6 * 0.10) + (c.f7 * 0.10) + (c.f8 * 0.10);
                        }
                        const heightPct = (yrCqv / 10.0) * 100;
                        let barColor = 'var(--text-secondary)';
                        if (yrCqv >= 9.0) barColor = 'var(--elite)';
                        else if (yrCqv >= 8.5) barColor = 'var(--strong)';
                        else if (yrCqv >= 8.0) barColor = 'var(--medium)';
                        else barColor = 'var(--weak)';
                        
                        bars += `<div class="sparkline-bar" style="height: ${heightPct}%; background-color: ${barColor};" title="Año ${yr}: ${formatDashboardScore(yrCqv)}"></div>`;
                    });
                    sparklineHtml = `<div class="sparkline-container">${bars}</div>`;
                } else {
                    // Fallback to show at least the current 2026 bar if no history database is available
                    const heightPct = (c.cqv / 10.0) * 100;
                    let barColor = 'var(--text-secondary)';
                    if (c.cqv >= 9.0) barColor = 'var(--elite)';
                    else if (c.cqv >= 8.5) barColor = 'var(--strong)';
                    else if (c.cqv >= 8.0) barColor = 'var(--medium)';
                    else barColor = 'var(--weak)';
                    
                    sparklineHtml = `
                        <div class="sparkline-container" style="justify-content: center;">
                            <div class="sparkline-bar" style="height: ${heightPct}%; background-color: ${barColor};" title="Año 2026 (Act.): ${formatDashboardScore(c.cqv)}"></div>
                        </div>
                    `;
                }

                // Calculate trend from 2025 to 2026
                let trendHtml = '';
                if (history && history["2025"]) {
                    let score2025 = history["2025"].cqv;
                    if (currentVersion === 'v2') {
                        score2025 = (history["2025"].f1 * 0.20) + (history["2025"].f2 * 0.10) + (history["2025"].f3 * 0.10) + (c.f4 * 0.20) + (c.f5 * 0.10) + (c.f6 * 0.10) + (c.f7 * 0.10) + (c.f8 * 0.10);
                    }
                    const score2026 = c.cqv;
                    const diff = score2026 - score2025;
                    
                    if (diff > 0.005) {
                        trendHtml = `<span style="color: var(--elite); font-weight: bold; font-size: 11px; display: inline-flex; align-items: center;" title="Mejorando vs 2025: +${formatDashboardScore(diff)}"><i class="fa-solid fa-arrow-trend-up"></i></span>`;
                    } else if (diff < -0.005) {
                        trendHtml = `<span style="color: var(--weak); font-weight: bold; font-size: 11px; display: inline-flex; align-items: center;" title="Empeorando vs 2025: ${formatDashboardScore(diff)}"><i class="fa-solid fa-arrow-trend-down"></i></span>`;
                    } else {
                        trendHtml = `<span style="color: var(--medium); font-weight: bold; font-size: 11px; display: inline-flex; align-items: center;" title="Sin cambios vs 2025"><i class="fa-solid fa-arrow-right"></i></span>`;
                    }
                } else {
                    trendHtml = `<span style="color: var(--text-secondary); opacity: 0.3; font-size: 11px;" title="Sin datos de 2025">-</span>`;
                }

                const sparklineCellHtml = `
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px; width: 68px;">
                        ${sparklineHtml}
                        ${trendHtml}
                    </div>
                `;

                tr.innerHTML = `
                    <td><span class="ticker-badge">${c.ticker}</span></td>
                    <td><span class="company-name">${c.name}</span></td>
                    <td><span class="q-badge">${c.quarter || 'Q1 2026'}</span></td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f1)}</td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f2)}</td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f3)}</td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f4)}</td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f5)}</td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f6)}</td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f7)}</td>
                    <td class="cqv-value-cell">${formatDashboardScore(c.f8)}</td>
                    <td class="cqv-value-cell">${c.pe ? c.pe.toFixed(1) + 'x' : '-'}</td>
                    <td class="cqv-value-cell score-high">${formatDashboardScore(c.cqv)}</td>
                    <td>${sparklineCellHtml}</td>
                    <td><span class="tier-badge ${tier.class}">${tier.name}</span></td>
                `;
                tbody.appendChild(tr);
            });
            
            // Update labels
            const totalCount = filteredData.length;
            const showingStart = totalCount === 0 ? 0 : start + 1;
            const showingEnd = rowsPerPage === 'all' ? totalCount : Math.min(end, totalCount);
            document.getElementById('showing-entries-label').innerText = `Mostrando ${showingStart} - ${showingEnd} de ${totalCount} empresas`;
            
            renderPagination(totalCount);
        }

        function renderPagination(totalCount) {
            const wrapper = document.getElementById('pagination-wrapper');
            if (!wrapper) return;
            wrapper.innerHTML = '';
            
            if (rowsPerPage === 'all' || totalCount <= rowsPerPage) return;
            
            const totalPages = Math.ceil(totalCount / rowsPerPage);
            
            // Prev btn
            const prevBtn = document.createElement('button');
            prevBtn.className = 'page-btn';
            prevBtn.innerHTML = '<i class="fa-solid fa-chevron-left"></i>';
            prevBtn.disabled = currentPage === 1;
            prevBtn.onclick = () => { currentPage--; renderTable(); };
            wrapper.appendChild(prevBtn);
            
            // Page numbers
            const maxVisible = 5;
            let startPage = Math.max(1, currentPage - 2);
            let endPage = Math.min(totalPages, startPage + maxVisible - 1);
            if (endPage - startPage < maxVisible - 1) {
                startPage = Math.max(1, endPage - maxVisible + 1);
            }
            
            for (let i = startPage; i <= endPage; i++) {
                const pageBtn = document.createElement('button');
                pageBtn.className = `page-btn ${i === currentPage ? 'active' : ''}`;
                pageBtn.innerText = i;
                pageBtn.onclick = () => { currentPage = i; renderTable(); };
                wrapper.appendChild(pageBtn);
            }
            
            // Next btn
            const nextBtn = document.createElement('button');
            nextBtn.className = 'page-btn';
            nextBtn.innerHTML = '<i class="fa-solid fa-chevron-right"></i>';
            nextBtn.disabled = currentPage === totalPages;
            nextBtn.onclick = () => { currentPage++; renderTable(); };
            wrapper.appendChild(nextBtn);
        }

        function handleSearch() {
            const query = document.getElementById('search-bar').value.toLowerCase().trim();
            applyFilters(query, document.getElementById('tier-filter').value);
        }

        function handleFilter() {
            const tier = document.getElementById('tier-filter').value;
            const query = document.getElementById('search-bar').value.toLowerCase().trim();
            applyFilters(query, tier);
        }

        function applyFilters(query, tier) {
            filteredData = companies.filter(c => {
                const matchesQuery = c.ticker.toLowerCase().includes(query) || c.name.toLowerCase().includes(query);
                
                let matchesTier = true;
                if (tier === 'elite') matchesTier = c.cqv >= 9.0;
                else if (tier === 'strong') matchesTier = c.cqv >= 8.5 && c.cqv < 9.0;
                else if (tier === 'medium') matchesTier = c.cqv >= 8.0 && c.cqv < 8.5;
                else if (tier === 'speculative') matchesTier = c.cqv < 8.0;
                
                return matchesQuery && matchesTier;
            });
            
            currentPage = 1;
            sortData();
            renderTable();
        }

        function handleRowsChange() {
            rowsPerPage = document.getElementById('rows-filter').value;
            currentPage = 1;
            renderTable();
        }

        function handleSort(column) {
            if (currentSort.column === column) {
                currentSort.direction = currentSort.direction === 'asc' ? 'desc' : 'asc';
            } else {
                currentSort.column = column;
                currentSort.direction = 'desc'; // default high to low
            }
            
            // Update UI headers indicators
            const colIndices = { 'ticker': 0, 'name': 1, 'quarter': 2, 'f1': 3, 'f2': 4, 'f3': 5, 'f4': 6, 'f5': 7, 'f6': 8, 'f7': 9, 'f8': 10, 'pe': 11, 'cqv': 12 };
            
            for (let key in colIndices) {
                const icon = document.getElementById('sort-icon-' + key);
                if (icon) {
                    if (key === column) {
                        icon.innerHTML = currentSort.direction === 'asc' ? '<i class="fa-solid fa-sort-up"></i>' : '<i class="fa-solid fa-sort-down"></i>';
                    } else {
                        icon.innerHTML = '<i class="fa-solid fa-sort"></i>';
                    }
                }
            }
            
            sortData();
            renderTable();
        }

        function sortData() {
            const col = currentSort.column;
            const dir = currentSort.direction === 'asc' ? 1 : -1;
            
            filteredData.sort((a, b) => {
                let valA = a[col];
                let valB = b[col];
                
                if (typeof valA === 'string') {
                    return valA.localeCompare(valB) * dir;
                }
                return (valA - valB) * dir;
            });
        }

        // Simulator Logic
        let simChart = null;
        function runSimulation() {
            const f1 = parseFloat(document.getElementById('slide-f1').value);
            const f2 = parseFloat(document.getElementById('slide-f2').value);
            const f3 = parseFloat(document.getElementById('slide-f3').value);
            const f4 = parseFloat(document.getElementById('slide-f4').value);
            const f5 = parseFloat(document.getElementById('slide-f5').value);
            const f6 = parseFloat(document.getElementById('slide-f6').value);
            const f7 = parseFloat(document.getElementById('slide-f7').value);
            const f8 = parseFloat(document.getElementById('slide-f8').value);
            
            // Update labels
            document.getElementById('val-f1').innerText = f1.toFixed(1);
            document.getElementById('val-f2').innerText = f2.toFixed(1);
            document.getElementById('val-f3').innerText = f3.toFixed(1);
            document.getElementById('val-f4').innerText = f4.toFixed(1);
            document.getElementById('val-f5').innerText = f5.toFixed(1);
            document.getElementById('val-f6').innerText = f6.toFixed(1);
            document.getElementById('val-f7').innerText = f7.toFixed(1);
            document.getElementById('val-f8').innerText = f8.toFixed(1);
            
            // Equation weights based on version
            let cqv = 0;
            if (currentVersion === 'v1') {
                cqv = (f1 * 0.25) + (f2 * 0.15) + (f3 * 0.15) + (f4 * 0.25) + (f5 * 0.20);
            } else {
                cqv = (f1 * 0.20) + (f2 * 0.10) + (f3 * 0.10) + (f4 * 0.20) + (f5 * 0.10) + (f6 * 0.10) + (f7 * 0.10) + (f8 * 0.10);
            }
            
            // Update display
            const display = document.getElementById('sim-cqv-score');
            display.innerText = cqv.toFixed(2);
            
            const tier = getTier(cqv);
            const tierBadge = document.getElementById('sim-tier-badge');
            tierBadge.innerText = tier.name;
            tierBadge.className = `tier-badge ${tier.class}`;
            
            const simData = [f1, f2, f3, f4, f5, f6, f7, f8];
            updateSimChart(simData, window.originalSimData);
        }

        function updateSimChart(data, originalBenchmark) {
            if (typeof Chart === 'undefined') {
                console.warn("Chart.js is not loaded. Skipping radar chart update.");
                return;
            }
            const canvasEl = document.getElementById('simChart');
            if (!canvasEl) return;
            const ctx = canvasEl.getContext('2d');
            
            if (simChart) {
                simChart.destroy();
            }
            
            const simDataFinal = (currentVersion === 'v2') ? data : data.slice(0, 5);
            const datasets = [{
                label: 'Valores Simulados',
                data: simDataFinal,
                backgroundColor: 'rgba(6, 182, 212, 0.2)',
                borderColor: '#06b6d4',
                borderWidth: 2,
                pointBackgroundColor: '#06b6d4',
                pointBorderColor: '#fff',
                pointHoverBackgroundColor: '#fff',
                pointHoverBorderColor: '#06b6d4'
            }];
            
            if (originalBenchmark) {
                const benchmarkDataFinal = (currentVersion === 'v2') ? originalBenchmark.data : originalBenchmark.data.slice(0, 5);
                datasets.push({
                    label: `Original: ${originalBenchmark.ticker}`,
                    data: benchmarkDataFinal,
                    backgroundColor: 'rgba(217, 70, 239, 0.05)',
                    borderColor: 'rgba(217, 70, 239, 0.6)',
                    borderWidth: 1.5,
                    borderDash: [5, 5],
                    pointBackgroundColor: '#d946ef',
                    pointBorderColor: '#fff',
                    pointHoverBackgroundColor: '#fff',
                    pointHoverBorderColor: '#d946ef'
                });
            }
            
            simChart = new Chart(ctx, {
                type: 'radar',
                data: {
                    labels: (currentVersion === 'v2') ?
                        ['F1 (Rent.)', 'F2 (Solidez)', 'F3 (Crec.)', 'F4 (Moat)', 'F5 (Proj.)', 'F6 (Asig.)', 'F7 (Yield)', 'F8 (Antif.)'] :
                        ['F1 (Rent.)', 'F2 (Solidez)', 'F3 (Crec.)', 'F4 (Moat)', 'F5 (Proj.)'],
                    datasets: datasets
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: {
                            display: originalBenchmark ? true : false,
                            labels: {
                                color: '#94a3b8',
                                font: { family: 'Inter', size: 10 }
                            }
                        }
                    },
                    scales: {
                        r: {
                            min: 0,
                            max: 10,
                            ticks: {
                                stepSize: 2,
                                display: false
                            },
                            grid: {
                                color: getChartGridColor()
                            },
                            angleLines: {
                                color: getChartGridColor()
                            },
                            pointLabels: {
                                color: getChartLabelColor(),
                                font: {
                                    family: 'Outfit',
                                    size: 11
                                }
                            }
                        }
                    }
                }
            });
        }

        // Initialize script logic
        function bootDashboard() {
            const hasGlobalData = (typeof window.companiesData !== 'undefined' && window.companiesData.length > 0) || (typeof companiesData !== 'undefined' && companiesData.length > 0);
            if (hasGlobalData) {
                initDashboard();
            } else {
                fetch('cqv_data.json')
                    .then(r => r.json())
                    .then(data => {
                        window.companiesData = data;
                        initDashboard();
                    })
                    .catch(err => {
                        console.warn("CORS block or missing file during fetch, falling back to window.companiesData if available", err);
                        if (window.companiesData) {
                            initDashboard();
                        }
                    });
            }
        }

        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', bootDashboard);
        } else {
            bootDashboard();
        }
        window.addEventListener('load', bootDashboard);
    