...
path_to_suprema.sort(key=lambda x: (x['cqv_curr'], x['delta']), reverse=True)

print(f"=== EMPRESAS ÉLITE CAMINO A ÉLITE SUPREMA (CQV 9.00 - 9.49 EN ASCENSO): {len(path_to_suprema)} ===\n")
for i, p in enumerate(path_to_suprema, 1):
    ...
    print(f"   Trayectoria: {p['first_sc']} ({p['first_yr']}) -> {p['cqv_curr']} (2026) | Delta: +{p['delta']} pts")
