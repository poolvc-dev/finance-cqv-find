...
cqv_map = {item['ticker']: item for item in cqv_data if 'ticker' in item}
...
| **Precio de Mercado a la Fecha de Valoración:** | **{price_str}** | En USD ($) | Cotización de cierre a la fecha de publicación ({val_date}). |
...
{name} ({ticker}) opera en el sector **{sector}**. La empresa ha demostrado una posición sólida en el mercado respaldada por elevados retornos sobre el capital, ventajas competitivas sostenibles y una consistente generación de flujo de caja libre.
...
| **F2: Solidez Financiera** | **{f2:.2f}** | 15.0% | **{f2*0.15:.4f}** | Balance sólido sin presiones de endeudamiento a corto plazo. |
...
| **F5: Asignación de Capital** | **{f5:.2f}** | 10.0% | **{f5*0.10:.4f}** | Asignación disciplinada de capital entre reinversión y retorno al accionista. |
...
for item in cqv_data:
    ...
    quarter = item.get('quarter', 'Q2 2026')
    q_suffix = quarter.replace(' ', '_').upper() # e.g. Q2_2026
    
    # We save as TICKER_2026_Q2.md (and TICKER.md)
    m = item.get('quarter', '')
    if 'Q3' in m:
        fn_upper = f"{ticker}_2026_Q3.md"
    elif 'Q4' in m:
        fn_upper = f"{ticker}_2026_Q4.md"
    else:
        fn_upper = f"{ticker}_2026_Q2.md"
        
    hist_data = cqv_history.get(ticker, {})
    content = generate_report_content(item, hist_data)
    
    # Save uppercase file
    with open(os.path.join('inform', fn_upper), 'w', encoding='utf-8') as f:
        f.write(content)
        
    # Save lowercase file
    with open(os.path.join('inform', fn_upper.lower()), 'w', encoding='utf-8') as f:
        f.write(content)

    count += 1

print(f"Successfully generated 100% complete 10-section Markdown investment theses for ALL {count} companies in inform/!")
