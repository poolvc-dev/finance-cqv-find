...
files = [f for f in os.listdir('inform') if f.endswith('_2026_Q2.md') and f[0].isupper()]

count = 0
for filename in files:
    ticker = filename.split('_')[0]
    item = cqv_map.get(ticker, {
        'ticker': ticker,
        'name': f"{ticker} Inc.",
        'quarter': 'Q2 2026',
        'cqv_v4': 8.50,
    ...
    # Save uppercase file
    with open(os.path.join('inform', filename), 'w', encoding='utf-8') as f:
        f.write(content)
        
    # Save lowercase file
    with open(os.path.join('inform', filename.lower()), 'w', encoding='utf-8') as f:
        f.write(content)
        
    count += 1

print(f"Successfully updated 100% of all {count} files ending in _2026_Q2.md!")
